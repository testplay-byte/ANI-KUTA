...
---
Task ID: DASHBOARD-TESTING-PAGE
Agent: full-stack-developer
Task: Create a dedicated testing page in the dashboard with checklists, log capture guide, and concerns

Work Log:
- Read worklog (last 2 entries — METADATA-FIX-v2 + DL-D3-D8) for context on what the 4 fixes actually were (metadata cache destructive overwrite, episode list built from downloaded-only, subtitle URIs not passed + SubtitleEngine couldn't handle content://, episode switching failed with sourceId=0).
- Read 7 existing codebase files to match style EXACTLY: layout.tsx (root wrapper), Sidebar.tsx (NavIcon record pattern), Checklist.tsx (local-only state reference), Card.tsx (Card + CardHeader), lib/data.ts (NAV_ITEMS array + ChecklistItem), globals.css (CSS var tokens), phase-d/page.tsx (hero/section patterns).
- Modified `lib/data.ts`: appended Testing entry to NAV_ITEMS array (icon="testing", href="/testing/").
- Modified `components/Sidebar.tsx`: added `testing` icon to the NavIcon `icons` record — clipboard with checkmark, stroke-based, 24x24 viewBox, matches existing icon style (strokeWidth 1.8, rounded caps/joins).
- Created `components/TestingChecklist.tsx` — a NEW client component distinct from the local-only Checklist.tsx. State persists to localStorage under `ani-kuta:testing:${id}`. Each item is a clickable row with rounded-[5px] 16px checkbox (green fill when checked). Includes progress bar (h-1.5, bg-[var(--c-success)]), done/total count in mono, optional detail text below main text, Reset button with window.confirm, and a collapsible chevron (collapse state also persisted to `ani-kuta:testing:${id}:collapsed`). SSR-safe hydration (starts unchecked/expanded, hydrates after mount to avoid localStorage flash). All design tokens match the rest of the dashboard (bg-surface, border-border, text-text-primary, bg-canvas, var(--c-success)).
- Created `app/testing/page.tsx` — "use client" page (needed for the TestingChecklist interactivity). Structured as: Hero (success-tinted kicker pill "BUILD 234ea15 · download-system-plan", title "On-Device Testing", subtitle, 4 quick-info pills for APK/Commit/CI/Date) → Section 0 Prerequisites (6 items) → Section 1 Fix 1 Metadata (8 items, accent success) → Section 2 Fix 2 Episode List (5 items) → Section 3 Fix 3 Subtitles (7 items) → Section 4 Fix 3b Switching (5 items) → Section 5 Regression (6 items) → Section 6 Log Capture Guide (static Card with Method A/B, responsive filtered-commands table desktop + stacked cards mobile, success-tinted tip box with the key log line, danger-tinted "don't capture full logcat" box) → Section 7 Reporting Results (4-item numbered list Card) → Section 8 Concerns & Open Questions (prominent Card with warning left-border, warning-tinted background, 10 concerns each with Risk: Low/Medium/High pill, the 3 open questions marked with a "Q" badge) → centered muted footer note "Testing page · Build 234ea15 · Generated for on-device verification of METADATA-FIX-v2".
- Ran `bun install --frozen-lockfile` → 51 packages installed (no lockfile changes, no new deps added).
- Ran `bun run build` → ✓ Compiled successfully in 5.5s · 14/14 static pages generated · /testing listed as static (○) · out/testing/index.html created with the expected content (verified "On-Device Testing", "Concerns", "Log Capture Guide" all present in the prerendered HTML).
- Skipped `bun run lint` — `next lint` requires an eslint config that isn't present in this project (pre-existing, not introduced by this change). Build-time TypeScript check (strict mode) passed cleanly.

Stage Summary:
- 3 files modified, 2 files created:
  * MODIFIED `lib/data.ts` — appended Testing nav entry
  * MODIFIED `components/Sidebar.tsx` — added `testing` clipboard-check icon
  * CREATED `components/TestingChecklist.tsx` — persistent (localStorage) checklist with collapse + reset
  * CREATED `app/testing/page.tsx` — full testing page (8 sections + hero + footer note)
- BUILD STATUS: ✅ PASS — `bun run build` succeeded with output: `output: "export"`, all 14 routes prerendered as static content, /testing included.
- All 5 TestingChecklist instances persist independently to localStorage keys: ani-kuta:testing:prereqs, :fix1-metadata, :fix2-episode-list, :fix3-subtitles, :fix4-switching, :regression (plus their :collapsed counterparts).
- No new npm dependencies added. No external icon library — all icons inline SVG. No server components with fetch. Static export compatible.
- Once the dashboard is deployed, the testing page will be live at: https://testplay-byte.github.io/ANI-KUTA/testing/
