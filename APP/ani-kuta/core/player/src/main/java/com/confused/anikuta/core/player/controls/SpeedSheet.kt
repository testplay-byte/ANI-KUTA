...
/**
 * Speed picker bottom sheet — presets + custom slider.
 *
 * Ported from the old project's `SpeedSheet.kt`. Shows:
 *  - Current speed (e.g. "1.00x")
 *  - Preset chips: 0.25x, 0.5x, 0.75x, 1.0x, 1.25x, 1.5x, 1.75x, 2.0x
 *  - Custom slider: 0.1x – 5.0x
 *
 * The speed is applied live via [onSpeedSelected] — the host calls
 * `AnikutaMPVView.playbackSpeed = speed` (which uses `setPropertyDouble`).
 */
...
fun SpeedSheet(
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            // Current speed display + presets in a single horizontal scroll Row
            Row(
                modifier = Modifier
                    .fillMaxWidth()
