...
- **SHIPPED RELEASES carry ALL FOUR ABIs (D-423, round 35 — supersedes D-251's arm64-only rule per user instruction).** The tag-driven `release-apk.yml` workflow builds with `-PreleaseAllAbis=true`: one SIGNED split APK per ABI (`arm64-v8a`, `armeabi-v7a`, `x86`, `x86_64`) + a signed UNIVERSAL APK, plus a release ZIP bundling them all. The dev/CI push path (`build-apk.yml`) stays arm64-v8a-only for speed.
  ...
  - Verified post-build in CI: every split APK must contain its OWN `lib/<abi>/` natives, and every shipped APK must pass the `apksigner verify` gate (below).
...
- **RELEASE SIGNING (D-423, round 35 — user instruction): CI builds AND signs every shipped APK.** The release keystore lives in the repository's **GitHub Actions secrets** (`ANIKUTA_KEYSTORE_BASE64` + `ANIKUTA_KEYSTORE_PASSWORD` + `ANIKUTA_KEY_ALIAS` + `ANIKUTA_KEY_PASSWORD`); the workflow decodes it to `keystore.properties` + the keystore file before `assembleRelease`. The **`apksigner verify` gate HARD-FAILS** the release workflow if ANY shipped APK is unsigned or fails verification — an unsigned artifact must NEVER reach a GitHub release (the round-34 lesson: the user downloaded `app-release-unsigned.apk` from a CI artifact and hit `INSTALL_PARSE_FAILED_NO_CERTIFICATES`). Local building/signing of shipped APKs remains FORBIDDEN (below).
...
- **VERSION DISCIPLINE (user instruction, D-425, round 36):** the version NEVER moves without the user's EXPLICIT instruction. `versionName`/`versionCode` in `AndroidConfig.kt` stay exactly where the user says (currently **1.1.1 / 10101**, the first official release line) until the user personally orders the bump. If a monotonic-versionCode concern arises (e.g. someone installed a higher-code test build), REPORT it to the user with options — NEVER resolve it by silently bumping the version (the round-35 v1.1.2 mistake, reverted in round 36). The current line: **v1.1.1 is the first official release** — the version stays 1.1.1 until the user says otherwise.
- **RELEASES (user instruction, D-251; updated D-423, D-425):** versioned releases are published via the `release-apk.yml` workflow on `v*` tag push — the release carries all-ABI SIGNED APKs + the release ZIP (see above) and the release must NOT be marked prerelease (the in-app updater relies on stable releases being visible). `versionCode`/`versionName` follow the version scheme in `AndroidConfig.kt`, the tag `v{versionName}` stays in sync with `AndroidConfig.kt`, and the release workflow verifies the match (bumps happen ONLY on user instruction — the version-discipline rule above). The updater's ABI-aware asset picker (D-423) downloads the APK matching the device's `Build.SUPPORTED_ABIS` (universal as the fallback).
- **DIVISION OF LABOR (user instruction, round 36):** the CODE/BUILD agent (this repo's agent) owns the dev-repo workflows and is responsible for building BOTH the release AND the debug APKs in GitHub Actions — the tag path (`release-apk.yml`) builds ONLY the release version (all ABI variants, signed, zipped: nothing else runs there); the push path (`build-apk.yml`) builds debug + release for verification. The RELEASE agent (the separate "new repository agent") handles creating the releases and managing the GitHub releases + tags on the PUBLISHED repo (re-hosting the CI-built artifacts per the starter prompt).
