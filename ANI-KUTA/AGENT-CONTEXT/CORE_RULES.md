...
- **PUSH PATH = DEBUG ONLY (user instruction, D-435, round 38):** any push to ANY branch (main, feature/**, functionality/**, release/**, streaming/**) builds ONLY the DEBUG APK (`build-apk.yml`: tests → assembleDebug → ABI check → the `anikuta-apk` artifact). A RELEASE APK is NEVER built on a branch push — the round-37 push-path assembleRelease was removed after the user found a release APK in the main-branch artifact next to the debug one. Release APKs exist ONLY via the tag-driven `release-apk.yml` (a `v*` tag on the release branch).
- **SHIPPED RELEASES carry ALL FOUR ABIs (D-423, round 35 — supersedes D-251's arm64-only rule per user instruction).** The tag-driven `release-apk.yml` workflow builds with `-PreleaseAllAbis=true`: one SIGNED split APK per ABI (`arm64-v8a`, `armeabi-v7a`, `x86`, `x86_64`) + a signed UNIVERSAL APK, plus a release ZIP bundling them all. The dev/CI push path (`build-apk.yml`) stays arm64-v8a-only for speed.
  - ABI set: `build-logic/.../AndroidConfig.kt` (`abiFilters` for dev, `releaseAllAbiFilters` for shipped releases), applied via the `anikuta.android.application` convention plugin + the `splits` block in `app/build.gradle.kts`.
  - Verified post-build in CI (both workflows inspect every APK's `lib/` folder and fail on any unexpected `lib/<abi>/`).
- **DEBUG PATH IS arm64-v8a ONLY (user instruction, round 40 — D-445; retires the D-246/D-251 emulator exception):** "from now we will only build the arm64v8 version for the debug version." The x86_64 emulator machinery is REMOVED (the build-apk.yml workflow_dispatch input + the emulator build step + the gradle `emulatorX64Build` flag + `emulatorAbiFilters`). No x86 APK is ever built on the dev/CI path. Shipped releases keep ALL FOUR ABIs (D-423 unchanged).
- **DEBUG LAUNCHER IDENTITY (user instruction, round 40 — D-444):** the debug build's launcher icon is the user-provided mascot artwork (`USER-UPLOADS/IMG_20260918_233212.png`) — resized per density, the artwork NEVER cropped (contain-fit; hairline padding filled with mirrored edge pixels). Adaptive icons (API 26+) carry it as the full-bleed background layer (same structure as the release line); the release build keeps the kawaii-mouth icon set unchanged.
- **DEBUG UPDATE SOURCE (D-440, reaffirmed round 40):** the debug build checks THIS repository (`testplay-byte/ANI-KUTA`) for new versions at runtime (FLAG_DEBUGGABLE switch); release builds check the published repo (`Confused-Creature-180/ANI-KUTA`).
- **RELEASES — THE DEBUG-ONLY LINE (user instruction, round 41 — D-447, supersedes the D-423/D-442 all-ABI-on-this-repo model):** THIS repository's GitHub releases carry ONLY the DEBUG build, ONLY arm64-v8a — built by the tag-driven release-apk.yml from the committed debug keystore (no release secrets), published as `ani-kuta-v{version}-debug-arm64-v8a.apk` + `SHA256SUMS.txt` (the `-arm64-v8a.` substring keeps the D-423 ABI-aware updater matching). The standalone debug app (app id `com.confused.anikuta.debug`, co-installable with the release app) updates itself in-app from exactly these releases (D-440). The RELEASE-signed, all-ABI build line belongs ONLY to the published repo (Confused-Creature-180/ANI-KUTA) and happens ONLY after everything is completed. The workflow hard-fails on any non-arm64 ABI or an unsigned APK. (The v1.1.3 all-ABI publication was deleted per this policy; the git tag remains as history.)
- **NO R8/MINIFICATION on the release line (user instruction, D-436, round 38):** the release buildType is `isMinifyEnabled=false` + `isShrinkResources=false` — no obfuscation, no code shrinking, no resource shrinking, NO proguard files. The v1.1.1 device round proved the obfuscated release builds broke the extension system (plugins loaded via DexClassLoader resolve host classes by their compiled names; keep-rule coverage cannot be guaranteed complete for a plugin ecosystem — R8's optimization passes alter runtime-resolved behavior beyond renaming). `app/proguard-rules.pro` is DELETED; if minification is ever deliberately re-enabled, the D-413/D-436 records in decisions.md must be honored. There is no mapping.txt anymore.
- **VERSION DISCIPLINE (user instruction, D-425, round 36):** the version NEVER moves without the user's EXPLICIT instruction. `versionName`/`versionCode` in `AndroidConfig.kt` stay exactly where the user says until the user personally orders the bump. If a monotonic-versionCode concern arises (e.g. someone installed a higher-code test build), REPORT it to the user with options — NEVER resolve it by silently bumping the version (the round-35 v1.1.2 mistake, reverted in round 36).
- **Sandbox stays LEAN (user instruction, round 39 — supersedes the D-246 emulator allowance):** the local Android SDK, JDK, and Gradle caches were DELETED (≈5GB freed) and must NEVER be reinstalled. No heavy software, no local processing of any kind — everything heavy (builds, verification, APK inspection beyond checksums) happens in GitHub Actions. This is the environment-optimization standing rule; the CI-first loop (D-281 above) is the ONLY verification loop. The release-artifact verification relies on the CI apksigner gate (inside the workflow logs) + local `sha256sum` checksum comparison (coreutils — allowed; it is not Android tooling).
- **RELEASE BRANCHES ARE CUT FROM MAIN (user instruction, round 39 — D-442):** the release flow is: the fixes land on main first (main carries EVERYTHING the release needs — the build line, the workflows, the features), then `release/<version>` is cut from the round's main head with ONE commit on top (the version bump + the branch-point docs), then the `v<version>` tag on that branch triggers release-apk.yml. The release is then re-hosted to the published repo (Confused-Creature-180/ANI-KUTA) with the EXACT version-prefixed asset names (the ABI-aware updater matches `-{abi}.` in the asset name — unprefixed names break the device's APK pick).
- **The update repo follows the BUILD TYPE (D-440, round 39):** debug builds check the DEV repo (testplay-byte/ANI-KUTA); release builds check the PUBLISHED repo (Confused-Creature-180/ANI-KUTA) — resolved at runtime from FLAG_DEBUGGABLE, so it is correct on every branch and build path. Supersedes D-411 (the release-line hardcode) and the D-430 note that kept the dev target on main.
