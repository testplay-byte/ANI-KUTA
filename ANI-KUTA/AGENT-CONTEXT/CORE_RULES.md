...
- **RELEASES (user instruction, D-251; updated D-423):** versioned releases are published via the `release-apk.yml` workflow on `v*` tag push — the release carries all-ABI SIGNED APKs + the release ZIP (see above) and the release must NOT be marked prerelease (the in-app updater relies on stable releases being visible). Bump `versionCode`/`versionName` per the version scheme in `AndroidConfig.kt`, keep the tag `v{versionName}` in sync with `AndroidConfig.kt`, and the release workflow verifies the match. The updater's ABI-aware asset picker (D-423) downloads the APK matching the device's `Build.SUPPORTED_ABIS` (universal as the fallback).
