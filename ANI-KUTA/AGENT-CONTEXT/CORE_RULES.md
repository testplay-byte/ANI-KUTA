...
- **NO ROUTINE CI UNIT TESTS (user instruction, round 44 — D-465):** the CloudStream-module unit-test steps do NOT run on routine CI builds (neither the branch push path NOR the tag release path) — they take unnecessary time. Run them ONLY when the user explicitly asks, or when the changed code specifically warrants a test pass (say so before adding one back).
...
- **RELEASES (user instruction, D-251; all-ABI D-423; debug-only push D-435):** versioned releases are published via the `release-apk.yml` workflow on `v*` tag push — every ABI variant (4 splits + universal) + the release ZIP, all SIGNED in CI from the repository secrets, and the release must NOT be marked prerelease (the in-app updater relies on stable releases being visible). Bump `versionCode`/`versionName` ONLY by the user's explicit instruction (D-425 version discipline — see below), keep the tag `v{versionName}` in sync with `AndroidConfig.kt`, and the release workflow verifies the match.
