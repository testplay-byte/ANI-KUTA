...
GitHub: `testplay-byte/ANI-KUTA`. Repo root contains a single wrapper folder `ANI-KUTA/` (per CORE_RULES §4). Active line: `main` (v0.2.63). Active WORK branch: **`streaming/CLOUDSTREAM-V2`** — Task 54 / round 14: THE UI-PARITY RELEASE (v0.4.2/67): CS playback WORKS (round-12/13: loadLinks resolution + Media3 engine + generation-lock fixes, device-verified) and now LOOKS like the aniyomi stack — the resolve sheet renders in the aniyomi ResolverSheet's design (server accordion + quality chips + RobotoFamily), the CS watch screen is a real two-mode watch PAGE (pill bar + 16:9 player + currently-playing description + episode rows + fullscreen controls w/ lock + canvas seekbar + speed sheet), and the player sheets use the aniyomi sheet languages (Qualities-and-Servers / Subtitles TrackRows / Speed). Per-episode metadata (title/thumb/date/desc/sub-dub) rides the SAME serialized wire format as the aniyomi WatchKey field — ONE DetailsScreen builder feeds both stacks. Aniyomi stack byte-untouched (parity via REPLICATED design tokens, zero code imports). NEXT: user device round 15 on v0.4.2 (CS episode tap → aniyomi-looking resolve sheet → the watch PAGE w/ description + episodes; fullscreen mode; sheets; aniyomi regression sweep again). Doc zone: DOCUMENTATION/cloudstream-v2/ (02 plan + 03 as-built + 04 fixes + 05 UI-parity plan). Debugging: the ONE logcat filter in doc 03 §3.
