...
GitHub: `testplay-byte/ANI-KUTA`. Repo root contains a single wrapper folder `ANI-KUTA/` (per CORE_RULES §4). Active line: `main` (v0.2.63, the trunk). Active WORK branch: **`streaming/CLOUDSTREAM`** — Task 40 (research) + **Task 41 (implementation phase 1: the CLEAN-ROOM compat module `:core:cloudstream-api`, the `:data:cloudstream` runtime, and the unified Aniyomi|CloudStream Extensions UI) are COMPLETE, CI GREEN (cb00e65)**. Gates answered (doc 23 §1): NO GPL — everything is clean-room. NEXT: user device feedback on the management flows, then provider-EXECUTION sessions (browse/search/load/loadLinks + real extractors) per doc 23 §7.
