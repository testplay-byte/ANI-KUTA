...
GitHub: `testplay-byte/ANI-KUTA`. Repo root contains a single wrapper folder `ANI-KUTA/` (per CORE_RULES §4). Active line: `main` (v0.2.63, the trunk). Active WORK branch: **`streaming/CLOUDSTREAM`** — Task 40 (research) + Task 41 (phase 1: clean-room compat module + runtime + unified UI, CI GREEN cb00e65) + **Task 42 (device-feedback round 1: loader IDEMPOTENT — the "Plugin already loaded"→Failed-to-load bug is FIXED; CS tab rebuilt with full aniyomi UI parity via shared `ExtensionListChrome.kt`; install animation completes visibly; repo delete KEEPS plugins; D-332)** — all pushed, local verify green (see lesson 12 for the local pre-push compile protocol + pending user ratification of the §8 deviation). Gates answered (doc 23 §1): NO GPL — everything is clean-room. NEXT: user device round 2 on this build, then provider-EXECUTION sessions (browse/search/load/loadLinks + real extractors) per doc 23 §7.
