...
> Refreshed in **Round 84 (2026-09-24)**. If `memory/progress.md` shows a NEWER round than this file's stamps, trust progress.md — and refresh this file at the phase boundary (the Round-77 lesson).
...
## 📍 Current State (refreshed Round 84, 2026-09-24)
...
- **Versions:** the mainline carries **1.1.20 / 10120** (D-430: version bumps ride the RELEASE branch only, never the mainline). Latest **debug** release: **v1.1.41 / 10141** (round 84 — D-580..D-583: the five-page extension testing system with the hard-isolation hang fix + Stop/Skip, the app-scoped run controller, the delete exit choreography on both extension systems, the true header pills, the button-feel filter menus + focus-ring search field, the taller/3D/user-driven link-source sheet). Latest **professional** release: **v1.1.3 / 10103** (`professional-v1.1.3`, 5 release-signed ABIs + universal — device-verified).
- **Latest records:** Round 84 implemented (D-580..D-583 — the five-page testing system, the hard-isolation hang fix, the delete exit choreography, the true pills, the filter menu cards + focus-ring search, the sheet's third pass); `DOCUMENTATION/cloudstream-v2/` numbering is sequential — the next record is **67**.
- **THE CURRENT PHASE (D-565) — DEBUG-FIRST:** the user directed that from now on ALL new features/QoL work lands on the mainline and ships via per-round **DEBUG** releases (the next one is v1.1.42/10142 — the 1.1.3N debug series continues). **Professional releases PAUSE** until the user explicitly orders the next one (D-425 version discipline unchanged — no bumps without the user's order).
