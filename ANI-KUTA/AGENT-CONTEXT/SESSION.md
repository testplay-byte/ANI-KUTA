...
> Refreshed in **Round 87 (2026-09-24)**. If `memory/progress.md` shows a NEWER round than this file's stamps, trust progress.md — and refresh this file at the phase boundary (the Round-77 lesson).
...
## 📍 Current State (refreshed Round 87, 2026-09-24)
...
- **Versions:** the mainline carries **1.1.20 / 10120** (D-430: version bumps ride the RELEASE branch only, never the mainline). Latest **debug** release: **v1.1.44 / 10144** (round 87 — D-593..D-613: the un-killable chain, the TestingPalette honest colors, the combined suite-health ring, the blocks that carry their own results, the 30-second stream cap, the two-group Link Sources sheet) — **LIVE**: `release/1.1.44` cut from the green head c3ce005 (the docs-ledger commit, above the round-87 implementation 31ce1887 whose Build APK run 36065055559 went GREEN first-try), Release APK run 36066139872 GREEN FIRST-TRY, the stable latest release carries the arm64-v8a debug APK 68.3 MB + SHA256SUMS.txt and the full 9-bullet tag body (D-466). The NEXT debug release is **v1.1.45 / 10145**. Latest **professional** release: **v1.1.3 / 10103** (`professional-v1.1.3`, 5 release-signed ABIs + universal — device-verified).
- **Latest records:** Round 87 implemented AND SHIPPED as v1.1.44 (D-593..D-613 — doc **69**; the round-86 decisions D-587..D-592 were backfilled into decisions.md during the round-87 ledger pass); `DOCUMENTATION/cloudstream-v2/` numbering is sequential — the next record is **70**.
- **THE CURRENT PHASE (D-565) — DEBUG-FIRST:** the user directed that from now on ALL new features/QoL work lands on the mainline and ships via per-round **DEBUG** releases (the latest: **v1.1.44/10144** — the 1.1.3N debug series continues; the next is v1.1.45/10145). **Professional releases PAUSE** until the user explicitly orders the next one (D-425 version discipline unchanged — no bumps without the user's order).
...
## 🚧 Open Items / Blocked (refreshed Round 87)
- **Round 87 SHIPPED v1.1.44:** the 22-point v1.1.43 device round implemented in ONE commit (31ce1887) — CI **GREEN FIRST-TRY** (Build APK run 36065055559), preceded by TWO sub-agent audits (the compile review found 2 missing `isActive` imports — fixed pre-push; the requirements audit verified 22/22 with file:line evidence). Docs: doc 69 + D-593..D-613 prepended + the round-86 decisions BACKFILLED into decisions.md + 4 lessons. The release: `release/1.1.44` + the 10144 bump (11e80db9) + the annotated tag v1.1.44 (full 9-bullet body) + Release APK run 36066139872 GREEN FIRST-TRY → **v1.1.44 is LIVE** (stable latest, arm64-v8a debug APK 68.3 MB + SHA256SUMS.txt). The dev PAT is stored repo-external at `/home/z/.secrets/github-credentials` (0600, account `testplay-byte` — re-provided by the user after the round-87 sandbox reset; the `official-repo-token` remains LOST and is needed only for official-repo work). The user's device round on v1.1.44 is the next input; the NEXT debug release is **v1.1.45/10145**. The `release/1.1.38`…`release/1.1.44` branches stay on the remote (deletion is user-gated).
