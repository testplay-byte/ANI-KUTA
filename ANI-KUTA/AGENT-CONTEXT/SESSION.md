...
GitHub: `testplay-byte/ANI-KUTA`. Repo root contains a single wrapper folder `ANI-KUTA/` (per CORE_RULES §4). Active line: `main` (v0.2.63, the trunk). Active WORK branch: **`streaming/CLOUDSTREAM`** — Task 40 (research) + Task 41 (phase 1: clean-room compat + runtime + unified UI) + Task 42 (round 1: idempotent loader, shared chrome) + Task 43 (round 2 + EXECUTION PHASE 1: trust flow, parallel installs, plugin detail page, search-page browse/search/load + CS content details screen, v0.2.64) + **Task 44 (round 3 / D-336..D-339: activity-context plugin-loading contract — MainActivity is now an AppCompatActivity and the loader passes the LIVE activity to Plugin.load() [ClassCastException fix]; CloudflareKiller REAL — challenge detect + headless WebView solve + per-host serialization + cookie cache + honest CloudflareBlockedException [0-results fix]; SECTIONED browse — every provider shelf as a titled horizontal row; detail-page button spec [available → bottom full-width Install, untrusted → Trust+Uninstall side-by-side, trusted unchanged]; retry spinners; v0.2.65)** — all pushed, CI GREEN (d2748114), v0.2.65 tagged + released. NEXT: user device round 4 on v0.2.65 (trust MovieBoxProvider → providers live; AniKoto browse → sectioned rows with the CF bypass; search; details) → then the PLAYBACK session (loadLinks + the 16 real extractors + WatchKey integration) per doc 23 §7.
