...
GitHub: `testplay-byte/ANI-KUTA`. Repo root contains a single wrapper folder `ANI-KUTA/` (per CORE_RULES §4). Active line: `main` (v0.2.63, the trunk). Active WORK branch: **`streaming/CLOUDSTREAM`** — Tasks 40-47 (research + clean-room compat + runtime + unified UI + six device-feedback rounds + Task 47's playback session: trust flow, provider EXECUTION, the source BRIDGE, the 8KB-truncation + search-memory root causes, the 35-extractor runtime, the P.A.C.K.E.R. unpacker + M3u8Helper) + **Task 48 (round 7 / D-351..D-356: THE loadLinks ROOT CAUSE — the shim's generic newEpisode JSON-quoted every String episode handle while upstream special-cases String ("just in case java is wack") — the %22 in MovieBox's subjectId was the smoking gun; fixUrl aligned to upstream verbatim; the bridge heals v0.2.68's quoted cached episodes; the INSTANT search-page browse cache (memory + disk snapshots, stale-while-revalidate, refresh failures never blank a shown page); the playback 403 RECOVERY LADDER (same-URL retry → pinned-link re-resolve → next mirror, position preserved, deferred switch-error surfacing at 8s); per-track subtitle headers END-TO-END (CS SubtitleFile.headers → Track → Resolver → WatchKey 3-field wire format → SubtitleEngine per-request headers); CS DOWNLOADS (bit-62 sources mint a rotating-link ResolveContext — expired extractor links self-heal via the existing ReResolver; mainId-race toast); playback HAPTICS (PlayerHaptic vocabulary, player_haptic_feedback pref) + the year in the details header; v0.2.69)** — pushed, CI pending. NEXT: user device round 8 on v0.2.69 — THE PLAYBACK VERIFICATION (CS episode → resolver sheet → MPV plays; 403 self-recovers; downloads produce playable files; search opens INSTANTLY) → then extractor hardening driven by device data + HLS variant-quality selection (doc 19 open questions).
