...
GitHub: `testplay-byte/ANI-KUTA`. Repo root contains a single wrapper folder `ANI-KUTA/` (per CORE_RULES §4). Active line: `main` (v0.2.63, the trunk). Active WORK branch: **`streaming/CLOUDSTREAM`** — Task 40 (research) + Task 41 (phase 1: clean-room compat + runtime + unified UI) + Task 42 (round 1: idempotent loader, shared chrome) + **Task 43 (round 2 + EXECUTION PHASE 1: trust flow [Untrusted section, trust gates code execution], parallel installs, plugin DETAIL page [every CS row clickable — metadata + live providers], left tabs + title-line repo badge, v0.2.64 + version-bump discipline, and provider EXECUTION on the SEARCH page — `CloudstreamContentRepository` [browse/search/load, `Anikuta:Data:Cloudstream:Exec` timed+prefixed logs], source-picker CS section, string sourceKey identity [aniyomi path byte-identical], NEW `:feature:cloudstream-content` details screen [hero/tags/description/season-grouped episodes/movie entry; "playback next update" note]; rule-8 ruling recorded: CI = primary verification, local only when cheap)** — all pushed, CI GREEN (7ee1cf21), v0.2.64 tagged + released. NEXT: user device round 3 on v0.2.64 (browse/search/details E2E checklist in the Task 43 session summary) → then the PLAYBACK session (loadLinks + the 16 real extractors + WatchKey integration) per doc 23 §7.
