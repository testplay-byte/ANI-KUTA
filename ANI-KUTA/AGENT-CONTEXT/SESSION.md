...
GitHub: `testplay-byte/ANI-KUTA`. Repo root contains a single wrapper folder `ANI-KUTA/` (per CORE_RULES §4). Active line: `main` (v0.2.63). Active WORK branch: **`streaming/CLOUDSTREAM-V2`** — Task 51 / round 11: THE CLOUDSTREAM V2 REBUILD from main (v0.3.0/64): the full CS plugin system + repos + trust UI + SEQUENTIAL multi-download, search + categories + memory + caching, details page, episodes/seasons — NO playback this phase (the bridge throws an honest 'playback arrives with the playback port' boundary), aniyomi untouched save 7 additive seams (DOCUMENTATION/cloudstream-v2/ is the doc zone; every phase CI-gated green). The PREVIOUS line `streaming/CLOUDSTREAM` (Tasks 40-50, v0.2.64-72) is SCRAPPED per the user's round-11 directive but kept read-only as REFERENCE. NEXT: user device round 12 on v0.3.0 (repo add → sequential installs → trust → search categories + memory + cache → details/episodes/seasons → honest playback boundary → aniyomi regression sweep → console export on any issue); then the playback port as its own phase.
