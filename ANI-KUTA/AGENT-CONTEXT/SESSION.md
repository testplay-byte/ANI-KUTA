...
> Refreshed in **Round 88 (2026-09-24)**. If `memory/progress.md` shows a NEWER round than this file's stamps, trust progress.md — and refresh this file at the phase boundary (the Round-77 lesson).
...
## 📍 Current State (refreshed Round 88, 2026-09-24)
...
- **Versions:** the mainline carries **1.1.20 / 10120** (D-430: version bumps ride the RELEASE branch only, never the mainline). Latest **debug** release: **v1.1.45 / 10145** (round 88 — D-614..D-624: the sheet's two true columns, the full-details page that finally behaves, the preview that asks before it plays) — **LIVE**: `release/1.1.45` cut from the green head 89d469e (the docs-ledger commit, above the round-88 implementation 05a79f80 whose Build APK run 36073179021 went GREEN first-try), Release APK run 36073541658 GREEN FIRST-TRY, the stable latest release carries the arm64-v8a debug APK 68.4 MB + SHA256SUMS.txt and the full 9-bullet tag body (D-466). Round 87 (v1.1.44/10144, D-593..D-613 — the un-killable chain, the TestingPalette honest colors, the combined ring) shipped GREEN first-try as well. The NEXT debug release is **v1.1.46 / 10146**. Latest **professional** release: **v1.1.3 / 10103** (`professional-v1.1.3`, 5 release-signed ABIs + universal — device-verified).
- **Latest records:** Round 88 implemented AND SHIPPED as v1.1.45 (D-614..D-624 — doc **70**; round 87 is doc **69**, D-593..D-613; the THREE-pass verification ledger the user ordered is in doc 70 §8); `DOCUMENTATION/cloudstream-v2/` numbering is sequential — the next record is **71**.
- **THE CURRENT PHASE (D-565) — DEBUG-FIRST:** the user directed that from now on ALL new features/QoL work lands on the mainline and ships via per-round **DEBUG** releases (the latest: **v1.1.45/10145** — the 1.1.3N debug series continues; the next is v1.1.46/10146). **Professional releases PAUSE** until the user explicitly orders the next one (D-425 version discipline unchanged — no bumps without the user's order).
...
## 🚧 Open Items / Blocked (refreshed Round 88)
- **Round 88 SHIPPED v1.1.45:** the v1.1.44 device round implemented in ONE commit (05a79f80, 4 files +892/−558) — CI **GREEN FIRST-TRY** (Build APK run 36073179021), preceded by the THREE sub-agent passes the user ordered (compile review 0 errors; requirements audit 21/21; the adversarial re-verification that caught the baseline-tertiary regression D-624 + one cap-reserve nuance — both fixed). Docs: doc 70 + D-614..D-624 prepended + 3 lessons. The release: `release/1.1.45` + the 10145 bump (8beee876) + the annotated tag v1.1.45 (full 9-bullet body) + Release APK run 36073541658 GREEN FIRST-TRY → **v1.1.45 is LIVE** (stable latest, arm64-v8a debug APK 68.4 MB + SHA256SUMS.txt). Prior round 87 shipped v1.1.44/10144 the same way (run 36066139872; the 22-point D-593..D-613 round). The dev PAT is stored repo-external at `/home/z/.secrets/github-credentials` (0600, account `testplay-byte` — re-provided by the user in round 87; the `official-repo-token` remains LOST and is needed only for official-repo work). The user's device round on v1.1.45 is the next input; the NEXT debug release is **v1.1.46/10146**. The `release/1.1.38`…`release/1.1.45` branches stay on the remote (deletion is user-gated).
