...
- **D-444:** the debug launcher = the user's mascot artwork (USER-UPLOADS/IMG_20260918_223706.png) — contain-fit (NEVER cropped, no distortion, mirrored-edge padding), density rasters (ic_launcher + ic_launcher_round, mdpi→xxxhdpi) + the adaptive bg layer (xxxhdpi/ic_launcher_bg) + the App Icon hero (icon_current.png); debug adaptive XMLs mirror the release structure (full-bleed bg + empty fg); the pre-26 lime vectors + the unused debug colors.xml removed. Release icon untouched.
