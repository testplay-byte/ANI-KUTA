...
## Phase CS-V2 Round 33 — Task 73: the v1.1.1 publishable release track
- **Merge (verified):** streaming/CLOUDSTREAM-V2 → main (`--no-ff`, fe15f95b; CI GREEN 33913230117); release/1.1.1 cut from merged main. The pre-merge workflow housekeeping honored the standing test-feature/** note and added release/**.
- **Debug bubble removed completely (D-409):** :feature:debug-bubble + :core:debug-api + the app debug/release source sets deleted (~24 files); every call site unwired (Koin, OkHttp/SqlDriver wrappers, MainActivity CompositionLocals, 4 screens' writer blocks, the Developer-tools section). The removal incidentally fixes the latent assembleRelease break (release DebugInit's debug-only import). Show-sources, the copy button, and the update-check history are KEPT per the user's explicit list.
- **Empty episode-check runs are silent (D-410):** the results notification early-returns when nothing was due (totalChecked == 0) — no more "nothing was due for a check this run" at every app open. Real checks still notify; per-episode notifications untouched.
- **The published repo (D-411):** the in-app updater + the release link point at Confused-Creature-180/ANI-KUTA (APK-only); version 1.1.1 / versionCode 10101 (install-over-monotonic over 0.4.20/85).
- **Release logging OFF (D-412):** Logger fully gated on BuildConfig.DEBUG; the plugin-facing com.lagradost.api.Log facade gated (E kept). Crash-handler + episode-dumper raw paths intentionally retained.
- **R8 full mode + resource shrinking (D-413):** release isMinifyEnabled + shrinkResources; the proguard keep surface covers the plugin-compat classpath (DexClassLoader parent-first by-name resolution), JNI surfaces, workers, serialization companions; SourceFile/LineNumberTable kept with mapping.txt uploaded per build. CI runs assembleRelease on every push + ABI-checks the release APK. Release signing via gitignored keystore.properties (absent ⇒ unsigned-but-building = the CI path; the final signed build is the tag-driven phase). DexGuard honestly declined (commercial-only).
- **Dependency cleanup (D-414):** logcat (×14 modules), stray rxjava (×3), seeker, androidx-media, truetype-parser removed (all verified zero-import); the 5 hardcoded deps moved into the version catalog (fragment-ktx added; the appcompat pin mismatch fixed).
- The final signing phase is deliberately ASK-FIRST (per instruction): the keystore is generated after the user answers; the delivery is a password-protected zip (APK + keystore + signing info), password in chat only.

## Phase CS-V2 Round 34 — Task 74: the v1.1.1 release round (share robustness, co-install, app icons, signing)
- **The plugin share bulletproofed (D-415):** the round's device report — "shows the loading on the share button but it does not load anything… does not show any error message" — fixed at every level: a hard 4s bound around the icon fetch (the per-read 2.5s timeouts could be stretched indefinitely by a trickle server — readBytes() bounds EACH read, not the total: the infinite-spinner class), a hard 15s bound around the whole export, a pre-flight source-file check with its own actionable message, the ContextWrapper unwrap to the Activity for the chooser launch (with NEW_TASK only when no activity is reachable), the CancellationException rethrow (leaving the screen mid-share is never a false error), TimeoutCancellationException with its own message, per-cause user-facing toasts, and a >1h stale-export sweep.
- **The co-installable debug line (D-416):** debug buildType gains applicationIdSuffix ".debug" + versionNameSuffix "-debug" + (via the app/src/debug/res overlay) the OLD lime launcher icon + the "ANI-KUTA Debug" label — release v1.1.1 and debug install SIDE BY SIDE (the user's exact request). All manifest placeholder authorities + context.packageName reads follow the suffix automatically; the committed debug keystore keeps debug builds cross-installable.
- **The launcher-icon system (D-417):** the release launcher icon is the user's kawaii-mouth image (1788604676536.jpg — adaptive: full-bleed 432px background WebPs + transparent foreground; legacy 48..192px WebPs at all 5 densities; the old main mipmap-anydpi vectors deleted). 8 activity-aliases (.icons.IconV1..V8) carry MAIN/LAUNCHER (moved off MainActivity); V1 = Original, V2..V8 = sakura/midnight/mint/sunset/mono/aqua/void (AI-generated in the same style, VLM-verified). ~465KB of new resources.
- **The App Icon page (D-418):** Appearance's very bottom, directly below the Details page row (the user's exact placement) → the current-icon hero, the 8-variant home-screen grid (tap = alias switch via setComponentEnabledSetting — enable-new-before-disable-old, DONT_KILL_APP; component classes are NAMESPACE-based, suffix-safe, verified against the built APK), the GitHub catalog section (live fetch of Confused-Creature-180/ANI-KUTA's icons/ folder, icon-NN filename matching, offline JSON cache, extras apply in-app with an honest "until the next release" note), and the custom-image import (SAF picker → center-crop → 512px → filesDir/app-icons). AppIconPreferences + Koin registration + AppIconKey navigation.
- **Debug descriptions minimized (D-419):** Show sources → "Show stream URLs"; Copy button → "Add a copy button to stream URL"; Update Check History → "Check the history" — the user's exact strings, nothing more.
- **The signing + delivery (D-420):** a brand-new JKS release keystore (RSA-2048, 9131-day ≈ 25-year validity, alias anikuta, CN=ANI-KUTA/O=Confused-Creature-180/C=US, distinct store/key passwords — PKCS12 was rejected precisely because it can't carry distinct passwords), generated + used LOCALLY: the Android SDK was installed in the sandbox (JDK 21 + cmdline-tools + platform-36 + build-tools) proving the code compiles + assembles locally (assembleDebug GREEN), but R8 exceeds the sandbox's 10-minute process window — so the SHIPPED APK is compiled by CI (the standing "CI is the compiler of record" rule) and signed locally with apksigner (the keystore + passwords never touch GitHub, not even as Actions secrets). The delivery: ONE AES-256 zip (signed release APK + co-installable debug APK + keystore + keystore.properties + SIGNING-DETAILS.txt + icons/ starter set + README-FIRST), uploaded as a temporary non-v* release asset (the token is pull-only on the new repo — verified) with the password in chat only. NO v* tag pushed (release-apk.yml would auto-publish a debug-signed APK — the v1.1.1 release is the user's own creation on the published repo, per the guide).

## Phase CS-V2 Round 35 — Task 75: the v1.1.2 CI-signed all-ABI release round (the unsigned-APK fix + the icon crop fix + the unified icon grid + the release-agent prompt)
- **The round-34 report's root cause CONFIRMED**: the user's downloaded APK was the push-path CI artifact `app-release-unsigned.apk` — the signing config existed (D-413) but no keystore in CI; the v* release APKs were debug-signed assembleDebug builds. `INSTALL_PARSE_FAILED_NO_CERTIFICATES` was the system telling the truth.
- **D-423 — CI builds AND signs EVERYTHING** (user-authorized secrets): four GitHub Actions secrets set on the dev repo (sealed-box API upload — keystore base64 + store password + alias + key password); `release-apk.yml` rewritten (tag → decode keystore → `assembleRelease -PreleaseAllAbis=true` → five APKs: arm64-v8a/armeabi-v7a/x86/x86_64 + universal → per-APK lib/ verification → the HARD `apksigner verify` gate (any unsigned APK fails the workflow before publishing) → `ANI-KUTA-v{V}-RELEASE.zip` + SHA256SUMS.txt + mapping → the stable release with the which-APK table); `build-apk.yml` also signs its push-path release APK (fork PRs no-op) — an unsigned artifact can never again be downloaded.
- **The ABI policy reversed** (CORE_RULES §8, D-425→D-251 superseded): shipped releases carry ALL FOUR ABIs via the app module's new `splits` block + the widened `ndk.abiFilters` (`-PreleaseAllAbis=true`, release-apk.yml only); the dev/push path stays arm64-only.
- **The in-app updater is ABI-AWARE**: GitHubUpdateSource walks `Build.SUPPORTED_ABIS` → `-{abi}.` asset match → `-universal.` fallback → first-APK legacy fallback (old releases keep working).
- **D-421 — the adaptive icon crop fix**: round-34's full-bleed bg layers lost their outer third to the launcher mask on EVERY icon (subject 60-90% of the source); regenerated with the subject scaled into the 66dp safe circle (adaptive-threshold bbox) + an EDGE-CLAMPED gradient-continuing surround + feathered paste; grid previews pre-masked circles — the picker now shows exactly what the launcher shows; all 8 VLM-verified complete + seamless.
- **D-422 — the App Icon page simplified to ONE grid**: the custom-image import REMOVED completely (user instruction — only provided options); the GitHub icons/ catalog merged into the home-screen grid (IconGridEntry Baked|Catalog); no "More icons" section; hero + cells circle-clipped; the honest unbaked-catalog note kept (limitation accepted).
