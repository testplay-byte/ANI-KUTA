...
## Round 39 (Task 79) — the crash-fix + the first release cut FROM main
- **D-439:** the library-open release crash (`NoWhenBranchMatchedException`, DetailsScreen.kt:367) fixed at ALL 7 sealed dispatches: total-when hardening (DetailsScreen ×4 + MainActivity ×3) + `describeDetailsKeyAnomaly` (the runtime-class/classloader diagnostic) — an unknown runtime variant now degrades to a logged no-op/null instead of killing the process.
- **D-440:** the update repo follows the BUILD TYPE (debuggable-flag runtime check: debug → testplay-byte/ANI-KUTA, release → Confused-Creature-180/ANI-KUTA; supersedes D-411) + the ABI-aware asset picker ported to main (D-423) so release-line update offers pick the device's APK; the Koin resolution moved inside the definition lambda (the Module receiver has no `get()` — the eager module-level call would not have compiled).
- **D-441:** notifications OFF by default — the master kill switch's default flipped; fresh installs post nothing until the user opts in.
- **D-442:** `release/1.1.2` cut FROM the round-39 main head at 1.1.2/10102 (the first release branch cut directly from main; the version the guide anticipated) → tag v1.1.2 → the all-ABI signed release → re-hosted to the published repo (Confused-Creature-180/ANI-KUTA) with the exact version-prefixed asset names.
- **Environment (user instruction):** the sandbox's Android SDK + JDK + Gradle caches DELETED (≈5GB freed) — GitHub Actions is the ONLY build AND verification path; no local Android tooling is ever reinstalled.
- Docs: 30-RELEASE-ROUND-39-PLAN.md; CORE_RULES §8 (the D-246 sandbox-emulator allowance superseded + the lean-environment rule + the release-from-main flow); BUILD-AND-BRANCH-GUIDE.md revised for the round-39 model.

## Round 40 (Task 80) — the smart-link floating return pill
- **D-443:** the ad system's redirection UX upgraded — when the user taps Continue and is redirected to the sponsor, a FLOATING RETURN PILL now floats over the browser: a theme-colored, animated capsule (countdown ring + "Stay a moment…" label + a "Go back" chip) that counts down `minTimeOutsideMs` and, on completion, crossfades to "You can go back now" + a gentle pulse; tapping it (any time — early returns still hit the existing Try-again gate) brings ANI-KUTA back to the foreground, where the existing return-gate completes the ad and opens the details page.
- Implementation: `:core:ads` gains `ReturnPillView` (classic Views — the TYPE_APPLICATION_OVERLAY window has no lifecycle owner, so ComposeView is not an honest fit; a custom `CountdownRingView` draws the 60fps arc + remaining seconds) + `SmartLinkReturnPillController` (WindowManager add/hide, replace-on-reshow for the Try-again loop, idempotent, orphan safety timeout at countdown+10min, SecurityException/BadTokenException caught → the ad flow never breaks over the pill). The interstitial drives it from the coordinator state it already observes (AdInProgress → show with colors captured from the live MaterialTheme palette; any in-app state → hide). `bringAppToForeground` uses the package launch intent — `:core:ads` stays free of :app/navigation knowledge.
- Manifest: SYSTEM_ALERT_WINDOW added; runtime-gated by `Settings.canDrawOverlays()` — no consent → silent no-pill fallback, the ad flow is NEVER blocked on the permission.
- Docs: ads/README.md (the return-pill section); decisions.md D-443.
- **D-444:** the debug launcher = the user's mascot artwork (USER-UPLOADS/IMG_20260918_233212.png) — contain-fit (NEVER cropped, no distortion, mirrored-edge padding), density rasters (ic_launcher + ic_launcher_round, mdpi→xxxhdpi) + the adaptive bg layer (xxxhdpi/ic_launcher_bg) + the App Icon hero (icon_current.png); debug adaptive XMLs mirror the release structure (full-bleed bg + empty fg); the pre-26 lime vectors + the unused debug colors.xml removed. Release icon untouched.
- **D-445:** debug builds are arm64-v8a ONLY — the x86_64 emulator exception retired end-to-end (workflow input + build step + ABI check + the gradle emulatorX64Build flag + emulatorAbiFilters). Debug update source re-affirmed: debug checks testplay-byte/ANI-KUTA (D-440, no change needed).

## Round 41 (Task 81) — the v1.1.3 device round: the release-policy fix + the pill redesign + the wizard overlay ask
- **D-447:** the release policy CORRECTED — this repo's releases = DEBUG arm64-v8a ONLY (the standalone debug app's in-app update line); the all-ABI release-signed line moved off this repo (it belongs to the published repo, after completion). release-apk.yml rewritten (no keystore secrets, assembleDebug + ABI hard-gate + apksigner gate, the debug asset name); the v1.1.3 all-ABI publication deleted.
- **D-448:** the floating return pill redesigned per the device round: bottom-center (48dp above the edge), compact counting state (NO Go back, NOT clickable), and on timer completion the pill GROWS (LayoutTransition width animation + chip entrance + 1.06 scale + pulse) and only then becomes tappable.
- **D-449:** the setup wizard gained the "Draw over other apps" step (fifth skippable permission step) — one tap opens the system's per-app overlay toggle; the ON_RESUME re-verify marks it granted on return; the finish summary gained the "Draw over apps" card; OnboardingPermissionStep gained an optional WHY-line description param; the step-machine test updated to the new order.
...
## Round 48 (Task 88) — the v1.1.10 device round: the poster pipeline fixes + the episode-type layout + the badge centering actually wired
- **D-486:** the poster pipeline hardening — the composer's whole body runs on Dispatchers.IO (the blocking SQLDelight reads left the main-thread produceState/tester path that made the preview fail + the test notifications post as plain text on the user's device), the art loads are time-bounded (12s withTimeoutOrNull per image — offline-first via Coil's memory→disk→network order over the 500MB disk cache; the fallback chain still yields a banner after a timeout), CancellationException rethrown, failure logs carry the exception class.
- **D-487:** the preview's "No episodes available yet" state made reachable (the hasContent-first gate — the old failed&&hasContent ordering turned the empty state into an eternal spinner); demo-picker contract verified (feed-first → random library content with cached episodes → honest empty state; no invented samples).
- **D-488:** the Notifications screen's episode-type block mirrors the Updates screen exactly — title + description on top, the FULL-WIDTH SegmentedToggle below (was squeezed into a 200dp trailing slot); same shared UpdatePreferences keys.
- **D-489:** the library badge positions actually reach the screen — the four CoverBadgeRow call sites pass the ViewModel's episodeBadgePosition/scoreBadgePosition (they were hard-coded TOP_START/TOP_END, making the v1.1.10 TOP_CENTER default dead code); the centered row floats 4dp inside the top edge.
- NEXT: CI green on the feature branch → release/1.1.11 cut from the branch head at 1.1.11/10111 → tag v1.1.11 (user-facing What's New bullets in the tag body) → the DEBUG release (ani-kuta-v1.1.11-debug-arm64-v8a.apk) → the user's device round. Merge to main still awaits the user's explicit confirmation.

## Round 49 (Task 89) — the poster pipeline's real root cause: hardware bitmaps (device-proven)
- **D-491:** the v1.1.11 device logcat proved the failure chain: Coil's default HARDWARE decode config cannot be drawn on the composer's software canvas ("Software rendering doesn't support hardware bitmaps" at drawCenterCrop ← compose ← buildBanner); the round-48 comment "toBitmap converts hardware bitmaps" was FALSE (BitmapImage is returned as-is — verified against the coil3 3.0.4 bytecode). THE FIX: bitmapConfig(ARGB_8888) at the single art choke point (loadBitmap) — fresh decodes software-safe AND the engine's isCacheValueValidForHardware auto-skips hardware-backed memory-cache entries (re-decodes from the disk cache, so the UI's loads can't poison the composer) + ensureSoftwareSafe() copy-or-null as the never-crash last line (fallback chain thumb → flat dark stage still yields a banner). The preview's failed message no longer blames the connection (with the root cause fixed, failed=true can only be an internal exception — art availability can no longer fail the banner at all).
- NEXT: CI green on the feature branch → the user's device round on the poster pipeline (preview renders real banners, shuffle works, test notifications arrive as posters, real notifications render as posters, airplane-mode offline re-test). Release 1.1.12 only after the device round goes green.
- **FINAL:** CI GREEN (run 35450863700 on 3614ed9c) → release/1.1.12 cut from 3614ed9c as ONE commit (the version bump 1.1.12/10112 + 41-RELEASE-1.1.12-BRANCH-POINT.md, D-492) → tag v1.1.12 → Release APK workflow GREEN (run 35451348176) → v1.1.12 LIVE (stable, --latest, ani-kuta-v1.1.12-debug-arm64-v8a.apk 66.7MB + SHA256SUMS.txt, body = the tag bullets — verified via the API). Awaiting the user's device round on v1.1.12.


## Round 50 (Task 90) — the presentation round: the banner becomes a banner, the glitch fixed, the shuffle + test notifications redefined
- **D-493:** the banner LAYOUT redesigned on the user's v1.1.12 feedback ("too tall for a banner", the background "glitched out", the SUB/DUB tags "not shown properly") — canvas 1024×576 (16:9) → 1024×440 (≈21:9) with a zone-anchored layout (title/EPISODE/episode-title from the top, the SUB/DUB chips + branding on a shared bottom baseline, the thumbnail aspect-preserved in a 210×330 right-centered box — any shape fits, nothing can overflow); THE GLITCH root-caused (the old drawCenterCrop's BitmapShader sampled at native size — sources smaller than the canvas CLAMP-smeared edge pixels across the banner) and replaced with src→dst cover-crop math + Coil `Scale.FILL` background decodes (verified against the coil3 3.0.4 source); the badge fix at the right layer (the engine's "unknown" variant for both-variant releases normalized to a random sub/dub on the DEMO paths, honest no-chip on the real path; case-insensitive chips; "both" renders two chips; FontMetrics-centered labels; the preview box adopts the composer's real ratio via the now-public CANVAS_WIDTH/HEIGHT — the private-companion facade blocker caught by the review round BEFORE the push).
- **D-494:** Shuffle actually shuffles — the feed-first path always re-picked the same newest row (the button looked dead); now the Shuffle tap records the on-stage mainId into a one-shot exclusion, skips the feed, and picks a random library item EXCLUDING it (two-pass picker: the exclusion is a preference, not a rule); the selection is cached so a toggle flip re-renders the same content with the new prefs instead of re-selecting.
- **D-495:** the test notifications redefined per the user's verbatim spec — library entries only: random distinct entry for the first post, ANOTHER for the second (any entry qualifies — the cached-episodes gate is gone; fewer entries cycle with different episodes); EMPTY library → pure-demo notifications (built-in titles on the styled no-art stage) so the tester ALWAYS delivers; the delayed post's title rides the WorkManager data; DI trimmed to the new constructor.
- **D-496:** the "EP 12.0" formatting fix — both post paths share episodeLabel (whole numbers drop the decimal, 12.5 specials survive); the composer's hero line too.
- Review discipline: THREE rounds — the self-review (tokenized brace/paren verification on every touched file), an independent agent review (compiler-verified the private-companion blocker + proved the toggle-flip re-select snap), and the fix round. Coil 3.0.4 APIs verified against the official source + the Maven Central artifact.
- NEXT: CI green on the feature branch → the user's device round (the checklist in progress.md round 50). Release 1.1.13 only after the device round goes green. Merge to main still awaits the user's explicit confirmation.
- **FINAL:** CI GREEN on the FIRST push (run 35455000790 on 2509a1de — no CI debug rounds). Awaiting the user's device round on the feature build (the checklist in progress.md round 50); release/1.1.13 + tag v1.1.13 only after it goes green. Merge to main still awaits the user's explicit confirmation.
- **RELEASED as v1.1.13 (Task 91, D-497/D-498):** the user directed the release NOW and set the standing RELEASE-FIRST process — the release IS the verification build; the device round happens on the release APK via the in-app updater. release/1.1.13 cut from 24be155b as ONE commit (c66db803: the version bump 1.1.13/10113 + 42-RELEASE-1.1.13-BRANCH-POINT.md) → tag v1.1.13 ("the banner finally looks like a banner" + 7 What's New bullets) → Release APK workflow GREEN on the FIRST run (35456726478, the only CI run this release cost) → v1.1.13 LIVE (stable, --latest, ani-kuta-v1.1.13-debug-arm64-v8a.apk 66.7MB + SHA256SUMS.txt, body = the tag bullets — verified via the API). The user's device round on v1.1.13 now decides the next loop. Merge to main still awaits the user's explicit confirmation.

## Round 51 (Task 92) — the presentation-depth + features round: banner v3, the dead-seam fix, the in-app heads-up banner, the planned-randomness deck
- **D-499:** the banner v3 — canvas 1024×400; the layout mirrored (the episode thumbnail as a LEFT-side 16:9 400×225 card, cover-cropped — too-wide art crops to the normal thumbnail shape — rounded, dark-bordered, drop-shadowed; absent URL/failed load → the banner composes without the card); the text column top-right (scrim mirrored); the episode number retired from the 58px hero line into the tag row ([EP n] lime chip + [SUB]/[DUB] dark chips); bold + shadow-backed title/episode-title/branding (light-art safe); THE CHIP TRUTH: the feed's per-variant rows are the evidence — new getVariantsForEpisode query + UpdateStore method + the composer's resolveAudioVariant union — a dual-variant episode renders BOTH chips (the user's SUB-only report).
- **D-500:** THE DEAD-SEAM DISCOVERY + the in-app heads-up banner — NotificationsModule's getOrNull<NotificationArtProvider>() never resolved the composer (Koin indexes by concrete type; every other nullable seam in the repo carries an explicit single<Interface> binding — the composer was the lone exception), so ALL system notifications since D-477 fell back to plain text — the user's "the notification showed properly but no banner alongside it / it never shows banner notifications" was this seam; fixed with the explicit binding, and the user's feature request implemented: InAppBannerController (SharedFlow, tryEmit — posting can never fail over it) + NotificationManager emitting the composed bitmap after every post + InAppBannerHost in AppRoot's overlay stack (foreground-only by construction — repeatOnLifecycle(STARTED); any screen; 5s auto-dismiss; tap-dismiss; the exit-animation state split the reviewer caught).
- **D-501:** PLANNED randomness — the ShuffleDeck: the library shuffled once per cycle, consumed in order, reshuffled on exhaustion with a no-immediate-repeat head-swap guard, stale ids pruned; eligibility via a caller lambda; @Synchronized; pickRandomPlanned preserves D-494's no-op-shuffle fallback; screen-open keeps feed-first → random.
- Review discipline: the tokenizer balance self-review (10 files) + the independent agent round (two compile blockers caught: the single-column SQLDelight scalar `.map` misuse; the missing InAppBannerController import — plus the dead seam and the exit-animation defect).
- **RELEASED FIRST (D-498):** the release branch + tag ship the moment CI is green — the user's device round happens ON the release APK (see the release record below).
- **RELEASED as v1.1.14 (D-502):** release/1.1.14 cut from 5141d1f1 as ONE commit (2f26f467: the version bump 1.1.14/10114 + 43-RELEASE-1.1.14-BRANCH-POINT.md) → tag v1.1.14 ("banners everywhere, and a banner that reads right") → Release APK workflow GREEN on the FIRST run (35462212024) → v1.1.14 LIVE (stable, --latest, ani-kuta-v1.1.14-debug-arm64-v8a.apk 66.7MB + SHA256SUMS.txt — verified via the API). The user's device round happens ON this release (D-498 release-first). Merge to main still awaits the user's explicit confirmation.

## Round 52 — the clean-notification + Poster-Studio round (D-503..D-506)
- **D-503:** the CLEAN banner — when a banner composes, it IS the notification: BigPictureStyle without bigContentTitle/summaryText and empty builder title/text on both post paths (the expanded notification shows the poster, nothing above it; a failed compose keeps the old text card). The REAL Android heads-up popup: the channel moved to `anikuta_new_episodes_high` at IMPORTANCE_HIGH (the DEFAULT-importance channel never pops heads-up; legacy channel deleted), PRIORITY_HIGH on the non-silent builders; the D-500 in-app overlay retired (InAppBannerController/InAppBannerHost deleted per the user's "I wanted the banner to be shown as a notification"). The second test notification: 30 seconds, not 5 minutes.
- **D-504:** the chip truth widened — resolveAudioVariant now unions the episode cache's own row parsed with parseAudioAvailability (the details page's pill parser) on top of the feed's per-variant rows and the passed variant → a dual-variant episode renders BOTH chips. The thumbnail evidence chain: the episode still (+ one 8s retry) → extras large cover → cover → banner — the left card stays on stage for rows with a null thumbnail_url. THE SCRIMS ARE GONE — the background art renders at full brightness (the D-499 text shadows carry readability).
- **D-505:** the POSTER STUDIO — the Customize button (left of Shuffle preview) opens a forced-landscape two-pane editor: left = the five elements (content title / EP number / SUB-DUB badges / episode title / thumbnail card) with visibility, a 12-swatch palette + default chip, per-element style reset, and the bottom size slider; right = the editable live preview rendered through the SAME PosterDrawing primitives as the composer (shared PosterCanvasMetrics — a faithful miniature). One-finger drag anywhere, two-finger pinch resize, magnetic snapping (8px grid + 14px magnetism to margins/centers/other elements, lime guide flashes), flow-seeding on first edit (the poster never jumps), Save persists PosterLayoutConfig JSON (notif_poster_layout_json) + dumps the FULL JSON to the console log (tag Anikuta:App:PosterStudio) so the layout can be shared back as the future default; Reset restores the factory flow layout. The composer consumes the config in ABSOLUTE mode on every real notification.
- **D-506:** the shuffle feedback — preview pulse + shuffle-icon 360° spin + banner crossfade (the D-494 semantics untouched: toggle flips still never re-select).
- **RELEASED as v1.1.15 (D-507):** release/1.1.15 cut from 048f9422 as ONE commit (5974113a: the version bump 1.1.15/10115 + 44-RELEASE-1.1.15-BRANCH-POINT.md) → tag v1.1.15 ("the notification IS the banner, and the poster is yours to redesign") → Release APK workflow GREEN on the FIRST run (35466505113) → v1.1.15 LIVE (stable, --latest, ani-kuta-v1.1.15-debug-arm64-v8a.apk 66.8MB + SHA256SUMS.txt — verified via the API). The user's device round happens ON this release (D-498 release-first). Merge to main still awaits the user's explicit confirmation.

## Round 53 (Task 94) — the studio-polish + notification-voice round: the drag obeys the finger, the options get rich, the notification speaks
- D-508: the Poster Studio drag REBUILT on absolute segment anchors — the snapped write-back no longer feeds the drag accumulation, so slow precise drags glide 1:1 (the round's headline fix) and the magnet only pulls near a snap line; the studio's clamp bounds now mirror the composer's and the coerceIn(0f, negative) crash edge is closed.
- D-509: the size slider moved below the preview (right pane, under the canvas, still bound to the selected element).
- D-510/D-515: real button feel — the studio's Back/Reset/Save as FilledTonal/Outlined/Filled-primary circles; the settings screen's Customize (outlined) + Shuffle preview (filled) with Material paddings.
- D-511: the rich options — six backward-compatible PosterElementLayout fields (fontKey, bold?, italic, shadow, chipBgArgb, labelOverride with comma-splitting), the sidebar's per-element OPTIONS section, ONE shared PosterDrawing.typefaceFor (composer AND studio), and the flow mode's title visibility gate.
- D-512: the hard per-line ellipsis in wrappedLines — long titles truncate inside their column, down to one word + "…", never past the margins.
- D-513: the studio as a design canvas (BOTH SUB and DUB always, full strength) + the carried preview selection (PosterCustomizeKey → @Serializable data class; onOpenCustomize hands the current selection over; PreviewSelection public).
- D-514: the smart adaptive scrim — measured luminance (24×10 probe) drives the veil: 0% ≤0.40, ≈15% at 0.60, 46% ceiling at 1.00; the same helper on composer AND studio (the round's reversal of the D-504 removal, SMARTER).
- D-516: the notification title + "New episode available for …" restored on both banner paths (the collapsed heads-up card is informative again; expanding reveals the banner; the BigPicture style stays clean).
- D-517: v1.1.16 released (release/1.1.16 from 3cb5adcc, ONE bump commit, tag, Release APK run 35470742809 GREEN on the first run — stable + --latest, verified via the API).

## Round 54 (Task 95) — the layout-planning round: the studio gets a real shell, the title gets aware, the shuffle always shuffles
- D-518: the studio shell REBUILT — statusBarsPadding at the root (the round's named bug: the header lived inside the left panel with zero insets, so SAVE rendered under the status bar and was unclickable), a full-width header bar with labelled Reset/Save buttons, the panels on tonal SectionCards, and the nav-bar inset on the whole body Row (SENSOR_LANDSCAPE can put the bar on either edge).
- D-519: the ELEMENT-AWARE text columns — awareWrapWidth in PosterDrawing (two-pass text-driven band, left-edge-only neighbour rects, AWARE_MIN_WIDTH floor) ends the title/episode-title wrap before any neighbour sharing the band; mirrored composer/studio with render-exact clamps; the studio's awareness funs declared before their first caller (the CI-validated forward-reference blocker) and the wrap widths hoisted out of the draw phase. BONUS: the composer's thumbnail render clamp was range-guarded — the last unguarded coerceIn had made every real notification silently lose its banner whenever the saved thumbnail scale exceeded ~1.78×.
- D-520: the shuffle fixed — an explicit shufflePending flag forces a re-selection on EVERY tap (the old exclude handshake silently no-op'd when the on-stage id was still null), and an honest indeterminate progress rail docks to the preview's bottom edge while the new banner composes (finally-cleared, cancellation-safe).
- D-521: the notification description is just "New Episode" (shortDescription() lost its parameter; both banner post paths updated; the headline keeps the content name).
- **RELEASED as v1.1.17 (D-522):** release/1.1.17 cut from 2b5d7730 as ONE commit (41919f46: the version bump 1.1.17/10117 + 46-RELEASE-1.1.17-BRANCH-POINT.md) → tag v1.1.17 ("the studio gets a real shell, the title gets aware, the shuffle always shuffles") → Release APK workflow GREEN on the FIRST run (35474920806) → v1.1.17 LIVE (stable, --latest, ani-kuta-v1.1.17-debug-arm64-v8a.apk 66.8MB + SHA256SUMS.txt — verified via the API). The user's device round happens ON this release (D-498 release-first). Merge to main still awaits the user's explicit confirmation.

## Round 55 (Task 96) — the template round: the studio retires, five templates ship
- **D-523:** the POSTER STUDIO removed completely — the screen (1,487 lines), the nav key, the layout-JSON pref and the composer's whole ABSOLUTE render mode deleted ("we should not give the users that much customizability").
- **D-524:** FIVE predefined poster templates replace it — Classic (the approved look), Spotlight (bottom-left billboard + bottom-right card), Split (tall left panel + centered column), Minimal (centered symmetric), Card (bottom info panel) — rendered deterministically by the composer, chosen via a five-way segmented toggle.
- **D-525:** the poster screen rebuilt — the live preview + Shuffle are stationary above a scrollable options list, the 32dp/side double gutter is one 8dp gutter, every description is one line, and the Shuffle is a single full-width 44dp ExtraBold button labeled "Shuffle".
- **D-526:** the Artwork three-way toggle (Auto/Cover/Episode) replaces the "Prefer cover art" switch — the episode's own still can now fill the background stage.
- **RELEASED as v1.1.18 (D-527):** release/1.1.18 cut from 827440c1 as ONE commit (45c0bdd9: the version bump 1.1.18/10118 + 47-RELEASE-1.1.18-BRANCH-POINT.md) → tag v1.1.18 ("the poster gets five templates, and the studio retires"; 7 What's New bullets) → Release APK workflow GREEN on the FIRST run (35503422555) → **v1.1.18 LIVE**: stable, --latest, ani-kuta-v1.1.18-debug-arm64-v8a.apk 66.7MB + SHA256SUMS.txt — verified via the API. The user's device round happens ON this release (D-498 release-first). Merge to main still awaits the user's explicit confirmation.

## Round 56 (Task 97) — the templates round two: reimagined Classic, Duo, the collapse, one line everywhere
- **D-528:** CLASSIC v2 — the eyebrow-first flow on one shared centerline (tag row leads, title follows, everything vertically centered; the still card shrinks to 360×202.5). The old top-hung asymmetric composition is dead.
- **D-529:** SPLIT → DUO — the true split-screen (left half full-bleed art, right half dark panel, 5px lime seam; new layout AND new name; the "split" key aliases to DUO).
- **D-530:** MINIMAL v2 — the left-aligned floating column (one left edge, title 44, tighter, vertically centered).
- **D-531:** the master-toggle collapse — the "Poster notifications" switch owns the top card; flipping it off fades+shrinks the preview, Shuffle, Layout, Artwork and Elements away; only the toggle remains.
- **D-532:** the ONE-LINE DESCRIPTION design language — structural maxLines=1 in the shared row components + every local helper, all wrapping copy shortened across Notifications/Updates/Settings (+ the Appearance/Details/CustomPalette/VideoCaching holdouts), and the notifications page's four trailing dividers removed.
- **D-533:** the library badge round — the Total icon is a playlist (play triangle + episode bars; the film-strip's two-color detail died under the Icon tint), the TOP_CENTER float 4→2dp, every badge shadow 2→4dp.
- **D-534:** the browse round — the score tag is the D-480 soft pill (the last pointed tag in the app), the hero score gets its own amber pill beside the rank pill, carousel covers lift on a 3dp shadow.
- **RELEASED as v1.1.19 (D-498):** release/1.1.19 cut from a0fe0919 (the branch-point record 48-RELEASE-1.1.19-BRANCH-POINT.md) → tag v1.1.19 → the user's device round happens ON this release. Implementation CI 35508580132 GREEN first-try. Merge to main still awaits the user's explicit confirmation.
- **v1.1.19 LIVE (D-535):** Release APK run 35509186985 GREEN; ani-kuta-v1.1.19-debug-arm64-v8a.apk 66.7MB + SHA256SUMS.txt, stable + --latest — verified via the API. The device round happens ON this release; the DASH download implementation is the next session's planned work (download-research/17).

## Round 57 (Task 98) — the CloudStream downloads round: DASH, the toggle's new home, the persistent skip, background parity, the data translation
- **D-536:** the poster master toggle is the Elements card's LAST row ("Poster") — the preview, Layout, Artwork and the element rows all collapse around it; only it never disappears.
- **D-537:** the CS watch screen pauses playback on app-background (ON_STOP) — the MPV watch screen's exact behavior; no auto-resume on return (parity).
- **D-538:** "Skip AniList link" is now PERSISTENT per entry (the D-238 blacklist pattern) — reopening content never re-attempts the auto-link; swipe-dismiss stays session-only; manual link/unlink clears the flag.
- **D-539:** CS DASH DOWNLOADS — the research doc 17's Option C implemented: segment-planning parser + CacheWriter into the offline SimpleCache (csdash| keys, NoOp eviction), the resolve sheet's DASH filter lifted, publishDashEpisode (.data.json+cover+subs, no video file), `csdash:` DB marker (no schema change), scanner/delete flows learn DASH, offline playback through the existing CS screen (startOfflineDash + CacheDataSource + quality pin), cache-preferred episode switching, sidecar subs via content://.
- **D-540:** ecosystem truth (CS content records system "cloudstream"; the bit-62 flag detection; the idempotent startup heal) + the link-switch translation (ContentIdentitySync → the previously-dead contentId sync queries + the verified .data.json identity rewrite — downloads survive source switches, files never move).
- **D-541:** the review round — 3 blockers + 4 risks found and ALL fixed pre-push (the offline key's navigation, the stale offline request, the suspend interface, dashManifestUrl in the rebuild, Locale.ROOT, direct-children addressing, the guarded switch).
- **RELEASED as v1.1.20 (D-498/D-542):** release/1.1.20 cut from cf067e68 (the branch-point record 49-RELEASE-1.1.20-BRANCH-POINT.md; the 1.1.20/10120 bump rode the release branch's first commit) → tag v1.1.20 ("DASH downloads land, the poster toggle finds its final home"; 8 What's New bullets) → Release APK run 35523678946 GREEN → **v1.1.20 LIVE**: stable, --latest, ani-kuta-v1.1.20-debug-arm64-v8a.apk 66.8MB + SHA256SUMS.txt — verified via the API. The device round happens ON this release. Merge to main still awaits the user's explicit confirmation.

## Round 58 (Task 99) — the DASH fetch fix + the repository cleanup
- **D-543:** the DASH manifest fetch is BYTE-FIRST — the v1.1.20 "Manifest could not be parsed" root cause (the String round-trip's forced UTF-8 decode + undecoded gzip; streaming parsed the same bytes fine) fixed in DashDownloader (fetchManifestBytes: bytes + gzip sniff + 8 MB guard + HttpException/raw-IOException retry semantics) and DashManifestPlanner (parse(ByteArray) + the REAL parser message + a sanitized body-head peek in every refusal). Full record + failure matrix: download-research/19.
- **D-544:** the branch cleanup — 23 remote branches deleted after per-branch verification (all release/1.1.x tag-preserved with docs-only deltas); main + feature/round-57-cloudstream-downloads + feature/test-controller-v5 remain; all 87 tags + every release intact.
- The findContentFolder WARN in the device log: benign (cover-thumbnail fallback) — documented in doc 19 §5, untouched.
- **v1.1.21 LIVE (D-545):** release/1.1.21 cut from 5f1796ad (the branch-point record 50-RELEASE-1.1.21-BRANCH-POINT.md; the 1.1.21/10121 bump rode the release branch's first commit) → tag v1.1.21 ("DASH downloads, fixed at the byte level"; 6 What's New bullets) → Release APK run 35528473234 GREEN → **v1.1.21 LIVE**: stable, --latest, ani-kuta-v1.1.21-debug-arm64-v8a.apk 66.8MB + SHA256SUMS.txt — verified via the API. The device round happens ON this release. Merge to main still awaits the user's explicit confirmation.

## Round 59 (Task 100) — the DASH parser factory fix (the REAL root cause)
- **D-546:** the v1.1.21 diagnostics exposed the true failure layer: the manifest body was VALID XML, and "This parser does not support specification "Unknown" version "0.0"" comes from AOSP libcore's `DocumentBuilderFactory` — `setXIncludeAware` (and setSchema/getSchema/isXIncludeAware) ALWAYS throw `UnsupportedOperationException` on Android, even with `false`. Both hardened factories (`DashManifestPlanner` + `MpdParser`) called `isXIncludeAware = false` unwrapped → the factory construction crashed before any parsing, on EVERY device, since the pipeline existed; v1.1.20 + v1.1.21 died at the same line. Fixed in both files (the supported hardening stays); parse errors now always name the exception class; the manifest fetch rides a patient derived client (20s/20s/45s — the 10s cold-start timeout burned an attempt in the device log). Full record + failure matrix: download-research/20.

## Round 60 (Task 3) — D-548: DASH downloads become real files in the user's SAF download folder
- **D-548:** the v1.1.22 device round validated the whole download pipeline but rejected the D-539 media home (app-private SimpleCache): the episode folder had no video file. The media now lives in the SAF download folder as real files — `<title> - E00001.mp4` (the concatenated fMP4) + `<title> - E00001.audioN.mp4` (one per audio set) + `<title> - E00001.mp4.dashmeta` (the manifest bytes + the local range index). Playback rides `CsPlayerEngine.startOfflineDashLocal` (sideloaded DashMediaSource + LocalDashDataSource over the local files) because an unindexed fMP4 is unseekable — the manifest stays the index. Pause/retry resume via a manifest-hash-pinned sidecar; the scanner reconstructs rows with the `csdash:` marker (both modes); deletes sweep audio+meta; the SimpleCache remains only for the v1.1.20–v1.1.22 legacy episodes. Independent review FIX-FIRST → 2 blockers (SAF mime/ext file renaming; the scanner dropping dashManifestUrl) + 1 major (DataSpec.position semantics) fixed before push. Record: download-research/21.

### v1.1.23 LIVE (release run 35541303979 GREEN first-try) — release/1.1.23 from 2270bfa6, bump 59addf73, tag v1.1.23

## Round 61/62 (Task 4) — D-550: the composed sidecar manifest + offline audio switching + the audio/ folder
- **D-550:** the v1.1.23 device round's playback failure root-caused (the sidecar's ORIGINAL manifest let ABR pick an undownloaded 720p rep — the 1 Mbps initial estimate sits below 1080p's 1.6 Mbps; the height pin cannot prevent SMALLER picks) and fixed both ends: the read-side prune (DashManifestPruner in :core:common — covered reps derived the planner's way; existing episodes play without re-downloading) + the write-side composition (DashOfflineManifestComposer — pruned + sibling imports + labels). Offline audio-version switching: sibling DASH variants ride AUDIO_VARIANT tracks (existing audio_tracks column — no migration), their audio sets download as extra groups, the sidecar manifest carries them as LABELED sets → the existing audio selector works offline. The audio/ folder: audio sets publish beside episodes/ + subtitles/; re-publish sweeps legacy-location copies; sweeps/scanner learn both locations; data.json gains additive audioUris. Record: download-research/22.
- **v1.1.24 LIVE (D-498/D-550):** release/1.1.24 cut from 784a9525 (the branch-point record 53-RELEASE-1.1.24-BRANCH-POINT.md; the 1.1.24/10124 bump rode the release branch's first commit) → tag v1.1.24 ("DASH downloads: playback fixed at the manifest level, audio versions switch offline"; 4 What's New bullets) → Release APK run 35555399865 GREEN on the FIRST run → **v1.1.24 LIVE**: stable, --latest, ani-kuta-v1.1.24-debug-arm64-v8a.apk 66.8MB + SHA256SUMS.txt — verified via the API. The device round happens ON this release. Merge to main still awaits the user's explicit confirmation. Branch cleanup this cycle: release/1.1.21/22/23 deleted (verified), 3 branches remain.
...
## Round 64 — D-552: the resolutions become pick targets + the audio snippet fix
- **D-552:** the v1.1.25 device round confirmed the D-551 grouping ("one single section … the two audio versions") and surfaced three demands: the probed "Available:" resolutions were display-only (not clickable, not downloadable); the declared quality lied (each variant's manifest carries the same spread — "1080p was available for the original audio too"); and BOTH audio versions downloaded but "audio 1 was just a random small snippet". ROOT CAUSE of the snippet: the planner's SegmentTimeline branch collapses every r="-1" entry to ONE segment when the MPD declares no parseable duration (periodTicks = -1 → r = 0) — audio packagers emit one negative-r entry for the whole track, and nothing downstream validated. THE FIX: the planner borrows the video timeline's span as the period clock (two passes per Period; DashSegmentPlan gains videoSpanSec/audioGroupSpansSec/audioSetsPresent; the estimate clock falls back to the derived span); the downloader refuses a primary audio whose known span is <50% of the video's (hard honest DownloadException — never a published snippet), skips known-short siblings (best-effort), and refuses declared-but-unplannable audio (a silent video); the sheet renders ONE CLICKABLE chip per probed height (replacing the declared chip + the "Available:" text) — pick target (link, height); playback pins the height via the engine's one-shot start-height override at READY (selectVideoTrack — the same mechanism as the player's quality section); download rides videoQuality ("720p") with zero schema change. VERIFICATION MILESTONE: pure-JVM planner tests EXECUTED in the sandbox for the first time (kotlinc + junit) — DashManifestPlannerTest 8/8 green; the D-551 DashManifestHeightsTest had NEVER compiled ($Number%05d$ is a Kotlin template; CI runs assembleDebug only) + a prolog-indent fixture bug — both repaired, 6/6 green. In passing: SegmentTemplate@duration is already in timescale ticks (the old duration*timescale double-scaling fixed to spec). Record: download-research/24.

### v1.1.26 LIVE (release run 35613828322 GREEN first-try) — release/1.1.26 from f036f855, bump 1.1.26/10126, tag v1.1.26 ("The resolutions become pick targets: one server, every audio version, every resolution CLICKABLE (play pins it, download takes it) — and the audio snippet is dead at the planner")
- **v1.1.26 LIVE (D-552):** release/1.1.26 cut from the round-64 feature head f036f855 (implementation run 35612188522 GREEN on the FIRST run — the sandbox-executed tests + the exhaustive call-site sweep paid off) → bump 1.1.26/10126 + the compact version-comment record + branch-point record 55 → tag v1.1.26 → Release APK run 35613828322 GREEN on the FIRST run → API-verified: stable, --latest, not draft/prerelease, ani-kuta-v1.1.26-debug-arm64-v8a.apk 66MB + SHA256SUMS.txt, body = the tag bullets; 10126 > 10125 → in-app update. CI ledger: EXACTLY 2 runs, both first-try green (the ≤2 budget honored).

## Round 65 — D-553: the selection TRUTH (the height memory + the honest markers + the reachable quality section)
- **D-553:** the v1.1.26 device round's LOGCAT (captured via the round-65 tooling deliverable: the `tag:Anikuta` filter + the AI log-formatting prompt) PROVED the D-552 chain exact — picked 480p → play request startAt=480p → "start-height pin: 480p → video group=0 track=2" → "video track selected: 480p (g0/t2)" → decoder 856x480: 480p genuinely played, yet the user "had concerns that maybe it was playing in 1080p". THE DIAGNOSIS: the truth was UNOBSERVABLE, behind three defects. (1) THE RE-ENTRY HOLE: CsWatchViewModel's same-key re-entry + autoStart re-requested play with NO initialHeight — only the SEEDED path pinned — so back→tap-same-episode and episode auto-advance fell back to ABR and climbed to 1080p over the user's pick; CsSourceMemory grew its second half (the height rides WITH the server per mainId; written by CsResolveSheet.pick + selectLink, a null-height pick CLEARS — a declared chip cannot pin so it must not fake a preference; read by the re-entry + autoStart paths; a stream lacking the height self-heals via the D-552 "not offered → ABR" semantics). (2) THE LYING MARKER: selectedTrackLabel was computed from group.isSelected (true for the WHOLE group when any track plays → always the FIRST rep = the top quality); fixed to group.isTrackSelected(trackIndex) + engineState.videoHeight in the open-effect keys (live refresh); the accordion's D-552 "no marking" compromise died — exactly ONE probed chip lights (current link × the LIVE decoder height) + the playing version's label tints + the "Now playing: <audio> · <label>" line. (3) THE UNREACHABLE SECTION: CsLinksSheet was a non-scrolling Column — one tall MovieBox card pushed "Quality for this stream" out of the clipped bottom (composed but INVISIBLE = the user's "no qualities for this stream"); the sheet body scrolls as ONE unit now (accordion/raw-list/variants LazyColumn→Column; the resolve sheet gets the same one-body scroll). PLUS FIX-A: at 3+ audio versions the collapsed header renders shortAudioLabel (Hindi→HIN, Original→ORIG, Malayalam→MAL, Tamil→TAM; curated map + 3-letter fallback; SUB/DUB pass through; ≤2 keep full names) — 6 unit tests, sandbox-executed green pre-push (kotlinc; the bare-"Audio" expectation was corrected by the execution — the 3-letter fallback is the honest output). PLUS the decoder truth log: onVideoSizeChanged emits "video size: WxH — playing <label>" (deduped). Record: download-research/25.

### v1.1.27 LIVE (release run 35627061143 GREEN) — release/1.1.27 from 942dc28b, bump 1.1.27/10127, tag v1.1.27 ("The selection truth: the height memory (the pick survives re-entry + auto-advance), the honest markers (the ACTUALLY-playing rep lights up + the Now-playing line), the reachable Quality-for-this-stream section (one scrollable body), the collapsed-header short forms, and the decoder's video-size truth log")
- **v1.1.27 LIVE (D-553):** release/1.1.27 cut from the round-65 feature head 942dc28b (implementation run 35625463149 GREEN on the FIRST run) → bump 1.1.27/10127 + branch-point record 56 → tag v1.1.27 → release run 35626504172 **RED**: the workflow titles the GitHub Release from the TAG ANNOTATION'S SUBJECT, and the tag carried the FULL release-commit message (1,900+ chars) → HTTP 422 "name is too long (maximum is 256 characters)" AFTER every build/APK gate passed; the tag was re-cut with the concise v1.1.26-convention subject → run 35627061143 GREEN → API-verified: stable, not draft/prerelease, ani-kuta-v1.1.27-debug-arm64-v8a.apk 70.1MB + SHA256SUMS.txt. CI ledger: 3 runs vs the ≤2 budget — 1 RED, disclosed (the round-63 honest-ledger pattern); LESSON: the tag subject is the release title — keep it ≤256 chars, the long why belongs in the COMMIT message.

## Round 66 — D-554: the CS quality sheets calm down to 0.60 + the episode-list customization FIRST DRAFT
- **D-554-A (the sheet height):** the v1.1.27 device round verified all three round-65 fixes and left ONE complaint — the qualities/servers bottom sheet "was a little bit taller in terms of its height". Both CS quality surfaces already content-fit under a 0.70 screen-fraction cap (material3 pinned at 1.3.1 has no sheetMaxHeight parameter — the cap must stay content-side), so the fix is a fraction change: CsLinksSheet → 0.60, CsResolveSheet → 0.60 (parity), CsEpisodesSheet pinned at an explicit 0.70 (a browsing surface), the app-wide default untouched, the DESIGN-SYSTEM sheets doc carrying the exception note. "Dynamic" = still content-fitting (the cap binds only on overflow); "smooth" = the accordion's expandVertically + the D-553 one-body scroll.
- **D-554-B (the episode-list customization first draft):** the dedicated settings page with the live preview, exactly like the poster page. EpisodeListPreferences grows 7 reactive prefs (rowStyle DETAILED/COMPACT/MINIMAL + 6 element toggles — every default == today's look); EpisodeRow MOVES out of the 4,243-line DetailsScreen.kt into its own file (public, with the style types + the pure pillsRowVisible algebra; the dead DownloadEpisodeButton deleted) so the app-module page can render it; DetailsScreen collects the prefs ONCE and rides one EpisodeListDisplayStyle per row; EpisodeListSettingsScreen replaces the EpisodeSettingsKey placeholder (stationary live preview = THREE inert sample rows through the REAL EpisodeRow — full/watched/bare content shapes, the thumbnail a generated PNG embedded as a data URI; options = the Layout 3-way toggle + the six-element switch card + an honest "More" pointer; the preview reads the SAME prefs the writes go to — zero drift); the Appearance row retitles to "Episode list". Scope discipline per D-523: curated presets, not a free-form editor; grids/per-show overrides/gestures recorded for later. The module's FIRST test source set locks fromKey + defaults + pillsRowVisible — 11/11 sandbox-executed green (the execution corrected two under-specified expectations: the relocated download control keeps the pills row alive). Record: download-research/26.

### v1.1.28 LIVE (release run 35645967935 GREEN first-try) — release/1.1.28 from bf436e73, bump 1.1.28/10128, tag v1.1.28 ("The episode list learns customization: a dedicated settings page with a live preview (Detailed/Compact/Minimal rows + element toggles), and the CS qualities/servers sheet calms to 60% of the screen")
- **v1.1.28 LIVE (D-554):** the implementation cycle cost 2 Build runs (1 RED: the interrupted-session extraction dropped the Modifier import, and publishing the row's parseAudioAvailability collided with EpisodeListProcessor's file-private twin — same-package top-level duplicates are a declaration-site conflict, not shadowing; both repaired in one commit) + 1 Release run GREEN first-try. The device round happens ON v1.1.28 (the checklist is in record 57 + progress.md).

## Round 67 — D-555: the CS sheets find the middle height (0.65) + the episode list becomes FOUR completely different layouts
- **D-555-A (the sheet height):** the v1.1.28 device round ruled 0.60 "way too smaller" and asked for "a bit more taller percentage but smaller than the previous one" — both CS quality surfaces (CsLinksSheet + CsResolveSheet) move to 0.65, the exact middle between the v1.1.27 complaint (0.70) and the over-correction (0.60); CsEpisodesSheet stays explicitly 0.70; the DESIGN-SYSTEM sheets doc's exception note carries the full 0.70→0.60→0.65 history.
- **D-555-B (the four-layout redesign):** the user rejected the D-554 three row variations ("definitely not good … a complete UI redesign … rather than just hiding some features like synopsis or maybe adjusting the shape a little bit") with four directives — preview = EXACTLY two episodes, proper thumbnail imagery, release date + metadata shown properly, FOUR options each a genuinely different design. The D-554 variations DIE; `EpisodeListRowStyle` becomes CLASSIC/GRID/TIMELINE/CINEMA dispatched through the new `EpisodeListEntry` (one entry point for the details list AND the settings preview): CLASSIC = the unchanged row; GRID = a two-column poster wall (badge overlays, chips below, long-press toggles watched); TIMELINE = an air-date schedule spine (continuous rail, date-first node labels with "EP n" fallback, nodes fill when watched); CINEMA = full-bleed banner cards (scrim, ghost number, overlaid title + translucent chips). The `EpisodeDownloadBadge` mirrors the 8-state contract in 32dp; `SwipeToToggleWatched` carries the Phase WP gesture verbatim (shared by CLASSIC/TIMELINE/CINEMA); `rememberEpisodeDisplayData` consolidates the display resolution into one pass. The preview's two samples carry REAL generated stills as PROPER data URIs (the D-554 draft's bare-base64 constant without the `data:` scheme prefix was why the thumbnails rendered empty) + dates on both. The GRID list branch chunks into paired rows inside the existing LazyColumn. Legacy keys migrate to CLASSIC via `fromKey`. Tests re-anchored + 5 new locks — 13/13 sandbox green. Record: download-research/27.
...
- **v1.1.33 LIVE (final):** release run 35765735656 GREEN on the FIRST run; 10133 > 10132 → in-app update; the cycle's CI ledger = EXACTLY 2 runs, both first-try green (the D-557/D-558 pre-push verification discipline paid: zero RED repairs this cycle). release/1.1.32 deleted after its post-tag delta verified docs-only.

## Round 72 — D-560: the sponsor popup's breathing room + the conditional overlay option + the one-time actual release build
- **The sponsor popup:** the cramped card grew air — a tinted hero bubble behind each state's glyph, a quiet SPONSORED eyebrow, ONE line of copy per state (the paragraphs are gone), a measured rhythm (28dp padding, 28dp corners). The frozen machinery is byte-identical.
- **The overlay option:** one tappable row — "Enable draw over other apps" — on the two sitting states, ONLY while the consent is missing (granted = nothing renders, the user's exact spec); re-read on every ON_RESUME, so the row vanishes the moment the user comes back with the toggle flipped; consent makes the floating return pill work for that very ad.
- **The one-time ACTUAL release build:** release-build-once.yml (dispatch-only) builds the FIVE release-signed variants (arm64-v8a / armeabi-v7a / x86 / x86_64 / universal) with the full pre-D-447 gate line, delivered as CI ARTIFACTS — never a release on this repo (the debug updater would be offered an uninstallable APK; D-447 stands). The Confused-Creature-180 re-host stays blocked on the release-agent token.
- **v1.1.34 LIVE:** the D-560 round released — CI: the implementation, the debug release, and the ONE-TIME all-ABI build all GREEN on their FIRST runs (3 disclosed runs: 2 in-budget + the user-ordered one-time exception). The release-signed set (arm64-v8a / armeabi-v7a / x86 / x86_64 / universal + sums + zip) shipped as a 469.7MB workflow artifact; no release page touched (D-447).

## Round 73 — D-561: the sponsor-less popup + the wizard one-liner + the hidden Always-sponsor page + the preset app icons
- **The wizard:** the "Draw over other apps" step now says EXACTLY one line — "Let's AniKuta show floating elements, which make things easier."
- **The popup:** the word "sponsor" is gone everywhere on the card — "Support AniKuta" → "(It just takes a few seconds)" → Continue → Not now; the overlay row is two quiet lines ("Enable draw over other apps" / "Makes things easier for you") and still hides itself the moment the consent is granted.
...
- **CI:** implementation + debug release + the user-ordered all-ABI build — 3 disclosed runs (the budget superseded by explicit instruction).
- **v1.1.36 LIVE (both lines):** the debug arm64 release (ani-kuta-v1.1.36-debug-arm64-v8a.apk 70.7MB) published as --latest; the actual release set (five release-signed APKs + sums + zip) delivered as the ani-kuta-v1.1.36-release-allabi workflow artifact (495MB, 90-day retention). Cycle ledger: 4 disclosed runs — implementation attempt 1 FAILED (a Kotlin replaceFirstChar overload ambiguity, fixed in 09f8de42), implementation attempt 2 + the debug release + the all-ABI build ALL first-try green.

## Round 75 — D-563: the launcher icons resized-not-cropped + the animated hero logo back + the bottom bar above the system buttons
- **The presets now render the FULL artwork:** the artwork moved out of the adaptive icon's cropped background layer into a 66%-canvas inset foreground over a solid edge-matched background — no more zoom, no more side cropping, exactly like the in-app preview (the alias switch machinery + preview untouched; the legacy API-24/25 JPGs regenerated the same way).
- **The popup's logo is back with animation:** the tinted hero circle (OpenInNew / spinner / Refresh) returns to all three states — a spring entrance pop replays on every state swap, and the mark gently breathes while the card waits.
- **Button navigation fixed:** the floating pill lifts above the system's back/home/recents buttons (navigationBarsPadding) and the root tabs' scroll tails grow by the same inset (bottomBarClearance) so the last rows never slide under it.
- **v1.1.37 LIVE (both lines):** the debug arm64 release (ani-kuta-v1.1.37-debug-arm64-v8a.apk 67.9MB) published as --latest; the actual release set (five release-signed APKs + sums + zip) delivered as the ani-kuta-v1.1.37-release-allabi workflow artifact (476.8MB, 90-day retention to 2026-12-22). Cycle ledger: EXACTLY 3 runs, ALL first-try green — implementation (35816889236) + the debug release (35817376243) + the all-ABI build (35817916342).

## Round 76 — D-564: the "You can go back" pill learns to disappear + the first PUBLISHED professional release since v1.1.2
- **The floating return pill's lifecycle is bounded:** the "You can go back / Go back" overlay used to float over other apps FOREVER if the user never returned — even after closing ANI-KUTA (the overlay window belongs to the app's still-running process, not to any screen). Now: the offer auto-dismisses by itself after 10 seconds if Go back is not pressed, and closing the app dismisses it immediately. The countdown ("Stay a moment…"), the grow, the Go back tap, and the exit animations are all unchanged.
- **The professional release v1.1.3:** the professional app line (the one installed as plain "ANI-KUTA", last at v1.1.2) ships version 1.1.3 — installable straight over 1.1.2 — carrying everything from the past months of device rounds: CloudStream extensions + downloads, the offline player stack, the setup wizard, the sponsor popup + the floating return pill, the six preset launcher icons, the episode/search/library redesigns, and this round's pill lifecycle fix. Published as a real GitHub Release (five release-signed APKs: arm64-v8a / armeabi-v7a / x86 / x86_64 / universal + checksums + the zip). Note: the official mirror repo still requires its own release token (pull-only access from here), so the release lives on the dev repo's Releases page.
- **v1.1.3 LIVE:** the professional release PUBLISHED — https://github.com/testplay-byte/ANI-KUTA/releases/tag/professional-v1.1.3 (stable, --latest, 7 assets). Cycle ledger: EXACTLY 2 runs, ALL first-try green — implementation (35824217115 @ 45fb7384) + the professional all-ABI build (35824777389, dispatched tag=v1.1.3 + ref=release/1.1.3 via the new optional ref input). release/1.1.37 deleted (post-tag delta docs-only).

## Round 77 — D-565: the project reset — the debug-first phase begins
- **The user verified the professional v1.1.3 end-to-end** (updates over 1.1.2 in place, clean release build, no debug features — "fully satisfactory") and directed the next chapter: all feature work happens on the DEBUG releases; a proper release comes later when everything is finalized (D-565). Professional releases PAUSE until explicitly ordered.
- **The agent bootstrap layer was rebuilt to the current era:** SESSION.md fully rewritten (it was frozen in the round-29 v0.2.63 / streaming-CLOUDSTREAM-V2 world), the twelve stale "Latest session" paragraphs atop progress.md relabeled Historical + a CURRENT STATUS block added on top, master.md's status block refreshed (56 modules / 569 Kotlin files / D-565), navigation.md's section count corrected, NEW_AGENT_SETUP.md's broken `blob/main/…` link fixed (`main` does not exist — the default branch IS the mainline), CORE_RULES §11's ntfy topic aligned with the actual `THE-TASK-IS-DONE`.
- **The GitHub repository setup was repaired:** the dashboard's Pages deploys had silently failed since `main` was deleted (D-552) — the `github-pages` environment's custom branch policy allowed only dead branches (the build job was green, the deploy job rejected). The mainline was added to the policy, three dead-branch policies removed, and the dashboard's status data refreshed by the full-stack-dev sub-agent (56 modules, D-001..D-565, both live releases, the debug-first banner; local build verified, 22/22 pages exported).
- **Known debt disclosed:** the dashboard's per-table DB transcription stays D-192-era and decisions D-277..D-562 remain representative — a full backfill on the user's request.
- **CI ledger:** docs-only + dashboard-only — zero APK runs; ONE deploy-dashboard run (disclosed).

## Round 80 — D-566..D-569: the notification-system rework — the "scanning" card learns to start on time, speak with sound, show live progress, and always end honestly
The user's order: improve the whole notification system — immediate audible "searching for new episode releases" at check start, live per-item progress, robust timing, modular maintainable code. The audit found 20 flaws (the stuck `setOngoing(true)` card with no cancel path anywhere; the silent IMPORTANCE_LOW progress channel; the REPLACE-on-every-app-open cadence reset; unbounded parallel source fetches; colliding banner IDs; the never-wired dedup retention). The rework shipped: the `UpdateProgressNotifier` contract v2 (onScanStarted → onCheckStart → per-item started/completed → finish/fail/cancelled), one `anikuta_update_activity` channel with real sound, the self-healing live card (only-alert-once + non-ongoing + 15-min timeout), honest caught-up cards for empty manual runs, single-flight checks, cancellation-safe cleanup, `checkActive` for the UI, the UPDATE-policy cadence fix, the AUTO-visible interval row, OFF-mode enforcement, the banner-ID collision fix, and the 90-day dedup sweep. Reviewed GREEN by an independent agent (tokenizer balance ×10 files, interface conformance, no-deadlock audit); its four recommendations were applied pre-push (completed-bar semantics, the double-fire doc, the T7 semaphore wiring, the stale KDoc). The release system is documented (D-569). State at close: complete + reviewed + docs on the mainline, unpushed pending the dev PAT (sandbox reset); CI 0/2 consumed.

## Round 81 — v1.1.38 LIVE: the notification rework shipped
- **The blocker cleared:** the dev PAT was re-provided (account `testplay-byte`, admin/push verified via the API before any push) and stored repo-external (`/home/z/.secrets/github-credentials`, 0600); the credential helper reads it at call time — never in URLs, never committed.
- **The mistake audit (user-ordered) found ZERO defects:** the Round-80 engine + notifier read end-to-end, the Koin arity + banner-ID seams verified, the official repo re-audited (v1.1.3 latest, 11 assets, sanitizer rules hold, Pages live), the playbook matched reality. One pre-existing stale CI comment noted, untouched.
- **The D-498 cut ran clean:** implementation run 35887267531 GREEN first-try → release/1.1.38 + the 1.1.38/10138 bump → tag v1.1.38 → Release APK run 35888008247 GREEN first-try → the stable latest release live (arm64-v8a debug APK 67.9 MB + SHA256SUMS.txt).
- **The release-notes wart fixed:** no tag-body Install line anymore — the workflow's own line is the only one (v1.1.37 shipped two).
- **CI ledger: EXACTLY 2 runs, both first-try green — within the ≤2 budget (D-472).**

## Round 82 — D-571..D-576: the extension uninstall fix, the alarm-clock source wheel, the search/filters polish, the repo copy, and the EXTENSION TESTING SCREEN
- **The uninstall flow fixed (D-571):** the three in-app Aniyomi uninstall dialogs REMOVED — the trash icon fires the SYSTEM uninstaller directly (Android's own "Do you want to uninstall this app?" prompt IS the confirmation); `ExtensionInstaller.uninstallApk` hardened into a logged 3-rung intent ladder (ACTION_DELETE → legacy UNINSTALL_PACKAGE → App-info) with every failure toasted; the CS `uninstallPlugin`'s silent-death bug fixed (catch + `uninstallError` StateFlow + UI toast + refresh-locked-in-finally).
- **The Link Source sheet rebuilt (D-575):** two snap-fling WHEEL columns (Aniyomi left / CloudStream right) with a center highlight band, distance falloff, rim fades and tap-to-center (the alarm-clock feel); per-source icons for both ecosystems with never-blank letter-tile fallbacks; the pill search bar with a filled circular Search button on the RIGHT; a results view with a change-source chip; the DetailsScreen call site untouched.
- **The search bar's button moved right (D-573):** the main SearchBar's left icon is decorative; a filled primary circle at the right edge is the submit trigger (full + compact sizes).
- **The extension filters reworked (D-572):** the header's Filters/Settings are labeled PILLS with a gap; the resting bar is a pill row (Search / Language / Sort / NSFW); the SEARCH pill expands into a dedicated auto-focused search view; the language menu is capped at 320dp (no more full-screen lists); tapping the ACTIVE sort flips ascending/descending (arrow + pill label show it) threaded through BOTH tabs; NSFW is its own toggle pill.
- **Repository long-press copies the URL (D-574):** combinedClickable + haptic + clipboard + toast on every repo row (both types) + a discoverability hint line.
- **THE EXTENSION TESTING SCREEN (D-576):** a new `testing/` package — models, waterfall context, a timeout/skip-aware engine, the chain factory, and SEVEN one-file tests (Ping, Search, Home page, Details, Episode list, Video resolve — the CS side routed through the real link resolver, Stream play via a Range-GET) — plus the screen itself: both ecosystems listed with icons and chips, a suite summary, a user-editable test query, per-card expandable per-test rows with live status/duration/reason, per-card Run, batch checkboxes with a sequential queue and Stop, and the entry banner at the top of the Extensions section. Built for the future roadmap (automated runs, statistics) per doc 64.
- **Review:** tokenizer balance ×22 files ALL BALANCED + an independent review agent (3 compile blockers + 1 defect found and FIXED before push).
- **v1.1.39 LIVE:** release/1.1.39 cut from the green head (6ad76dee) → the 1.1.39/10139 bump → tag v1.1.39 → Release APK run 35906141111 GREEN first-try → the stable latest release (arm64-v8a debug APK 68.0 MB + SHA256SUMS.txt). CI ledger DISCLOSED: 4 runs this cycle (2 fast-fail compile runs on the new screen, both diagnosed from logs; 1 green implementation; 1 green release) — the honest cost of CI-only builds on a round this size.

## Round 83 — D-577..D-579: the uninstall permission, the 19ms testing bug, and the second device pass on the sheet/filters/testing screen
- **The uninstall root cause (D-577):** the manifest lacked `REQUEST_DELETE_PACKAGES` — the platform UninstallerActivity starts-and-finishes silently without it (exactly the user's 70ms log signature). One permission line fixed what the round-82 intent-ladder hardening could never reach: startActivity() succeeds, so the ladder saw no failure.
- **The 19ms bug (D-578):** the round-82 tests called the source methods on the MAIN dispatcher — real aniyomi extensions threw NetworkOnMainThreadException instantly while the CS bridge (internal dispatching) passed everything. All five test files now run `withContext(Dispatchers.IO)`, matching the production pattern.
- **The smart search (D-578):** the Search test runs the custom query then the well-known ladder (One Piece / Naruto / Breaking Bad / Interstellar across anime/series/movie); first result-set wins; per-attempt 12s bound inside a 60s kind budget; honest aggregate failures. `TestTimeFormat` renders "1.2 s" / "2m 05s" everywhere.
- **The testing screen rework (D-578):** the Science icon pill entry (banner gone), All/Aniyomi/CloudStream sections with select-all + run-section, tap/long-press multi-select (chevron-only expand), the live batch overlay ("Testing 3 of 7" + animated progress + live rows + Stop), the summary's animated proportion bar + "Failed (n)"/"Passed (n)" precision re-runs, and the modular file split.
- **The memory (D-579):** `ExtensionTestResultStore` persists every finished target's verdicts (per-target JSON in SharedPreferences), restores them on open, prunes uninstalled targets, and powers the precision re-runs + Clear.
- **The sheet (D-578):** linked-source pre-selection + ✓ marker (id match, name fallback), dedicated section cards with distinct backgrounds, center-only highlight, the short hint, the wrap-content sheet, and the exact-spec search bar (far-right X, vanishing magnifier, use-only search button, border, bottom padding).
- **The filters (D-578):** styled Language/Sort menus + the bordered dedicated search field; the header pills went icon-only (Science · Filter · Gear).
- **CI ledger:** the implementation run 35975795395 FAILED fast (the four rewritten test files dropped the `.tests` package suffix; `Modifier.weight` outside a RowScope) — fixed from the logs; the fix push is the cycle's second run.

## Round 84 — D-580..D-583: the five-page testing system + the hard-isolation hang fix, the delete exit choreography, the true pills, the button-feel menus, and the sheet's third pass
- **THE HANG root-caused + fixed (D-583):** the device report (unresponsive on SOME Aniyomi extensions at the search test, Stop dead, CloudStream never hanging) traced to fully-cooperative cancellation — blocking extension code (no suspension points) defeats `withTimeout`/`Job.cancel()`, and the old screen's `isBatchRunning` gated every control including Stop. `TestIsolation.runKind`: each kind on a dedicated single-thread daemon dispatcher, the caller awaiting in 250ms slices under its own deadline (always returns), abort = cancel + `shutdownNow()` interrupt; `context.ioDispatcher` routes the tests' blocking work onto that thread so the interrupt lands on the blocked call; SearchTest CE-rethrow + TCE disambiguation; SEARCH 60s→80s; test client `callTimeout(30s)`.
- **THE FIVE-PAGE SYSTEM (D-583):** Home → per-system List (checkbox-free rows, tap=expand/long-press=select) → the dedicated Run page (Stop + SKIP-TARGET always live, live per-kind rows, queue, completion verdicts, Re-run failed/passed) → Target detail (seven full verdict cards) → Stats (totals, per-system bars, per-kind avg/failure table, needs-attention list, run history). The app-scoped `ExtensionTestRunController` owns the lifecycle (survives navigation; zero DI wiring via GlobalContext); 4 new nav keys + the `pushTesting` duplicate-class guard; the 4 superseded screen files deleted; shared visuals in `TestingSharedUi.kt`; verdicts persist per-target + a 40-capped run-history feed.
- **The delete exit choreography (D-580):** the Downloads D-384 motion (110ms settle dip → 240ms slide+fade via graphicsLayer) shared into `ExtensionListChrome` and adopted by all six delete-capable rows of BOTH tabs (Aniyomi optimistically — the system dialog can be dismissed, so a 3s watcher fades the row back; CS after its AlertDialog confirm) + `animateItem()` on every row of both lists (the rows below GLIDE up).
- **The true header pills (D-580):** Tests/Filters/Settings — icon + label, content-adaptive widths.
- **The filter menus + search (D-581):** button-feel bordered option cards with 4dp separation (language + sort), a focus-ring search field with a steady magnifier and a boxed clear.
- **The sheet's third pass (D-582):** taller (wheel 236dp + 60% min height); the wrong-initial-highlight BUG root-caused (the last-composed wheel's initial centerIndex=0 emission overwrote activeSide — now only user-driven taps/drag-settles claim focus); 3D cards (animated shadow, primary rim, gradient sheen); falloff into draw phase + lightTick haptics; focus-ring search bar with an elevated button; EMPTY-on-open with first-focus paste of the content name.
- **CI ledger (disclosed):** run 1 (the UI batch, 839c7c8) GREEN first-try; run 2 (the testing system, 713ba6e) FAILED — 4 errors (a dropped `?.` in a chain, a smart-cast that doesn't extend into an items lambda, the parent-package `matchesSearch` import, and an implicit-receiver AnimatedVisibility capturing the deprecated RowScope overload) — all diagnosed from the logs; run 3 (23eab3f) GREEN. Total: 3 implementation runs + 1 release run (below).

## Round 85 — D-584..D-586: honest stop/settle semantics, in-place runs, rich payloads, the stats dashboard, the hub rework, the patient resolve, the confirmed-removal choreography, and the sheet's fourth pass
- **The run-state machine fixed (D-584):** the stop residue (the hero's eternal "Now testing", the spinning rows) traced to three stacked defects — the settle never cleared `currentTargetId`, the in-flight RUNNING result was never terminalized, and the whole queue started `isRunning=true`. The settle now clears the live target, rewrites RUNNING → SKIPPED("Stopped by user"), marks the interrupted target finished+aborted, and COMPLETED clears it too. The rerun blocker was the Run page's `rememberSaveable` armed-set being restored by the nav shell's per-CLASS `SaveableStateHolder` — instance-local arming after a successful start now.
- **In-place runs (D-584):** every run entry (list Run-all/per-row/selected, the detail page's button, the hub's Run-all) calls the app-scoped controller directly — rows animate Queued → Testing… → verdict on the same screen; the Run page is a deep view via the banner/View and the list's pinned run strip.
- **The list rework (D-584):** expansion = actions FIRST + only compact status/duration rows (no messages, no PENDING wall); tap toggles selection while a selection is open; the trailing chevron GONE; `animateItem()` on the rows; the double height-animator resolved to one.
- **The patient resolve (D-584):** the CS resolver no longer coerces a missing/degenerate `loadLinksTimeoutMs` into a hard 5s wall (the 120s default applies; the 30s first-link watchdog still bounds UX); the Aniyomi test mirrors the production hoster ladder (getHosterList → lazy getVideoList(hoster) ×4 → legacy, per-rung 20s, ×3 episodes); the kind budget 45s→90s.
- **The smart phrases + the candidate walk (D-584):** Jujutsu Kaisen (24 ep) + the Chinese donghua "Link Click" (11 ep) replace One Piece/Naruto; DETAILS/EPISODE_LIST walk the full search+home candidate pools forward on dead entries (the winner becomes the chain's anime).
- **The rich payloads (D-585):** a `TestPayload` chain (TestOutcome → TestResult → a JSON store projection → the UI) carries the ACTUAL results — poster cards (search/home), the details dossier, EP chips (48 cap), server/quality rows (the ABI quality scale), HTTP/RTT/bytes chips (ping/stream) — rendered below the run page's tap-expanded rows and inline on the detail page (detail text no longer truncated).
- **The stats dashboard + the hub (D-585):** the new `TestingCharts.kt` (animated-sweep donut, grow-in bars, sparkline, count-ups) powers the stats page (health-ring hero + the pass-rate trend + per-system mini donuts + stage-reliability bars + the history feed with wall time) and the hub's rework (health-ring hero + recent-run chips + the morphing command footer with a badged Stats pill — the stock buttons gone).
- **The confirmed-removal choreography (D-586):** the Aniyomi trash fires the system uninstaller DIRECTLY (the row stays visible under the prompt; cancel costs nothing; the 3s blind-restore heuristic deleted); a screen-local `ACTION_PACKAGE_REMOVED` receiver freezes the removed extension as a GHOST at its captured index and the exit choreography plays ONCE on the confirmed removal; trust/untrust animates the same way on BOTH ecosystems (tap-driven: exit first, then the data change).
- **The sheet's fourth pass (D-586):** the 60% min-height dead slab GONE (wrap-content; results = a ceiling); `navigationBars ∪ ime` padding + `adjustResize` (the keyboard never covers the search bar); both wheels alphabetically sorted case-insensitively + remember-keyed (no per-keystroke refilter); the BLUR CONTRACT (focus loss or keyboard-back resets the field unless results are showing; a 100ms IME-visibility poll while focused handles keyboard-back without fighting the M3 sheet's inset listener); query/showResults/autoPasted are `rememberSaveable`.
- **The icon-only header pills (D-586):** Tests/Filters/Settings lost their labels (the device report) but kept the stadium shape (44dp min width) with the accessible name hoisted onto the Surface semantics.
- **The special orders:** the agentic UI-designer prompt delivered FIRST to the upload folder (`ANI-KUTA-UI-DESIGN-AGENT-PROMPT.md` — a self-contained brief: domain, payloads, pain points, deliverable spec); five research sub-agents audited the testing screens independently and returned file:line root-causes that drove the fixes.
- **CI ledger (disclosed):** batch 1 (77f8185) GREEN first-try; batch 2 (6360bae) FAILED on ONE error (the missing `TestPayload` import in PingTest) — diagnosed from the logs; batch 3 (541a3ec) FAILED on two classes (the missing `android.content.Intent` import + `WindowInsets.ime` being @Composable inside snapshotFlow) — both fixed (c820d91). Total: 3 implementation runs + the release run (below).

### Round 86 (2026-09-24) — D-587..D-592: the v1.1.42 device round — honest verdicts, live timers, the timeline, the normal source list
- **The sheet's fifth pass (D-587):** the search-bar focus-killer ROOT-CAUSED — the round-85 keyboard-back IME poll cleared focus on its first check, BEFORE the async keyboard opened, so the field lost focus the same frame it won it (the device: "the selection would not appear"; paste still worked because it edits the buffer programmatically); the watcher now acts only after the IME was OBSERVED visible once. The two alarm-clock drums DELETED (~400 lines) for ONE normal alphabetical list: rounded-RECT headings (bold 14sp, count right — no more bubble pills), the selected row tinted + bordered + bold + a primary check, the linked ✓ kept, the hint moved to the very bottom and reworded.
- **The hub + stats (D-588):** the suite-health ring split into SIX per-system segments (Aniyomi primary/error/dim → a 4° gap → CloudStream tertiary/error/dim; `DonutSegment.gapAfterDegrees` + a usable-sweep rebase; two-caption legend); recently-tested = extension ICON + NAME chips only (store-ordered targets, letter-tile fallback); system cards: right-side count chip + chevron gone + accent edge; the Stats pill restyled (icon + label + failed-count bubble); the stats page's Needs-attention + Recent-runs sections REMOVED (the user: "no need to show any of those at all"), Clear-all relocated.
- **The list (D-589):** Run all / Run failed / Run passed pills (eco-scoped, counts in the labels, empty scopes disabled); the VERDICT SORT (passed alphabetical → untested → failed by failedCount ascending then name — "failed in a lot of things → much further down"), re-sorting LIVE mid-run (store refresh + animateItem, and stateFor prefers the store for fresh queue states); the expansion split into TWO blocks (a separated kind-row container + the actions bottom-right); per-row cards + the one-at-a-time entrance reveal; LIVE TICKING timers + LEADER-DOT trails + honest skip reasons on the compact rows.
- **The engine (D-590):** chain order Ping → Home page → Search (enum + chain swapped; persistence by name); the HONEST GATE (missing prerequisites = FAILED "Not run — … failed", the four defensive skips aligned — dead sources no longer cascade as "skipped"); the DETAILS FORGIVENESS (details fail + episode list pass ⇒ the details verdict rewritten "Forgiven — the episode list loaded" and the target is HEALTHY; the reverse stays a fail); the CAUSE-AWARE SETTLE — the false "Stopped by user" root-caused to a stray CancellationException killing the run job while the round-85 settle stamped every completion-while-RUNNING as a user stop; now `stopRequested` + the completion cause + a session-GENERATION latch (identity never matched — the loop copies the session) split user stops (SKIPPED "Stopped by user" + aborted) from unexpected deaths (RUNNING kinds → FAILED "Run aborted: …", target NOT aborted, Logger.e with the stack); structured TEST TARGET/KIND START-FINISH logging (verdict, durationMs, http, url, reason, detail) + per-phrase search DEBUG lines.
- **The detail page (D-591):** single-target ROUTING (the run-strip/hub "Details" opens the dedicated page when the session has exactly one target); the DOSSIER header (System/Version/Package/Plugin/NSFW/Site/Last-tested rows from the live managers); the 7-STAGE LIVE progress bar (each stage fills against its own budget, snaps to its verdict color); the tertiary non-full-width run pill; the TIMELINE (a left rail of connected status bubbles + the cards on the right, IntrinsicSize rows); the cards' static descriptions and duplicated ping/stream chips REMOVED; the CODE-WINDOW detail blocks (dark monospace + window dots).
- **The run page + payloads (D-592):** the bespoke top section (breathing pulse dot, phase chip, hand-drawn bar — zero stock Material; Stop/Skip/aftermath as stadium pills); the FINISH BLOCK (verdict headline + counts + wall time via the new `RunSession.finishedAtMs`; primary/error variants); the 3×2 one-line result grid (top 6); the RANDOM top-10 search-preferred details pick (`detailsWalkOrder`); the search ADVANCED STATS in the payload (attempts + winning phrase/category) + the LIVE per-phrase status piped into the rows/cards; the video links as numbered quality cards; THE STREAM LIVE PREVIEW (the payload carries streamUrl + referer/UA/headers, store round-trips them; a muted looping ExoPlayer card with the stream's own headers via DefaultHttpDataSource; media3-ui added to the impl module); the LEAVE GUARD (hub + run page: BackHandler + confirm — leaving stops the run).
- **CI ledger:** the round-86 compile review (a dedicated agent) found ZERO errors across the 20 changed files before the push; the build runs are recorded in the next round's ledger.
- **The release (the user's 'do the release properly' order):** `release/1.1.43` cut from the green mainline head 4835627 (the bump 10120→10143 rides the branch per D-430), the annotated tag v1.1.43 written with the FULL user-facing bullet body (D-466 — the nine round-86 bullets, so the release notes are the real changelog, not the '- Bug fixes and improvements.' fallback), Release APK run 36039846671 **GREEN FIRST-TRY** → **v1.1.43 is LIVE** (stable latest, arm64-v8a debug APK 68.3 MB + SHA256SUMS.txt). Housekeeping: the v1.1.41 + v1.1.42 release bodies (which had shipped with the fallback because their tag bodies were empty) were backfilled with their proper round-84/85 bullets via the API — no re-tagging, no rebuild.

### Round 87 (2026-09-24) — D-593..D-613: the v1.1.43 device round — the un-killable chain, the honest palette, the combined ring, and the blocks that carry their own results
- **The engine (D-593) — THE UN-KILLABLE CHAIN:** the "one test stops midway and the other tests stop with it" report root-caused to three stacked defects — SearchTest caught `CancellationException` BEFORE `TimeoutCancellationException` (TCE IS-A CE) so its own 12s phrase timeout escaped as a cancellation and killed the whole run job; TestIsolation's awaiter rethrew every body-origin CE into that same job; and the queue loop had no blast-radius containment. Fixed: the TCE-first catch ladder + `currentCoroutineContext().isActive` guards on every test's catches (Search/Details/EpisodeList/VideoResolve); the isolation async block converts body-origin CEs into FAILED verdicts (the isActive check splits "the body died" from "the user pressed Stop"); per-target containment — one target settles FAILED "Run aborted: <Class: msg>" and the queue CONTINUES — under a CoroutineExceptionHandler net; `stop()` remains the only whole-run unwind.
- **The palette (D-594):** `TestingPalette.kt` NEW — the single source of truth for every color the testing system uses. The ring's old pair (the selectable accent `primary` vs the never-themed M3 baseline `tertiary` #EFB8C8 — pale pink ~20° from error red; the user: "way too close together. They look ugly") replaced by FIXED hues that never follow the accent preset: Aniyomi emerald #34D399, CloudStream sky #38BDF8; failures red/orange per system; untested two DISTINCT grays (the camouflage complaint); seven per-kind accents (ping sky, homepage amber, search pink, details coral, episodes teal, video-resolve lime, stream violet).
- **The payloads + the 30-second stream (D-595..D-598):** every resolved video listed (the cap gone, payload 24); the CS resolver ACCUMULATES links across snapshots with dedup so early finds survive later snapshots; the stream preview capped at 30 SECONDS — the final seconds tick down, the player auto-stops, "Stream played successfully" prints, REPEAT_MODE_OFF ends the looping, the PlayerView detaches in onRelease (player, surface, session closed cleanly).
- **The hub (D-599..D-602):** the suite-health ring COMBINED — the user's model ("Aniyomi 10 passed + CloudStream 5 passed → show 15, with the split inside") replaces the round-86 six-segment split: pass/fail/untested groups, each subdivided by system, group gaps REPLACED out of the sweep (no duplicate arcs, one revolution); the "· 10 + 5" legend split; the system cards' count chips beside their titles, full-width bars, centered idle content; the untested track VISIBLE (onSurfaceVariant@0.25 resolved IN composition — a Canvas lambda cannot call composable resolvers); the Run-all pill centered and redrawn.
- **The list (D-603/D-604):** the double-check (select-all) button DELETED (the round-86 order that had not landed); the count + the three compact run pills share ONE controls row; ALL seven tests always render in the expansion — pending at 0.42 alpha, no placeholder.
- **The detail page, rewritten (D-605..D-610):** the DOSSIER header (uppercase muted labels in a fixed column, wrapping values, hairline dividers); the TIME-PROPORTIONAL STAGE BAR (each segment's LENGTH = its test's actual ms — running segments grow on a live ticker, pending hold their budget's share on the faint track; per-kind colors, failures dim their hue); the COLORED RESULT BLOCKS carry their results INSIDE as a labeled section (message/detail text rows, the search ladder as stat pills, the payload — grid/dossier/episodes/links/preview — within the block); `CodeWindowBlock` DELETED entirely (the user: "remove it completely"); the run pill redesigned (bordered tint + icon badge, wrap-content); the TRANSIENT VERDICT BANNER (a fresh verdict slides in under the actions, lingers ~2.6s, slides away; nothing on the page auto-scrolls — ever).
- **The run page (D-597/D-611):** the LIVE elapsed timer finally reaches the run page's rows (`LiveElapsedText` → `KindResultRow` → `LiveKindRow`) — the frozen "0 ms" report is dead; the auto-scroll effect DELETED (the user owns the viewport; the sections update in place).
- **The Link Sources sheet (D-612/D-613):** the two ecosystems back as TWO SEPARATE section cards (accent dot + bold heading + count, the lists visibly apart); the list cap IME-AWARE (screen − nav − IME − 214dp reserved, recomputed live) — the search bar can never be squished out of view again.
- **Review + CI (the user's explicit sub-agent order):** TWO audits ran against the working tree before the push — the COMPILE review (every changed file against real Kotlin 2.2.0 + material3 1.3.1 + compose-ui sources: exactly 2 confirmed errors, missing `kotlinx.coroutines.isActive` imports in DetailsTest + EpisodeListTest, both fixed pre-push; braces/smart-casts/Canvas rules/Surface overloads verified) and the REQUIREMENTS audit (all 22 device-report points with file:line evidence: 22/22 landed, 4 stale comments corrected). CI: 31ce1887, Build APK run 36065055559 **GREEN FIRST-TRY**.
- **The release:** `release/1.1.44` cut from the green mainline head, the bump 10143→10144 rides the branch per D-430, the annotated tag v1.1.44 carries the full round-87 bullet body (D-466), the Release APK run goes green → **v1.1.44 is LIVE** (see the SESSION/progress record for the run id and artifacts).

### Round 88 (2026-09-24) — D-614..D-624: the v1.1.44 device round — the sheet's two true columns, the full-details page that finally behaves, and the preview that asks before it plays
- **The sheet's two true columns (D-614):** the user dictated the layout exactly — Aniyomi LEFT, CloudStream RIGHT, side by side; the round-87 stacked section cards had read as one tall column. A Row of two weighted cards, each with a FIXED heading (palette dot + bold label + count) above its OWN independently-scrolling LazyColumn; rows shrunk to the ~160dp budget (20dp icons, 11sp one-line names, the selected treatment intact, the linked ✓ kept); honest empty notes; the ime-aware cap's reserve grown 214→244dp for the headings (the search bar keeps its room).
- **The stale-banner fix (D-615):** the verdict banner's seen-set started empty at every composition, so opening a tested extension replayed an unearned banner for its last stored verdict EVERY time. The seen-set is now SEEDED at first composition — only verdicts arriving while the screen watches earn the banner.
- **The banner floats (D-616):** the old inline LazyColumn item's height snapped in and out, shoving the timeline ~34dp on every completion (up to 14 jolts per 7-test run). The banner is an overlay pinned under the header — appearance and exit shift nothing.
- **The hero compressed (D-617):** the seven stacked dossier rows became a two-column meta grid (wide Package/Site cells take full rows and wrap); the duplicated "System" row is gone; the first result lands on screen.
- **The failures handled (D-618):** a verdict summary (Passed n / Failed n / Not run n chips, non-zero only) at the top of the hero; the failure REASON renders in full (the 2-line ellipsis on the full-details page was backwards).
- **Live-run awareness (D-619):** a queued target's chip reads "Queued" (like the list's); the run pill has three honest states — "View live run" (navigates to the observe-only live session), "A run is in progress…" (disabled, truthful), "Run all tests".
- **The stage bar honest (D-620):** pending segments wear their kind's color (the pre-run gray strip is dead); the 2% clamp is a renormalized 3% floor (widths sum to one bar, no theft); the per-segment animation waves are gone (the running segment grows on its live ticker).
- **The breathing rail (D-621):** a RUNNING test's bubble PULSES (ring + dot) — running vs passed is unmistakable without reading; pending cards fade ONCE (the double-dim was unreadable).
- **The preview asks first (D-622):** stored pages render a tap-to-play strip (they used to stream video on open); the ended preview collapses to a one-row strip ("Stream played successfully" / "Preview unavailable — the verdict came from the stream's data"); the ExoPlayer exists only inside the playing branch and is released on exit; PING's empty labeled results box is skipped entirely.
- **The payloads whole (D-623):** the grid shows all 12 captured entries (was 6), the episode chips all 48 (was 24), the genres all 8 (was 3); labels carry counts; 9sp → 10sp.
- **The color discipline (D-624):** the final re-verification pass caught the new pill reaching for the never-themed M3 baseline tertiary (pale pink) — the exact round-87 bug class — replaced with the palette's sky.
- **The three-pass verification (the user's explicit order):** PASS 1 the compile review (0 confirmed errors, real-signature checks, 2 dead imports removed); PASS 2 the requirements audit (21/21 landed, file:line evidence, the four GOOD pages regression-swept clean); PASS 3 the adversarial re-verification (runtime hazards, strings, the theme grep that caught D-624, the observe-only push contract). Verdict after fixes: SHIP.

### Round 89 (2026-09-26) — D-625: the round-89 direction order — the link sources sheet reverts to v1.1.43
- **The order:** a session order, not a device report — the v1.1.45 state "is not satisfactory according to my requirements"; the LINK SOURCES changes specifically are to revert to v1.1.43, and NOTHING else ("Nothing in the sense of extension testing needs to be reverted"). Plus: release v1.1.46 properly so the app updates in-app, keep the sandbox project folder separate, keep the doc/comment discipline.
- **The scope audit (before touching anything):** the `v1.1.43..HEAD` diff falls into exactly four buckets — the Link Sources sheet (ManualSearchSheet.kt, the only anime-details file that moved: the D-612/D-613/D-614 changes), the extension-testing system (15 files + MainActivity's D-619 wiring), the D-430 version-doctrine difference (AndroidConfig 10143-at-tag vs 10120-on-mainline — not a real change), and the docs/memory. Only the FIRST bucket reverts.
- **The revert (D-625):** ManualSearchSheet.kt restored BYTE-IDENTICAL to tag v1.1.43 — the D-587 single alphabetical list (rounded-rect Aniyomi/CloudStream headings, full-width rows, 28dp icons, 12/13sp names, the full selected treatment, the fixed 430dp cap) — plus ONE round-89 annotation comment in the section banner so a future edit knows the D-614 two-column implementation lives at tag v1.1.45 (commit 05a79f80) and must be cherry-picked, never re-implemented. The removed symbols (SourceColumnCard/SourceColumnRow/listMaxHeight) were all file-private; module-wide grep: zero dangling references.
- **The disclosed consequence:** the D-612 squish fix rode the same lines — with the fixed cap back, the search bar can again clip when keyboard + full list exceed the screen (the v1.1.43 behavior, reported in the v1.1.44 round). Flagged to the user; a one-line cap change re-applies it on the old layout if ordered.
- **What STAYED:** every round-87/88 testing change (the un-killable chain D-593, TestingPalette D-594, the payloads D-595/D-596, the hub/list/run/detail rework D-597..D-611 + D-615..D-624), the MainActivity live-run pill, the mainline 1.1.20/10120 doctrine.
- **CI (the RESET verified itself):** revert commit d87b8d02 → Build APK run 36243004630 **GREEN** — the exact v1.1.43-green blob restored, verified before the docs were written.
- **The release (the user's "do the release properly" order):** `release/1.1.46` cut from the green head, the bump 10120→10146/1.1.20→1.1.46 rides the branch (D-430), the annotated tag v1.1.46 carries the honest what-you'll-see bullet body for a revert release (D-466), the Release APK run green → **v1.1.46 is LIVE** (stable latest, arm64-v8a debug APK + SHA256SUMS.txt) — the in-app updater picks it up. Run id + artifacts in the progress ledger.

### Round 90 (2026-09-27) — D-626 + D-627: the v1.1.46 device round — the sheet's two true columns WITH scroll-selection and the linked-content card, plus the suite-health ring's rounded grouped slices
- **The order:** the v1.1.46 device verdict — the reverted v1.1.43 single list is "most definitely not good" too; the two-column direction is what was wanted all along ("on the left side and the right side, two different sources should be shown… let's improve it and let's move forward and not go backward anymore"). Plus the suite-health ring: rounded spaced slices, the six sections combined into three (no spacing or rounding where the two systems meet inside a group, rounding only on each group's outer ends, never too much), and the legend aligned as a proper table. Plus: analyze the testing systems properly (recorded in doc 72 §3 — the pipeline, the isolation model, the honest gate, the forgiveness rule, the verdict math, the persistence layer).
- **The sheet, rebuilt to spec (D-626) — NOT cherry-picked:** the D-614 blob (v1.1.45/05a79f80) lacked four of the five things this round exists to fix, so the sheet was rewritten forward. TWO COLUMNS: Aniyomi LEFT, CloudStream RIGHT, each its own card (fixed heading: accent dot + label + count; own independently scrolling list; both buckets pre-alphabetized). SCROLL-DRIVEN SELECTION: the row nearest a column's vertical center becomes the selection LIVE while the user scrolls and the search bar's placeholder renames itself to that source — an interaction guard (per-column hasScrolled) keeps the seeded pre-centering scroll from ever stealing the selection, and the seed is the CURRENT selection so "Change" reopens centered on it. THE PASTE OPTIMIZED: the query is a TextFieldValue — the first-focus name paste lands with the caret at the END (the String overload could drop it at offset 0, so the first keystroke prepended); the round-85 blur contract and the D-587 IME watcher unchanged. THE HINT FIXED: "Tap a source to select it, then search above." THE LINKED-CONTENT CARD (new): at the very bottom in both modes — cover left, name top-right, episodes · status · score · season/year underneath, "Linked via <source>" in primary; built at the call site from the UnifiedAnime into a decoupled LinkedContentInfo. THE IME-AWARE CAP RESTORED: the D-612 mechanism with the card in the reserve (300dp, floor 140dp) — the round-89 disclosed squish trade-off closes.
- **The ring (D-627):** new GroupedDonutChart in TestingCharts.kt — three verdict groups, the two systems' sub-arcs meeting inside each group with NO gap and NO rounding (butt-cap color seams), round caps ONLY on each group's outer ends (zero-sweep StrokeCap.Round arcs exactly on the boundary, in the touching sub-arc's color), the group gap DERIVED from the cap geometry (2×capAngle+3° ≈ 23° — neighboring caps can never touch; the visual gap lands ≈1.15× the stroke width), per-group idle-track underlays so the gaps stay pure card surface, zero-count groups contributing nothing, a single-group ring centering its gap at 12 o'clock. The legend rebuilt as a FIXED-COLUMN table — [swatches][total][label centered][A][+][B], structurally identical rows (the split always renders, each count in its system color) — every row lines up with its siblings, exactly the user's alignment spec. The stats screen's own donuts untouched (different surface, not in the order).
- **CI:** implementation commit 3f81856f → Build APK run 36248211351 (recorded in the progress ledger).
- **The release (the standing per-round debug loop, D-565):** `release/1.1.47` cut from the green round-90 ledger head, the bump 10120→10147/1.1.20→1.1.47 rides the branch (D-430), the annotated tag v1.1.47 carries the what-you'll-see bullet body (D-466), the Release APK run green → **v1.1.47 is LIVE** (stable latest, arm64-v8a debug APK + SHA256SUMS.txt) — the in-app updater picks it up. Run id + artifacts in the progress ledger.

### Round 91 (2026-09-27) — D-628..D-633: the v1.1.47 device round — the sheet's five-row wheels + the extension-side card, and the testing system's depth pass
- **The wheels (D-628, the headline):** the user's v1.1.47 sheet verdict ("not handled properly — a lot of issues"), executed item for item — the sheet is capped at 60% of the screen (chrome reserve 330dp); each column is a FIXED five-row 36dp WHEEL (snap fling + half-viewport contentPadding so even the first/last rows center with empty space above/below; tap-to-center via the canonical animateScrollToItem(index, −centerPadding)); non-selected rows GRAYED with a 1.2dp slight blur (readable, never a smear; no-op below API 31); the column holding the selection lights up (accent border + tint) and its heading wears an accent gradient band + count chip + hairline; 16dp of real space between the systems and the search bar; the ime guard keeps its D-612 lineage.
- **The card goes extension-side (D-628):** "it is showing the details from AniList… it should be showing the details from the extension side" — extensionBase is now a MutableStateFlow-backed observable property, and the DetailsScreen builds the card from the EXTENSION's own title/cover/status/score/year + the episode count from the linked source's OWN episode list; AniList-only entries render the sparse honest "No source linked yet" card.
- **Suite health (D-629):** labels "pass"/"fail"; each legend row in its own hairline-bordered depth section, separated by spacers; the D-627 aligned table + ring geometry untouched (the user confirmed them good).
- **The home depth pass (D-630):** rectangular recently-tested chips tinted with each extension's OWN icon color (new rememberIconTint in :core:designsystem — the CoverColorExtractor swatch pipeline over a bounded icon bitmap, Palette resizeBitmapSize(48), letter-tile hue fallback); system cards with Tv/Cloud glyphs in tinted wells + hairline borders; the command footer's two buttons became rounded RECTANGLES — Run-all a SOLID MUTED ACCENT (lerp(primary, background, 0.20)) labeled just "Run all tests" behind a CONFIRMATION dialog; and THE SPLIT LIVE EXPERIENCE — the top banner shows the extension name + the STAGE (a poll-loop machine queues verdicts, each holding ≥1s, yielding to the live stage — "some padding there going on"), the footer shows the live elapsed time + n-of-m + Stop. Header blur joins all five testing pages.
- **The list pass (D-631):** headings are just "Aniyomi"/"CloudStream"; the lang·system row subtitle is gone; a never-run expansion shows the test MANIFEST (name + theme-color dots) until results exist.
- **The full-details pass (D-632):** the dossier COLLAPSES by default (verdict chips + meta grid on tap; Language moves into the grid); "Stage timings" is its own card with the EQUAL-BY-DEFAULT model (untested = equal 10s shares, the running segment starts smallest and grows live, finished = actual durations, every weight animates); the run pill is a solid muted-accent "Run Tests" and nothing else; the header gear opens the extension's own settings (SourcePreferences via MainActivity); centered timeline bubbles; compact bright name+color sections for never-run kinds (no "Not run yet"); animateContentSize so completions expand smoothly; the verdict banner is SOLID (surfaceContainerHigh + shadow + accent leading edge) with darkening gradient scrims above and below.
- **The run screen's free back (D-633):** the leave guard is GONE from the Test Run screen (back pops, the run continues in the app-scoped controller); the prompt lives on the home screen only.
- **CI (honestly over the D-472 ≤2 budget by one):** run 36257993430 FAILURE (Int×Dp operand order ×2; rememberSnapFlingBehavior's `lazyListState` parameter name — designsystem GREEN), run 36258319641 FAILURE (one error: the home banner's AnimatedVisibility bound to the ColumnScope member extension and rejected in the new receiver tower — anime-details GREEN, the rest of extensions-settings type-checked), run 36258963397 on 196cfbc4 **GREEN** (the banner extracted into a top-level LiveRunBanner composable).
- **The release (the standing per-round debug loop, D-565):** `release/1.1.48` cut from the green round-91 ledger head, the bump 10120→10148/1.1.20→1.1.48 rides the branch (D-430), the annotated tag v1.1.48 carries the what-you'll-see bullet body (D-466), the Release APK run green → **v1.1.48 is LIVE** (stable latest, arm64-v8a debug APK + SHA256SUMS.txt) — the in-app updater picks it up. Run id + artifacts in the progress ledger.
