...
## Task 43 — CloudStream Device-Feedback Round 2 + Provider EXECUTION Phase 1 (2026-08-29, D-333..D-335)

- **process:** version-bump protocol restored — v0.2.64 (versionCode 64); every downloadable build gets a bump (the round-2 report flagged the stuck 0.2.63 across Tasks 41/42; Task 42's build was never even released).
- **process:** core-rule-8 ruling recorded — GitHub Actions is the PRIMARY verification path (used freely, multiple fix-and-repush rounds); local Gradle only when trivially cheap; this session ran CI-only (sandbox wiped = re-setup resource-intensive).
- **fix:** extension source tabs LEFT-aligned (round-2 reversal of round-1's right-edge request).
- **fix:** repo-row type badge moved onto the TITLE line (was vertically centered across the two-line row — read as "aligned to the URL too").
- **fix:** PARALLEL INSTALLS — Pending state + download moved OUTSIDE the install mutex (a second install previously blocked silently with no row feedback); only the instance swap + load + refresh serialize; per-plugin double-tap guard.
- **feat:** TRUST FLOW — `CsPluginRecord.isTrusted` gates code execution; new Untrusted section (listed but never classloaded — aniyomi TrustService parity); Trust loads + promotes; Untrust unloads + demotes; updates preserve trust; legacy records grandfathered as trusted (unit-locked).
- **feat:** PLUGIN DETAIL PAGE — every CS row clickable → new `CloudstreamPluginDetailScreen`: identity/status header, authors/description/language/size (disk-or-catalog)/NSFW/repo linkage, Supported Modes chips (tvTypes), live provider list (lang/types/providerType/browse support), per-state actions (Trust/Untrust/Retry/Uninstall/Install). Catalog metadata captured at install — renders fully after repo deletion.
- **feat (D-334):** provider EXECUTION phase 1 — `CloudstreamContentRepository` (sources/mainPage/search/load with `Anikuta:Data:Cloudstream:Exec` prefixed + timed logging, D-304 dedupe, never-silent errors); Search page integration via string `sourceKey` identity ("cloudstream:<provider>", aniyomi path byte-identical); picker sheet gains Anime Extensions + CloudStream sections; `ExtensionNoBrowse` honest state for search-only providers; NSFW gate filters the picker; selection healing; NEW `:feature:cloudstream-content` module — `CloudstreamContentDetailsScreen` (poster hero/type/year/score/status/duration, tags, description, season-grouped episodes with Dub/Sub labels, movie entry, Loading/Error/Retry, explicit "playback next update" note — loadLinks + real extractors stay doc 23 §7's next session).
- **divergence (user direction over docs 16/18):** CS rides the EXISTING search page instead of the deferred 5th Cloud tab.
- **status:** CI rounds converging (7aadb9d2 → 5b4a5c95 → d389058f → c805b21e → 888971af → 7ee1cf21: DubStatus property name, cross-module smart-cast, sealed-base elvis chain, Errored-has-no-repoUrl, navigateToDetails NavKey widening); tag v0.2.64 + release after green.

## Task 44 — CloudStream Device-Feedback Round 3: Execution Root-Causes + Sectioned Browse (2026-08-29, D-336..D-339)

- **fix (D-336):** ClassCastException on trust (MovieBoxProvider) — the plugin casts `load(context)`'s context to AppCompatActivity (upstream passes `this@MainActivity`); our loader passed the app context. MainActivity now extends **AppCompatActivity** (theme parent → `Theme.AppCompat.NoActionBar`, all visual attributes pinned — rendering identical), registers with `CommonActivity` (identity-guarded clear in onDestroy), and the loader passes the LIVE activity to `Plugin.load()`; `appcompat:1.7.1` added (also satisfies androidx.appcompat.* references in real plugin dexes — parent-first); `AnikutaApp : CloudStreamApp()` wires `CloudStreamApp.context`.
- **fix (D-337):** browse/search → 0 items (AniKoto) — Cloudflare challenge interstitial on the device (site + selectors verified working with plain GETs; the sandbox received cached 200s). **CloudflareKiller is real**: challenge detection (cf-mitigated header + body markers via peekBody on 403/429/503 + 200-HTML scan), headless WebView solve on the main thread (UA pinned to USER_AGENT, cf_clearance capture, 20s watchdog, main-frame-error early-out), per-host cookie cache, **per-host solve serialization** (parallel shelf requests reuse the winner), 60s failed-solve cooldown, `CloudflareBlockedException` (friendly userMessage) when un-bypassable — never silently parsed as "no results". Wired into plugin `app`/`insecureApp`; logs under `Anikuta:Data:Cloudstream:Net` (`cf:` prefixes).
- **feat (D-338):** SECTIONED browse — `browseSections()` fetches EVERY provider shelf (page 1, parallel, per-shelf failure tolerated + logged, all-CF-blocked rethrows); `ExtensionBrowseSuccess` renders titled horizontal rows ("Latest Updated", "Most Popular", …) reusing the flat-grid card at 110dp; search stays the flat grid; aniyomi path untouched.
- **fix (D-338):** detail-page buttons per the round-3 spec — available → Install at the VERY BOTTOM, full width; untrusted → [Trust Plugin][Uninstall] side-by-side under the header; trusted → bottom [Untrust][Uninstall] row UNCHANGED (explicitly approved); errored → [Retry][Uninstall].
- **fix (D-338):** retry animation — manager `retrying` StateFlow drives a spinner on the Failed-to-Load row's retry icon + the detail Retry button ("no animation while it was reloading").
- **status:** CI chain: 2 red rounds (kotlinx extension fns can't be called fully-qualified; retry spinner takes a Boolean, not the Set) → GREEN d2748114 → tag v0.2.65 + release.

## Task 45 — CloudStream Device-Feedback Round 4: 8KB-Truncation Root Cause + STANDARD Details Screen (Source Bridge) + Untrust-in-List + CF Manual-Solve Loop (2026-08-29, D-340..D-343)

- **fix (D-340):** THE round-4 root cause — `NiceResponse.readBody()` did ONE `source.read(buffer, MAX_TEXT_SIZE)`, but okio's `Source.read` performs a SINGLE underlying read returning at most ONE 8KB segment: EVERY body > 8KB was silently truncated. This alone explains all three reported symptoms: AniKoto search "0 item(s)" in 150ms (HTML cut after `<head>` → jsoup finds no items, no error), Anikage `JsonEOFException` at column 8083 (~8192 bytes of CJK JSON cut mid-token), and browse shelves empty on EVERY provider; AllMovieLand's 6 results only worked because its response fit in one segment. Fix: loop until EOF (cap 5MB). Plus the diagnostic layer the report demanded: `CsNetLoggingInterceptor` (`http: →`/`http: ←` lines with status/type/len/duration + WARN body snippet on non-2xx) and `body: read N chars … first="…"` per read — one filter (`Anikuta:Data:Cloudstream:Net`) shows the whole plugin network pipeline.
- **feat (D-341):** CloudStream results now open the **EXACT SAME details screen as aniyomi** (round-4 directive; the custom CS details page from Task 43 is DELETED — `:feature:cloudstream-content` removed entirely). Mechanism: the **source bridge** — every trusted provider exposed as an `AnimeHttpSource` (`CloudstreamAnimeSourceBridge`, stable synthetic id `0x4000… | hash(name)`, resolves the live provider by name per call) and merged into ExtensionManager (`setExternalSources`, wired in AnikutaApp). Details/episodes/save button/back/tags/background/auto-link all flow through the untouched aniyomi paths; plugin-thrown Errors are converted to Exception (the aniyomi catch sites catch Exception); `getVideoList` honestly reports "CloudStream playback arrives in the next update" (loadLinks session). Picker excludes bridged ids from the aniyomi section; the details manual-search sheet gains them (manual CS↔AniList linking works).
- **fix (D-342):** Cloudflare manual-solve loop CLOSED — CS Cloudflare errors now render the `CloudflareBlocked` card WITH the "Open in WebView" button (was: a text-only error mentioning a WebView that couldn't be opened); `ExtensionEmpty` carries the provider `mainUrl` so its WebView button appears too; **CloudflareKiller merges the system WebView CookieManager cookies per host** (the manual solver writes there — previously the plugin client never saw a manual solve); the manual WebView launches with the CS pinned USER_AGENT (cf_clearance is UA-bound); more challenge markers (JS-disabled 200 interstitials).
- **fix (D-342):** **Untrust directly in the Trusted Sources list** (round-4 report: no untrust option visible) — RemoveModerator icon + confirmation dialog on every trusted row; the file stays on disk (re-trust needs no re-download).
- **status:** CI on c41cbc8d; tag v0.2.66 + release after green.

## Task 46 — CloudStream Device-Feedback Round 5: Activity-Gated Plugin Load + Picker Selection State + Details Enrichment + Trust-UX Parity (2026-08-29, D-344..D-347)

- **context:** the round-5 report VALIDATED the whole execution phase ("Every single extension or plugin is working perfectly and properly… results exactly how I hoped for them… trending, latest updates, popular movies, popular OVAs… everything is being shown properly") and the standard details page ("perfect… exactly how it was supposed to be") — the remaining findings were state-machine and enrichment defects, fixed root-cause-first below.
- **fix (D-344):** MovieBox "Failed to load" after EVERY restart (works when trusted interactively) — the manager's `init{}` ran `loadAll()` inside Application.onCreate BEFORE MainActivity exists, so `Plugin.load(context)` got the Application context and `context as AppCompatActivity` plugins threw. `CommonActivity.activityFlow` (StateFlow mirror of the activity ref) + the initial load SUSPENDS until the first Activity (15s timeout → app-context fallback); `loadedOnce` StateFlow signals first-load completion.
- **fix (D-345):** source-picker state machine — the double-checkmark (stale aniyomi row AND CS row both checked: `selectedSourceId` was passed unconditionally, now kind-gated like the CS param); heading "Anime Extensions" → "Aniyomi" (user request); the persisted CloudStream selection reset to aniyomi after a full app restart (the heal collector fired on the legitimately-empty cold-start csSources emission — it now awaits `sourcesLoaded` first, 20s bounded).
- **fix (D-346):** details-page enrichment for CS entries — (a) `SAnime.year/score` optional channel (binary-safe interface properties, only the CS bridge populates) → Year + "★ N%" + "Score N/100" render; (b) `Episode.season` encoded into episode names as leading "Season N - Episode M" tags → the existing season chip selector + per-season numbers activate for multi-season CS shows (was: one flat list); (c) relative/protocol-relative image URLs absolutized against the provider mainUrl (details poster/background, episode preview_url, grid cards) — the details poster only overwrites the incoming thumbnail when resolvable; (d) `details:` + `episodes:` diagnostic lines under `Anikuta:Data:Cloudstream:Bridge` (per the explicit "add some proper console logging" request).
- **fix (D-347):** trust-UX parity — Untrusted rows' Trust icon is the SHIELD (`VerifiedUser`, was the checkmark badge `Verified`); untrust from the Trusted Sources list is ONE TAP (confirmation dialog removed — aniyomi parity, explicitly requested; Uninstall keeps its confirm).
- **status:** CI on 5fd4a458; tag v0.2.67 + release after green.

## Task 47 — CloudStream Device-Feedback Round 6 + THE PLAYBACK SESSION: bridge getVideoList + Real Extractor Runtime + Search Memory + Year Seeding (2026-08-29, D-348..D-350)

- **context:** the round-6 report validated the MovieBox restart fix ("it does not crash… works properly"), the single-checkmark picker on live switching, rating/seasons/thumbnails on details, and the whole trust UX ("much better… satisfactory results") — and cleared the way for playback ("I think we can move on to the actual video functionality… a full-fledged working system in the end").
- **fix (D-348):** search-page memory — the CS selection STILL reset after a full restart because the Task-46 heal validated against the two-layer WhileSubscribed chain's `emptyList()` INITIAL value (the derived flow only carries real providers several dispatches after its FIRST subscriber attaches; the heal was that subscriber). `awaitCsSource` (raw-flow fast path → sourcesLoaded gate → bounded non-empty wait) now fronts the heal collector AND both CS loaders; the top AniList/Extension tab is persisted (`search_selected_source`) and restored at construction.
- **fix (D-350):** details year — the rating rendering proved the Task-46 SAnime channel correct; the provider's load() simply omits year while its SEARCH responses carry it. The search-time year now threads ExtensionAnime → AnimeDetailsKey.Extension → the stub-SAnime seed (load().year wins, the seed fills nulls; every enrich/refresh path re-seeds), and year+score persist in the additive ext-extras JSON so cache-first reopens (Library/History) keep both rows.
- **feat (D-349):** PLAYBACK — (a) bridge `getVideoList`: `provider.loadLinks(Episode.data, false, …)` with CS3 streaming semantics (ConcurrentLinkedQueues; links already emitted SURVIVE mid-call failures; total failure → IllegalStateException with the real reason; URL dedup; DRM/DASH/TORRENT/MAGNET filtered + counted; CS3 header fold with UA default; episode-level subs → Track on every Video; audioTracks) → Video with "Source - label 720p" titles (the shared resolver parses server/audio/quality tiers; unlabeled links get per-source "Mirror N"); (b) `getHosterList` throws for instant fallback (the inherited impl would GET(baseUrl+data)); (c) `videoListTimeoutMs` open val on AnimeHttpSource (CS: provider loadLinksTimeoutMs clamped 5s–8min, default 120s; aniyomi keeps 30s) replaces the resolver's fixed budget; (d) the REAL extractor runtime — 35 built-ins: the shared `extractJwPlayerLinks` engine (multi-pass unpack → sources/file/bare-URL harvest + caption tracks) for the StreamWish/VidStack/Filesim/VidHide/Filemoon/Vidmoly/Emturbovid/Mp4Upload/mirror families + dedicated Dood (pass_md5 + token/expiry), StreamTape (robotlink concat), MixDrop (MDCore.wurl), StreamSB (sources49/48/50/51 API probe), Voe (multi-layout), Dailymotion (metadata API), PixelDrain (list/file API), Ok.ru (videoPlayerMetadata API), Streamlare (stream/get POST) + the 19 census variant classes (FileMoon, VidHidePro3/5/6, OkRuSSL/HTTP, …); all registered at manager init BEFORE plugins load (reverse-order dispatch → plugin mirror subclasses win); (e) the REAL P.A.C.K.E.R. unpacker (escape-aware args extraction, balanced-paren block replacement, multi-pass — JsUnpackerTest unit-locks it); (f) the REAL M3u8Helper (#EXTM3U validation, master→variant fan-out with RESOLUTION heights, URI absolutization, quality dedupe); (g) `links:` diagnostics under Anikuta:Data:Cloudstream:Bridge (start/finish lines: per-type counts, hidden count, subs/audio totals, samples).
- **scope note:** ZERO downstream watch-flow changes were needed — VideoResolver → ResolverSheet → WatchKey → WatchScreen → MPV, episode switching (re-resolve through the bridge id), watch progress, and the apply/continue-watching system are source-agnostic Video consumers already. Downloads of CS content + the 403 re-resolve ladder (doc 19 §6–§7) remain the NEXT session.
- **status:** CI on 01c3be5c; tag v0.2.68 + release after green.

## Task 48 — Device round 7 + PLAYBACK FIXED + resilience (streaming/CLOUDSTREAM, v0.2.69)
- **THE loadLinks root cause (D-351):** the shim's generic `newEpisode(data: T)` JSON-quoted every String episode handle (upstream special-cases String — verified in both AniKoto + MovieBox smali calling the Object-typed overload); fixUrl aligned to upstream verbatim; bridge strips quotes from v0.2.68-cached episodes; regression tests pin the contract. Every CS provider's playback unblocked.
- **Instant search page (D-352):** CloudstreamBrowseCache (memory + per-provider disk snapshots, 10-min freshness TTL) + stale-while-revalidate in loadCloudstreamPopular — cached feed renders before plugins even load; refresh failures never blank a shown page.
- **403 recovery ladder (D-353):** bounded 3-step auto-recovery (same-URL retry → pinned-link re-resolve → next mirror) with position restore; deferred switch-error surfacing (8s instead of the 30s timeout); errorGeneration re-trigger for identical error strings.
- **Per-track subtitle headers (D-354):** CS SubtitleFile.headers → Track → ResolverSubtitleTrack → WatchKey 3-field wire format → PendingExternalTrack → per-request SubtitleEngine headers (per-track wins, video headers fallback); download side uses the comma-smart header parser.
- **CS downloads (D-355):** bit-62 sources mint a rotating-link ResolveContext — expired extractor links self-heal mid-download through the existing ReResolver machinery; the silent mainId-race no-op now toasts; HLS first-variant limitation documented.
- **Playback haptics + header year (D-356):** PlayerHaptic vocabulary at 6 gesture sites, gated by the new player_haptic_feedback pref (default ON); seasonYear now renders in the details header meta row next to the title.
- v0.2.69 (versionCode 69).

## Task 48.1 — Device round 8: THE CRASH + THE 428 + watch-page metadata + self-heal (streaming/CLOUDSTREAM, v0.2.70)

- **context:** round-8 report on v0.2.69 — details pages good; (a) AniKoto episode tap → resolving spinner 10–15s → APP CRASH (full logcat uploaded as USER-UPLOADS/LOGCAT.MD); (b) MovieBox episode resolves to one 720p MP4 → player → HTTP 428 Precondition Required (proxy upstream AND direct retry) → player dies; (c) no episode description on the player page; (d) no thumbnails/synopsis in the watch episode list. Two Explore sub-agents distilled the 337KB logcat + mapped every implicated code path; a Plan sub-agent verified the fix plan (GO + amendments A1–A8, all applied).
- **fix (D-357) — THE CRASH:** `CloudflareBlockedException` extended Exception, not IOException — OkHttp's AsyncCall only catches IOException, so it was rethrown on the dispatcher thread → uncaught handler → process death; the bridge's partial-links rescue never saw it. Now an IOException subclass + a terminal `CsInterceptorSafetyNet` (outermost interceptor on the plugin client: IOException passes typed, every other Throwable wraps — plugin NoClassDefFoundErrors become honest resolver errors instead of crashes) + headless-solver FAST-FAIL (zero cookies 8s after page-finish = unsolvable interactive challenge; was 20s/host serialized ×N) with 1s early-success cookie polling.
- **fix (D-358) — THE 428:** two independent comma-truncation bugs mangled every User-Agent: the proxy's MpvHeaderParser split+dropped fragments (`…(KHTML, like Gecko)…` → cut at "(KHTML"), and mpv splits http-header-fields on ',' honoring ONLY backslash escapes (verified against mpv m_option.c — quotes do NOT work). New canonical `MpvHeaderFields` (:core:network): comma-GLUING parse (proxy + SubtitleEngine now delegate) + `escapeForMpv()` applied at ALL 7 MPV boundaries (raw csv everywhere else — A1 rule). 428/429 join the ladder's looksExpired set (stale sign → straight to fresh-sign re-resolve) + PlayerObserver capture. 10 unit tests lock the parser/escaper.
- **fix (D-359) — watch-page metadata:** buildEpisodeMetadataSerialized iterated the AniList-only map (EMPTY for CS content) → player page starved of description/thumbnails/synopses. Now the union of extension episodes ∪ metadata (extension-first), newline-sanitized, desc capped at 400 (TransactionTooLargeException safety).
- **fix (D-360) — self-heal + corruption:** (a) crash-restart lands in ErrorActivity where no activity registers → MovieBox errored all session; a late-activity watcher now reloads plugins. (b) SAF openOutputStream("w") does NOT truncate (Google 146330523) — 7 .data.json files corrupted with stale tails; now "rwt" + delete-recreate fallback + per-folder write mutex.
- **deferred (documented):** DASH (.mpd) surfacing — 5 of MovieBox's 6 sources are DASH manifests mpv cannot play; needs an MPD parser (video+audio rendition extraction + mpv --audio-file composition) built against REAL manifests (the sandbox is region-blocked from the MovieBox API). Next-round item with the roadmap.
- v0.2.70 (versionCode 70).

## Task 49 (2026-08-30) — device round 9: general resolution + the console logging tool; v0.2.71

Round-9 report: "no extension resolves any videos… MovieBox cannot resolve either… make it work in general like the original app… implement a proper console logging tool… extractor hardening… AI HLS quality selection." Three research sub-agents (R9-A failure catalog, R9-B logging audit, R9-C extractor/DASH gap + live probes) + a plan-verification agent (R9-PLAN: GO with amendments 1–8, all applied) preceded any code.

- **fix (D-361) — THE DEAD-DISPATCH BUG:** loadExtractor normalized the embed URL but NOT the extractor mainUrl — dispatch could never match (scheme on one side only) → 53/80 census plugins silently resolved 0 links since v0.2.68. Both sides now normalized (`^(https?://)?(www\.)?` + lowercase + trailing-slash — the old regex didn't even cover `http://`); no-match WARNs the host; CloudflareBlockedException rethrows out of getSafeUrl; LoadExtractorDispatchTest (8 cases) locks it in CI.
- **fix (D-362) — error visibility:** VideoResolver's blanket catch turned every failure (CF block / timeout / provider down / hidden-links) into a generic "No videos available"; failures are now captured with isolation kept and rethrown when the result is empty — the resolver sheet shows the REAL reason; timeouts get distinct messages; DetailsViewModel's linked==null silent no-op now errors honestly; bridge diagnostics itemize hidden vs dropped counts.
- **feat (D-362) — CONSOLE LOGGING TOOL (user-requested):** release-available in-app console (Settings → Developer tools → Console logs): RingLogBuffer (:core:common, 10k ring) wired in ALL builds (Logger min-level INFO in release — deliberate CORE_RULES §20 reversal, D-362), plugin-facade sink (com.lagradost.api.Log — 48/80 plugins log through it) + 21 mirrored raw-Log sites, live 1s view with pause/tail-follow, text+level+namespace filters, and Export&Share = version/device header + ring snapshot + own-process logcat dump → share sheet. The next device round has DATA.
- **feat (D-363) — HLS quality selection:** the bridge expands unlabeled M3U8 masters into one link per quality variant (M3u8Helper.parseMasterPlaylist extracted pure + tested; fail-open; ≤4 fetches/resolve, ≤8 variants/master) — raw masters now give the original app's per-quality picker.
- **feat (D-363) — DASH surfacing:** MpdParser (XXE-hardened) + bridge sniffer — static single-file .mpd manifests become directly playable VIDEO links (separate audio → mpv audio-add plumbing); dynamic/multi-segment stay hidden but are LOGGED with structure. MovieBox's 5 .mpd finally diagnosable.
- **fix (D-363) — CF solver hardening:** 200-HTML challenges need ≥2 markers (Turnstile-embedding pages false-positived); solver loads the challenged PATH; the WebView ATTACHES to the live activity (1dp) — attached views pass challenge-JS probes detached headless views fail.
- **tests:** LoadExtractorDispatchTest (8), M3u8ParseMasterPlaylistTest (5), MpdParserTest (6), RingLogBufferTest (4); :core:common joined CI.
- v0.2.71 (versionCode 71).

## Task 50 (2026-08-30) — device round 10: THE SEPARATION (aniyomi restored + CloudStream upstream parity); v0.2.72

Round-10 report: "It does not resolve the videos properly for some series… the old system, the aniyomi system, is not working either. I think you have mixed them up… Cloud stream handles things a bit differently so we cannot use the exact same system… This is going to be your last attempt." Three research agents FIRST (R10-A aniyomi-path audit + old-kuta comparison; R10-B upstream recloudstream/cloudstream clone + semantic spec; R10-C 80-plugin census + plumbing audit) + R10-PLAN adversarial review (GO + amendments all applied: the `kotlin.` exclusion prefix, label-neutral dub rows instead of a dedup-key change, `data_cache_episode` purge SQL).

- **fix (D-364) — THE ANIYOMI REGRESSION:** D-294's unconditional parent-first loader made the host's serialization-2.x classes win at resolution time → NoSuchMethodError/NoClassDefFoundError for extensions bundling 1.x ("some episodes don't resolve" while browse/search worked). Old-kuta's child-first-with-parent-fallback RESTORED + hardened with parent-first exclusion prefixes (kotlin., the API, the network stack) — the instantiation-time LinkageError retry keeps D-294's crash fix intact.
- **feat (D-365) — THE SEPARATION:** `isCloudStreamBridged` dispatch marker → VideoResolver splits into AniyomiSourcePipeline (restored + fixed) and CloudstreamSourcePipeline (upstream semantics). The bridge's budget now wraps ONLY provider.loadLinks — a timeout KEEPS every streamed link (upstream never discards; 0/80 census plugins declare loadLinksTimeoutMs so all ran the 120s cliff), Cloudflare blocks keep partials, caller-cancellation never masquerades as a timeout, and the HLS/MPD expansion runs outside the budget.
- **fix (D-365) — aniyomi pipeline restorations:** probe memoization (no more wasted AbstractMethodError probe per resolve for ext-lib ≤15), honest hoster-API-only fallback errors, lazy `resolveVideo()` honored for blank/"null"-URL videos (Video.kt:81 literal-"null" now filtered).
- **feat (D-366) — 20-min CloudStream link cache** (upstream RepoLinkGenerator parity — re-resolve/mirror-switch/episode-return replay instantly; forceRefresh on the recovery ladder + dead-download re-resolve) + the v0.2.68 stale episode-cache purge (NULL/blank episode_url rows could NEVER resolve — deleted once at startup).
- **fix (D-367) — honest episodes:** comingSoon → real error (not silent empty), TorrentLoadResponse → one honest torrent row, shared dub data handles → label-neutral rows (no downstream key changes); +8 census mirror-host extractors (dood.to/.wf/d000d, vidhidevip/plus, mixdrop.ag, streamsb.net, waaw.to; 35→43) + provider-name collision WARN.
- **feat (D-368) — selection intelligence + un-mixing:** old-kuta server-grouping intelligence (audio tokens never server names; resolution priority; Server A/B/C; "All Videos" raw fallback), old-kuta 5-tier episode-switch preference (keeps server/audio/quality across episodes), and the Link-Source sheet sectioned Aniyomi / CloudStream.
- **tests:** CloudstreamLinkCacheTest (6), CloudstreamBridgeEpisodesTest (8); :core:video-resolver joined CI.
- v0.2.72 (versionCode 72).
