...
- **D-485:** the poster pipeline hardening — the composer's whole body runs on Dispatchers.IO (the blocking SQLDelight reads left the main-thread produceState/tester path that made the preview fail + the test notifications post as plain text on the user's device), the art loads are time-bounded (12s withTimeoutOrNull per image — offline-first via Coil's memory→disk→network order over the 500MB disk cache; the fallback chain still yields a banner after a timeout), CancellationException rethrown, failure logs carry the exception class.
- **D-486:** the preview's "No episodes available yet" state made reachable (the hasContent-first gate — the old failed&&hasContent ordering turned the empty state into an eternal spinner); demo-picker contract verified (feed-first → random library content with cached episodes → honest empty state; no invented samples).
- **D-487:** the Notifications screen's episode-type block mirrors the Updates screen exactly — title + description on top, the FULL-WIDTH SegmentedToggle below (was squeezed into a 200dp trailing slot); same shared UpdatePreferences keys.
- **D-488:** the library badge positions actually reach the screen — the four CoverBadgeRow call sites pass the ViewModel's episodeBadgePosition/scoreBadgePosition (they were hard-coded TOP_START/TOP_END, making the v1.1.10 TOP_CENTER default dead code); the centered row floats 4dp inside the top edge.
