...
- **Round 42 FINAL:** CI GREEN on the feature line (8432122c, after the LayoutTransition-token/removeCallbacks compile rounds) → release/1.1.5 cut at 8432122c (D-453) → release-branch CI green → tag v1.1.5 → Release APK workflow GREEN → **v1.1.5 LIVE: the debug-only release (ani-kuta-v1.1.5-debug-arm64-v8a.apk + SHA256SUMS.txt, stable, --latest)** — the debug app updates over its installed v1.1.4 in-app. ntfy.sh sent to THE-TASK-IS-DONE. **AWAITING the user's device round on v1.1.5 + the explicit merge confirmation — main has NOT received the feature merge.**
