...
**Latest session (2026-08-26, D-277..D-280): Browse hero v4 (full uncropped banner + palette gradient) + Search/Browse offline resilience — on `test-feature/video-cache-new-download` (now 67 commits ahead of `main`, **v0.2.54**, NOT merged).** User device-tested v0.2.53: the Browse hero was still "ugly, not showing things properly" — root cause: D-275's forced `aspectRatio(16f/9f)` + `ContentScale.Crop` discarded ~half of AniList's natively ~3:1 banner, and the blurred-cover strip had a hard rectangular seam + sat directly behind the foreground poster (visually redundant). **D-277 (hero v4):** banner now `ContentScale.Fit` + `Alignment.TopCenter` (WHOLE banner, no crop), card ratio 16:9 → 1.2:1 (stable pager height), the blurred-cover strip REPLACED by a palette-derived vertical gradient — cover dominant color via new `rememberCoverDominantColor` composable in `:core:designsystem` (wraps the existing `CoverColorExtractor`; inline-constructed via `context.imageLoader` — NO Koin dep added to the framework-light core module), darkened `lerp(color, Black, 0.55)` for white-text contrast; gradient `[Transparent, Transparent, coverColor, darkCoverColor]` feathers the banner's bottom edge into solid color (details-page `DetailBanner` recipe adapted — NO hard seam). Rank pill + poster bg → translucent black (readable on any cover hue). **D-278 (Search offline):** new shared `BrowseCacheCodec` in `:core:anilist` (encode/decode + SECTION_* keys — no parser duplication across Browse+Search); `SearchViewModel.loadTrending` now CACHE-FIRST — serves the cached trending browse payload instantly (the EXACT same AniList TRENDING query Browse caches) then refreshes from network → "search shows default results without internet" (falls to Idle only when no cache row exists). `:feature:anime-search:impl` gained `:core:data-cache` dep + a 5th ctor param (DataCacheRepository — already a Koin single). **D-279 (Browse partial-success):** trending fetch fail on cold start + popular/topRated cached → `Success(empty)` instead of hard Error (screen renders the cached sections); `heroItems` now `combine(_state, _popular)` with a popular fallback so the hero renders with whatever cache exists. **D-280:** "data removed after update" audit — NO code path wipes data on update (schema version 1, additive idempotent onOpen, zero startup purges; the only bulk deletes are user-triggered with confirm dialogs). Most likely environmental: APK signature mismatch → forced uninstall+reinstall (allowBackup=false = no restore), or OS-cleared `cacheDir/image_cache`. NOT fixable in code — offline resilience improved instead. Version 0.2.53 → **0.2.54** (versionCode 54); tag + release after CI green. **Sandbox was CLEARED at session start** — restored from GitHub (clone → branch checkout → worklog-tail + AGENT-CONTEXT catch-up per CORE_RULES §15). 3 parallel research sub-agents (R-1 Browse-hero code / R-2 details-page reference / R-3 db-cache-offline) → implement → compile-review sub-agent COMPILE-CLEAN (coil3 3.0.4 AsyncImage alignment/contentScale named-params verified via bytecode dump; Koin bindings; dataJson/results param names confirmed). Pushed commits `3517d414` (D-277) + `ccd8aafe` (D-278+D-279); CI pending.
