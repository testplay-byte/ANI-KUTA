...
- **Round 47 FINAL:** CI GREEN on the feature line (60975de2 + 4 fix rounds: the SQLDelight/imports/brace/align-scope chain — each root-caused, e.g. the SettingsGroupScope lambda has no ColumnScope so align() was replaced with an End-arranged Row) → release/1.1.10 cut at 4b8961bd (D-485) → tag v1.1.10 (the D-466 bullet process) → Release APK workflow GREEN → **v1.1.10 LIVE: the debug-only release, What's New = the clean tag bullets** — the debug app updates over its installed v1.1.9 in-app. ntfy.sh sent to THE-TASK-IS-DONE. **AWAITING the user's device round on v1.1.10 + the explicit merge confirmation — main has NOT received the feature merge.**
