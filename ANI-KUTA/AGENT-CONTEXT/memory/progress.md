...
- **Round 44 FINAL:** CI GREEN on the feature line FIRST ROUND (0db9a8c7) → release/1.1.7 cut at 0db9a8c7 (D-464) → release-branch CI green → the D-465 workflow commit (the routine unit-test steps removed from BOTH workflows, cherry-picked to both branches) → tag v1.1.7 → Release APK workflow GREEN (the first test-free run — faster) → **v1.1.7 LIVE: the debug-only release (ani-kuta-v1.1.7-debug-arm64-v8a.apk + SHA256SUMS.txt, stable, --latest)** — the debug app updates over its installed v1.1.6 in-app. ntfy.sh sent to THE-TASK-IS-DONE. **AWAITING the user's device round on v1.1.7 + the explicit merge confirmation — main has NOT received the feature merge.**
