...
**Latest session (2026-08-26, D-272..D-276): smart-link ad system + Browse Hero sharp-banner/blurred-cover fix — on `test-feature/video-cache-new-download` (now 67 commits ahead of `main`, **v0.2.53**, NOT merged).** New `:core:ads` module (47 modules total, was 46) — isolated + extensible ad system: `AdsConfig` (bundled in APK bytecode via `DefaultAdsConfig` object — no user-facing setting, updates when the app updates), `AdsCoordinator` state machine (Idle→AdPending→AdInProgress→AdTryAgain→Idle), `SmartLinkAdInterstitial` full-screen Compose Dialog, `AppLifecycleObserver` (ProcessLifecycleOwner — the "time spent outside" gate), `AdPreferences` (6h cooldown persisted across cold starts). Smart-link flow: tap content → interstitial → "Continue" opens URL in browser → app backgrounds (ON_STOP recorded) → user returns (ON_START) → if elapsed ≥ 15s → proceed to Details + 6h cooldown; if < 15s → "Try again" (re-opens URL); max 3 retries then safety-cap. Back = cancel (non-intrusive escape — no cooldown set). Navigation interception in `MainActivity.kt` AppRoot: ALL 10 user-tap navigate-to-Details call sites (Browse/Library/Search/Downloads/Updates/History/Profile) route through `navigateToDetails` → ad gate. Notification deep-link EXCLUDED (system-initiated, not a user tap on an entry). Browse Hero: removed the D-262 CPU `boxBlur` (banner now SHARP `AsyncImage`) + added blurred-cover bottom strip matching details-page `Modifier.blur(8.dp).scale(1.15f)` EXACTLY + lightened scrim (0.22/0/0/0.45/0.82 → 0.15/0/0/0.30/0.55). Compile-review sub-agent (Explore, Task 10): 0 errors, 5 unused-import warnings (cleaned). Pushed commit `15653cf1`; CI pending.
