...
- **Round 46 FINAL:** CI GREEN on the feature line (54990265 + the D-475 follow-ups: the SQLDelight scalar-COUNT accessor, the Flow import, the audit fixes — the Long limits, no allowHardware, the preview state, the title from main_entry, the audio-gate state, the ScrollBlurOverlay backgroundColor — through 16a70d05/f74a3266/aa8cc0b4/15c9dbc9) → release/1.1.9 cut at 15c9dbc9 (D-482) → NO branch CI run (the D-472 optimization working: release branches don't trigger build-apk) → tag v1.1.9 (the D-466 process: user-facing bullets in the annotation) → Release APK workflow GREEN → **v1.1.9 LIVE: the debug-only release, its What's New = exactly the clean tag bullets (verified via the API)** — the debug app updates over its installed v1.1.8 in-app. ntfy.sh sent to THE-TASK-IS-DONE. **AWAITING the user's device round on v1.1.9 + the explicit merge confirmation — main has NOT received the feature merge.**
