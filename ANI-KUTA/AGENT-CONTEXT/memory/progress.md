...
- **Round 45 FINAL:** CI GREEN on the feature line FIRST ROUND (0db9a8c7 → efcccf24 + the stray-patch-script cleanup) → release/1.1.8 cut at efcccf24 (D-471) → release-branch CI green → tag v1.1.8 (the FIRST D-466-process tag: the annotation carries the user-facing bullets) → Release APK workflow GREEN → **v1.1.8 LIVE: the debug-only release, its What's New body = EXACTLY the clean tag bullets (verified via the API)** — the debug app updates over its installed v1.1.7 in-app. ntfy.sh sent to THE-TASK-IS-DONE. **AWAITING the user's device round on v1.1.8 + the explicit merge confirmation — main has NOT received the feature merge.**
