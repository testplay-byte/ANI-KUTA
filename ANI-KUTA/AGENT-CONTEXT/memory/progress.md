...
## Task 63 / round 23 — the library performance deep pass + 8 fixes (v0.4.11/76)
- The v0.4.10 device round CONFIRMED (do not regress): the post-import crash fix, the plugin↔repository linkage, the Format-sources menu (position + randomization), the cover pinch-zoom.
- The v0.4.10 device round FOUND: library scrolling into never-loaded cover areas drops to ~5-6 FPS; category switch to an unloaded set lags; scrolling back to "never shown" areas jitters. Root causes (plan doc cloudstream-v2/14): R1 the app-wide Coil cap of 2 concurrent requests (round-21's search spec applied GLOBALLY — a library fling enqueues dozens of cover requests; cells leaving the LazyGrid cache window get CANCELLED and re-entering cells re-enqueue at the queue TAIL — visible covers starve); R2 every category tap re-ran the full 7-query DB pipeline; R3 chips snapped to the start edge, never centered.
- Shipped (plan doc 14): **A1** ImageLoaderFactory concurrency 2 → 8 total / 6 per-host (search-page spirit kept — bounded FIFO, OkHttp never preempts); **A2** LibraryViewModel full-set cache (allEntries + allLibraryItemRows + allEntriesLoaded; selectCategory = in-memory re-filter ~1-2ms on Default with a race guard; full DB reload stays for init/PTR/silent resume refresh); **A3** category chips auto-CENTER (snap → measure → negative-offset scrollToItem; instant on first composition, animated on manual taps; edges clamp = the spec's "excluding the left and right side ones"); **B** the downloads tag lost its parentheses and bolds ONLY the episode count (buildAnnotatedString); **C** background-task STATUS notifications — UpdateProgressNotifier interface in :core:updates + engine calls (onCheckStart/onProgress/onFinish/onFailed) + UpdateProgressNotifierImpl in :app (silent IMPORTANCE_LOW channel anikuta_update_progress, ONE notification id, 500ms throttle, SecurityException-safe) covering the periodic worker AND the manual check; **D** Developer tools + console logging REMOVED (feature:debug-bubble + core:debug-api modules, dual debug/release source sets, DebugSettingsScreen, ConsoleLogsScreen, DebugPreferences, ResolverDebugReport + tests, EpisodeListDumper, LogAppender/RingLogBuffer + test, every gated affordance in ResolverSheet/PlayerSheets/CsSourceListUi/CsResolveSheet/WatchScreen/DetailsScreen/DetailsViewModel/BrowseScreen/DownloadsScreen, Settings rows, MainActivity keys, gradle wiring; Logger back to CORE_RULES §20 shape — v/d/i debug-build-only, w/e ALWAYS; serverNameOf/groupServers + the CS long-press URL copy STAY user-facing); **E** the Profile genres radar CATEGORY filter (All + library categories — the AniList statuses ARE the category names; genre counts + the genre sheet restricted to the filter's mainIds; persisted via profile_genre_filter, default All, gone-category → All); **F** the CS browse fixes — CsBrowseSection.shelfIndex = the ORIGINAL mainPage index captured BEFORE compaction (the mixing root cause: the search page derived the index from the COMPACTED list position, so one failed shelf shifted every later row's subpage onto the WRONG category), same-title sections MERGE case-insensitively (concat + distinctBy url + re-cap, first shelf's index), CsBrowseDisplayRow.title = the restore-side validation key (size/index/title mismatch → fresh shuffle; put() carries the previous display so background refreshes keep the round-22 arrangement), LazyColumn row keys = the stable shelfIndex; **G** Watch Activity left day markers lifted 4dp (offset y=-4 + bottom padding 20→24 keeps the alignment math); **H** DB hardening — the covering index idx_library_item_main_added(main_id, added_at) (.sq + idempotent onOpen migration) for the getAllLibraryMainEntries GROUP BY, deleteCategoryAndMoveItemsTo = ONE transaction (kills the per-item implicit-transaction INSERT loop).
- Verification: the reference sweep for every deleted symbol CLEAN (comments only); signature sweep (ResolverSheet/QualitySheet/SettingsScreen/UpdateEngine/CsBrowseSection/CsBrowseDisplayRow/GenreRadarChart call sites) CLEAN; import sweep CLEAN; the full-diff static review found ONE blocker (kotlinx.coroutines.withFrameNanos — it is androidx.compose.runtime's API) — fixed before the push.
- Aniyomi stack: additive/display-layer only; engine/resolver/MPV/download byte-untouched. Reveal-once covers, draw-phase fades, D-285 batching, H2/M3 stay.
- NEXT: push → CI green → tag v0.4.11 → release → the 2 ntfy.sh notifications → the user's device round (gallery-grade library scroll, chips centering, the notification-bar check status, the clean settings page, the radar filter, correct + deduped CS categories).
