...
## Task 73 / round 33 — the v1.1.1 PUBLISHABLE release track (merge → release/1.1.1)
- The v0.4.20 device round: FULLY APPROVED — "The subtitles now show how they are meant to be and all the other things are well managed, well handled, and properly working." The subtitle system is CLOSED.
- **A (the merge)**: streaming/CLOUDSTREAM-V2 (111 commits, 0 divergence) merged into main as `--no-ff` (fe15f95b) after the pre-merge workflow housekeeping (the standing test-feature/** trigger note honored + release/** added, 128c78e9). **CI GREEN 33913230117 on the merge commit** — the user's "verify by building from the main branch" satisfied. DASHBOARD paths untouched (no Pages deploy triggered).
- **B (release/1.1.1, from merged main)** — the publishable line, six decisions:
  - **D-409 THE DEBUG BUBBLE REMOVED COMPLETELY**: the whole :feature:debug-bubble module (15 files) + :core:debug-api (3) + app/src/debug + app/src/release source sets (6) DELETED; unwired at settings.gradle (2 includes), app/build.gradle (2 deps), AnikutaApp (debugKoinModules + the OkHttp/SqlDriver debug wrappers → bare factories), MainActivity (DebugBubbleHost + the hoisted debugContext + the two debugapi provides — LocalLibrarySelectionMode kept), the 4 screens' DB-7 writer blocks + their 4 gradle deps, the DebugSettingsScreen Developer-tools section, the SettingsScreen row subtitle. This also FIXES the latent assembleRelease break (the release DebugInit imported the debug-only DebugBuildInfo — release never compiled before). KEPT per instruction: show-sources + copy-button + update-check history + the crash reporter + the debug keystore.
  - **D-410 THE EMPTY-RUN NOTIFICATION SILENCED**: UpdateProgressNotifierImpl.onFinish early-returns on totalChecked == 0 (the report's "nothing was due for a check this run" fired at every app open — WorkManager's REPLACE-on-open policy lands the periodic check at open). Real runs still notify (found episodes or not); the live progress notification, onFailed, and every per-episode notification untouched.
  - **D-411 THE PUBLISHED REPO**: the updater's Koin owner → Confused-Creature-180 (AppUpdateModule), the sheet's release URL, and AndroidConfig versionName 1.1.1 / versionCode 10101 (the major·10000+minor·100+patch scheme — 10101 > 85 keeps install-over monotonic).
  - **D-412 THE RELEASE LOG STRIP**: Logger.setEnabled(BuildConfig.DEBUG) (the lambda-based wrapper → ~1.2k call sites dead in release, R8 strips them) + the com.lagradost.api.Log facade gained an enabled gate (D/I/W silent in release; E kept for support), set from Application.onCreate.
  - **D-413 R8 + RESOURCE SHRINKING + SIGNING PLUMBING**: the release buildType flips to isMinifyEnabled + isShrinkResources (R8 FULL MODE); app/proguard-rules.pro rewritten with the plugin-compat keep surface (com.lagradost/eu.kanade/injekt/gson/jackson/okhttp3/okio/appcompat/kotlin/kotlinx — plugin dexes resolve host classes BY NAME parent-first, R8 cannot see those refs), the JNI surfaces (is.xyz.mpv + arthenica), WorkManager workers (+constructors), serialization companions, and SourceFile/LineNumberTable (mapping.txt uploaded per CI build). DexGuard assessed + HONESTLY declined (commercial Guardsquare license — cannot be provisioned; R8 full mode is the standard equivalent). CI builds assembleRelease ON EVERY PUSH (a keep-rule regression breaks at the commit that caused it, not at tag time) + the release APK ABI-checked + mapping.txt artifact; timeout 30→45. The release signing is keystore.properties-driven (gitignored; ABSENT ⇒ unsigned release that still builds = the CI verification path; PRESENT ⇒ signed — the file is written from CI secrets in the FINAL phase).
  - **D-414 THE DEPENDENCY/CATALOG CLEANUP**: logcat lib (14 modules, zero imports), stray rxjava (player/video-resolver/anime-details-impl — source-api KEEPS rxjava+rxandroid: the plugin ABI), seeker + androidx-media + truetype-parser (core/player, zero imports) — all removed + their catalog entries; the 5 HARDCODED deps (extensions-settings/impl ×4 incl. an appcompat 1.6.1-vs-1.7.1 pin mismatch, notifications ×1) moved into the catalog (fragment-ktx entry added).
- The new-repo setup + signing guide: DOCUMENTATION/release/REPO-SETUP-AND-SIGNING-GUIDE.md (creating the APK-only repo, the updater's exact discovery contract, the release-creation steps, the keystore rules).
- **The pre-push compile-verification agent found ONE blocker (a Python-style `#` comment leaked into core/notifications/build.gradle.kts — a guaranteed configuration-phase failure) + the worker-rule constructor hardening; both fixed before the push.** Everything else verified clean: zero residual debug-bubble/debug-api references in code, all 21 build scripts brace-balanced, the version catalog consistent, both workflows YAML-valid, the proguard packages all real.
- The FINAL phase (after the user provides the keystore details — ASKED FIRST per instruction): generate/obtain the release keystore → CI secrets → release-apk.yml converts to the signed assembleRelease → tag v1.1.1 → the SIGNED APK downloaded → the password-protected zip (APK + keystore + keystore.properties + signing-info.txt) → password in chat ONLY.
- **CI: THREE rounds to GREEN (the R8 verification, honestly recorded):** (1) d1068ad2 — the Kotlin-DSL script-compilation lesson (a fully-qualified `java.util.Properties` reference does NOT resolve in build.gradle.kts — the `java` name is shadowed in the DSL receiver chain; fixed with the standard `import java.util.Properties` + an explicit local, cd7ded02); (2) cd7ded02 — the classic R8+jackson missing-class pair (java.beans.ConstructorProperties + java.beans.Transient — jackson-databind's Java7SupportImpl references the desktop-JVM-only java.beans package, a path that can never execute on Android; the standard `-dontwarn java.beans.**`, 95a18448); (3) 95a18448 — **CI GREEN 33917309841: all unit tests + assembleDebug + assembleRelease (R8 full mode + resource shrinking) + the ABI check on BOTH APKs + the mapping.txt artifact (8.4MB) + the APK artifact (73.1MB — debug + unsigned release)**. Everything before minifyReleaseWithR8 was green from round 1 (tests, debug build, all release compilation) — the two failures were both first-R8-run classpath surface polish.
- **NEXT (the FINAL phase — ASK-FIRST per the user's instruction):** the keystore details are requested from the user (fresh generate vs provided; passwords generated vs provided; alias/validity/DN); then keytool → GitHub Actions secret → release-apk.yml converts to the signed assembleRelease → tag v1.1.1 → the signed APK downloaded → the password-protected zip (APK + keystore + keystore.properties + signing-info.txt) → password in chat ONLY → the user creates the release in Confused-Creature-180/ANI-KUTA from the guide (DOCUMENTATION/release/REPO-SETUP-AND-SIGNING-GUIDE.md).

## Task 74 / round 34 — the v1.1.1 release round (share robustness, co-install, app icons, THE SIGNING)
- Round 33's state confirmed by the user: merge + main-build verification + release/1.1.1 + the publishable polish all APPROVED ("good work" on every line); the signing details were answered: brand-new keystore, generated passwords (shared in chat only), default alias, 25-year validity, default certificate name.
- **The local-build capability (this round's unlock):** the Android SDK is now installed in the sandbox (/home/z/android-sdk — JDK 21 Temurin + cmdline-tools + platforms;android-36 + build-tools 35/36; dl.google.com verified fast). assembleDebug ran GREEN locally (the round's compile verification: the coil3 import + the hoisted override local + the missing MainActivity import were the three fixes). R8 cannot finish inside the sandbox's 10-minute foreground-process limit (background processes are killed between commands — tested twice) → the shipped APK is compiled by CI and SIGNED LOCALLY with apksigner (the keystore never touches GitHub — the security intent honored exactly; CORE_RULES "CI is the compiler of record" kept: CI compiles, local signs).
- **D-415 the share bulletproofing** (the round's device report: loading forever, sheet never appears, no error): 4s hard icon-fetch bound (the trickle-server readBytes() class), 15s hard total bound, pre-flight source check, Activity-context chooser launch, cancellation rethrow + timeout-specific messages, per-cause toasts, stale-export sweep.
- **D-416 the co-install line:** debug applicationIdSuffix .debug + versionNameSuffix -debug + the old lime icon via app/src/debug/res overlay + "ANI-KUTA Debug" label. Verified against the built APK: package com.confused.anikuta.debug, all 8 aliases resolve to com.confused.anikuta.icons.IconV{n} (namespace-based classes — the ComponentName construction is suffix-safe by design), the debug default icon = @color #ffb1f256 + the old foreground drawable.
- **D-417/D-418 the app-icon system:** the release icon = the user's kawaii-mouth image (adaptive full-bleed bg + transparent fg + legacy WebPs all densities; the old anydpi vectors deleted); 8 aliases (V1 original + 7 AI-generated same-style variants, VLM-verified each) switch the real home-screen icon; the App Icon page (Appearance → very bottom, below the Details page row) = hero + baked grid + GitHub catalog (Confused-Creature-180/ANI-KUTA icons/ folder — live fetch, icon-NN match, offline cache, honest in-app-only note for unbaked icons) + custom-image import (center-crop 512). The icons/ starter set (8 × 512px PNGs) rides the delivery zip for the user to upload (the token is PULL-ONLY on the new repo — verified via API; push stays on the source repo).
- **D-419 the minimal debug descriptions** (the user's exact three strings).
- **D-420 the keystore + delivery:** JKS (distinct store/key passwords — PKCS12 can't carry distinct ones), RSA-2048, 9131 days, alias anikuta, CN=ANI-KUTA/OU=ANI-KUTA/O=Confused-Creature-180/C=US; three strong random passwords (store/key/zip) generated locally; the delivery = ONE pyzipper AES-256 zip (CI-built signed release APK + co-installable debug APK + keystore + keystore.properties + SIGNING-DETAILS.txt + icons/ + README-FIRST.txt) uploaded as a temporary `assets-v1.1.1-delivery` release asset (deliberately NOT v* — release-apk.yml would auto-publish a debug-signed APK) with the password in CHAT ONLY; deleted after the user confirms safekeeping. NO v1.1.1 tag on the source repo — the published-repo release is the user's own creation per DOCUMENTATION/release/REPO-SETUP-AND-SIGNING-GUIDE.md (updated with §4a icons/ + §4b delivery).
- NEXT: push → CI green (tests + assembleDebug + assembleRelease-R8) → download the artifact → local apksigner sign → verify (v2/v3 schemes + aliases in the release APK) → the AES zip → upload → password in chat → the release-vs-main summary for the user's porting decisions.
