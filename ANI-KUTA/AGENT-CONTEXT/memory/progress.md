...
- **Round 43 FINAL:** CI GREEN on the feature line through 1 fix round (3648ee2c → the GradientDrawable-alpha ValueAnimator round → 4b7b2042) → release/1.1.6 cut at 4b7b2042 (D-459) → release-branch CI green → tag v1.1.6 → Release APK workflow GREEN → **v1.1.6 LIVE: the debug-only release (ani-kuta-v1.1.6-debug-arm64-v8a.apk + SHA256SUMS.txt, stable, --latest)** — the debug app updates over its installed v1.1.5 in-app. ntfy.sh sent to THE-TASK-IS-DONE. **AWAITING the user's device round on v1.1.6 + the explicit merge confirmation — main has NOT received the feature merge.**
