...
- **Round 41 FINAL:** CI GREEN on the feature line through 3 fix rounds (6e999daa → the LayoutTransition import + the dp-helper rounds → 056373e2) → release/1.1.4 cut at 056373e2 (D-450) → release-branch CI green → tag v1.1.4 → the REWRITTEN Release APK workflow GREEN → **THE FIRST DEBUG-ONLY RELEASE LIVE: testplay-byte/ANI-KUTA v1.1.4 (stable, --latest, EXACTLY 2 assets: ani-kuta-v1.1.4-debug-arm64-v8a.apk 66.6MB + SHA256SUMS.txt)** — the standalone debug app (com.confused.anikuta.debug) updates itself in-app from it. The v1.1.3 all-ABI publication DELETED per D-447 (the git tag remains). CORE_RULES §8 release bullets updated (the debug-only line + the D-442 from-main default noted as superseded by the round-40/41 from-feature-branch instructions). ntfy.sh sent to THE-TASK-IS-DONE. **AWAITING the user's device round on v1.1.4 + the explicit merge confirmation — main has NOT received the feature merge.**
