...
**Latest session (2026-08-30, Task 50 / D-364..D-368): DEVICE-FEEDBACK ROUND 10 — THE SEPARATION: aniyomi restored + CloudStream upstream parity — v0.2.72.** Round-10 report (last-attempt round): "It does not resolve the videos properly for some series… the old system, the aniyomi system, is not working either. I think you have mixed them up… Cloud stream handles things a bit differently so we cannot use the exact same system as the old one." Three research agents FIRST (R10-A: aniyomi-path audit + old-kuta comparison + 341-APK dex survey; R10-B: upstream recloudstream/cloudstream clone → THE semantic spec — upstream NEVER discards streamed links, timeout wraps only provider.loadLinks, partials cached 20 min; R10-C: 80-plugin census + plumbing audit — 0/80 declare loadLinksTimeoutMs so every CS source ran the resolver's 120s discard-everything cliff) + R10-PLAN adversarial review (GO + amendments all applied). **(D-364) THE ANIYOMI REGRESSION:** D-294's unconditional parent-first loader made host serialization-2.x classes win at resolution time → NoSuchMethodError for extensions bundling 1.x ("some episodes don't resolve" while browse/search worked) — old-kuta's child-first-with-LinkageError-fallback RESTORED + hardened with parent-first exclusion prefixes (`kotlin.` critical — sb-template v16 extensions bundle a partial stdlib incl. Continuation; naive child-first would ICCE every suspend call). **(D-365) THE SEPARATION:** `isCloudStreamBridged` marker → VideoResolver splits into AniyomiSourcePipeline (restored: probe memoization, honest hoster-only errors, lazy resolveVideo, "null"-URL filter) + CloudstreamSourcePipeline (upstream semantics); the bridge's budget wraps ONLY provider.loadLinks — timeout KEEPS streamed partials, CF blocks keep partials, expansion runs outside the budget. **(D-366)** 20-min CloudStream link cache (mirror-switch/episode-return replay instantly; forceRefresh on the ladder + dead-download re-resolve) + the v0.2.68 stale episode-cache purge (NULL/blank episode_url rows could NEVER resolve — startup DELETE). **(D-367)** honest episodes (comingSoon error, Torrent row, shared dub handles → label-neutral rows) + 8 census mirror extractors (35→43) + provider-name collision WARN. **(D-368)** old-kuta selection intelligence (server grouping, 5-tier episode-switch, Link-Source sheet sectioned Aniyomi/CloudStream). Tests: link-cache 6 + bridge-episodes 8; :core:video-resolver joined CI. v0.2.72. **NEXT: user device round 11 on v0.2.72 — (a) ANIYOMI extensions that previously failed mid-resolve now resolve (the classloader fix); (b) CloudStream multi-extractor series resolve within/after the budget WITH partial results (no more 120s-then-nothing); (c) episode switch keeps server/audio/quality; (d) re-entering an episode resolves instantly (cache); (e) coming-soon series show the honest error; (f) console export on any failure.**
