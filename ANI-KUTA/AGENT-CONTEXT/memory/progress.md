...
**This session — Autonomous Test Controller (branch `TEST_BETA_FEATURE`, commits f7a04aa3 → 3b459ef7, NOT merged to main):**
- User instructed: build an autonomous testing system inside the app (debug-only, like the debug-bubble) that lets a remote AI agent inspect + control the running app on the user's phone — interact with the UI, take screenshots, read logcat/DB/network/activity logs. Work ONLY on branch `TEST_BETA_FEATURE`; never touch `main`.
- Read CORE_RULES + all AGENT-CONTEXT + codebase (4 parallel Explore sub-agents) before coding. 3 research sub-agents verified: (A) AccessibilityService capabilities + screenshot APIs, (B) transport — ntfy.sh limits (250 msg/day, too low) + sandbox URL discoverability (Caddy XTransformPort is generic, verified), (C) codebase patterns (debug-bubble, nav hook, reuse map).
- **Decisions D-197..D-202**: module structure (`:core:test-api` + `:core:test-controller`, both `debugImplementation`), hybrid transport (ntfy bootstrap + Bun relay :3030), AccessibilityService + triple addressing (nodeId/coords/text-query), PixelCopy screenshots (all API levels — `takeScreenshot` 2-arg form removed in SDK 36), reuse of DebugLogBuffer/DebugNetworkStats/DebugDatabaseBrowser/AnikutaDatabase/SettingsRepository, debug-only removability.
- **Implementation**: 2 new Gradle modules (46→48 modules), 30 Kotlin files, `:app/src/debug` wiring (AppRouteRegistryImpl, DebugNavBinder, DebugInit extensions), MainActivity hooks (DebugNavBinder + testTagsAsResourceId), Bun relay mini-service (`mini-services/agent-bridge/`).
- **CI compile-error fix loop (4 iterations → GREEN)**: Run 1 (8 errors) → Run 2 (2 errors: GestureResultCallback 1-param + takeScreenshot removed in SDK 36) → Run 3 (:app missing `:core:test-api` dep — Gradle implementation not transitive) → Run 4 ✅ GREEN. All fixes verified via GitHub Actions API log downloads + grep.
- **Branch**: `TEST_BETA_FEATURE` — NOT merged to `main`. User will verify on device before merge.
- **Pending**: User to paste the sandbox preview URL for the live smoke test (phone → relay connection). Optional 6-digit PIN second factor not yet implemented.
