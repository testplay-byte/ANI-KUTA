...
- **Round 40 FINAL:** CI GREEN through 3 feature rounds (the pill compile rounds: run for 6d24ddec + the icon/ABI rounds for 5413b7e4/789f4e44) → release/1.1.3 cut FROM THE FEATURE BRANCH at 789f4e44 (D-446, the user's one-off; the FIRST cut at 5413b7e4 discarded pre-publish for the D-444 artwork revision) → release-branch CI green → tag v1.1.3 → Release APK workflow GREEN → RELEASE LIVE: testplay-byte/ANI-KUTA v1.1.3 (stable, --latest, 7 assets: 4 ABI APKs + universal + ZIP + SHA256SUMS). The published-repo re-host remains BLOCKED on the release-agent credentials (same as v1.1.2 — the dev PAT has no write access to Confused-Creature-180/ANI-KUTA). ntfy.sh notification sent to THE-TASK-IS-DONE (the user's round-40 topic). **AWAITING the user's device round on v1.1.3 + the explicit merge confirmation — main has NOT received the feature merge.**
