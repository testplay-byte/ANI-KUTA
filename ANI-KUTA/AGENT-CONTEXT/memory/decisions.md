...
## D-565 (round 77): THE DEBUG-FIRST PHASE — professional releases pause, the debug line carries all future feature work; plus the project reset (the bootstrap/GitHub-Pages/dashboard housekeeping)
The professional v1.1.3 came back FULLY verified on the user's device ("I downloaded the version 1.1.3, and it updated, showed me that it will update over the old one… successfully got updated… everything was looking proper, good, clean, and beautiful… no debug features… fully satisfactory") and the user closed the release chapter: "from now on forward, we are going to be working on the debug releases and we are going to implement features there… afterwards, we will do a proper release again when everything has been finalized and properly managed." THE DOCTRINE: (1) all new features/QoL land on the mainline `feature/round-57-cloudstream-downloads` and ship via the per-round DEBUG releases — the next one is v1.1.38/10138 (the 1.1.3N debug series continues; the mainline itself stays 1.1.20/10120 per D-430 — the bump still rides the release branch); (2) the PROFESSIONAL line (release/1.1.3 + professional-vN tags + published GitHub releases) PAUSES until the user explicitly orders the next professional release — no professional bumps/tags/publishes without that order (D-425 unchanged); (3) the rest of the per-round loop is unchanged (the ≤2 CI budget, the docs set, ntfy). THE ROUND-77 RESET (docs-only + dashboard — zero APK CI): the agent bootstrap layer had rotted to the round-29 era — SESSION.md still said "Active branch streaming/CLOUDSTREAM-V2, v0.4.17, latest decision D-329" — so: SESSION.md fully rewritten to the current state (mainline identity, both live releases, the debug-first protocol, the D-425/D-430/D-440/D-445 digest, the open items), the twelve stale "Latest session" paragraphs atop progress.md relabeled **Historical** with a CURRENT STATUS block added on top, master.md's status block refreshed (56 modules / 569 Kotlin files / D-565), navigation.md's CORE_RULES section count corrected (31), NEW_AGENT_SETUP.md's `blob/main/…` link fixed (main does not exist — the default branch IS the mainline), and CORE_RULES §11's ntfy topic aligned with the actual practice (`THE-TASK-IS-DONE`). THE GITHUB REPO SETUP: the dashboard's Pages deploys had silently failed since D-552 deleted `main` — the `github-pages` environment's custom branch policy allowed ONLY dead branches (main, feature/debug-bubble, functionality/improvements, test-feature/video-cache-new-download), so the deploy job was rejected while the build job stayed green (run 35615314575). The mainline was ADDED to the policy via the environments API, the three dead-branch policies deleted (main kept for the future), and the DASHBOARD DATA refreshed by the full-stack-dev sub-agent (status-level only, no history backfill): 56 Gradle modules (1 app + 32 core + 2 data + 21 feature — 9 modules the dashboard had never recorded), 25 tables / 17 .sq files, 569 Kotlin files, D-001..D-565, both live releases, the debug-first phase banner; the local build verified (22/22 pages exported); the deploy run is expected green on push (1 disclosed CI run). Known debt DISCLOSED (also in progress.md): the dashboard's per-table DB transcription stays D-192-era and decisions D-277..D-562 remain representative — a full backfill on the user's request. LESSON (lessons-learned.md): phase boundaries are where bootstrap files rot — refresh SESSION.md + the progress.md top block at every phase change; and the Pages API 404s unauthenticated even when configured — the deploy-blocker was the environment's branch policy, not Pages itself. Record: download-research/37.

## D-564 (round 76): the return pill's lifecycle is bounded (the 10-second ready expiry + the app-close dismissal), and the professional line ships v1.1.3 — the first PUBLISHED professional release since v1.1.2 (the v1.1.37 device round)
The v1.1.37 round came back positive on the carried surface ("good work so far… handled almost everything properly") and set two directives. (A) THE PILL LIFECYCLE ("the You can go back floating overlay stays there forever… it does not disappear even if I change to some other application… if I close the application too, then it still keeps on showing" + the explicit spec: "the You can go back options will automatically disappear after ten seconds if the user does not press the Go Back button"): ROOT CAUSE — the pill had exactly ONE exit (the coordinator's state change on the user's return); a user who never returned left the TYPE_APPLICATION_OVERLAY window up forever because it belongs to the still-cached PROCESS, not to any activity. THE FIX is contained in SmartLinkReturnPillController (the pill's visuals, the coordinator's state machine, and the popup card untouched): (1) THE READY WINDOW EXPIRES — onReady arms READY_AUTO_DISMISS_MS = 10_000L; firing it plays the normal green-check exit (the stay DID complete; only the offer expired) and removes the window — the pill's total life is bounded (~5s countdown + 10s ready) in every scenario; (2) CLOSING THE APP DISMISSES — while a pill is alive, an Application.ActivityLifecycleCallbacks close-watcher counts LIVE activities (starts at ONE — show() runs inside the foregrounded single-activity UI's tap handler) and dismisses immediately when the LAST activity is destroyed (back-press finish, the recents swipe, a system kill); the zero-check rides a one-looper-tick post beat so a destroy-then-recreate swap (locale/density — rotation/uiMode are absorbed by MainActivity's manifest configChanges) never zeroes spuriously; registration happens only after addView succeeds (a failed add must not leak a watcher), `as? Application` degrades to expiry-only with a log, and hideInternal cancels the timer AND unregisters the watcher on EVERY path (the animated hide, the immediate replace, the auto-dismiss, the close-hook, the coordinator hide) so the old pill's watch never survives a re-show. (B) THE PROFESSIONAL RELEASE ("previously our professional release version was version 1.1.2, and this time the professional actual released version is going to be version 1.1.3 so that it can be updated on the old one. And it is going to be actually published, and it is actually going to be released"): THE TWO LINES made explicit — the shipped identities are the co-installable DEBUG line (.debug id, the committed debug keystore, updated in-app from testplay-byte releases, at v1.1.37) and the PROFESSIONAL line (the bare com.confused.anikuta, release-keystore-signed, D-440: its updater checks Confused-Creature-180/ANI-KUTA — the official repo whose ONLY release is v1.1.2); "updated on the old one" = the professional 1.1.2 → 1.1.3, so the release branch carries versionName "1.1.3" / versionCode 10103 (> 10102 — install-as-update; the same release keystore via the same CI secrets) while the feature line stays 1.1.20/10120 (D-430). PUBLISHING — the user's "actually published, actually released" order supersedes the D-447 artifact-only doctrine for THIS release, disclosed; the danger is verified ABSENT for this tag: the debug updater picks the highest parsed version TUPLE of the last 30 releases and `professional-v1.1.3` parses as major="professional-v1" (not an Int) → skipped outright, v1.1.37 stays the debug best; the professional release publishes on testplay-byte (where we hold write access) as stable --latest with the five release-signed APKs + SHA256SUMS + the zip; the OFFICIAL re-host (Confused-Creature-180 — where the professional app's updater actually points) stays BLOCKED on the release-agent token (verified this round: the dev PAT is pull-only there), and the artifact set is kept in the exact round-39 re-host shape. BUILD — the tag v1.1.3 on testplay-byte is TAKEN (the old internal round of September 7; never re-pointed), so release-build-once.yml gains an optional `ref` input (defaults to the tag; the edit rides the release branch): the dispatch checks out release/1.1.3 (the tag↔version gate reads the CHECKED-OUT tree and passes) while the artifacts stay named from the tag. CI ledger: EXACTLY 2 runs (implementation + the professional all-ABI build) — the release-branch push matches no build-apk.yml branch pattern, the professional tag does not match the v* release trigger, and .github/** is paths-ignored. Record: download-research/36.

## D-563 (round 75): the launcher icons are RESIZED-not-cropped (the adaptive geometry fixed), the animated hero logo returns to the popup, and the bottom bar clears the system buttons (the v1.1.36 device round)
The v1.1.36 device round accepted the icon FUNCTION, the debug page, and the entry-click trigger — then set three fixes. (A) THE CROP ROOT CAUSE: round 74 put each preset artwork in the adaptive icon's BACKGROUND layer — every layer is a 108×108dp canvas but the launcher's mask shows only the central ~72dp (~66%), so the artwork rendered ~1.5× zoomed with ~16.7% cut per side ("cropped badly on the right and left sides… the presets are shown properly, but when the app icon gets applied, it is zoomed in"); the DEFAULT icon survived because its artwork was designed full-bleed-safe. THE FIX (resources only — the D-562 alias/switch/reconcile machinery and the preview JPGs untouched): per key a NEW inset foreground (drawable-nodpi/preset_icon_<key>_fg.png — the artwork LANCZOS-resized to 66% (338px) centered on transparency; 66% of 108dp ≈ 71dp fits inside every mask shape, so the ENTIRE design is always visible — "not cropped but rather resized"), the six adaptive XMLs rewritten to background = a SOLID color machine-sampled from the artwork's own outer 6px frame (values/colors.xml preset_icon_<key>_bg — the near-uniform artwork backgrounds melt in seamlessly, no ring, no seam) + foreground = the inset PNG, and the legacy API-24/25 JPGs regenerated with the same treatment (artwork at 72% on the matched background). A machine-composited preview sheet (full/circle/squircle per preset) verified the complete mouth in every shape BEFORE shipping. (B) THE ANIMATED HERO RETURNS ("previously at the top of the pop-up it had an SVG logo, but now it does not. So I would like you to add that SVG logo with a proper animation"): the D-560 HeroBubble shape back on ALL THREE states with its original glyphs (OpenInNew / spinner / Refresh), now with TWO animation layers — a spring entrance pop (Animatable 0→1, MediumBouncy + MediumLow stiffness, scale 0.6→1 with overshoot + fade-in, fired per composition so every Crossfade state swap replays it) and a quiet breathing loop (±4.5%, 1.2s reverse, FastOutSlowInEasing) — both read inside the draw-phase graphicsLayer (zero recompositions). The frozen card shape stands: the heading stays the first TEXT element, the overlay row untouched, zero sponsor strings. (C) THE BOTTOM BAR vs BUTTON NAVIGATION ("the bottom navigation bar shows under the bottom navigation bar buttons… shown a little bit up, properly adjusted for and calculated"): the app is edge-to-edge and the pill carried only a fixed 16dp vertical padding, so with 3-button nav it rendered UNDER the system buttons. AnikutaBottomNavBar's outer Box now applies navigationBarsPadding BEFORE the fixed margin (lifts above buttons AND the gesture pill in every mode; scrolls-behind unchanged), and a new designsystem helper bottomBarClearance(base) = base + WindowInsets.navigationBars.bottom feeds the ROOT-TAB scroll tails (Browse 90dp; Library's 4 containers incl. the 160dp selection tails; Search's 5 tails; More 110dp) so the last rows clear the LIFTED pill too — lifting the pill alone would have pushed its top edge (~122dp in button mode) over the old hard-coded tails. Tail-space only: lists still draw edge-to-edge; sub-screens and the details page (frozen gradients) untouched. (D) THE RELEASE: v1.1.37/10137 — the user's "exactly the same version, 1.1.3" is the same 1.1.3x shorthand as round 74's accepted 1.1.36 order → the next record in the sequence, both releases from the SAME tag; the ≤2 budget superseded by explicit instruction, fourth consecutive cycle. Verification: XMLs minidom-parsed + every reference cross-checked; nesting-aware brace balance machine-verified on all six touched Kotlin files. Record: download-research/35.
...
### D-379 — Task 55: Media3 sidecars need LABELS + LIVE indices + content-sniffed mimes; display modes are preference-shared, replication-isolated

The v0.4.2 device round surfaced three real root causes behind the subtitle complaints: (1) the sidecar `SubtitleConfiguration` never set a LABEL, so `CsTextTrack.name` fell back to the format id — which is `"$url|$name"` — hence URL rows; (2) `selectTextTrack` built its `TrackSelectionOverride` from the sheet's SNAPSHOT group/track indices — after any re-prepare those shift, the override landed on the wrong/absent group, and clicks "did nothing"; (3) the extension-only mime guess mislabels extension-less VTT as SubRip → zero cues ("reloaded but never attached"). Fixes: labels + `CsSubtitle.displayName` (tag→locale name, URL→filename via `CsLanguageNames`), LIVE id-based index resolution in the engine, and a bounded 256-byte content sniff in the resolver (`sniffedMime`). Style customization: `CsSubtitleSettingsSheet` (cs-watch replica of the aniyomi sheet, Media3-mapped options only) writes the SAME `PlayerPreferences` and applies them live via `CaptionStyleCompat` + fractional text size. The new UX features are preference-SHARED but code-REPLICATED: `PlayerPreferences.resolveSheetFormatted` (the formatting toggle — small DropdownMenu ABOVE the sheet title, per user spec) and `EpisodeListPreferences.subDubMode` (SEPARATE default / COMBINED — the pairing helper `CsSubDubSiblings` lives in :feature:cs-watch:api so the resolve sheet, watch VM, details screen, watch page and episodes sheet share ONE definition). Aniyomi-side changes are strictly ADDITIVE (popup + raw-list branches in ResolverSheet/QualitySheet; one Display-tab section).
- **Files:** CsPlayerEngine.kt, CsLinkModels.kt, CsMediaTypes.kt, CsLanguageNames.kt (NEW), CsAudioTag.kt (NEW), CsSubtitleStyle.kt (NEW), CloudstreamLinkResolver.kt, CsResolveSheet.kt, CsSourceListUi.kt (NEW), CsPlayerSheets.kt, CsSubtitleSettingsSheet.kt (NEW), CsWatchScreen.kt, CsWatchPage.kt, CsWatchViewModel.kt, CsSubDubSiblings.kt (NEW), PlayerPreferences.kt, EpisodeListPreferences.kt, ResolverSheet.kt (aniyomi — additive), PlayerSheets.kt (watch — additive), EpisodeListSettingsSheet.kt, DetailsScreen.kt, docs 03/06.
- **Status:** ✅ Implemented (Task 55 / round 15, v0.4.3/68).
- **Date:** 2026-08-31.

### D-380 — Task 55 tail: the CI break was TWO local-scoped declaration bugs; downstream verification must be STATIC (CI aborts hide everything below the failure point)

The round-15 push (7b108be8) failed CI in 2 tasks: `:data:cloudstream:compileDebugKotlin` (139 errors, ALL cascading from `sniffSubtitleMime` being a channelFlow-LOCAL function carrying an illegal `private` modifier — Kotlin locals take no visibility modifiers, and the unresolved declaration poisoned every downstream inference) and `:core:cs-player:testDebugUnitTest` (`CsMediaTypesTestKt > initializationError` — the Task-55 sniff tests were at FILE top level, so JUnit4 ran the Kotlin file-facade class, which has no public zero-arg constructor). Because Gradle aborted at :data:cloudstream, NOTHING downstream compiled (:feature:cs-watch:impl, :feature:anime-details:impl, :app) — the round-15 UI work was entirely UNVERIFIED. Decision: before re-pushing, run a 3-way parallel STATIC compile review over the never-compiled modules (resolve-sheet group, player/subs group, animedetails+seams). It caught a real BLOCKER the compiler WOULD have hit next: the aniyomi ResolverSheet's new raw branch passed its `(ResolvedVideo, String, String) -> Unit` callback straight into `RawVideoList`, whose `onPickVideo` expects `(ResolverVideo, String, String) -> Unit` — function types are contravariant in parameters, and unrelated final classes can't bridge; the accordion branch already had the URL-lookup adapter, so both branches now share ONE extracted `pickVideo` local. Also applied: single-flavor sub/dub lists render unfiltered (DUB-only + default SUB flavor + no switcher = blank list), the sub/dub display feature is now GATED on `viewModel.isLinkedSourceCloudStream()` (structural ADDITIVE-ONLY guarantee for the aniyomi stack — the scanlator match is also narrowed to the bridge's exact "Sub"/"Dub" mirror), merged rows copy `fillermark`, and 11 extraction-leftover imports removed. The sniff function itself is now a private class member using the `subSniffClient` ctor param, with `SNIFF_HEAD_BYTES` in the companion.
...
### D-384 — Task 58: the debug toolkit is ONE flag pair gating BOTH extension stacks; the aniyomi report builder lives in :core:video-resolver (pure), the row chrome stays per-sheet

The user's v0.4.5 feedback: the toolkit was implemented "only for the cloud stream extensions" and must cover the regular ANIYOMI extensions too. Decision: the SAME DebugPreferences flags (`debug_show_resolve_sources` / `debug_resolve_copy_button`, default OFF) now gate all four resolve surfaces (CS entry sheet, CS in-player links sheet, aniyomi ResolverSheet, aniyomi QualitySheet) — one user-facing mental model, one Debug page section ("Resolve lists (all extensions)"). Placement: the pure aniyomi report builder went into `:core:video-resolver` (both aniyomi sheets already depend on it and consume ResolverServer from there) while the Compose row chrome (copy icons, URL lines, the rememberCopyFeedback helper) stays duplicated per sheet file — the replication rule (no cross-stack imports, no cross-feature imports) is preserved. Privacy rule carried over: header VALUES never ride the clipboard (keys only, sorted).
- **Files:** ResolverDebugReport.kt (NEW), ResolverDebugReportTest.kt (NEW), ResolverSheet.kt, PlayerSheets.kt, DetailsScreen.kt, WatchScreen.kt, DebugSettingsScreen.kt.
- **Status:** ✅ Implemented (Task 58 / round 18, v0.4.6/71).
- **Date:** 2026-08-31.

### D-385 — Task 58: overlay-subtitle style rides HOISTED Compose state (live while paused); geometry is MPV-unit-parity pure math

The v0.4.5 "settings apply only while playing" finding had a two-part root cause: (1) the overlay read the style NON-reactively (plain SharedPreferences getters inside composition — no snapshot state, no invalidation) and (2) the only recurring recomposition trigger was the 100 ms engine ticker, whose `MutableStateFlow` equality-dedup suppresses emissions while paused (position/duration/buffered all constant → `copy()` equals → no emission → no recomposition → no fresh style read). Decision: hoist `liveSubtitleStyle` into `mutableStateOf`, refreshed by the settings sheet's onApplySettings (which fires on EVERY change) + a sheet-open sync LaunchedEffect; the overlay consumes the state → restyling is immediate, paused or not. The formatting fixes went through the same extraction: all fractions moved to pure `CsSubtitleGeometry` (border = borderSize/55 of the font height — LINEAR, the old 0.035f-per-unit was ≈1.9× MPV and its 0.15 fraction cap saturated every setting ≥ 5; per-line ASS BorderStyle=3 boxes padded by the border width; shadow IN ADDITION to the border; no maxLines truncation) and `applySubtitleStyle` now rides the SAME fontFraction (fontScale finally scales the Media3 view too — embedded + overlay scale identically).
- **Files:** CsWatchScreen.kt, CsSubtitleOverlay.kt, CsSubtitleGeometry.kt (NEW), CsSubtitleGeometryTest.kt (NEW), CsPlayerEngine.kt, CsSubtitleStyle.kt, CsSubtitleSettingsSheet.kt.
- **Status:** ✅ Implemented (Task 58 / round 18, v0.4.6/71).
- **Date:** 2026-08-31.

### D-386 — Task 58: CS downloads reuse the engine AS-IS — identity rides (mainId, episodeKey = data handle); the resolve step moves to the SHEET (download mode), never the classic resolver

The engine is source-agnostic by construction (DownloadRequest keyed mainId|episodeKey; queue/service/SAF/notifications/DB/playback never touch extension code) and CS-bridged episodes already set SEpisode.url = the provider data handle — so a CS download needs ONLY a resolve step + a translation layer. Decision: the details page's download button routes CS-bridged episodes to the CS resolve sheet in DOWNLOAD mode (same 9-arg context as the play path — `onDownloadCsEpisode`); a pick enqueues via `CsDownloadRequestBuilder` (allHeaders → the engine's comma-safe MPV header string; provider sidecars → DownloadTracks with the same-source header fallback; DASH links filtered + counted because the engine has no DASH path; resolveContext null — CS links re-resolve through the sheet, not the aniyomi ReResolver). Offline playback of downloaded CS episodes stays on the MPV local-file path (the details page's downloaded-branch runs before the CS branch) — deliberate: identical to aniyomi downloads, no player-stack change. The classic auto path is CS-guarded with guidance text (defense in depth).
- **Files:** CsResolveSheet.kt, DetailsScreen.kt, DetailsViewModel.kt, MainActivity.kt, CsDownloadRequestBuilder.kt (NEW), app/build.gradle.kts (:core:cs-player dep).
- **Status:** ✅ Implemented (Task 58 / round 18, v0.4.6/71).
- **Date:** 2026-08-31.

### D-387 — Task 58: plugin sharing is a FILE FORMAT (.moviebox.WHITECAT = untouched .cs3 bytes + identity-bearing filename); the receiver filters by MIME and gates in-activity; imports land UNTRUSTED with repo linkage when the catalog matches

Android cannot intent-filter on file extensions for content:// URIs, so the manifest filters are deliberately MIME-based (VIEW content/file × octet-stream + zip; SEND octet-stream) and the exported PluginImportActivity gates in-activity on the display name's custom extension + the zip's manifest.json — non-plugin files that reach the broad filter are rejected gracefully. The exported bytes are the untouched .cs3 (DexClassLoader-compatible as-is); the FILENAME carries the identity (stem = internalName, mirroring the repo .cs3 naming) under the user's custom extension. ONE confirm dialog before anything installs (the user's explicit spec); the import lands UNTRUSTED (the session-3 trust model — consent adds the FILE, trusting gates the CODE), links its record to an added repository cataloging the same internalName when one exists (updates flow; otherwise repo-less, "shared-file" path salt), and hands off to the main app through a SharedPreferences pending-nav note consumed at MainActivity cold-start AND ON_RESUME (the import activity lives outside the main task).
...
### D-392 — Task 60: the subtitle overlay's line-height must be EXPLICIT (the ambient LocalTextStyle leaks a fixed 24sp line box)

The round-19 overlay Text passed `fontSize` but NOT `lineHeight` — material3's `MaterialTheme` provides `typography.bodyLarge` as the ambient `LocalTextStyle`, and `AnikutaTypography.bodyLarge` carries `lineHeight = 24.sp` FIXED. `Text(fontSize = X)` merges over it: fontSize is overridden, lineHeight is inherited — a 24sp line box against a varying effective font (~11sp at 16:9-box font 100 + scale 0.5; ~21sp at 2x; ~43sp at 4x), which is EXACTLY the v0.4.7 device readings ("way too much gap" / "overlapping" / "distorted"). The fix is structural: `lineHeight = CsSubtitleGeometry.lineHeightSp(fontSizeSp.value).sp` (fontSize × LINE_HEIGHT_RATIO 1.2 — above Roboto's natural ~1.18 line box, so lines never touch; a constant ~20% gap at every scale) + `lineHeightStyle` via a BARE `style = TextStyle(...)` (no LocalTextStyle merge — the overlay's line metric is fully self-contained). RULE: any Text whose size is computed must pin BOTH fontSize and lineHeight.
- **Files:** CsSubtitleOverlay.kt, CsSubtitleGeometry.kt (+ test).
- **Status:** ✅ Implemented (Task 60 / round 20, v0.4.8/73).
- **Date:** 2026-09-01.

### D-393 — Task 60: the formatting control lives ON the sheet heading; the legacy share extension is a REJECTION marker; the post-Add landing is the CloudStream tab

(1) The user's round-20 spec replaced the round-19 pill: the TITLE TEXT itself (only the text — intrinsic width, ellipsized) opens a small menu — a 14dp-cornered FLAT surface (shape + containerColor + tonalElevation 0) with a DISTINCT 1dp outline via `Modifier.border` (version-proof — no reliance on DropdownMenu's `border` param) holding one "Formatted sources" row + trailing Switch; the menu STAYS OPEN while toggling (the user flips on AND off inside it); outside-tap/back dismisses. Shared `CsFormattingTitle` for the 2 CS sheets + local copies for the aniyomi sheets (the replication rule). (2) Strict .WHITECAT: `"X.moviebox.WHITECAT"` ALSO ends with `.WHITECAT`, so merely removing the legacy branch still matched the old name (the offline test caught it — suffix shadowing AGAIN); the legacy tail is now an explicit REJECTION marker checked FIRST → old-format names are not shared-plugin names (identity falls to the manifest on the content-first path). (3) The extensions landing: `ExtensionsSettingsKey` object→`data class(initialTab)`; the plugin-import push sets "cloudstream"; `rememberSaveable(initialTab)` seeds the tab (StateFlow current values mean the tab row + content are correct at first frame both same-process and cold-start). (4) Download rows: the server pill is `weight(1f, fill=false)` + Ellipsis inside a weighted inner Row, the status/percentage pill outside it (both stacks — shared components). (5) The Downloaded-page crash: group by the STABLE `content.contentId` (the table denormalizes title/cover per row; different metadata on the same contentId split into two same-key LazyColumn items).
...
## D-397 (round 27): the armed delete glyph grows 2.5x in the LAYOUT phase
animateDpAsState grows the glyph's measured dp (16→40dp episode rows, 20→50dp delete-all header — 2.5x per the round-27 re-spec) and the call sites animate the IconButton frame with it (32→48dp, 36→56dp). NO draw-phase scale anywhere in the armed state: the rounded card Surface clipped the painted-outside glyph's top/left/right and the rasterized layer blurred. ARMED_ICON_GROWTH = 2.5f replaces ARMED_ICON_SCALE = 3f. The episode rows are wrapped in key(episodeKey) — positional remember slots made the post-delete moving row inherit the dead row's exit Animatables.

## D-393 (round 27): disk-truth deletion — the DB row is the key, the filename sweep is the contract, survivors are the metric
deleteDownloadedEpisode: Phase 2a = the .data.json URI deletes (best-effort); Phase 2b = deleteEpisodeFilesOnDisk(contentDir, episodeNumber-from-DB-row) — a census→delete→verify sweep of episodes/ + subtitles/ + legacy root files, matching the canonical filename token (VIDEO_NAME_REGEX ` - E(\d{5}(\.\d+)?)\.[^.]+$`, SUBTITLE_NAME_REGEX `^subtitle_E(\d{5}(\.\d+)?)_`, full-token compare), DISK_SWEEP_PASSES=3 with settle pauses, returning DiskSweepReport (survivors must be empty). Phase 4 (maybeDeleteSeriesFolder) decides by dbRemaining==0 ONLY (the .data.json count is advisory); on a failed folder delete it checks countEpisodeVideoFiles() before the DB sweep — the sweep only runs when nothing playable remains.

## D-395 (round 27): the search top bar is a nested-scroll reveal latch
ONE NestedScrollConnection on the content Box (inside PullToRefreshBox): downward delta > 8px (SEARCH_BAR_DELTA_THRESHOLD_PX) collapses, upward delta reveals; LaunchedEffect(uiState::class) re-reveals on every content-mode transition. The old OR-of-all-three-scroll-states derived expression is deleted (the Idle column's remembered ScrollState outlived its mode and latched the bar collapsed; the top-only reveal threshold never fired mid-list).

## D-396 (round 27): the update-check history records + resolves the smart-schedule math
UpdateCheckItemLog + nextAiringEpisode/nextAiringAt/learnedOffsetMs/expectedCheckAt (nullable defaults — old JSON decodes; the engine captures them from the same AnimeUpdateState it acted on, expectedCheckAt = airing + the SmartReleaseScheduler-identical clamped offset, future airings only). SmartReleaseCheckWorker.schedule stamps every one-shot with sr_main_<mainId> (mainTag()); the history VM's queryLandedScheduleByAnime() resolves each anime's earliest ENQUEUED nextScheduleTimeMillis on every refresh; CheckItemRow renders the 4-line fact panel (Next release / Learned delay / Calculated / Landed + drift), only when schedule data exists.

## D-399 (round 28): one animation progress drives the whole armed delete choreography
(D-398 is retired-unused — a numbering gap from a plan revision.) TwoStepDeleteIcon now takes a single `armedProgress: Float` (260ms tween, Motion.EasingEmphasized — the shared-element navigation's curve) and derives the glyph size (`idle + idle*(2.5-1)*p`), the staggered overlap-free crossfade (idle gone by p=0.625, armed in from p=0.375, + a 0.7→1.0 pop), while the caller derives the IconButton frame (32+16*p / 36+20*p) from the SAME progress. Rationale: the round-27 version's THREE independent animations (150ms AnimatedContent morph, 220ms size grow, 220ms frame grow) desynced — the morph landed early and the grow read as a two-stage stutter. ONE progress = nothing to desync.

## D-400 (round 28): the armed-delete state is screen-level + an ancestor interceptor disarms it
ArmedDelete(cardId, targetKey) lives in DownloadedFilesScreen (one armed button in the whole list). The content root Column installs ONE pointerInput(armedDelete) → awaitEachGesture → awaitFirstDown(requireUnconsumed=false): every pointer DOWN that passes through the tree is observed (children keep priority, nothing consumed); if the touch's window position (the root's positionInWindow + the event position) is OUTSIDE the armed button's window-space rect (reported via onGloballyPositioned → boundsInWindow(), only by the armed button), the state disarms. Covers: other rows, other cards, the back header, blank space, scroll starts. The button's own tap hits its clickable (the interceptor stays silent inside the rect).

## D-401 (round 28): the deletion pipeline is data.json-first, serialized, and strictly verified
The user's pipeline, verbatim: "if the user clicks the delete button then it will update the data.json file of that specific content and then it will delete that content properly afterwards." Phases: (0) DB capture (episodeNumber + the row's videoUri — a URI independent of .data.json); (1) findContentFolder once; (2) capture the .data.json entry (URIs) → removeEpisodeFromDataJson(contentDir, episodeKey, episodeNumber) with the 3-attempt ladder + number-drift reconciliation + STRICT verification (DeletionMatching.removalVerified: re-read must be non-null and free of the removed entries by key OR number); (3) the captured-URI deletes + deleteEpisodeFilesOnDisk (the disk-truth sweep); (4) maybeDeleteSeriesFolder (dbRemaining==0, now accurate under the lock); (5) DB row + cache LAST. Serialization: DefaultDownloadManager.deleteMutex wraps every public delete (delete-all's fallback loop runs OUTSIDE and calls the PUBLIC per-episode delete — the non-reentrant mutex can't self-deadlock); DownloadStorageProvider.treeMutex serializes ALL its tree mutations (writeDataJson/upsert/remove/replace/deleteContentFolder/deleteEpisodeFilesOnDisk). Lock order: deleteMutex→treeMutex, scanMutex→treeMutex — no cycles. The write now lands while the tree is untouched — the round-27 stale-URI window (sibling deletions invalidating DocumentFile URIs, the reason the ladder existed) is gone by construction; the ladder stays as defense-in-depth.

## D-402 (round 28): the search top-bar reveal follows the FINGER, and the top is sacred
The nested-scroll sign convention (proven from material3 1.3.1's PullToRefresh source): available.y > 0 = finger DOWN = the content toward the TOP (and the P2R pull — its own source labels the positive-y branch "Swiping down"); available.y < 0 = finger UP = scrolling INTO the content. The latch: finger up past the threshold collapses; finger down reveals; derivedStateOf on the ACTIVE mode's scroll state (Idle: scrollState.value==0; grids: firstVisibleItemIndex==0 && offset==0; browse list likewise; static states always true — the other states' objects survive mode switches with stale offsets so only the active one counts) force-reveals at the very top; the dead zone (|dy| ≤ 8) keeps state. The decision is the pure, unit-tested searchBarNextCollapsed(current, dy, atTop, threshold).

## D-403 (round 28): the onboarding setup wizard (feature:onboarding) + the no-folder gate
The wizard (single module, the feature:download precedent): WELCOME (custom non-Material animated landing — the time-driven aurora canvas + 28-particle drift field in ONE drawBehind pass, the 90ms/letter staggered ANI-KUTA wordmark with the live-accent tail, the chained tagline + gradient-glow CTA), THEME (8 curated cards — System/Midnight/Daylight/AMOLED/Dusk/Night/Teal Mist/Coral Day — ids map app-side to ThemeMode/AccentPreset/amoled; selection applies LIVE via ThemePreferences' mutableStates, the wizard itself re-themes), STORAGE/NOTIFICATIONS/BATTERY (REAL verification — DocumentFile.canWrite for the folder, checkSelfPermission, isIgnoringBatteryOptimizations — re-derived on every pref change + every ON_RESUME, never "we asked once" flags; each SKIPPABLE), FINISH (the granted-vs-skipped summary). No ViewModel — ephemeral UI + app-side persistence via callbacks. AppPreferences.onboardingCompleted gates AppRoot's start destination (never reset — once per install); FirstRunSetupDialog (the every-launch M3 nag) is DELETED; the startup update check is suppressed during onboarding. The no-folder gate: all FIVE download-initiation call sites check DocumentFile.fromTreeUri(...)?.canWrite() at TAP time (a local, main-thread-safe permission check) and show a clear "No download folder selected" AlertDialog with an inline OpenDocumentTree picker that VERIFIES the pick and RETRIES the download automatically — the other half of the skip contract.

## D-404 (round 29): every SAF write truncates ("wt"), .data.json heals, and the delete flow rebuilds from DB truth
THE root cause behind every "data.json not updated / corrupted" device report since round 25 was ONE primitive: `ContentResolver.openOutputStream(uri, "w")` on AOSP ExternalStorageProvider maps to MODE_WRITE_ONLY WITHOUT MODE_TRUNCATE (FileSystemProvider.openDocument → ParcelFileDescriptor.parseMode) — only "wt" truncates. Every .data.json write that SHRANK the content (any entry removal) left new-json-head + old-json-tail = an unparseable file; the next findContentFolder walk then skipped the (now-invisible) folder, so the LAST-episode delete skipped every disk phase. The 1-episode case always looked clean only because the folder-delete path proceeds on unreadable json — a four-round false positive. The fix stack: (1) copyFile + the cover write open "wt" + a post-write byte-length check; (2) readDataJsonIndexed SALVAGES corrupted files (DataJsonRepair.salvageCompleteJsonHead — a string/escape-aware balanced-brace scanner recovers the first complete top-level object; every read path self-heals, including the exact file v0.4.16 left on the user's disk); (3) the delete flow no longer match-and-prays — DataJsonRepair.rebuildEpisodesAfterDelete derives the new episodes list FROM THE DB (rows-for-anime minus the deleted row, DB identity fields always win, metadata enriched from the existing entries, ghosts dropped, null file healed from rows) and rewriteDataJsonEpisodes writes it through the VERIFIED ladder (truncating write → strict re-read with salvage DISABLED + exact (key,number) set equality → retry → nuclear delete+recreate — a corrupted document can no longer wall the ladder off: the read is metadata enrichment, never a gate); (4) findContentFolderByTitle (sanitized-name match, skipping folders whose valid json claims a different mainId) locates destroyed-json folders for BOTH delete paths. DeletionMatching.matchRemoval stays for the Phase-2a URI-delete entry capture; removalVerified is removed (superseded by the exact-set check). Unit-tested in DataJsonRepairTest.

## D-405 (round 29): the onboarding wizard v2 — morphing-blob welcome, snap-carousel themes, combined permission buttons, and the Browse preload
The v0.4.16 device report's redesign: (WELCOME) the background is five large MORPHING ORGANIC BLOBS (8 control points each with independent wobble amplitude/speed/phase, closed Catmull-style cubic paths, soft radial-gradient fills, Lissajous center drifts — live accent, electric violet, rose, amber, teal on a deeper base) + a rotating outline-geometry layer, replacing the radial-glow aurora + rising-particle field ("animated shapes… blobs moving around… different colors"); the "offline-first anime streaming" footer is REMOVED; the tagline ROTATES smoothly ("Your anime. Your rules." / "Your content. Your rules." / "I don't make any promises." / "Don't expect anything." / "It is what it is."); (THEME) a horizontal SNAP CAROUSEL of 7 large mock-screen cards with LIVE theme application on settle (220ms collectLatest debounce), + the System/Light/Dark MODE ROW (applies live, independent of the carousel — absorbs the old "system" card) + "You can further customize in the settings later."; (PERMISSIONS) a big 92dp centered icon, the Allow/Choose action REPLACED by a granted state once verified (the folder step keeps "Change folder"), and ONE combined bottom button ("Skip for now" while ungranted → "Continue" once granted, neutral→accent); (FINISH) a 2×2 summary grid + the relocated version line; (PRELOAD) the new BrowsePreloader warms the 3 sections' data exactly as BrowseViewModel will read them (cache-first, fetch+upsert only when missing/expired) + enqueues every cover into Coil at the EXACT render sizes (128×192dp cards, 84×126dp hero posters — SectionPreloader's memory-cache contract), gated by a once-captured remember flag so finishing the wizard mid-warmup never cancels it. D-406: the wizard⇄app handoff crossfades 250ms emphasized (quick to enter, no visual cut).

## D-426 (round 37): the D-410 port — the empty episode-check run is silent on main too
`UpdateProgressNotifierImpl.onFinish` early-returns when `summary.totalChecked == 0` — the "Episode check complete — nothing was due for a check this run." notification (fired on every app open via the WorkManager REPLACE-on-open policy) is gone. Identical hunk to release/1.1.1's D-410 (the files were identical between the lines); the live progress notification, real result notifications, onFailed, and the per-episode "new episode found" path are untouched.

## D-427 (round 37): the D-419 port — the three minimal debug descriptions on main
The Debug page subtitles replaced with the user's exact round-34 strings: Show sources → "Show stream URLs"; Copy button → "Add a copy button to stream URL"; Update Check History → "Check the history". The debug-bubble Developer-tools section is UNTOUCHED (main keeps the bubble per the user's standing round-37 instruction).

## D-428 (round 37): the D-415 port — the share button fully hardened on main
`CloudstreamPluginDetailScreen.sharePluginFile` taken wholesale from the release line (the diff was purely D-415): (1) pre-flight source check with the specific "The plugin file is missing — reinstall the plugin and try again" message; (2) the hard 4s bound around the icon fetch (the trickle-server readBytes() class); (3) the hard 15s total bound (the spinner can never hang); (4) the ContextWrapper→Activity unwrap with FLAG_ACTIVITY_NEW_TASK only when no activity is reachable; (5) CancellationException rethrown + TimeoutCancellationException gets its own toast; (6) per-cause error messages; (7) the >1h stale-export sweep.

## D-429 (round 37): the co-installable debug line + THE LATENT assembleRelease-BREAK FIX
The D-416 port: debug gains `applicationIdSuffix ".debug"` + `versionNameSuffix "-debug"` + the app/src/debug/res overlay (the OLD lime icon + the "ANI-KUTA Debug" label) — a release install and a debug build live side by side (the authorities + packageName follow the suffix automatically; the committed debug keystore keeps debug-over-debug installs). THE FIX the release line's D-409 found: `app/src/release/DebugInit.kt` imported `DebugBuildInfo` from `:feature:debug-bubble` — a debugImplementation-only classpath, so release NEVER compiled (main's CI only ever ran assembleDebug). Main KEEPS the bubble, so the release DebugInit now registers NOTHING (emptyList) — release compiles with the bubble modules still present, proven by the local pre-push :app:compileReleaseKotlin + the CI assembleRelease.

## D-430 (round 37): the release-line build config ported to main (minus the release-only polish)
(a) The convention plugin flips release to R8 full mode + resource shrinking, with the D-413 keep surface (app/proguard-rules.pro: the plugin-compat classpath, the JNI surfaces, WorkManager workers, serialization companions). (b) The keystore.properties-driven anikutaRelease signing (absent → unsigned-but-building). (c) The ABI splits block + releaseAllAbiFilters (active ONLY under -PreleaseAllAbis=true — the tag path; main pushes stay single-ABI arm64). (d) build-apk.yml gets the D-423 treatment: the secrets decode + the SIGNED push-path assembleRelease + the apksigner gate + mapping.txt. NOT ported (the user: "those things will not be followed on the main branch"): D-411's updater repoint (main keeps the dev-repo target) + D-412's release logging strip. The version STAYS 0.4.20/85 (D-425 discipline — main is the dev line).

## D-431 (round 37): the unused-dependency + directory cleanup (the D-414 port + the root audit)
Deps (each re-verified zero-import on main): logcat (14 module declarations — the com.squareup.logcat lib predates the custom Logger), stray rxjava ×3 (core/player, core/video-resolver, feature/anime-details — core/source-api's aniyomi-compat rx STAYS), seeker, androidx-media, truetype-parser; the 5 hardcoded deps (core-ktx in notifications; preference-ktx/fragment-ktx/appcompat/core-ktx in extensions-settings) moved into the catalog. Root audit: DATABASE.json (the old debug-bubble DB export) DELETED; the stray root 1788604676536.jpg moved into USER-UPLOADS/ (user data preserved; the app res embeds the processed icon). KEPT deliberately: USER-UPLOADS/ (the user's zone — the extension APKs saved there per a past user request), REFERENCES/ (the old-vs-new knowledge base), DASHBOARD/ (deployed by deploy-dashboard.yml), :feature:debug-bubble + :core:debug-api (the bubble stays on main).

## D-432 (round 37): the app-icon system rebuilt CATALOG-ONLY + the rounded display format
The user's round-37 instructions: (1) the 8 premade baked app icons REMOVED COMPLETELY ("There are 8 premade app icons on the app icon page but I told you to remove all of them and you did not. Remove them properly.") — main never had the aliases (they were release-line only); main gets the system WITHOUT them: the launcher icon is the user's kawaii-mouth icon (the release line's D-421-safe adaptive layers + legacy WebPs), the OLD lime icon becomes the debug-build identity, and the App Icon page shows ONLY the GitHub repository's icons/ catalog (live fetch → offline cache → refresh; Confused-Creature-180/ANI-KUTA/contents/icons). AppIconPreferences loses the alias machinery (catalogJson + inAppOverridePath only). (2) The display-format fix ("I told you to show them in a circular kind of format but you are showing the app icons in a circle, which is not good.") — INTERPRETED + DOCUMENTED: every icon now shows its FULL artwork in a ROUNDED-CORNER (squircle-style) cell; the round-35 page's circle clips (and pre-masked circle previews) cut the artwork to a disc — the same cropping class the user rejected on the launcher in round 35. The selection ring matches the rounded shape; the hero (84dp) + the grid cells (64dp) share the format. The honest in-app-apply note + the clear-override row stay.

## D-435 (round 38): the debug-only push path + the stale main release-apk.yml replaced
The user's round-38 instruction: "From the main branch and any other branch, it should only build the debug version, and it should not build the release version alongside it." build-apk.yml REWRITTEN: tests → assembleDebug → the ABI check (the debug APK only) → the `anikuta-apk` artifact (the debug APK only). The D-423/D-430 push-path release machinery (the keystore decode, the SIGNED assembleRelease, the apksigner gate, the mapping upload) is REMOVED from every push — release APKs exist ONLY through the tag-driven release-apk.yml. BONUS FIX found during the round: main's release-apk.yml was still the STALE pre-round-35 file (assembleDebug + a debug APK attached to releases!) — the rewritten all-ABI SIGNED pipeline had only ever existed on release/1.1.1, so a main-line tag could never have cut a correct release. Main now carries the same pipeline as the release line (minus the mapping asset — D-436). The trade-off is accepted and documented: a release-compile regression now surfaces at TAG time (not on push) — with minification retired the release compile is near-identical to debug, and the tag workflow remains the release gate.

## D-436 (round 38): the R8 retirement — minification REMOVED from the release line
The user's diagnosis of the v1.1.1 device round ("I think it is maybe the R8 obfuscation which is affecting it… Maybe we should remove the R8 obfuscation completely") was correct and is now standing policy: the release buildType in the convention plugin is `isMinifyEnabled=false` + `isShrinkResources=false`, NO proguard files. `app/proguard-rules.pro` is DELETED (the full D-413 keep-rule set is preserved in this record: SourceFile/LineNumberTable + Signature/InnerClasses/EnclosingMethod/Exceptions/annotations attributes; kotlin.Metadata; com.lagradost.**; eu.kanade.**; uy.kohesive.injekt.**; gson; jackson; okhttp3; okio; androidx.appcompat; kotlin.**; kotlinx.**; is.xyz.mpv.**; com.arthenica.ffmpegkit/smartexception.**; ListenableWorker subclasses + ctors; the serialization Companion/serializer() members on com.confused.anikuta.**; the -dontwarn set incl. java.beans). Root-cause analysis: the compat surfaces genuinely live under the kept packages (core/source-api = eu.kanade.tachiyomi.**, core/cloudstream-api = com.lagradost.**) — but keep-rule coverage can NEVER be guaranteed complete for a DexClassLoader plugin ecosystem; R8's optimization passes (inlining, merging, dead-code elimination) alter runtime-resolved behavior beyond renaming. Field evidence: the unminified debug build (identical code) worked on the user's device; BOTH R8 builds (the main push artifact + the v1.1.1 tag release) broke the extension system (some extensions loaded, others failed, the loaded ones returned no results). The APK size cost is accepted (the app is native-lib dominated). No mapping.txt exists anymore — release-apk.yml's mapping staging + asset upload removed on both lines. If minification is ever deliberately re-enabled, re-create the keep rules from this record FIRST.

## D-437 (round 38): the App Icon page's minimal empty state
The user's round-38 instruction: remove the whole description set from the icon page. The empty state (catalog empty OR unreachable — the catalogError distinction is gone from the UI) says ONLY "There aren't any icons yet." The bottom note ("Icons come from the repository's icons folder on GitHub — refresh to pick up newly added icons. Tapping an icon applies it inside the app; it becomes the home screen launcher icon when it's included in a release.") is deleted outright. The loading row is a bare centered spinner (no "Checking the icons folder on GitHub…" text). Kept: the hero + its labels, the refresh button, the catalog cells + the selected ring, the "Back to the app's icon" clear row, and the two functional toasts ("Couldn't load that icon" / "Applied inside the app") — those are feedback, not descriptions.

## D-438 (round 38): the debug identity everywhere — icon_current + the lime density rasters
The user's report: "in the debug version, the app icon was shown as the release versions." Root cause: the App Icon page hero's fallback `R.drawable.icon_current` (the kawaii artwork) lives in the MAIN source set — the debug overlay never overrode it, so the debug build's icon page showed the RELEASE artwork. Fix: the debug overlay gains its own `drawable-nodpi/icon_current.png` (288×288, the lime artwork — lime #B1F256 + the dark #14111F play triangle, generated programmatically, pixel-verified). ALSO fixed (the pre-26 gap, minSdk 24): the debug overlay only had anydpi/anydpi-v26 XMLs, and Android resource resolution prefers main's density-specific kawaii `ic_launcher.webp` rasters over anydpi on API 24–25 — the overlay now ships its own lime `ic_launcher.webp` at every density (48/72/108/144/192, matching main's exact raster sizes) + its own `values/colors.xml` (ic_launcher_background #B1F256) so the overlay is self-contained. The debug build never shows the release kawaii artwork anywhere (launcher at every API level, the in-app hero).

## D-439 (round 39): the total-when hardening — the library-open crash fixed at every sealed dispatch
The v1.1.1 device crash (`kotlin.NoWhenBranchMatchedException`, DetailsScreen.kt:367 — EVERY library open crashed the release build while the debug build was unaffected) is fixed by defense-in-depth at EVERY `when` over `AnimeDetailsKey` (7 sites total): DetailsScreen's load-dispatch (the LaunchedEffect), the line-367 cover-hint when + the title hint + the autoPlayEpisode when, and MainActivity's AppRoot dispatch-when + `handleDownloadEpisode` + `handleDownloadSpecificVideo` mainId whens. Each unknown-variant branch now logs via the NEW `describeDetailsKeyAnomaly(key)` helper (feature/anime-details/api — pure JVM, no Android deps) which captures the exact runtime identity: runtime class name, classloader class + identity hash, whether the object still passes `is AnimeDetailsKey`, and the object identity — one log line pins the mechanism if the anomaly ever recurs. Degradation behavior per site: the load-dispatch stays on its loading skeleton (logged no-op), the cover/title hints resolve to null (no shared-element morph, the skeleton renders), autoPlayEpisode null (no auto-play), the AppRoot dispatch renders nothing, the download handlers resolve mainId to null (the existing null-path handles it). Mechanism analysis (honest): the sealed hierarchy declares exactly AniList + Extension and the app only constructs those two — a runtime object matching NEITHER `is` check can only exist through a class-identity anomaly (the same class resolved through two classloaders — e.g. a plugin/extension dex carrying host-class copies, a hybrid install's leftover state, or R8's class synthesis in the round-37 minified builds the crash was reported against). The R8 retirement (D-436) removes the most plausible mechanism; this hardening guarantees NO mechanism can kill the process at these dispatch points again.

## D-440 (round 39): the update repo follows the BUILD TYPE (+ the ABI-aware picker ported to main)
AppUpdateModule now selects the GitHub update repo from the MERGED MANIFEST's `android:debuggable` flag (ApplicationInfo.FLAG_DEBUGGABLE — the RUNTIME truth of the installed build type, correct on every branch and build path): debug builds (`com.confused.anikuta.debug`) check the DEV repo `testplay-byte/ANI-KUTA`; release builds (`com.confused.anikuta`) check the PUBLISHED repo `Confused-Creature-180/ANI-KUTA` (where official releases are re-hosted — the same repo the App Icon catalog lives in). This supersedes the release-line-only D-411 hardcode: the debug app no longer "accidentally" points at the published repo (the user observed the debug build checking the new repository). CRITICAL implementation fix caught in static review before CI: the initial wiring called `get<Context>()` at MODULE-declaration level — Koin's `Module` receiver exposes no `get()`, and even a resolvable form would evaluate eagerly at module-load time; the resolution now lives INSIDE the `single<UpdateSource>(named("github"))` lambda (the Scope receiver provides `get()`; resolution defers to first use against startKoin's androidContext). ALSO ported to main: the D-423 ABI-aware asset picker from release/1.1.1 (GitHubUpdateSource — main still had the round-34 first-APK pick, which would offer a device the WRONG APK now that releases carry 5): walks `Build.SUPPORTED_ABIS` in preference order matching `-{abi}.` in the asset name, falls back to `-universal.`, then the first APK. Required because the round-39 release branch cuts FROM MAIN — everything the release line needs must live on main.

## D-441 (round 39): notifications OFF by default
The master notification kill switch (`NotificationPreferences.notificationsEnabled`) default flipped true → false: a fresh install posts NO notifications until the user opts in (Settings → Updates → Notifications). Users who already toggled the switch keep their stored value (the default only applies when the preference key was never written). The master switch gates every episode-release notification (NotificationManager checks it at every post site), so the single default flip covers the whole system; the onboarding flow only requests the OS-level POST_NOTIFICATIONS permission (the user's own tap) — it never flipped the app-level switch.

## D-442 (round 39): the release cut FROM MAIN — release/1.1.2 at 1.1.2/10102
The user's round-39 instruction: update main → create the new release FROM main → create the release branch → complete the release flow. `release/1.1.2` is cut from the round-39 main head (the first release branch cut directly from main — the previous release/1.1.1 line was a long-lived branch) and carries ONLY the version commit: AndroidConfig 1.1.2/10102 + the branch-point docs. Everything the release needs now lives on main (D-430's build line, D-435's debug-only push path + the tag pipeline, D-436's no-R8, the round-37/38 feature set, D-439/D-440/D-441). Version rationale: v1.1.2 is the next number after v1.1.1 (exactly what BUILD-AND-BRANCH-GUIDE §5 anticipated: "a future release cut from main would take the next number the USER chooses, e.g. 1.1.2"); the round-35 v1.1.2 was deleted before any user install (the user's certificate-error report was on the CI artifact, never a release APK), so 1.1.2/10102 is clean AND monotonic over the installed 10101 (upgrade-in-place works, no uninstall churn). The old release/1.1.1 branch stays for reference. The release is re-hosted to Confused-Creature-180/ANI-KUTA with the EXACT dev-release asset names (version-prefixed `ani-kuta-v1.1.2-{abi}.apk`) so the D-440/D-423 ABI-aware updater matches correctly on the published repo.

## D-443 (round 40): the smart-link floating return pill — a theme-colored countdown capsule over the browser
The user's round-40 instruction: "When the user is redirected what should happen is that a floating button should start to show up with the timer and the option to go back. It will be a pill-shaped one, animated, beautiful, and theme-colored." Interpreted against the smart-link flow: while the user is IN THE BROWSER (the app backgrounded), the only surface that can reach them is a system overlay window (TYPE_APPLICATION_OVERLAY) — an in-app Compose overlay is invisible over another app. Design: `ReturnPillView` (classic Views + a custom `CountdownRingView`; the overlay window has no lifecycle owner/saved-state registry, so faking them for a ComposeView is fragile overhead for one capsule) renders [ring][label][Go-back chip] in a 30dp-radius capsule; colors are captured from the interstitial's live `MaterialTheme.colorScheme` (surface/onSurface/primary/onPrimary) at the Continue tap, so the pill follows the user's active theme palette. Animations: 280ms fade+rise+overshoot entrance, 0.96 press-scale with spring-back, a linear (honest-time) 60fps ring countdown, and a ready-state crossfade to "You can go back now" + looping 1.04 pulse. Semantics: the ring counts down EXACTLY `SmartLinkConfig.minTimeOutsideMs` (pill and state machine can never disagree); the pill is tappable at ANY time — an early tap returns to the app and the EXISTING coordinator gate does its thing (elapsed < min → the Try-again state; ≥ min → complete + proceed); on return-to-foreground the pill hides (state leaves AdInProgress → the interstitial's hide branch). `SmartLinkReturnPillController` is a Koin single: replace-on-reshow (the Try-again loop re-shows per attempt with a fresh countdown), idempotent hide with an animated exit, an orphan safety removal at countdown+10min, and add/remove wrapped in try/catch (SecurityException/BadTokenException) so the ad flow NEVER breaks over the pill. Permission honesty: SYSTEM_ALERT_WINDOW is declared, runtime-gated by `Settings.canDrawOverlays()` — no consent → a silent no-pill fallback (the previous behavior), no settings nag (the ad system's non-intrusive standing rule). `bringAppToForeground` uses `packageManager.getLaunchIntentForPackage` — `:core:ads` still knows nothing about :app or navigation. Isolation unchanged: the coordinator/state machine gained ZERO changes; the pill is driven entirely by the state the interstitial already observes.

## D-444 (round 40): the debug launcher = the user's mascot artwork, NEVER cropped
The user's round-40 instruction: "for the debug version use this image as the logo. make sure to resize it properly as needed and such and also remember dont crop the image" — REVISED the same round ("i told you to use the image as the logo and not crop it now use this… you can reduce the resolution but you cant crop it"): the artwork is the user's SECOND upload USER-UPLOADS/IMG_20260918_233212.png (the lime-tongue mascot on warm dark, 1024×1024 RGB — perfectly square, so every icon canvas maps 1:1 with ZERO padding, zero crop, zero distortion; the first upload 223706 was 1024×1039 and is superseded+removed). Processing: the WHOLE artwork is scaled uniformly (LANCZOS) into each square canvas — contain-fit, never cover/crop, no aspect distortion; the two hairline pad bands (the source is 1.5% taller than square) are filled with MIRRORED edge pixels so there is no visible seam and no pixel of the artwork is lost. Files: the 5 density rasters `mipmap-{m,h,x,xx,xxx}dpi/ic_launcher.webp` (48/72/96/144/192) + matching `ic_launcher_round.webp` (new — pre-26 round launchers previously fell through to main's kawaii round), the adaptive-background `mipmap-xxxhdpi/ic_launcher_bg.webp` (432px, overrides main's kawaii bg in debug builds), and the App Icon page hero `drawable-nodpi/icon_current.png` (1024px). Adaptive XMLs (anydpi-v26, ic_launcher + ic_launcher_round) now mirror the release line's structure — full-bleed `@mipmap/ic_launcher_bg` background + the empty `@mipmap/ic_launcher_fg_none` foreground — so the artwork renders on API 26+ too (the debug set previously only carried the lime vector/rasters and a color background). The launcher's own shape mask (circle/squircle) applies as it does to EVERY app icon — that is the launcher's shape, not our crop. Removed: the pre-26 `mipmap-anydpi` lime-vector overrides (the new rasters take over) + the now-unused `debug/values/colors.xml` (ic_launcher_background). RELEASE BUILDS UNTOUCHED — the release line keeps the kawaii-mouth adaptive set (the debug source set only overrides in debug builds).

## D-445 (round 40): the debug path is arm64-v8a ONLY — the x86_64 emulator exception RETIRED
The user's round-40 instruction: "from now we will only build the arm64v8 version for the debug version." The D-246/D-251 TEST-ONLY x86_64 emulator-build exception (which since D-251 only ran behind an explicit workflow_dispatch input, default off, and was already orphaned by the round-39 lean-environment rule) is now RETIRED COMPLETELY: build-apk.yml loses the `build_emulator_apk` input, the emulator build step, and the emulator ABI sanity check; the convention plugin loses the `emulatorX64Build` property branch; AndroidConfig loses `emulatorAbiFilters`. arm64-v8a is the ONLY legal ABI on the dev/CI path (the workflow's ABI gate now checks that single fact); shipped releases keep ALL FOUR ABIs (D-423 unchanged — the release path is not touched). Debug update source re-affirmed as-is (D-440): the debug build checks testplay-byte/ANI-KUTA (this repository) for new versions at runtime via the FLAG_DEBUGGABLE switch — exactly the user's "the debug version application will check the current github repository for the new version".

## D-447 (round 41): THE DEBUG-ONLY RELEASE LINE — this repo's releases carry ONLY the debug arm64-v8a APK
The user's v1.1.3 device-round correction: "In the releases of this GitHub repository you were supposed to do the releases of the debug version. The release version was not supposed to be released here and only one of the debug versions, the arm64v8, was supposed to be built and was supposed to be released." Root cause of the miss: the tag pipeline carried over the D-423 all-ABI SIGNED release machinery from the era before the debug app became the user's standalone daily build. The policy now (CORE_RULES §8): testplay-byte/ANI-KUTA releases = the DEBUG build (the committed debug keystore, applicationId com.confused.anikuta.debug — the user's STANDALONE co-installed app that updates itself in-app from these releases per D-440), arm64-v8a ONLY (D-445's debug ABI rule extended to the release line), asset ani-kuta-v{version}-debug-arm64-v8a.apk (the "-arm64-v8a." substring keeps the D-423 ABI-aware picker matching) + SHA256SUMS.txt. The RELEASE-signed all-ABI line exists ONLY on the published repo (Confused-Creature-180/ANI-KUTA) and ONLY after everything is completed. release-apk.yml REWRITTEN: no keystore secrets, assembleDebug, the ABI hard-gate (arm64-v8a only), the apksigner gate, the debug-install release notes. The v1.1.3 RELEASE (the all-ABI publication) was DELETED from this repo per the policy — the tag remains as history; the re-release happens as v1.1.4 debug. Older releases (v1.1.1/v1.1.2) left untouched ("from now on" scope — the user can decide their fate).

## D-448 (round 41): the floating return pill REDESIGNED — bottom + Go-back-only-after-timer + grow
The user's v1.1.3 device-round spec: (1) the pill shows at the BOTTOM of the screen "with some space" (was top) — the controller's LayoutParams moved to BOTTOM|CENTER_HORIZONTAL, y = 48dp above the bottom edge; (2) "the Go Back button should not show until the timer has successfully been completed" — the counting state is a COMPACT capsule (ring + "Stay a moment…" label only); (3) on completion the pill GROWS — the chip animates in via a LayoutTransition (width grow + chip entrance on the exact views), the capsule scales 1.0→1.06, ring → checkmark, label → "You can go back now", gentle pulse; (4) "clicking the pill when the timer has not been completed should not lead the user back" — the counting capsule has NO click listener at all; the whole-capsule tap target attaches ONLY in the ready state (early returns still work through the browser's own back, covered by the coordinator's Try-again gate). The ready-state press feedback scales relative to READY_SCALE so the pulse never fights the touch animation.

## D-449 (round 41): the setup wizard asks for "Draw over other apps"
The user's v1.1.3 device round: "in the setup wizard it did not ask me for the 'Draw over other apps' option or permission… It should open up the drawer or it should ask for the permission and the user should easily be able to allow it by clicking one button and just turning on the toggle." An OVERLAY permission step now sits between BATTERY and FINISH (the fifth skippable step): one tap on "Allow drawing over apps" opens the system's per-app "Display over other apps" screen (ACTION_MANAGE_OVERLAY_PERMISSION with the package URI, the general list as fallback), the user flips ONE toggle, and the wizard's existing ON_RESUME re-verify flips the step to granted the moment they return. The step carries a WHY line (the ad-return pill) via the new optional `description` param on OnboardingPermissionStep; the FINISH summary grid gains the "Draw over apps" card. Honest note recorded: there is NO way to float a view over another app without this consent (no runtime dialog exists for it, unlike notifications) — so the wizard ask IS the minimal-friction path; without the grant the ad flow still works, just without the pill.

## D-451 (round 42): the debug launcher icon's safe-zone fix — the mascot is NEVER cropped by the launcher mask
The user's v1.1.4 device round: "The app's icon itself is not proper. It is cut off on all four sides." ROOT CAUSE: adaptive icons (API 26+) are 108dp layers but launchers mask to the center ~61-67% — the D-444 full-bleed background put the artwork's edges exactly where every launcher's mask cuts, so all four sides were visibly chopped (that masking is the launcher's shape, not our crop, but the user experiences it as cropping — and they're right that the mascot's square was being clipped). FIX: the adaptive background (mipmap-xxxhdpi/ic_launcher_bg.webp) is regenerated as the COMPLETE artwork at 46% of the canvas — the whole square INCLUDING its corners sits inside the ~61% safe circle — feather-blended (a 12% alpha fade at the artwork's edges) into a solid canvas of the artwork's own mean border color, so the mask band reads as the artwork's dark background continuing outward. Verified against a simulated 61% circular mask: the full mascot (mouth + tongue + all four artwork edges) is always visible; the mask cuts only self-colored dark pixels. The legacy rasters (ic_launcher/ic_launcher_round, pre-26) stay the full-square artwork (no mask there). Note recorded: the two-pass blur-extend variant was REJECTED — the blurred tongue bled a green halo past the artwork's dark corners.

## D-452 (round 42): the pill's side-clipping fix + the shrink-to-bubble exit
The user's v1.1.4 device round: (1) "the right and left sides of the bigger size were cut off" — ROOT CAUSE: the ready-state scale (1.06) + the pulse (to 1.09) enlarge the capsule BEYOND its laid-out bounds, and the overlay window is sized exactly to the laid-out content, so the window's surface clipped the capsule's left/right sides. FIX: the root gains 16dp horizontal / 8dp vertical padding headroom (the window measures root incl. padding) and the capsule is centered within it; the pulse amplitude trimmed 0.03 → 0.02 (max scale 1.08 < the headroom). (2) The exit animation: "it will become smaller from the right and left sides and only the checkmark will remain and the checkmark will pop up like a bubble" — animateOut is now a staged exit: the label + chip collapse horizontally (pivot on their left edge, scaleX→0 + fade, 170ms) → they're GONE-d and the transition-armed capsule animates the width collapse → the checkmark ring POPS like a bubble (scale 1→1.4 with a 2× overshoot + a trailing fade, ~340ms) → the window is removed. One code path for BOTH exit routes (the Go-back tap and the state-driven hide); idempotent (isExiting) with the tap disabled mid-animation and staged callbacks cancelable via a shared token.

## D-454 (round 43): the pill REBUILT — zero window resizes, ON_STOP-synced countdown, the red X
The v1.1.5 device round found three pill problems and all three had the same class of root causes:
1. **"Stuttering / glitchy" animations** — the grow + exit animated the WINDOW's size (LayoutTransition bounds inside a TYPE_APPLICATION_OVERLAY window): a WM surface resize is asynchronous to the driving animations, so frames rendered against the old surface — visible jitter. REBUILT: the view is constructed in its READY shape and MEASURED before the window is added; the window takes that exact size ONCE and never resizes. The counting presentation is transforms inside the fixed window (chip GONE, capsule at 0.92 scale, FLAG_NOT_TOUCHABLE so the whole window passes touches to the browser); the grow does ONE compensated in-surface relayout (the capsule's scale is set the same frame the layout widens so the on-screen width is continuous) followed by a pure property spring to 1.06; the exit is 100% property animation (label+chip fade/collapse → capsule background dissolves → the ring pops like a bubble). Zero surface resizes per cycle.
2. **"Returned before the time passed but it showed the green check"** — TWO stacked causes: (a) the ring counted from the Continue tap while the coordinator's clock starts at the app's ON_STOP (the browser actually covering us) — the ring ran up to ~0.5s AHEAD, so its checkmark could appear while the gate would still reject; (b) the exit had only one look. FIXED: the controller registers a ProcessLifecycleOwner ON_STOP observer while the pill shows and starts the countdown on THAT signal (2.5s fallback), and hide(returnedTooEarly) picks the exit mark — the RED X (theme error color) when the coordinator says AdTryAgain, the green check on success.
3. **"Right and left sides of the bigger size were cut off"** — the grow's OvershootInterpolator peaked ABOVE the v1.1.5 16dp headroom. The pre-sized window carries 20dp/8dp padding headroom measured-in; the overshoot can only grow into it. (Also: the replace path removes the old pill INSTANTLY so the Try-again loop never overlaps two pills.)
Files: ReturnPillView.kt + SmartLinkReturnPillController.kt rewritten; SmartLinkAdInterstitial passes theme error + the verdict; zero changes to the coordinator/state machine.

## D-455 (round 43): the debug launcher zoomed IN — a slight mask crop is now the intent
The v1.1.5 device round: the icon was "not cut off but way too zoomed out… it should be cropped on all four sides a little bit." The D-451 safe-zone sizing (46% of the canvas) kept the artwork fully inside the mask but read as shrunken. The adaptive background's sharp artwork is now 66% of the canvas — the ~61% mask circle trims ~2.5% per side at the edge midpoints (the corners, dark background, trim a bit more) — with the same feathered blend into the border color. The user's own reference for "perfect" is the App Icon page hero (icon_current.png), which stays the FULL square artwork (that page shows the rounded-cell format, not the launcher mask).

## D-456 (round 43): the cumulative double-tap seek + 2× the visible hold (BOTH stacks, BOTH orientations)
The v1.1.5 device round: "+10, +10, +10" on every double-tap and the pill disappearing immediately. NEW: a shared `DoubleTapSeekState` + `DoubleTapSeekIndicator` in :core:player (used by the MPV stack's MinimizedControls AND FullscreenControls) and a private CS replica (`CsDoubleTapSeekState` + indicator in CsPlayerControls.kt, per the stacks' zero-shared-code rule): consecutive same-side double-taps ACCUMULATE the label (+10 → +20 → +30 …), switching sides restarts the count; the visible hold is ~1s (2× the old ~500ms life: 120ms in + 1000ms hold + 300ms out) and every re-tap RESETS the hold by cancelling the previous animation job — which also fixes the old overlapping-animations flicker on rapid taps. FULLSCREEN PARITY: the fullscreen controls in BOTH stacks previously seeked SILENTLY (the gesture existed, no indicator was rendered — the explore map confirmed it) — the indicator now renders there too, identical to portrait.

## D-457 (round 43): the player button/seekbar visual pass (BOTH stacks)
Per the device round: (1) the PLAY/PAUSE buttons (minimized AND fullscreen, both stacks) get a dedicated lighter glass — `themedPlayPauseColor()` / `csThemedPlayPauseColor()`: primary lerped only ~30% toward black at 42% alpha (was 55%/62%) — with the icon enlarged 32dp → 38dp INSIDE the unchanged button size (56/60dp); (2) the fullscreen -10s/+10s skip buttons get `shadowElevation = 6.dp`; the fullscreen play/pause too; (3) the fullscreen seekbar gains a canvas "shadow" (two stacked translucent dark rounds behind the track) and the THUMB gets a holder treatment — a dark shadow halo + a White-55% border ring around the thumb square so it reads as a distinct knob; (4) the EXIT-fullscreen button now carries the SAME White-12% per-button background as its neighbours (the top-right four + the bottom-right cluster — the old primary-35% tint read as backgroundless next to them) + a 4dp shadow. Applied identically in FullscreenControls.kt/:core:player and CsPlayerControls.kt.

## D-458 (round 43): the CS stack's fullscreen indicator + the isolation-respecting duplication
The CS fullscreen double-tap seek indicator is a file-private replica (`CsDoubleTapSeekState`/`CsDoubleTapSeekIndicator` in CsPlayerControls.kt) — NOT an import of :core:player — honoring the CS stack's stated zero-shared-code isolation (CsPlayerControls.kt header). Same behavior/timing as D-456. If the seek indicator ever changes again, BOTH copies must change (documented here).

## D-460 (round 44): the pill's grow-clipping ROOT CAUSE — the LayoutTransition lag
The v1.1.6 device round: "'You can go back now' was OUTSIDE the container — the right and left sides were outside the view area." ROOT CAUSE FOUND: the capsule's LayoutTransition animated the capsule's bounds over 260ms while the chip + the longer label rendered IMMEDIATELY — for the transition's duration the text painted OUTSIDE the still-narrow rounded rect (a visual clip that no window padding could fix, because the clip was the capsule's own lagging bounds). Also the label's ready-state padding swap (+4dp) desynced the pre-measured window size from the laid-out content. FIXES: the LayoutTransition is REMOVED (the grow is a single-frame relayout, visually seamless via the existing scale compensation; the chip now FADES in after the capsule reaches its final width); the label padding is CONSTANT (measured == laid out, always); headroom raised 20dp → 24dp; the label text is now "You can go back" (the user's spec — "remove the now").

## D-461 (round 44): the exit bubble pops DEAD-CENTER
The v1.1.6 device round: "the checkmark [was] on the very left side… it should move to the right and then center and then an animation should play." The ring sits at the capsule's LEFT while ready (it's the first child; the label + chip follow), so the old bubble popped left-of-center. FIX: beginExit now SLIDES the ring to the window's horizontal center (translationX = windowCenter − ringCenter, 220ms, FastOutSlowIn) while the label + chip fade away — then the check/X bubble plays centered.

## D-462 (round 44): the seek pill — closer to the middle + the switch pulse
The v1.1.6 device round: the +10/+20/+30 pill sat "way too much to the right or left" (40dp from the edge), and the value switches were instant. FIXES (both stacks): the pill's side padding is 40dp → 96dp (clearly inward of the edges), and every accumulating tap POPS the pill (scale 1 → 1.12 → 1, 230ms, a separate coroutine job so the hold's cancellation never kills it).

## D-463 (round 44): the fullscreen shadows CORRECTED — on the text, not the blocks; the track's shadow moved to the thumb; the exit button JOINS the tray
The v1.1.6 device round: (1) the D-457 shadows landed on the BUTTON BLOCKS ("an ugly way of putting things") — REMOVED from the skip buttons + the fullscreen play/pause in BOTH stacks, and a real TEXT shadow added to the -10s/+10s labels (TextStyle.shadow: black 70%, offset 2px down, blur 8 — a legibility drop under the glyphs); (2) the seekbar's dark backing was on the BAR while the thumb's halo was lighter — INVERTED per the user: the track backing is REMOVED entirely and the thumb's halo is now the dark one (50% black, 4dp, the light border ring stays); (3) the exit-fullscreen button: the D-457 change removed its theme color AND left it outside the cluster tray — the user wants it EXPERIENCED like its neighbours: it now sits INSIDE the bottom-right cluster's dark tray (the same Surface as speed/rotate/skip, styled as a plain White-12% FSSmallButton/CsFsSmallButton with NO shadow). The FSExitButton/CsFsExitButton composables are deleted (in-tray FSSmallButton calls replace them).
...
## D-472 (round 46): the CI runs per release cut to TWO
The user's round-46 instruction: ~5 Actions runs per release is too many; docs pushes triggering full APK builds is wasteful; at most 1-2 workflows for the release. build-apk.yml: 'release/**' removed from the push triggers (a release branch differs from its feature head by only the version bump — the TAG's Release run builds + gates the exact same code, so the branch run was redundant) and paths-ignore added (**.md, AGENT-CONTEXT, DASHBOARD, USER-UPLOADS, .github) so docs/agent edits never trigger Gradle. Per release now: 1 feature-push run + 1 tag run = 2. build-apk.yml + release-apk.yml remain the two workflows (merging them into one file risks the paths-ignore/tag interaction — a tag on a docs-only commit would be skipped — and buys nothing). The removed test steps (D-465) stay removed; the user confirmed they saved no meaningful time.
...
## D-486 (round 48): the poster pipeline hardening — the composer owns Dispatchers.IO, the art loads are time-bounded
THE v1.1.10 DEVICE FAILURE (the poster preview's "Couldn't load the preview art" + the plain-text test notifications): the ONLY unguarded throwing statements inside `EpisodeBannerComposer.buildBanner`'s catch were the two BLOCKING SQLDelight reads (`getContentDetails`, `getEpisodeMetadata`), and the preview's `produceState` + the tester's `rememberCoroutineScope` run on the MAIN dispatcher — so those blocking reads (and the preview's own feed/title/picker reads, the picker doing 1+2N queries) executed on the main thread, colliding with concurrent writers (the update worker, the download scanner); the catch converted the throw into a null banner → the failed state + the BigText fallback. FIX (three layers): (1) `buildBanner` now wraps its whole body in `withContext(Dispatchers.IO)` — the composer owns its dispatcher so EVERY caller (preview on Main, tester on Main, the real notification path in a WorkManager worker) is safe without per-call-site discipline; (2) the preview's produceState body + `EpisodeNotificationTester.postRecentUpdateNotifications` run in `withContext(Dispatchers.IO)` for their own reads; (3) `loadBitmap` is bounded by `withTimeoutOrNull(12s)` — the image client's 30s-connect/60s-read ladder must not wedge a preview or a notification for minutes on a dead CDN; Coil's memory→disk→network order already gives offline-first (the 500MB disk cache serves cached art instantly, offline) — the timeout only caps the never-cached case, after which the fallback chain (thumb as background → flat dark stage) still yields a banner. CancellationException is rethrown (never swallowed), and the failure log now carries the exception CLASS ("banner composition failed (X) for mainId=…") so the next device round-trip needs one logcat line, not a stack-trace hunt.

## D-487 (round 48): the preview's "No episodes available yet" state was UNREACHABLE — the state machine is hasContent-first
Static-analysis find while fixing D-486: the empty path set `failed = false, hasContent = false`, but the UI's when-gate required `failed && !hasContent` — so the honest empty state could never render and an eternal spinner showed instead. The gate is now `!result.hasContent` FIRST (before `failed`), and the empty path keeps `failed = false` (an empty state is not a failure). Verified against the user's Task-2 spec: feed-first → random library content WITH cached episodes (EpisodeDemoPicker reads the real `library_item` table via `getLibraryMainIds` + requires cached episode metadata; re-rolled per open via the roll counter + the Shuffle action) → the honest empty state; no invented "Sample" text anywhere.

## D-488 (round 48): the Notifications screen's episode-type block mirrors the Updates screen's layout EXACTLY
The user's v1.1.10 round: the Sub/Dub/Both control was squeezed into a SettingRow's trailing slot (200dp). The block is now the Updates screen's structure verbatim: title (16sp Medium) + description (13sp onSurfaceVariant) stacked at the top, the FULL-WIDTH SegmentedToggle BELOW them (no width cap — SegmentedToggle fills its container, same as the Updates screen which passes no modifier), inside the same SettingsGroupCard with showDividers=false. STILL writes the SAME shared UpdatePreferences checkSub/checkDub keys — both screens stay in sync (D-476 unchanged).

## D-489 (round 48): the library badge positions are REAL now — the ViewModel state reaches CoverBadgeRow
THE v1.1.10 DISCONNECT: "centered at top" shipped the TOP_CENTER enum + the ViewModel default + the CoverBadgeRow branch — but `LibraryGridCard`'s CoverBadgeRow call sites HARD-CODED `BadgePosition.TOP_START`/`TOP_END` ("positions hardcoded (no user-selectable position)"), so the state the grid passed in was dead and the device still rendered episode badges at top-end. The four call sites (comfortable + compact/COVER_ONLY branches) now pass `episodeBadgePosition`/`scoreBadgePosition` through, and the card's `episodeBadgePosition` default moves TOP_END → TOP_CENTER to match the D-483 default + the ViewModel's prefs fallback. The centered row floats 4dp INSIDE the top edge (D-480's floating-pill language); corner rows stay flush so their outer corner keeps matching the cover rounding. The bucket names (topStartBadges/topEndBadges) are historical — they hold the score-group and episode-group badges respectively.

> **NUMBERING NOTE (round 48):** D-485 was already burned by the round-47
> agent — it names the release/1.1.10 branch-point decision (the release
> branch's bump commit message + `39-RELEASE-1.1.10-BRANCH-POINT.md`), though
...
## D-493 (round 50): the banner becomes a BANNER — the ≈21:9 layout redesign, the cover-crop glitch fix, the badge fix
The v1.1.12 device round APPROVED the pipeline ("the results are much more satisfactory — they work and are handled quite properly") and flagged the presentation: the banner is "too tall for a banner kind of size", the background art renders "glitched out", and "the sub and dub tags are not being shown properly". THREE fixes, all in EpisodeBannerComposer (+ the preview box):
- LAYOUT: canvas 1024×576 (16:9) → 1024×440 (≈21:9, the classic banner proportion). The vertical layout is ZONE-ANCHORED, not flow-laid: the text block hangs from the TOP (title bold 46px ≤2 lines → EPISODE hero 58px lime, letter-spaced → episode title 28px), the SUB/DUB chip row hangs from the BOTTOM-LEFT at a fixed baseline (H−36) the ANI-KUTA wordmark aligns to, and the thumbnail is an aspect-PRESERVED fit into a 210×330 box, right-centered — correct for ANY thumbnail shape (16:9 stills, square, portrait). Zone anchoring means no element can overflow the canvas no matter how long the text is (the old flow layout pushed the chip toward the bottom edge and clipped when title+episode-title stacked). The text column widens when the thumbnail is absent. The preview box adopts the composer's REAL ratio via the now-public `CANVAS_WIDTH`/`CANVAS_HEIGHT` — the companion had to go public: Kotlin forbids cross-file member access through a PRIVATE companion's class-name facade ("cannot access 'companion object Companion': it is private" — compiler-verified on the project's Kotlin 2.2.0 by the review round BEFORE the push; every other member stays explicitly private so only the two consts leak).
- THE GLITCH (root cause proven): the old `drawCenterCrop` used a `BitmapShader` WITHOUT a local scale matrix — a shader samples its bitmap at the bitmap's NATIVE pixel size, so any source SMALLER than the canvas (every portrait cover against a 1024-wide banner) tiled with `TileMode.CLAMP` and SMEARED the right/bottom edge pixels across the rest of the banner. That was the device's "glitched background". Replaced with the classic src→dst rect cover-crop math (`canvas.drawBitmap(src, null, dstRect, FILTER_BITMAP|ANTI_ALIAS|dither)`) — correct for ANY input geometry (portrait poster, landscape banner, square logo, smaller or larger). Background loads now pass `Scale.FILL` to Coil (verified against the coil3 3.0.4 source: `coil3.size.Scale`) so the decode COVERS the canvas instead of squeezing into a fit box (a portrait cover decoded at fit-box 311×440 then upscaled 3.3× — now the covering region decodes and the crop math stays exact). `ensureSoftwareSafe` + the 12s timeout (D-491/D-486) untouched.
- THE BADGES: the engine writes `audioVariant="unknown"` for releases carrying BOTH variants (`UpdateEngine.parseAudioVariant`: `hasDub && hasSub -> "unknown"`) — the composer's chip gate matched only "sub"/"dub", so the badge VANISHED for most feed-driven demos. Demo paths now normalize unknown→random sub/dub BEFORE composing (the D-483 demo-honesty rule; `normalizeForDemo` in the poster screen, the picker's random variant); the REAL notification path keeps the truth (real posts only ever carry sub/dub anyway, and unknown → no chip is deliberately honest). The composer's chips are now case-insensitive, "both" renders TWO chips, and the chip label centers via FontMetrics (`centerY − (ascent+descent)/2` — the old `textSize/3` guess sat the label visibly low). The no-art stage upgraded from a flat rectangle to a STYLED stage (vertical dark gradient + a soft lime glow top-right) — the empty-library demo posts land there.

## D-494 (round 50): Shuffle actually shuffles — the one-shot exclusion + the selection cache
The user: "the shuffle preview button was not working. It was not shuffling between the other library items." ROOT CAUSE: the D-483 feed-first selection ALWAYS re-picked the same newest feed row on every produceState re-run — the button re-ran the pipeline but the feed-first path landed on the same row every time, so it looked dead. THE FIX (two parts):
- THE ONE-SHOT EXCLUSION: the Shuffle button records the on-stage mainId into a ref, bumps the roll counter; the next produceState pass consumes the exclusion (cleared immediately — it can never leak into a later pass), SKIPS the feed path entirely, and picks a random library item EXCLUDING the on-stage one. `EpisodeDemoPicker.pickRandom(excludeMainId)` is two-pass: pass 0 skips the excluded id, pass 1 re-admits it as the LAST resort (a single-entry library degrades to a no-op shuffle, never an empty preview). The shuffle is LIBRARY-ONLY by design — the user asked for it to cycle library items.
- THE SELECTION CACHE: the selected payload is cached across re-runs; a toggle flip re-renders the content ON STAGE with the new prefs instead of re-selecting. The independent review round proved a bare re-select snaps the preview back to the feed row / re-rolls a random item / re-randomizes the demo chip on EVERY flip — only a screen open (no cache yet) or a Shuffle tap (exclusion set) may re-select. The exclusion refs + the cache are deliberately NOT produceState keys (read/written inside the producer, never driving it — no restart loop).

## D-495 (round 50): the test notifications redefined — random library entries, always something to show
The user's round-50 spec (verbatim): "If there are no entries in the library, then the test notification should just show a random test notification. If there are some entries saved in the library, then it should randomly pick one of the entries for the first notification and should randomly pick another entry for the second notification." The tester's selection rewritten accordingly:
- The episode_update FEED is no longer consulted for tests (the D-483 feed-first rule is GONE for the test path; it remains the PREVIEW's initial-selection source). The LIBRARY is the source: [count] random entries — ANY entry qualifies (the old cached-episodes gate is gone: an entry without cached episodes still has cover art in its details row and composes a full banner).
- Fewer entries than requested → the same entries CYCLE with a different cached episode (the base's own episode filtered out of the re-roll) so both posts go out and never read identically.
- EMPTY library → `EpisodeDemoPicker.pickPureDemo()`: a built-in demo payload (a recognizable title from a small list, random episode 1–24, random sub/dub, blank mainId) that composes on the styled no-art stage (D-493). The tester can therefore ALWAYS deliver — the old "no feed rows AND no eligible library content — nothing to demo" silent bail is gone.
- The delayed post's TITLE rides the WorkManager data now (a pure demo has no library row to look up at fire time; the repo lookup remains the fallback for older queued work). DI updated to the new constructor (repository, data cache, notification manager, demo picker — the unused composer dep and the update store dropped).

## D-496 (round 50): "EP 12.0" — the episode-number formatting fix in the notification texts
`postNotification` interpolated the raw Double — the device showed "EP 12.0 · SUB is now available" (the user's "no proper formatting") — and `postPosterNotification` truncated with `toInt()` (a 12.5 release became "EP 12"). Both paths now share `episodeLabel` in NotificationManager's companion: whole numbers render without the decimal, fractional specials survive ("EP 12", "EP 12.5"). The composer's hero line gets the same treatment ("EPISODE 12" / "EPISODE 12.5" — previously `toInt()` truncated the specials).

## D-497 (round 50): release/1.1.13 — the release cut (the record lives on the branch)
`release/1.1.13` cut from the round-50 feature head `24be155b` (CI green: run 35455000790) as ONE commit (c66db803: the version bump 1.1.13/10113 in AndroidConfig.kt + the branch-point record
`DOCUMENTATION/cloudstream-v2/42-RELEASE-1.1.13-BRANCH-POINT.md`). Tag `v1.1.13` carries the user-facing What's New bullets (D-466) — the headline: "the banner finally looks like a banner"; 7 bullets covering D-493 (the ≈21:9 redesign + the glitch fix), the chips, D-494 (Shuffle), D-495 (the test notifications), D-496 ("EP 12") and offline-first. Release APK run 35456726478 GREEN on the FIRST run — the only CI run this release cost (build-apk.yml ignores `release/**` branches and docs-only paths) — publishing `ani-kuta-v1.1.13-debug-arm64-v8a.apk` (66.7 MB) + `SHA256SUMS.txt`, stable + `--latest`, body = the tag bullets + the Install line — verified via the API. 10113 > 10112: the debug app updates over its installed v1.1.12 in-app. Main stays at 0.4.20/85 (D-425). Full detail: the branch-point doc on the release branch.

## D-498 (round 50): RELEASE-FIRST — the release IS the verification build (the user's standing process)
The user's verbatim instruction: "directly try to do the debug release rather than trying to build it separately… This time you did a build but you did not do the release. Instead of doing the build directly, you can directly do the release. If the release fails we will know that the APK was not built properly… Remember to handle the future changes exactly like this too." THE STANDING PROCESS from now on: after an implementation push lands CI-green, cut the release branch + bump the version + tag IMMEDIATELY — no device round on a FEATURE build before the tag, no separate verification builds; the user's device round happens ON the RELEASE APK via the in-app updater (the tag's workflow run IS the build the user installs). The ≤2-runs budget (D-472) maps naturally: run 1 = the implementation push on the feature branch, run 2 = the tag's Release APK run. A failed release run is diagnosed from the RELEASE CI logs like any other build failure (fix on the feature line → re-cut/re-tag within the budget). Doc'd in the 42-RELEASE branch-point record + the AndroidConfig D-497 comment + progress.md.

## D-499 (round 51): the banner v3 — mirrored, tag-first, thumbnail-card-on-the-left, and the SUB/DUB chip TRUTH from the feed
The v1.1.13 device round approved the pipeline and the banner's SHAPE ("almost exactly how I wanted") but asked for more: less height, the thumbnail LEFT, a bigger bordered/shadowed thumbnail that crops too-wide art to the normal thumbnail shape, the episode number as a proper TAG, bolder shadow-backed text, and the language tags reflecting the episode's REAL variant availability (it showed SUB only while the episode clearly had a dub too). THE LAYOUT: canvas 1024×440 → **1024×400** (2.56:1); the layout MIRRORS — the thumbnail is now a LEFT-side CARD (fixed 16:9 box 400×225 — bigger in area than the old 210×330 chip, and the "normal thumbnail shape" the user named), COVER-CROPPED into it (`drawThumbBox`: max-cover scale + centered src→dst + rounded clipPath — a too-wide banner crops horizontally, a portrait cover crops its middle band, the BOX's shape always wins), rounded 20px with a 6px dark border + a two-layer fake-blur drop shadow; the text column hangs from the top-RIGHT (scrim mirrored), reclaiming full width when the thumb is absent (pref off OR no URL OR failed load — all null-safe, the banner composes without the card). THE TAG ROW: the 58px "EPISODE 12" hero line is RETIRED — the episode number is the row's first TAG chip ("EP 12"/"EP 12.5", lime fill + dark bold label), followed by the audio chips (dark translucent fill + lime label; "both" → two chips); the row sits under the ≤2-line bold title, and the stack (48 + 2×53 + 18 + 54) can never overflow the 400px canvas. TEXT TREATMENT: title/episode-title/branding all carry a soft dark shadow layer (readable on light art — the user's explicit rule). THE CHIP TRUTH (the user's "only showing sub even though a dub was available"): the engine inserts SUB + DUB of the same episode as SEPARATE feed rows (episodeUpdate.sq), so the composer now resolves variants for (mainId, episodeNumber) — new `getVariantsForEpisode` query (DISTINCT audio_variant, ABS on REAL) + `UpdateStore.getVariantsForEpisode` + `resolveAudioVariant` in the composer UNIONS the feed evidence with the passed-in variant (a passed "both" carries both; a DB failure degrades to the passed variant; blank mainId skips the lookup; "unknown" with no evidence still draws nothing — honest). The composer gains the UpdateStore dep (AnikutaApp's DI passes the 5th get()).

## D-500 (round 51): the IN-APP heads-up banner + THE DEAD-SEAM DISCOVERY (why system notifications never showed the banner)
Two device reports, ONE root cause: the test notification "showed the notification itself properly but it apparently did not show the banner alongside it", and "it never shows banner notifications at all… the application [should] show the banner whenever the user is on any screen, rather than just silently giving a notification". (A) THE DEAD SEAM (the review round's headline catch, now device-explained): NotificationsModule's `getOrNull<NotificationArtProvider>()` NEVER resolved the composer — Koin indexes a definition under its CONCRETE type only (verified against koin-core 4.2.2 sources; the repo's own DI research doc says explicit binding is required, and EVERY other nullable seam in AnikutaApp is bound as `single<Interface> { … }` — NotificationSender, UpdateProgressNotifier — the composer was the lone exception). So every SYSTEM notification (real + test) fell back to the plain text style since D-477 while the live preview worked (it injects the CONCRETE EpisodeBannerComposer). FIX: the explicit `single<NotificationArtProvider> { get<EpisodeBannerComposer>() }` binding in AnikutaApp — the poster path (BigPictureStyle) is finally reachable at runtime. (B) THE IN-APP BANNER (the actual feature): `InAppBannerController` (:core:notifications — MutableSharedFlow replay=0, DROP_OLDEST, tryEmit fire-and-forget — posting can never fail over the banner) + `InAppBannerEvent(bitmap, title, text, mainId)`; NotificationManager emits right after nm.notify() on BOTH post paths carrying the SAME composed bitmap the system notification renders; `InAppBannerHost` (:app) collects under repeatOnLifecycle(STARTED) in AppRoot's overlay stack (a sibling of DebugBubbleHost) — foreground-only BY CONSTRUCTION (backgrounded posts drop; the system notification covers them; no stale banner on app-open), shows the composed banner as a rounded shadowed card sliding from the top on WHATEVER screen the user is on (or a compact title/text card when bitmap==null), auto-dismiss 5s, tap dismiss; the current/visible state split keeps the card composed THROUGH the exit animation (the reviewer caught the null-before-exit variant rendering an empty Box).

## D-501 (round 51): PLANNED randomness — the ShuffleDeck
The user: "We need to use better randomness handling… planned randomness rather than just simple randomness because simple randomness does not feel that random." The Shuffle no longer re-rolls dice per tap: `EpisodeDemoPicker.ShuffleDeck` holds the whole library as a shuffled queue (Fisher-Yates via kotlin.random), consumed one id per tap; on exhaustion it refills with a no-immediate-repeat guard (a fresh deck never STARTS with the last served id — the head swaps into a random later position); stale ids (removed from the library) are pruned per call, added ids join on the next refill. Eligibility stays with the caller (a lambda: cached episodes + a title + not-the-excluded-id) so the deck never touches the DB; `@Synchronized next()` because rapid taps can overlap two produceState passes on IO. `pickRandomPlanned(deck, excludeMainId)` serves the deck walk and PRESERVES D-494's no-op-shuffle rule (deck empty of alternatives → the fallback re-picks randomly — the exclusion is a preference, never an empty preview). Screen-open selection keeps the old feed-first → random path (a fresh random per open is correct); only SHUFFLE taps consume the deck. The deck lives in the screen's composition (remember{}) — a new screen open is a new session.

## D-502 (round 51): release/1.1.14 — the release cut (the record lives on the branch)
`release/1.1.14` cut from the round-51 feature head `5141d1f1` (CI green: run 35461484463 — the implementation needed one CI round for a cross-module smart-cast fix on InAppBannerEvent.bitmap, the round-50 pattern) as ONE commit (2f26f467: the version bump 1.1.14/10114 in AndroidConfig.kt + the branch-point record
`DOCUMENTATION/cloudstream-v2/43-RELEASE-1.1.14-BRANCH-POINT.md`). Tag `v1.1.14` carries the user-facing What's New bullets (D-466) — the headline: "banners everywhere, and a banner that reads right"; 7 bullets covering the mirrored layout + the thumbnail card, the shadowed bold title, the EP tag, the chip truth, the dead-seam fix (plainly: "a wiring bug has been silently disabling the poster on every system notification"), the in-app banner popups and the planned Shuffle. Release APK run 35462212024 GREEN on the FIRST run — publishing `ani-kuta-v1.1.14-debug-arm64-v8a.apk` (66.7 MB) + `SHA256SUMS.txt`, stable + `--latest`, body = the tag bullets + the Install line — verified via the API. 10114 > 10113: the debug app updates over its installed v1.1.13 in-app. Main stays at 0.4.20/85 (D-425). Full detail: the branch-point doc on the release branch.

## D-503 (round 52): the CLEAN banner — no text above the poster, the REAL Android heads-up popup, and the in-app overlay retired
Three device findings, one theme — the notification IS the banner: (A) "It was showing me the banner and along with the banner it was still showing me the text above it … If there is a banner and a proper banner has been generated, then it will not show those or the top info at all." ROOT CAUSE: the D-477 BigPictureStyle carried `.setBigContentTitle(title)` + `.setSummaryText(text)`, which render title + "EP n · DUB is now available" INSIDE the expanded big-picture layout (plus the builder's own contentTitle/contentText). FIX on both post paths (`postNotification` + `postPosterNotification`): the style sets ONLY `.bigPicture(banner).bigLargeIcon(null)` and the builder's title/text collapse to `""` when a banner composed (a failed compose keeps the old informative text card — the fallback is untouched). (B) "When I am in the app and I click 'Send test notification,' the banner shows at the top of the app itself, which is kind of not what I wanted. I wanted the banner to be shown as a notification. … it does not show me the notification as a device popup." TWO fixes: the channel — `anikuta_new_episodes` was created at IMPORTANCE_DEFAULT, which NEVER produces the Android heads-up popup; channels are immutable after creation, so the id moved to `anikuta_new_episodes_high` (IMPORTANCE_HIGH + vibration, `enableVibration`), `ensureChannel` DELETES the legacy channel (no duplicate "New episodes" rows in the settings app), and the non-silent builders moved to PRIORITY_HIGH — the real system heads-up banner now drops over any screen. And the D-500 in-app overlay is RETIRED per the user's verdict: InAppBannerController/InAppBannerHost DELETED, the emission blocks and the `inAppBanner` constructor param removed (NotificationsModule wiring + the AppRoot host call gone with them). (C) The stagger: "have that one sent after 30 seconds rather than 5 minutes" — EpisodeNotificationTester's `index * 5L MINUTES` → `index * 30L SECONDS` (the WorkManager path kept so the second post still survives app death); the legacy core `postTestNotification` (no callers) left as-is.

## D-504 (round 52): the chip truth WIDENED (episode-cache audio parse) + the thumbnail evidence chain
The v1.1.14 device round: "It was only showing sub or dub … not both of the badges together even though the content was actually available in both audio variants" — the D-499 feed-union is thinner than reality: a sub release posts before any dub row exists, 'initial' batches record one variant, and a source may never emit the dub row at all. The composer's `resolveAudioVariant` now unions a THIRD evidence layer: the EPISODE CACHE's own row (`data_cache_episode.source_name`/`scanlator`) parsed with `parseAudioAvailability` (:core:common) — the EXACT parser the details page renders its per-episode SUB/DUB pills from, i.e. the evidence the user cross-checks against. Both available → BOTH chips; one → one; genuine nothing → no chip (still honest). ALSO: "on some of the content it was apparently not showing the thumbnail images at all even though the thumbnail images of those contents were available" — the thumb now loads through an evidence chain: the episode's own still (with ONE retry at 8s for transient CDN silence — the "available but not shown" case) → the extras' large cover → the cover → the banner, each at a 6s ceiling; the card stays on stage for rows whose `thumbnail_url` is null, and an empty chain still composes WITHOUT the card (the graceful path, unchanged). The D-504 BACKGROUND rule from the same round: the D-499 right-column + bottom SCRIMS ARE GONE — the art renders at FULL BRIGHTNESS ("the banner image is apparently getting a darkening kind of effect applied to it … it should not"), and the D-499 text shadows are the only readability carrier.

## D-505 (round 52): the POSTER STUDIO — the five-element editor (drag / pinch / magnetic snap / save + console log)
The user's headline feature: "just to the left of the shuffle preview button … it will say 'Customize' and clicking it will lead the user to a new page … switched from portrait to landscape automatically … on the left side about 30% the options for editing and on the right side the editable live preview," five elements (content title, episode thumbnail, episode number, audio-version availability, episode name), each freely draggable ("anywhere he wants"), zoomable, per-element advanced options in the sidebar (color "and maybe some other editing options"), a SIZE SLIDER at the very bottom when needed, "a magnetic kind of effect so that everything can be aligned properly — the increments will be smaller but they will be there," settings "properly saved," and — the explicit tooling request — "if I go to the notification poster and then customize it and after customizing I save it, then all the saved details will be shown in the console log. That way if I want to make them default, I can easily give you those details." BUILT AS: `PosterLayoutConfig` (@Serializable, one JSON blob in `NotificationPreferences.posterLayoutJson` = `notif_poster_layout_json`; per-element x/y/scale/colorArgb/visible; `customized` gate) + `PosterCanvasMetrics` (the 1024×400 geometry, the single source of truth for composer AND studio) + `PosterDrawing` (the composer's draw primitives extracted — wrap, chips, thumb card, cover-crop, fallback stage — so the studio preview is a faithful MINIATURE, not a lookalike) + `EpisodeBannerComposer.loadEditorArt` (the exact art loads minus the compose) + `resolveAudioVariant` made public (the studio resolves chips through the same resolver). The screen (`PosterCustomizeScreen`, app:settings): DisposableEffect forces SENSOR_LANDSCAPE and restores the previous orientation on exit; left panel (~32%) = the element list with visibility switches (pref-synced where a pref exists), a 12-swatch color palette + "A" default chip, a per-element style reset, and the size slider pinned at the bottom (0.4–2.0×); right pane = the 2.56:1 preview fitting the tighter axis, rendered on a Canvas through PosterDrawing. GESTURES: one `pointerInput(Unit)` (deliberately NO state keys — a key change would restart the detector MID-DRAG and kill the gesture; the handlers read live state through the `by remember` delegates) with `awaitEachGesture`: one finger = drag, two fingers = pinch (span ratio scales 0.4–2.5, centroid drift moves), topmost-first hit-testing, touch-to-select. MAGNETISM: a fine 8px grid plus a 14px snap window onto the canvas margins/centers, the thumb-box lines, and the other elements' center lines — lime guide lines flash on the snapped axis and clear on gesture end (and the pinch→drag transition re-anchors `prev` to the remaining pointer so the element never jumps). FLOW SEEDING: while `customized=false` the studio renders the composer's FLOW layout; the FIRST edit seeds every untouched element at its current flow anchor (the poster never jumps under the finger) and flips `customized=true` — the composer then renders ABSOLUTE mode (anchors + scale + color per element, the v1.1.14 prefs still gating audio badge/episode title/thumbnail on top). SAVE: persists the JSON + `Logger.i("Anikuta:App:PosterStudio") { "POSTER LAYOUT SAVED: $json" }` (the full dump the user asked for) + a toast; Reset-to-defaults restores the factory flow layout (and logs). The element draw order/hit order and the sample placeholders (a dimmed [SUB][DUB] pair when the variant resolves unknown, a dashed "thumbnail" box when no art) keep every element editable even when the sample lacks data.

## D-506 (round 52): the shuffle feedback — "I should be given an animation so that I know that the shuffling did happen"
The deck (D-501) worked but was silent. The preview now stacks THREE cues on every shuffle tap: the preview box pulses (scale 1 → 0.965 → spring back, DampingRatioMediumBouncy), the shuffle icon spins a full 360° (tween 550ms), and the recomposed banner CROSSFADES in when the compose lands (Crossfade 300ms — which also smooths the toggle-flip re-renders). D-494's semantics untouched: toggle flips still never re-select (the crossfade animates the re-render, the CONTENT on stage stays).

## D-507 (round 52): release/1.1.15 — the release cut (the record lives on the branch)
`release/1.1.15` cut from the round-52 feature head `048f9422` (CI green: run 35465964501 — the implementation needed one CI round: the studio imported the composer from `:core:notifications` while `EpisodeBannerComposer` lives in `:app`'s `com.confused.anikuta.notifications` package, collapsing every `PosterEditorArt`/`loadEditorArt`/`resolveAudioVariant` reference in PosterCustomizeScreen — the documented one-fix-push pattern) as ONE commit (5974113a: the version bump 1.1.15/10115 in AndroidConfig.kt + the branch-point record
`DOCUMENTATION/cloudstream-v2/44-RELEASE-1.1.15-BRANCH-POINT.md`). Tag `v1.1.15` carries the user-facing What's New bullets (D-466) — the headline: "the notification IS the banner, and the poster is yours to redesign"; 7 bullets covering the clean banner (banner only, no text above), the REAL heads-up popups (high-importance channel; the in-app card gone), SUB+DUB together, the thumbnail fixes, the full-brightness art, the Poster Studio (drag/pinch/magnetic snap/recolor/save + the console-log dump), and the 30s stagger + the shuffle animation. Release APK run 35466505113 GREEN on the FIRST run — publishing `ani-kuta-v1.1.15-debug-arm64-v8a.apk` (66.8 MB) + `SHA256SUMS.txt`, stable + `--latest`, body = the tag bullets + the Install line — verified via the API. 10115 > 10114: the debug app updates over its installed v1.1.14 in-app. Main stays at 0.4.20/85 (D-425). Full detail: the branch-point doc on the release branch.

## D-508 (round 53): the STUDIO DRAG REBUILT — absolute segment anchors (the slow-drag stick fix)
The round's headline device report: "it would get stuck somewhere occasionally … if I quickly tried to move it, there were no issues but if I tried to move it slowly and precisely, it would never allow me to get out of an area … if I slowly moved my finger down for a while, it would not do anything. After a while, after some limit had passed … it would start to move down a bit until it reached the next magnetic location." ROOT CAUSE: the D-503 loop accumulated per-event deltas THROUGH the element's snapped origin — every event read origin() (already snapped), added the finger delta, snapped it back, and WROTE the snapped value as the new accumulation base; any per-event movement smaller than the 14px snap window was therefore swallowed every frame, and slow drags (sub-14px deltas) NEVER escaped while fast swipes (one big delta) did. FIX: the position model is ABSOLUTE — the gesture captures a SEGMENT anchor pair (the element's origin + the pointer position) once at gesture down, at pinch start, and at the pinch→drag handoff, and every event computes the raw target from THAT pair (`moveElementTo(kind, rawX, rawY)`); the snapped value never feeds the accumulation. Slow precise drags now glide 1:1 with the finger; the magnet only pulls while the target is genuinely close to a snap line (the same 8px grid + 14px window — "magnetic kind of effect … smaller but they will be there", intact). The clamp bounds now MIRROR the composer's composeAbsolute (texts x ≤ W−120 / y ≤ H−lineHeight×scale; chips/cards clamp to their measured rect) so the studio can never place an anchor the real banner would re-clamp, and the `coerceIn(0f, negative)` crash edge for canvas-wider rects is closed with `coerceAtLeast(0f)` (the reviewer's check).

## D-509 (round 53): the SIZE slider moved BELOW the preview
"The size adjustment bar was supposed to be shown below the preview of the notification poster but it was not." The slider moved from the left panel's bottom to the RIGHT pane, directly under the editable preview (the Column now stacks BoxWithConstraints → the size row → the preview caption), still bound to the selected element with the live % readout and the gesture-hint line.

## D-510 (round 53): the real-button header + the settings-screen button feel
Two "button feel" verdicts: the studio header's bare IconButtons "not proper", and the Customize/Shuffle actions needing "an actual button kind of feel" with "a little bit of padding". The studio's Back / Reset / Save are now FilledTonalIconButton / OutlinedIconButton / FilledIconButton (Save in the primary container colors); the settings screen's Customize is an OutlinedButton and Shuffle preview a filled Button, both with icons at 18dp + ButtonDefaults.IconSpacing and a spacedBy(10.dp, End) row. The pulse/spin/crossfade shuffle feedback is untouched.

## D-511 (round 53): the RICH OPTIONS — font family, bold/italic/shadow, tag backgrounds, custom tag labels
"The customizability options … do need to be improved … for the text I should be given quite a lot of options … font style, the boldness … for the tags I should be able to change the background color of the tags and also the text of the tags, the formatting of the text." `PosterElementLayout` grew SIX backward-compatible fields (defaults reproduce the factory look exactly; old saved JSON parses untouched — kotlinx defaults): `fontKey` (""/"condensed"/"serif"/"mono"), `bold: Boolean?` (null = the element's FACTORY weight — title/chips bold, the episode title regular), `italic`, `shadow` (the D-499 soft shadow becomes a per-element toggle), `chipBgArgb` (a tag's background; 0 = the factory lime/dark palette), `labelOverride` (custom tag text; on the audio row a COMMA renders several tags — "Subbed, Dubbed" → two chips). The sidebar gained an OPTIONS section per selected element (text: FONT 2×2 family chips + Bold/Italic/Soft-shadow toggles; chips: TAG BACKGROUND palette + LABEL TEXT field with the comma hint + Bold/Italic label toggles). Rendering goes through ONE shared resolver, `PosterDrawing.typefaceFor(fontKey, bold, italic, fallbackBoldDefault)` — the composer's ABSOLUTE mode and the studio's drawPoster both call it, so WYSIWYG holds; the FLOW mode keeps the factory look but gained the title VISIBILITY gate (composeFlow now honors layout.title.visible — hiding the title lifts the tag row to the top instead of leaving a phantom gap).

## D-512 (round 53): the HARD per-line ellipsis — long titles never leave their column
"The title should not be handled in a bad way … adjust its length accordingly. If the title is longer then it will only show the first half of the title and then show the dots … It will make sure that there is space and if there is no space then it will just not show the full title, even if it is able to show only one line or one word." `PosterDrawing.wrappedLines` gained a final per-line pass: ANY line measuring wider than its column (the greedy wrap can emit one when a SINGLE long word lands on a fresh line) is hard-ellipsized down to the width — even to a bare "…". Combined with the existing ≤2-line cap + the truncated-last-line ellipsis, every title now truncates to the space available, never paints past the right margin, and the studio's hit-rects (built from the SAME wrappedLines) match. The wrap-width floor in the studio's elementRect dropped 120→24px to mirror the composer's floorless W−x−margin math (the x ≤ W−120 clamp guarantees the real ≥84px).

## D-513 (round 53): the studio is a DESIGN CANVAS (BOTH tags always) + the carried preview selection
Two verdicts: "In the post studio both of the tags should be shown regardless of availability" and "the exact same one should be shown as the live preview, which was being shown previously on the notification poster. Currently when I go to the customized one, it picks another one." (A) The studio now renders the override list or BOTH tags unconditionally at full strength (the dimmed-placeholder path is gone — availability is the real banner's concern); the composer's ABSOLUTE mode keeps the resolved-variant truth with `audioChipLabels` (override first). (B) `PosterCustomizeKey` became a @Serializable data class (mainId/title/episodeNumber/audioVariant); the settings screen's Customize tap hands over its CURRENT `selectionCache` value (`onOpenCustomize: (PreviewSelection?) -> Unit`), and the studio seeds its selection from the carried payload (falling back to its own feed-first/random pick only when the preview was still loading). `PreviewSelection` went PUBLIC (the CI round: the public composable's lambda parameter may not expose an internal type — the single compile error of the implementation push). The studio's sample resolve also dropped the variant call (chips are always both now); the episode-title load stays.

## D-514 (round 53): the SMART ADAPTIVE SCRIM — darkening returns, by measured luminance
The round REVERSES half of D-504: "I think the background banner is meant to be darkened a little bit. Maybe we should go with that: the whole background banner will be darkened a little bit, maybe utilizing a smart algorithm to adjust the darkness to a proper level. If it is already dark then it will not be darkened but if it is lighter then it will be made darker." `PosterDrawing.applyAdaptiveScrim` measures the background's average luminance (a 24×10 createScaledBitmap probe, Rec. 709 weights, software-safe + try/catch — every failure degrades to "no scrim") and draws a proportional full-canvas black veil: lum ≤ 0.40 → NOTHING (already-dark art untouched), 0.60 → ≈15%, ≥1.00 → the 46% ceiling ("a little bit", never a void). The SAME helper runs on the composer (right after drawCoverFit, before any element) AND the studio's drawPoster — WYSIWYG holds. The D-499 text shadows stay ON top of it (the second readability carrier; now a per-element toggle per D-511).

## D-515 (round 53): the settings-screen half of the button feel
See D-510 (split here because the round's feedback listed the poster-section UI separately): the Customize (OutlinedButton, Tune icon) and Shuffle preview (filled Button, Shuffle icon with the D-506 spin) buttons on NotificationPosterSettingsScreen.

## D-516 (round 53): the notification title + "New episode available for …" BACK on the banner path
The D-503 empty-title experiment ends. The device round: "when I checked the send test notification, it properly showed me the banner of my device's own notification panel, which was good, but there was this issue: it was not expanded. I did not see the cover banner image itself; it only showed me the notification. … we should show the title at least in the notification, regardless of the banner … We should include the text description. We should say 'new episode available for' then the first two words or so of the content." BOTH banner post paths (`postNotification` + `postPosterNotification`) now set the builder title/text to `headline(title)` (one line, 40 chars + "…", surrogate-pair-safe) and `shortDescription(title)` ("New episode available for " + the first two words + "…" when truncated) when a banner composed — the collapsed heads-up card is informative again and expanding shows the banner under the title. The BigPictureStyle stays clean (no bigContentTitle/summaryText — the D-503 three-stack stays dead); a failed compose keeps the old full text card. Note: Android heads-up is a system presentation — the app cannot force-expand it; title+text restore is the honest fix for the "nothing but the app header" look.

## D-517 (round 53): release/1.1.16 — the release cut (the record lives on the branch)
`release/1.1.16` cut from the round-53 feature head `3cb5adcc` (CI green: run 35470221593 — the implementation needed one CI round: the D-513 signature change made the public NotificationPosterSettingsScreen expose the D-503-internal PreviewSelection through its onOpenCustomize parameter; the class went public) as ONE commit (b87f7fd0: the version bump 1.1.16/10116 in AndroidConfig.kt + the branch-point record `DOCUMENTATION/cloudstream-v2/45-RELEASE-1.1.16-BRANCH-POINT.md`). Tag `v1.1.16` carries the user-facing What's New bullets (D-466) — the headline: "the studio obeys the finger, and the notification speaks again". Release APK run 35470742809 GREEN on the FIRST run — publishing `ani-kuta-v1.1.16-debug-arm64-v8a.apk` (66.8 MB) + `SHA256SUMS.txt`, stable + `--latest`, body = the tag bullets — verified via the API. 10116 > 10115: the debug app updates over its installed v1.1.15 in-app. Main stays at 0.4.20/85 (D-425). Full detail: the branch-point doc on the release branch.

## D-518 (round 54): the STUDIO SHELL REBUILT — insets, the full-width header, the reachable SAVE
The round-54 verdict: the customize screen "does not look that well managed, that well planned, that well handled ... You are not considering the layout, the UI, the top notification area ... There should be padding at the top for the notification bar so that the buttons do not show under it. The save button: I am unable to click the save button due to it being under the notification bar." ROOT CAUSE: the D-510 header row lived INSIDE the left panel with ZERO inset handling on a forced-landscape edge-to-edge (enableEdgeToEdge) screen — the whole control row, Save included, rendered UNDER the status bar. The shell is now the canonical editor shape: root Column → `.statusBarsPadding()` once; a FULL-WIDTH header bar (FilledTonalIconButton back · "Poster studio" + a "Drag to move · pinch to resize" subtitle · weight-spacer · labelled OutlinedButton Reset · labelled primary Button Save — real ≥40dp targets, the round's named unclickable Save included); HorizontalDivider; a body Row carrying `.navigationBarsPadding()` on the WHOLE body (review fix: in SENSOR_LANDSCAPE the 3-button bar can sit on EITHER vertical edge, so both panes clear it); left panel = three tonal SectionCards (ELEMENTS / COLOR / OPTIONS — planned structure, not a raw list); right pane = the untouched preview + the slider strip on its own card + the caption. The section cards solved a DSL trap: LazyListScope `items{}` cannot run inside a plain composable lambda, so the ELEMENTS card iterates a plain `for` over the five kinds.

## D-519 (round 54): the ELEMENT-AWARE text columns — the title never paints under corner elements again
"the content title are not being handled that well and they are not aware of their elements surrounding them so they do not adjust their length properly as needed ... the content title is showing under some corner elements." In ABSOLUTE mode the title/episode-title wrap width ran to the right margin regardless of everything, so any chip or thumbnail pinned on the text's row painted ON TOP of it (later draw order). `PosterDrawing.awareWrapWidth(text, x, y, fullWidth, textSize, typeface, maxLines, lineHeightFactor, others)` ends the column BEFORE the left edge of any other VISIBLE element whose vertical band intersects the text's band: the band is TWO-PASS and text-driven (lines counted at the FULL width first — a one-line title is never shrunk by chips that merely sit below it), the neighbour rects carry ONLY left/top/bottom (zero measurement → deterministic and identical on the composer AND the studio), and a candidate narrower than AWARE_MIN_WIDTH=120 (or a neighbour entirely to the text's left) is skipped rather than collapsing the column (AWARE_TEXT_GAP=24 breathing room). The composer's composeAbsolute builds `awarenessRects(exclude)` inline — clamped EXACTLY as the render path clamps (measured chipWidth for the EP chip, rowWidth for the audio row, W−120/H−size for texts, W−w/H−h for the thumb — legacy/hand-edited JSON can carry out-of-range anchors and an awareness rect must never sit where nothing draws); the studio builds THE SAME list (`awarenessRects`/`awareWidthFor` local funs, declared BEFORE elementRect — a local fun is only visible below its declaration point, the round's one CI-validated blocker) and passes the two hoisted wrap widths into drawPoster (computed once per recomposition, never per draw frame). THE STUDIO'S DIVERGENCES, deliberate: the blank-episode-title placeholder and the dashed no-art thumbnail are editing affordances the real banner never paints, so neither narrows a column in the studio (thumb gate = `hasThumb`, real art). BONUS CRASH FIX (the reviewer's find): the composer's thumbnail RENDER clamp was the one unguarded `coerceIn(0f, H−h)` — at scale > ~1.78 (the studio's own slider allows 2.0) 225×s > 400 threw IllegalArgumentException and EVERY real notification silently fell back to plain text while the studio still previewed a banner; all four clamps are now range-guarded like their siblings.

## D-520 (round 54): the shuffle that always shuffles + the honest progress rail
The round-54 verdict: "the customize button and the shuffle preview buttons were looking good but the shuffle preview button was giving me a bad experience." TWO defects, both fixed in NotificationPosterSettingsScreen: (1) the D-494 handshake (exclude = the on-stage id) silently DEGRADED into a no-op whenever the on-stage id was still null — a tap landing before the first compose finished produced a null exclude, which the producer read as "not a shuffle tap" and REUSED the cached selection: the pulse played and NOTHING changed. The handshake is now an explicit `shufflePending` flag consumed FIRST on the main thread of every producer pass (a tap mid-flight cancels the pass and the next pass re-reads the flag — a request can never leak into an unrelated pass), forcing a re-selection on EVERY tap; a null on-stage id still reaches the deck path via the "" sentinel (excludes nothing, matches no real id — pickRandomPlanned filters `id != ""`). (2) the compose can take seconds (a first-pick's art over the network) with zero visible progress — the producer now arms a `shuffling` state (cleared in a finally that also runs on cancellation, so a newer tap/toggle always retires the rail) and the preview answers with a slim indeterminate LinearProgressIndicator docked to the box's bottom edge while the new banner composes. The pulse/spin/crossfade feedback and the D-499 no-repeat deck semantics are untouched; toggle flips still never re-select.

## D-521 (round 54): the notification description is just "New Episode"
The round-54 verdict simplifies the round-53 wording (D-516): "it was showing me the title of the content there but below it it was showing 'New Episode Available' followed by the name of it. It was good but I think the bottom description could be made simpler. It could just say 'New Episode Available' or maybe we can simplify it more to just 'New Episode'. Let's make it simpler like that." `shortDescription()` (the title parameter is gone) returns the constant "New Episode" on BOTH banner post paths — the headline above it already carries the content's name, so the collapsed heads-up card reads TITLE / New Episode with nothing else. The non-banner fallback text card keeps its fuller episode text.

## D-522 (round 54): release/1.1.17 — the release cut (the record lives on the branch)
`release/1.1.17` cut from the round-54 feature head `2b5d7730` (CI green on the FIRST implementation run — 35474291318; the pre-push independent review caught the one compile blocker — the studio's local-function forward reference — before the push, so the implementation needed NO fix CI) as ONE commit (41919f46: the version bump 1.1.17/10117 in AndroidConfig.kt + the branch-point record `DOCUMENTATION/cloudstream-v2/46-RELEASE-1.1.17-BRANCH-POINT.md`). Tag `v1.1.17` carries the user-facing What's New bullets — the headline: "the studio gets a real shell, the title gets aware, the shuffle always shuffles". Release APK run 35474920806 GREEN on the FIRST run — publishing `ani-kuta-v1.1.17-debug-arm64-v8a.apk` (66.8 MB) + `SHA256SUMS.txt`, stable + `--latest`, body = the tag bullets — verified via the API. 10117 > 10116: the debug app updates over its installed v1.1.16 in-app. Main stays at 0.4.20/85 (D-425). The docs commit (64b68a70) cherry-picked to the release branch as 69cb7b4c (docs-only, no CI). Full detail: the branch-point doc on the release branch.

## D-523 (round 55): the POSTER STUDIO IS RETIRED — "we should not give the users that much customizability"
The round-55 verdict, verbatim: "I looked at your work and let me tell you, I am not satisfied with the poster banner notifications at all. I think we are doing a little bit overboard with these things ... We should not give the users that much customizability. Giving them options for the notification poster and giving them full customizability flexibility is not a good option ... I am proposing that we should remove the poster studio functionality completely. Instead of that we should provide the user with some predefined templates for the notification poster." EXECUTED COMPLETELY: `PosterCustomizeScreen.kt` (1,487 lines) deleted; `PosterCustomizeKey` + its nav branch + `onOpenCustomize` deleted from MainActivity; `PosterLayoutConfig.kt` (the whole free-form element model — positions/scales/colors/font overrides) deleted; `EpisodeBannerComposer`'s ABSOLUTE mode (composeAbsolute, drawScaledChip, element-aware awarenessRects, loadEditorArt/PosterEditorArt) deleted; `NotificationPreferences.posterLayoutJson`/`notif_poster_layout_json` deleted (legacy installs just carry a dead key). `PosterDrawing` lost typefaceFor + awareWrapWidth + the AWARE_* constants (absolute-mode-only machinery). `PosterCanvasMetrics` moved into `PosterTemplate.kt` — the template file is now the layout's home. Customizability that remains: the five templates (D-524), the three-way artwork source (D-526), and the five element switches — expressed as segmented toggles + one-line descriptions, the episode-type block's language.

## D-524 (round 55): the FIVE PREDEFINED TEMPLATES — the layout lives in the CODE, not in saved anchors
"I would like you to look into it properly ... for the predefined layouts of the notification poster, I think we should give the user a total of five options by default. You will decide the layout: how their layouts should actually look, how they should function, and all of that stuff. All of them will be quite unique." `PosterTemplate` enum (key + label): **CLASSIC** — the approved v1.1.14 look unchanged (left 16:9 still card centered, text column top-right, branding bottom-right); **SPOTLIGHT** — the billboard: the text stack hangs BOTTOM-LEFT (title 46 ≤2 → tag row → episode title, the stack's top found by measuring its own height), the still (if on) docks BOTTOM-RIGHT as a small 280px card whose left edge ENDS the text's wrap width (the two anchors can never overlap — the round's no-overlap rule by construction), branding top-right; **SPLIT** — the magazine cover: a tall 420×344 still panel fills the left half, the right column is vertically CENTERED (title 40 ≤3 → tags → episode title 24); **MINIMAL** — the symmetric poster: no card, title/tags/episode-title all centered (40/24px, stacked from y=88) — the thumbnail toggle intentionally has no home here (the KDoc says so; the live preview shows the truth); **CARD** — the info panel: a dark 200px rounded panel docks to the bottom, the text lives INSIDE it (34/22px, vertically centered, no shadows needed), the still (if on) rises from the panel's left edge poking 28px above it, branding top-right. Every template reuses the shared primitives (adaptive scrim, cover-crop, soft shadows, the extracted drawTagRow/tagRowWidth helpers) — only the ARRANGEMENT differs. The choice rides `NotificationPreferences.posterTemplate` (`notif_poster_template`, default "classic"); `PosterTemplate.fromKey` is lenient (unknown/legacy → CLASSIC, never a broken banner). The composer dispatches in compose(); no mode flag, no JSON, nothing that can drift between preview and notification.

## D-525 (round 55): the SCREEN REBUILT — stationary preview, one gutter, one-line descriptions, the proper Shuffle
Four explicit asks, all executed in the rewritten `NotificationPosterSettingsScreen`: (1) "The live preview at the top will never disappear. It will be stationary. Only the bottom section will be scrollable. The top live preview section will be kept exactly as it is." — the preview card + Shuffle moved OUT of the LazyColumn into a fixed region under the CollapsingHeader; the LazyColumn below owns ONLY the option cards; the preview's card label, 2.56:1 ratio, all seven render states (off/empty/failed/banner-crossfade/spinner/pulse/progress-rail) are unchanged. (2) "There is a lot of padding on the right and left sides of it" — the root cause was STACKED padding (the list's 16dp contentPadding on top of SettingsGroupCard's own 16dp = 32dp per side); the screen now carries a single 8dp gutter via the local `PosterCard` (the SettingsGroupCard look minus its horizontal padding) and a horizontal-8dp preview region — the preview sits 16dp from each edge, the option rows 24dp. (3) "The description should only be one line" — every description rewritten short ("Render notifications as banners" / "Shown under the tags" / "The art card beside the text" / "The audio chips row" / "The corner wordmark" / "How the banner arranges art and text" / "Which art fills the background") AND maxLines=1 + ellipsis on both description sites. (4) "The shuffle preview: I would like you to improve the UI of the button ... much better, much more well-defined, and much more proper. Also simplify it to just shuffle." — one full-width 44dp filled Button, shape 12dp, primary container, RobotoFamily 15sp ExtraBold "Shuffle" + the D-506 icon spin; the Customize button died with the studio. The option groups use the episode-type block's structure (title + one-line description above, full-width SegmentedToggle below) via the local `SegmentedOptionBlock`; `SegmentedToggle` gained a backward-compatible `compact` (12sp) mode for the five-way picker. Selection logic untouched: the D-483 feed-first pick, D-494/D-499/D-520 shuffle deck semantics, the D-494 selectionCache (toggle flips never re-select) — the produceState keys now include templateKey + backgroundSource so both new toggles re-render the SAME content on stage.

## D-526 (round 55): the THREE-WAY ARTWORK SOURCE — "Auto / Cover / Episode"
"Make sure to give the user some customizability with the three-way, four-way, or five-way toggles, as they are in the episode type selection options and such." The old "Prefer cover art" switch (D-483 item) becomes a three-way segmented toggle over `posterBackgroundSource`: **Auto** = "banner" (the wide art first, falling back to the cover — the approved default), **Cover** = "cover" (always the poster art), **Episode** = "episode" (NEW — the episode's OWN still fills the stage first, falling back through the art chain). The "episode" load runs UNGATED by the thumbnail-card toggle (the still is the STAGE there, not just the card — `loadThumbBitmap` gained a `gated` param, default true) — but only when the gate made the first call a no-op (the review fix: when the card toggle is ON the first call already walked the full chain, and re-walking it on a dead CDN would stack a second 12s+8s+6s-per-link ladder onto every build). The chain of last resort is unchanged: the card thumb (when loaded) still doubles as the background of last resort, then the styled fallback stage.

## D-527 (round 55): release/1.1.18 — the release cut (the record lives on the branch)
`release/1.1.18` cut from the round-55 feature head `827440c1` (CI: the implementation push 35502594912 RED on exactly ONE error — `drawCenteredText`'s unqualified `textSize = textSize` inside `Paint.apply{}` resolved the LHS to the shadowing function PARAMETER, a val, not the receiver property — fixed by qualification in 827440c1, green on 35502967752; the independent pre-push review had already eliminated the two structural blockers — 4× nullable-episodeTitle smart casts and PosterCanvasMetrics living in the deleted PosterLayoutConfig.kt) as ONE commit (45c0bdd9: the version bump 1.1.18/10118 in AndroidConfig.kt + the branch-point record `DOCUMENTATION/cloudstream-v2/47-RELEASE-1.1.18-BRANCH-POINT.md`). Tag `v1.1.18` carries the user-facing What's New bullets — the headline: "the poster gets five templates, and the studio retires". Release APK run 35503422555 GREEN on the FIRST run — publishing `ani-kuta-v1.1.18-debug-arm64-v8a.apk` (66.7 MB) + `SHA256SUMS.txt`, stable + `--latest`, body = the tag bullets — verified via the API. 10118 > 10117: the debug app updates over its installed v1.1.17 in-app. Main stays at 0.4.20/85 (D-425). Full detail: the branch-point doc on the release branch.

## D-528 (round 56): CLASSIC v2 — the label-first flow on one shared centerline
The v1.1.18 device verdict: "the classic one is most definitely not good ... it needs to be redone properly ... improve the layout, the UI, and the overall look and feel much better." Two structural defects drove the redo: the old Classic hung the text from the TOP edge while the 400×225 thumbnail floated on the vertical centerline (a lopsided composition with the whole lower-right quadrant dead), and the tag row sat UNDER the title like an afterthought. The reimagined composition puts everything on ONE shared centerline: the still card shrinks to 360×202.5 (CLASSIC_THUMB_W/H — the column breathes) vertically centered on the left, and the right column is ALSO vertically centered (a measured stack, the Spotlight technique) reading top-down in the classic movie-poster hierarchy: the [EP n][SUB][DUB] tag row as an EYEBROW, then the ≤2-line 44px title, then the episode title. Thumbnail off → the text reclaims the full width, still centered. drawFlowThumbBox (the old 400×225 fixed-position helper) deleted with its only call site.

## D-529 (round 56): SPLIT → DUO — the true split-screen (new layout AND new name)
The verdict: "not satisfied ... we should completely rethink it. We should also rethink its name and other stuff." The old floating 420×344 panel read as a card on a background, not a SPLIT. The new composition is two full-height halves: the LEFT half is the episode art run edge-to-edge (0→512, corner 0 — a panel, not a card), the RIGHT half is a solid dark panel (0xE80C0A12, ~91% — the D-524 Card panel color, a touch more opaque because it backs the whole text column), and a 5px LIME SEAM (the brand accent, the EP-chip lime) welds them at the split. The text stack (title ≤2 @38 → tag row → episode title ≤2 @22 — the narrow 439px column needs the extra ep-title line) lives vertically centered in the panel. Thumbnail off → the background art shows through the left half; the split structure never breaks. Branding bottom-right sits ON the panel, legible. The enum entry is DUO("duo","Duo"); the retired "split" pref key ALIASES to DUO in fromKey (a pre-rename selection must not silently fall back to Classic), and the settings screen seeds templateKey through fromKey (the review's RISK-1 fix: the toggle highlights the template that actually renders).

## D-530 (round 56): MINIMAL v2 — the left-aligned floating column
The verdict: satisfied "but ... the title could be made better ... the height of the poster ... can be reduced ... maybe align it to the left side." The centered symmetry read scattered (every line started at a different x) and the stack sat tall. The new composition keeps the no-card purity but: the ≤2-line title, the tag row and the episode title all start on ONE left edge (a true column), the title grows 40→44 (the shared display size), the stack gaps tighten (14/12 vs 16/14), and the whole block is vertically CENTERED — a compact floating column, not a full-height banner of text. drawCenteredText and tagRowWidth (the centered-layout helpers) had no callers left and were deleted.

## D-531 (round 56): the MASTER-TOGGLE COLLAPSE — poster off = only the toggle remains
"When the poster notifications have been turned off, then almost everything will disappear properly with smooth animations. The top live preview itself will disappear. The layout options, the artwork, the elements, everything will disappear. The only toggle which will show is the poster notification toggle at the very top." The "Poster notifications" switch moved OUT of the Elements card into its own unlabeled PosterCard at the very top of the stationary region (PosterCard's label param became String? = null — a labelled card containing an identically-titled row would say everything twice), and BOTH the preview region (preview + Shuffle) and the scrollable options region (Layout/Artwork/Elements) wrap in AnimatedVisibility(posterEnabled) with fadeIn+expandVertically / fadeOut+shrinkVertically. The preview's produceState early-returns when off — the whole selection/compose pipeline (network art loads included) would be invisible work; a flip back ON re-runs it (posterEnabled is a key). The "poster notifications are off" preview state remains only as the collapse animation's exit content.

## D-532 (round 56): the ONE-LINE DESCRIPTION design language — structural, app-wide
"The descriptions will be one line only. I want you to apply this design language to the notifications page ... same goes for the updates page too ... the settings page too." TWO layers: (1) STRUCTURAL — the shared SettingsGroupCard.SettingRow description gains maxLines=1 + ellipsis, MoreListRow's subtitle drops 2→1, and every local row helper (the Updates SwitchRowContent/NavRowContent/mode-card/history/check-now texts, both Appearance/Details SwitchCards) matches — the clamp is the guarantee, the copy is the design; (2) COPY — every description that wrapped was shortened on the named pages (the notifications page's four trigger/customize/test strings, the episode-type blocks on both pages, the poster row) AND the sweep took the obviously-long holdouts the shared clamp would have ellipsized mid-word (Settings hub rows, Appearance adaptive/blur, Details tint/pan/morph, CustomPalette element descriptions, VideoCaching, the Episode-settings placeholder). DIVIDERS: the notifications page's trailing divider lines below single-row cards (Enable notifications / On schedule / Customize / Send test) were pure noise — showDividers=false on all four cards (the round-56 verdict: the line below entries is "not good").

## D-533 (round 56): the library badge round — playlist icon, 2dp float, 4dp shadow
Three device findings, one badge system: (1) THE TOTAL ICON — the film-strip read "like a chip rather than anything relatable": ROOT CAUSE is that Compose Icon tint flattens ALL fills into the single contentColor, so the film-strip's white perforations vanished and the silhouette was an abstract blocky rectangle. The redesign (BadgeIcons.Total) is a PLAYLIST — a solid play triangle + three episode bars — which works AS a one-color silhouette (the same constraint Sub's caption-frame and Dub's microphone already honored) and pairs semantically with them. (2) THE FLOAT — the TOP_CENTER floating badge sits 2dp inside the top edge (was 4dp): "move it up a little bit and leave just a slight amount of space". (3) THE PROMINENCE — every badge shadow (simple + compound) lifts 2dp→4dp: the counts were "blending into the background cover images". (PointedSide/PointedTagShape imports removed from LibraryScreen — the last executable-code references were already gone in D-480.)

## D-534 (round 56): the browse round — the soft-pill rating + the hero score pill
The verdict: the page "does not look well handled ... the rating is not shown properly either." The carousel score tag was the LAST 45°-pointed corner tag in the app (the Library retired its tips in D-480 — "sharp", off-theme) — it becomes the D-480 SOFT PILL: fully rounded, floating 4dp inside the cover corner, soft 3dp shadow, 10sp. The hero card buried the score in the meta text ("★ 85 · 12 eps · 2024") — the score now lives in its OWN amber pill (the shared rememberBadgeColorScheme language) BESIDE the rank pill, and the meta row keeps episodes · year. Every carousel cover gains a 3dp shadow so the cards lift off the page. The page now speaks one badge language end to end.

## D-535 (round 56): release/1.1.19 — the release cut (the record lives on the branch)
`release/1.1.19` cut from the round-56 feature head `d592cd12` (the implementation commit `a0fe0919` carried the 1.1.19/10119 bump; CI run 35508580132 GREEN on the FIRST run — the independent pre-push review's compile-risk sweep held, and its RISK-1, the templateKey seeding through fromKey, was fixed before the push) + ONE docs commit (the branch-point record `DOCUMENTATION/cloudstream-v2/48-RELEASE-1.1.19-BRANCH-POINT.md`). Tag `v1.1.19` carries the user-facing What's New bullets — the headline: "the templates round two: Classic reimagined, Duo replaces Split". Release APK run 35509186985 GREEN — publishing `ani-kuta-v1.1.19-debug-arm64-v8a.apk` (66.7 MB) + `SHA256SUMS.txt`, stable + `--latest`, body = the tag bullets — verified via the API. 10119 > 10118: the debug app updates over its installed v1.1.18 in-app. Main stays at 0.4.20/85 (D-425). Full detail: the branch-point doc on the release branch. The DASH download research (doc 17) rode this cycle as analysis-only — next session's launch pad.

## D-536 (round 57): the POSTER TOGGLE LIVES IN ELEMENTS — "Poster", the last row
The verdict: "the poster notification toggle should be shown at the bottom in the elements section itself and it should be named 'Poster' rather than 'Poster Notifications'." D-531's own top card is GONE: the master switch is now the LAST row of the Elements card, titled "Poster" (description unchanged: "Render notifications as banners") — and it is the ONE row that never collapses. Everything else wraps AnimatedVisibility(posterEnabled) individually: the stationary preview card (+Shuffle), the Layout card, the Artwork card, the Elements LABEL (PosterCard gained labelVisible so the label fades with its card) and the four element rows (one AnimatedVisibility Column inside the card). The whole-list AnimatedVisibility around the LazyColumn was removed — the list must stay composed because the toggle lives inside it. Off-state: the header + an unlabeled Elements card holding the lone "Poster" switch (the D-531 unlabeled-card look, relocated). The preview's produceState early-return stays.

## D-537 (round 57): the CS player pauses on app-background — MPV parity
The device report: "when I am playing a video and I exit the app ... the video keeps on playing in the background." The MPV watch screen has paused on ON_STOP since forever (WatchScreen.kt:467-490 — LifecycleEventObserver, ON_STOP → pause, ON_START → deliberately NO auto-resume); the CS screen had NOTHING (grepped both modules — zero lifecycle observers), so Media3 audio kept running under the home screen. The fix is the exact MPV block in CsWatchScreen (after the keep-screen-on effect): ON_STOP → engine.pause() (playWhenReady=false — unconditional, the MPV semantic), ON_START → log-only (parity: the user taps play; no auto-resume). ON_STOP/ON_START (not ON_PAUSE/ON_RESUME) so multi-window focus changes don't pause; no PiP exists in the app so ON_STOP unambiguously means "home screen/recents". CS audio-focus handling (Media3 handleAudioFocus=true) is orthogonal and stays.

## D-538 (round 57): the auto-link SKIP is PERSISTENT — no more re-attempts after a skip
The report: skipping the linking ("Skip AniList link" / the manual-link sheet) only lasted for the session — reopening the content from the library re-ran the auto-link and re-popped the sheet. Root cause: skipAniListLink only wrote in-memory state (AutoLinkState.Skipped), wiped on DisposableEffect reset AND on loadFromExtension's state reset; AutoLinkService had no "user skipped" lookup at all. The fix mirrors the D-238 reverse-direction blacklist EXACTLY: AutoLinkPreferences.markUserSkipped/isUserSkipped/clearUserSkipped keyed "auto_link_user_skipped:$sourceId:${animeUrl.hashCode()}" (the (sourceId, animeUrl) identity — the same pair the link cache keys on); attemptAutoLink returns Skipped (the SILENT result — no popup, no sheet) when flagged, checked after the per-source gate and BEFORE the cache check; skipAniListLink persists the flag from the Success-state identity; linkAniListEntry AND unlinkAniList CLEAR it (a deliberate link-cycle decision re-enables auto-linking); the sheet stays reachable from the three-dot menu (openManualLinkSheet never consults the flag). DELIBERATE SEMANTIC SPLIT: the sheet's explicit SKIP persists; a casual swipe-DISMISS does NOT (onDismiss → dismissManualLinkSheet — close-only, the pre-D-538 semantics) — an accidental dismissal must not permanently silence auto-linking.

## D-539 (round 57): CS DASH DOWNLOADS — the manifest+segments offline cache (the round's headline)
The device failure: "No downloadable sources. Only DASH streams were found for this episode: 3 DASH streams — stream them instead." Research doc 17's Option C refined and IMPLEMENTED: the app's OWN queue schedules the task; media3-datasource does the byte work into a SimpleCache; playback rides the EXISTING CS screen through a cache-wrapped factory. NO muxer (fMP4 has no single file without one the app doesn't ship), NO Media3 DownloadService (the queue/foreground service/notifications stay the ONE system), NO DB schema change (no migration chain exists — a row change would crash updated installs).
- THE ARTIFACT: a DASH download = the manifest + the chosen video rep's + ALL audio reps' init/media segments cached in SimpleCache(filesDir/cs_dash_cache, NoOpCacheEvictor — eviction would be silent data loss) under keys `csdash|<manifestUrl>|<resourceUrl>` (DashCacheKeys, core/common — the ONE contract both ends share; core/download and core/cs-player never depend on each other). The SAF folder still gets .data.json (with dashManifestUrl per entry) + .cover.jpg + .nomedia + the subtitle files — the durable metadata story is IDENTICAL to file downloads; only the video file is absent.
- THE PLANNER: DashManifestPlanner (core/download) — XXE-hardened, SegmentTemplate (SegmentTimeline S@t/@d/@r incl. negative repeats to period end; @duration numbering), SegmentList (media/range + Initialization sourceURL/range), SegmentBase/plain BaseURL single files, $Number%0Nd$/$Time$/$RepresentationID$/$Bandwidth$ substitution ($$ escapes), attribute inheritance rep→set→period (DIRECT-children lookups — a rep-level template must not leak into siblings), any ContentProtection → drmProtected (honest refusal: "This stream is DRM-protected — it cannot be downloaded. Stream it instead."), dynamic → unsupported, >20k segments → unsupported. Best video rep = the picked quality label's height (else highest bandwidth); ALL audio reps (one per AdaptationSet — usually SUB/DUB sets) so every audio selection plays offline.
- THE DOWNLOADER: DashDownloader — manifest fetched with the flattened MPV-format headers (DownloadHeaderParser), every part cached via media3's CacheWriter over CacheDataSource.createDataSourceForDownloading() (the writer skips already-cached spans natively = pause→resume re-caches nothing; writer.cancel() on coroutine cancellation = prompt pause), progress = accumulated bytes vs the bandwidth-derived estimate. Subtitles ride the D-FIX-SUB pattern. The upstream client = the CS RUNTIME's base client when on the Koin graph (named("cloudstreamPlayback") — the same cookies/interceptors the WORKING playback path uses; a resolve just succeeded so its clearance state is live) with the DOWNLOAD client as fallback.
- THE ROUTING: VideoTypeDetector gains DASH (.mpd / dash+xml); HttpDownloader routes DASH at the top of download() AND re-detects small MPD files after a generic fetch (the HLS redetect pattern); CsResolveSheet's DASH filter + the "Only DASH streams were found" empty card + the "can't be downloaded" footer are GONE — DASH links are pickable in download mode like everything else.
- THE PLAYBACK: downloaded_episode rows for DASH carry video_uri = "csdash:<manifestUrl>" (the marker — the mapper's video_uri ?: file_path fallback keeps serving it; NO schema change), file_size = real cached bytes. DetailsScreen's downloaded branch + the Downloads page route DASH rows to a CsWatchKey(offlineManifestUrl=...) — DIRECT backstack.add (the review caught csResolveRequest opening the resolve sheet for an already-downloaded episode). CsPlayerEngine.startOfflineDash: DashMediaSource over CacheDataSource(cache, OkHttp upstream) with the shared key factory + a maxVideoHeight pin (the manifest may list more video reps than were cached — an unpinned ABR could select a missing one; startInternal clears the pin so online playback never inherits it). CsWatchViewModel: startOfflinePlayback (phase straight to PLAYING — no resolution), requestPlayOffline (the online requestPlay CLEARS the offline fields — the review caught the stale-request leak), selectEpisode prefers the offline cache for downloaded targets (binge-watching downloaded episodes never touches the network; non-downloaded targets fall back to online resolution via the provider name), generation-guarded. Sidecar subtitles: the D-407 resolveSubtitleTracks chain (real SAF files) → CsSubtitleFetcher's new content:// branch → the existing overlay; embedded DASH text tracks ride the engine as always.
- THE DELETE: the DASH branch purges the cache (DashCacheStore.purgeEpisode — every key under the episode's manifest prefix) instead of deleting a video URI; subtitle deletes + the .data.json rebuild + the series-folder cleanup run as usual; DataJsonRepair.rebuildEpisodesAfterDelete now PROPAGATES dashManifestUrl (the review catch — losing it would turn every later scan into anti-shrink churn and a reinstall into an unrecoverable download). The scanner registers cache episodes straight from .data.json (no file walk possible) and the anti-shrink guard holds.
- THE QUEUE: untouched — one task type among the rest (the same foreground service, notifications, concurrency, RetryPolicy, auto-clear).

## D-540 (round 57): ECOSYSTEM TRUTH + the link-switch data.json translation
The ask: "if a user has downloaded episodes using the Cloudflare extensions ... and then switches ... to the normal Aniyomi extensions then it should ... update the data.json file accordingly ... properly translate the data.json file between these two formats and systems" (+ future providers). The research's code-verified gaps, fixed:
- ECOSYSTEM TRUTH: CS content was recorded as system "aniyomi" EVERYWHERE (DetailsViewModel ×3, ContentResolver.linkExtensionToExisting) while the seeded "cloudstream" system row sat unused. Now: CLOUDSTREAM_SOURCE_ID_FLAG (bit 62 — the documented twin of data:cloudstream's CsSourceIds constant) + Long?.isCloudstreamBridgedId() in core/content; linkExtensionToExisting picks the system row + contentId system section + extensionType default from the FLAG; resolveContentForExtension computes the ecosystem from the source id (new writes are truthful).
- THE HEAL: ContentRepository.healCloudstreamEcosystem (idempotent) relabels existing flagged rows (main_entry system_id → the cloudstream row; content_details extension_type 'aniyomi' → 'cloudstream') — runs at app start BEFORE the download scan (AnikutaApp), so the scan reconciles already-healed rows into .data.json.
- THE TRANSLATION: ContentIdentitySync (core/content, NON-suspend — the review catch; the manager owns its threading) fired by linkExtensionToExisting after the transaction; DefaultDownloadManager implements it: (1) the TWO PREVIOUSLY-DEAD .sq queries (updateDownloadedContentId / updateDownloadContentId — zero callers since REVIEW-5) now sync both download tables' content_id; (2) the durable .data.json identity block is rewritten through the VERIFIED 3-attempt ladder with the regenerated contentId (rewriteDataJsonEpisodes now takes contentId from the caller — the identity-rewrite path). The FILES never move (mainId-keyed folders): downloads stay playable across the switch; only the metadata translates. The episode keys remain source-specific (SEpisode.url) BY DESIGN — a cross-source episode-handle re-match is the smart-matcher's future domain, documented honestly.
- SELF-DESCRIBING METADATA: DownloadContentInfo + ContentDataJson gain providerName (the CS bridge name; null for aniyomi) — .data.json now states its own ecosystem, the future-proofing base for any new provider system.

## D-541 (round 57): the review round — 3 blockers + 4 risks, all fixed pre-push
The independent reviewer (general-purpose, review-only) verified the media3 1.9.3 surface against the real AARs, the SQLDelight param orders, the Koin graph, and traced the offline integration. BLOCKERS all fixed: (1) the suspend fun-interface call inside non-suspend linkExtensionToExisting (guaranteed compile failure) → the interface is non-suspend, the manager launches on its scope; (2) the offline CsWatchKey pushed via csResolveRequest opened the RESOLVE SHEET (a live network re-resolve of a downloaded episode — dead end or online playback, the offline branch unreachable) → DIRECT backstack.add at both call sites; (3) requestPlay left the stale playOfflineManifestUrl set (the trigger gives offline priority — EP A's cache under EP B's UI) → requestPlay clears both offline fields. RISKS fixed: dashManifestUrl survives DataJsonRepair's rebuild; direct-children addressing lookups (rep-level templates must not leak into siblings); Locale.ROOT for $Number%05d$ (Eastern-Arabic digits would 404 CDNs); the offline episode switch is generation-guarded. The 33-file brace/token sweep: ALL BALANCED.

## D-542 (round 57): release/1.1.20 — the release cut (the record lives on the branch)
`release/1.1.20` cut from the round-57 feature head `cf067e68` + ONE bump commit (1.1.20/10120 + the branch-point record `DOCUMENTATION/cloudstream-v2/49-RELEASE-1.1.20-BRANCH-POINT.md`). Tag `v1.1.20` carries the user-facing What's New — the headline: "DASH downloads land, the poster toggle finds its final home". Release APK run 35523678946 **GREEN** — `ani-kuta-v1.1.20-debug-arm64-v8a.apk` (66.8 MB) + `SHA256SUMS.txt`, stable + `--latest`, body = the tag bullets — verified via the API. 10120 > 10119: the debug app updates over its installed v1.1.19 in-app. Main stays at 0.4.20/85 (D-425). CI ledger this cycle: 4 one-fix implementation rounds (the media3 setter shape, the Kotlin $-template escapes + resume imports, the module-isolation primitives rule, the AnimatedVisibility receiver + content-type) + the green release run — every fix single-cause and log-diagnosed; the pre-push review's three blockers were real and would have been FOUR of those red runs otherwise.

## D-543 (round 58): the DASH manifest fetch is BYTE-FIRST — the v1.1.20 "Manifest could not be parsed" fix
The device report (MovieBox, "Today's Asuka Show" EP 5): the D-539 picker resolved 2 DASH links, the download queued, the manifest fetched clean (`200 application/octet-stream len=4012`, full body), and THEN "Manifest could not be parsed" — `DashDownloader.kt:87` re-throwing the planner's catch-all. Root cause: `fetchText` decoded the body to a String (forced UTF-8 — octet-stream declares no charset) and re-encoded it via `byteInputStream()` before `DocumentBuilder.parse`, while the STREAMING path hands media3's parser the RAW byte stream (parser-owned encoding detection). A UTF-16 body arrives as NUL-riddled mojibake; a gzip body (OkHttp only transparently gunzips when IT supplied Accept-Encoding — the visible Content-Length proves none happened) is binary either way; a plain UTF-8 body round-trips byte-identically and was never the failure. SECOND defect in the same catch: the `Exception → DownloadException` wrap swallowed IOExceptions, classifying genuine CDN blips non-retryable AND bypassing the queue's `pauseForNetwork` gate. The fix (doc download-research/19 is the full record + failure matrix): `fetchManifestBytes` reads `body.bytes()` with empty/8 MB guards, explicit gzip handling (Content-Encoding header OR the 1F 8B magic → bounded GZIPInputStream), `HttpException(code)` for non-2xx (5xx/429 retry, 4xx fail fast), RAW IOException passthrough (retryable + pause-for-network eligible); `DashManifestPlanner.parse(ByteArray, …)` parses the untouched stream — BOM/UTF-16 detection rides the platform — and parse failures now carry the REAL parser message + a sanitized 80-char body-head peek (an HTML/JSON/binary body is diagnosable from the task's error line alone). DRM/live/segment planning untouched; a hostile >8 MB response still buffers before the guard (pre-existing, acknowledged — streamed-read budget is a future candidate). The `findContentFolder — NO folder` WARN in the same logcat is BENIGN (the notification's cover-thumbnail lookup; the folder only exists after publish) — left untouched deliberately.

## D-544 (round 58): the repository branch cleanup — 26 → 3 remote branches
The user's ask: "a lot of unnecessary branches … we only need one of the latest branches … and one of the older branches, which is the test controller version 5." VERIFIED before deleting anything (the git proof, not trust): the 20 `release/1.1.x` branches are ALL preserved by their `v1.1.x` tags (the GitHub releases live on tags, not branches) and every post-tag branch delta is 100% AGENT-CONTEXT docs (progress/changelog/decisions/handoff records — zero code files, checked `git diff --name-only v1.1.N origin/release/1.1.N` for all 16 + 1.1.17/18/19/20 ancestry); the 3 old feature branches' unmerged tail commits are likewise docs-only (the release-record LIVE lines, duplicated content-wise by later rounds' docs). DELETED (23 remote, API 204 × 23): `feature/ads-return-pill`, `feature/poster-templates`, `feature/round-56-polish`, `release/1.1.1`…`release/1.1.20`. KEPT (3): `main` (default), `feature/round-57-cloudstream-downloads` (the latest branch — every change so far), `feature/test-controller-v5` (the user's requested keeper). Local mirrors cleaned to the same shape. Post-verify: 3 branches, all 87 tags intact, releases/latest = v1.1.20 stable unchanged.

## D-545 (round 58): release/1.1.21 — the release cut (the record lives on the branch)
`release/1.1.21` cut from the round-58 feature head `5f1796ad` + ONE bump commit (1.1.21/10121 + the branch-point record `DOCUMENTATION/cloudstream-v2/50-RELEASE-1.1.21-BRANCH-POINT.md`; the stale round-54/57 version-comment block replaced with the compact round-58 record). Tag `v1.1.21` ("DASH downloads, fixed at the byte level") carries the What's New: the parse-failure fix, the honest error lines, the smarter retries, the honest guards. Implementation run 35527713450 **GREEN on the FIRST run** (the review's SHIP verdict held); Release APK run 35528473234 **GREEN** — `ani-kuta-v1.1.21-debug-arm64-v8a.apk` (66.8 MB) + `SHA256SUMS.txt`, stable + `--latest`, body = the tag bullets — verified via the API. 10121 > 10120: the debug app updates over its installed v1.1.20 in-app. The release-branch push cost NO Build APK run (the D-472 filter held — release/** is not a trigger and the bump commit's only code file is build-logic, which rides the tag's own release run). Main stays at 0.4.20/85 (D-425/D-430). CI spend this cycle: EXACTLY 2 runs. The user's device round happens ON v1.1.21 — re-run the MovieBox EP-5 DASH download (the "Manifest could not be parsed" case must die), then airplane-mode playback; the poster/background/skip verdicts carry over unchanged.

## D-546 (round 59): the DASH parser factory fix — `setXIncludeAware` ALWAYS throws on Android; it was the REAL root cause all along
The v1.1.21 device round's "Manifest could not be parsed" carried the D-543 body-peek diagnostics, and they exposed the truth: the body was a VALID `<?xml …` manifest and the error ("This parser does not support specification "Unknown" version "0.0"") was unrelated to its content. PROVEN against AOSP libcore (`luni …/javax/xml/parsers/DocumentBuilderFactory.java`): `setXIncludeAware`, `isXIncludeAware`, `setSchema`, `getSchema` throw `UnsupportedOperationException` with that EXACT message UNCONDITIONALLY on every Android device — even `setXIncludeAware(false)` — because the package spec metadata is unset ("Unknown"/"0.0"). Both `DashManifestPlanner.newHardenedFactory()` AND `MpdParser.newHardenedFactory()` called `isXIncludeAware = false` UNWRAPPED inside the factory `.apply{}`, so the factory construction exploded before one byte of XML was read — on EVERY device, since the pipeline existed. v1.1.20 and v1.1.21 failed at the SAME line (D-543's transport theory was real but secondary — the byte-first contract stays, it's what made this visible). MpdParser's `runCatching` swallowed the same crash into the empty-info return, so its on-device "DASH hidden" verdicts were never real parse verdicts. THE FIX: the always-throwing line removed from BOTH factories (XInclude is not implemented by the platform parser — "not XInclude-aware" is already the default; DOCTYPE/external-entity feature refusals + `isExpandEntityReferences` stay, all supported); parse-failure messages now ALWAYS carry the exception class (`simpleName: message` — the old `message ?: simpleName` dropped the class and cost a device round-trip); the DashDownloader manifest fetch rides a derived patient client (`newBuilder()` — shared pool/interceptors/cookies) with connect/read 20s + call 45s (the device round burned attempt 1/3 on a 10.0s CDN cold-start timeout; attempt 2 answered in 823ms), segments/subtitles keep the shared client. THE RULE: on Android NEVER call setXIncludeAware/setSchema/getSchema/isXIncludeAware on DocumentBuilderFactory — any of them throws, always. Full record: download-research/20 (the failure matrix update). Blast radius: planner factory + MpdParser factory + the manifest-fetch timeouts only; streaming/queue/storage/notifications untouched.

## D-548 (round 60): DASH downloads become REAL files in the user's SAF folder — the SimpleCache media home is retired for new downloads
The v1.1.22 device round: the pipeline finally ran clean (D-546 + D-543 held), but the user's file manager showed the episode folder WITHOUT the video file — "the actual video file was not saved in the actual directory where it was meant to be saved". That was D-539's design (app-private SimpleCache; SAF folder = metadata only; `videoUri` null + the `csdash:<manifestUrl>` marker), not a bug. THE MOVE: (1) the planner exposes `videoParts` + `audioGroups`; (2) DashDownloader downloads each part with plain OkHttp (Range-aware, 200/206 slicing, short read = raw retryable IOException) and APPENDS into per-representation temp files, recording placements; a resume sidecar (`dash-resume.json`: manifest SHA-1 pin + per-group parts-done/bytes + the PLACEMENTS list) persists after every part — pause/retry resumes, crash-heal truncates to the recorded length, a short file restarts the group and subtracts its bytes; (3) publishDashEpisode copies them into the SAF folder as `<title> - E00001.mp4` + `<title> - E00001.audioN.mp4` + `<title> - E00001.mp4.dashmeta` (manifest bytes + the URL→file-range index) and returns `videoUri = csdash:<metaDocUri>` (the D-539 marker scheme, now with a LOCAL payload) + `dataJsonVideoUri` (the real file uri); (4) playback = `CsPlayerEngine.startOfflineDashLocal`: DashManifestParser over the STORED manifest bytes → SIDeloaded DashMediaSource + LocalDashDataSource serving file ranges — because an unindexed fMP4 is UNSEEKABLE in ExoPlayer (Android docs' own item), the manifest IS the index; seek points/duration/multi-audio all come from the DASH timeline; an index miss throws honestly (never a silent stream). ROUTING: DashCacheKeys gains payloadFromUri/offlineMetaUriFromUri/isOfflineDashUri; CsWatchKey.offlineManifestUrl → offlineMediaUri; the screen's play trigger branches content:// (local files) vs http (legacy cache); the scanners reconstruct DB rows with the marker (meta-sibling discovery, legacy fallback); the delete path deletes the video file for local-file episodes (token sweep takes audio+meta) and purges the cache for legacy ones. SAF TRAP (review blocker): FileUtils.splitFileName reconciles display-name ext vs mime — "audio/mp4" renames …audio1.mp4→…m4a and "application/json" renames …mp4.dashmeta→…json; fixed with "video/mp4" (exact match) + "application/octet-stream" (bypasses reconciliation). REVIEW: independent verdict FIX-FIRST → both blockers (SAF rename; the scanner rebuild dropping dashManifestUrl — the next scan would have rerouted D-548 episodes to MPV silent/unseekable) + one major (DataSpec.position is the ABSOLUTE remote range start — in-part offset = position − partPosition) + both minors (doneBytes inflation on group restart; the async load generation gate on every loader) all fixed before push. Full record: download-research/21. Blast radius: DashDownloader + planner groups + publishDashEpisode + scanner/delete/token-matching + the CS watch routing; streaming, queue, retry policy, progressive/HLS pipelines, notifications untouched; the SimpleCache binding stays for the v1.1.20–v1.1.22 legacy episodes.

## D-550 (round 61/62): the sidecar manifest is COMPOSED, not original — the ABR pick dies; audio versions switch offline; the audio/ folder
The v1.1.23 device round: the download layout validated, but the episode WOULD NOT PLAY. ROOT CAUSE: the sidecar stored the ORIGINAL manifest (three video reps: 1080p@1.6M, 720p@0.8M, 480p@0.35M) while only the picked rep is on disk — ExoPlayer's AdaptiveTrackSelection boots on DefaultBandwidthMeter's INITIAL 1 Mbps estimate → picked the 720p rep → chunk-stream1-* not in the local index → the honest IOException → no playback; any ABR down-switch would kill it mid-stream; the maxVideoHeight pin only CAPS (smaller undownloaded reps stay selectable). THE FIX both ends: READ side (fixes existing v1.1.23 episodes without re-downloading) — startOfflineDashLocal prunes the sideloaded manifest to the reps the range index covers (DashManifestPruner in :core:common — coveredRepIds derives each rep's FIRST planned URL with the planner's exact semantics: BaseURL inheritance chain, the same tag substitution, URI.resolve; pruneRepresentations removes uncovered reps; parse failure = untouched fallback; empty coverage = untouched fallback) + the maxVideoHeight pin dropped for content:// payloads (stale-label risk; redundant post-prune; KEPT for the legacy cache path); WRITE side — DashOfflineManifestComposer composes the sidecar manifest (prune to plan.videoRepIds+audioRepIds; import sibling audio sets with absolutized addressing; label every audio set). OFFLINE AUDIO SWITCH: sibling DASH variants (same server, different audio label, .mpd) ride AUDIO_VARIANT DownloadTracks through the EXISTING audio_tracks JSON column (no DB migration, no other downloader reads them) → DashDownloader fetches+plans+downloads their audio sets as extra groups (the resume sha pins primary+sibling manifests together; failures best-effort; total-hint adds plan.audioEstimatedBytes) → the composed manifest carries them as LABELED sets → the existing audio track selector works offline. THE AUDIO/ FOLDER (the user's requested layout): publishDashEpisode puts audio sets in audio/<base>.audio<N>.mp4; re-publish sweeps the legacy episodes/-located copies; delete sweeps + the scanner's sibling accounting cover BOTH locations; .data.json gains additive audioUris (scanner rebuilds from disk truth; ignoreUnknownKeys keeps every older reader safe). The .dashmeta sidecar STAYS (the manifest IS the index media3 cannot synthesize over an unindexed fMP4 — seek + duration + the multi-audio list live there). Record: download-research/22.

## D-551 (round 63): the link-name vocabulary learns LANGUAGE-AUDIO — one server, every audio version, every resolution
The v1.1.24 device round CONFIRMED the D-548/D-550 chain ("quite satisfied") and surfaced two symptoms of ONE root cause on the MovieBox dual-audio episode (Hindi + Original; one DASH manifest per variant — the aoneroom shape): (1) formatted mode showed the audio versions as TWO separate servers; (2) only ONE audio version downloaded — the D-550 sibling pipeline never engaged. ROOT CAUSE: "(Hindi Audio)"/"(Original Audio)" were invisible to the vocabulary — CsAudioTag.parse → "Default" for both, serverNameOf kept the bracket glued into the server name, and siblingAudioVariants matched by RAW name equality (never equal → zero AUDIO_VARIANT tracks → DashDownloader's D-550 machinery correct but never fed). THE FIX — one vocabulary, all consumers: (1) CsAudioTag gains the language-audio pass (bracketed "(Hindi Audio)" + whole-segment "MovieBox - Hindi Audio" forms; ~35-abbreviation normalization map ("eng"→"English", "orig"→"Original"); multi-audio/sub/dub families excluded; NO free-form matching — "MovieBox Audio Server" stays Default; prose brackets too long for the ≤24-char capture stay Default; the regexes are PUBLIC so the server-name derivation strips with the EXACT patterns the label parse recognizes); (2) serverNameOf MOVED from :feature:cs-watch:impl to CsServerNames.of (:core:cs-player) because a THIRD consumer needs the identical derivation — the :app sibling matcher now matches on the derived server ("MovieBox" == "MovieBox") and the D-550 pipeline finally receives the variant (its manifest's audio sets download as extra groups; the composed sidecar labels them "Hindi"/"Original" — the offline selector offers both); (3) the user's conditional wish — every available resolution: DashManifestHeights (:core:common, pure, the pruner's hardened-factory discipline — audio sets skipped via contentType/ContentComponent/mimeType; distinct descending; null on any failure) + CsDashQualityProbe (impl-private, the LINK'S OWN headers — the MovieBox manifest answers only with its CloudFront-Policy cookie + UA; bounded 5s connect/read, 8s call, 2 MiB cap; silent by contract) + additive CsVideoLink.availableQualities → the formatted card renders "Available: 1080p · 720p · 480p" under a version whose single DASH link was probed (the same list the player's per-stream quality section shows; the chip stays the pick target — ABR serves every listed rep). The probe runs OFF the resolve path (per-snapshot SupervisorJob scope, dies with the sheet/retry; the resolver flow byte-identical); pickableLinks is a STABLE remember(links, probeQualities.size) enriched view — a per-recomposition new list would reset the accordion's expanded-server remember every frame; the enriched list rides PreResolvedSeed so the in-player sheet inherits the lines. Untouched: RAW mode, the player, the downloaders, the queue, the scanner, the publish layout, data.json's schema (videoAudio now records "Hindi"/"Original" instead of "Default" — same field, richer truth), COMBINED sub/dub explicit tags. DASH-only probing this round; the field is type-agnostic for a future HLS master-playlist probe. Record: download-research/23.

## D-552 (round 64): the resolutions become PICK TARGETS — and the audio snippet dies at the planner
The v1.1.25 device round confirmed the D-551 grouping ("it did show me everything as one single section and it showed me the two audio versions") but surfaced three demands: (1) the probed "Available: 1080p · 720p · 480p" line was display-only — "I was not able to click on them. I was not able to directly download them"; (2) the declared quality lied — the provider stamps each variant's manifest with its MAX rep, so the sheet's "1080p = Hindi / 720p = Original" chips presented resolution as an audio-version property ("the actual thing was that 1080p resolution was available for the original audio too"); (3) both audio versions downloaded (the sibling matcher engaged) but "audio 1 … was just a random small snippet". SNIPPET ROOT CAUSE: the planner's SegmentTimeline branch expands every r="-1" entry to ONE segment when the MPD carries no parseable mediaPresentationDuration/Period@duration (periodTicks = -1 → r = 0) — audio packagers emit the whole track as a single negative-r entry (init + 1 segment ≈ seconds), the explicit video timeline stays complete, and NOTHING validated. THE FIX, four layers: (1) the planner borrows the video timeline's span as the period clock — two passes per Period (video plans first, each rep carries spanSec; audio expands against declared ?: videoSpanSec), DashSegmentPlan gains videoSpanSec + audioGroupSpansSec (single-period only) + audioSetsPresent, and the estimate clock falls back to the derived span; (2) the downloader validates — primary audio known-short (<50% of video span) is a hard honest DownloadException, known-short siblings are SKIPPED (the picked variant never pays), declared-but-unplannable audio is refused (a silent video is garbage too), spans log as evidence; unknown spans never fire (the guard refuses only what it can prove); (3) the sheet renders ONE CLICKABLE CsQualityChip PER PROBED HEIGHT for a single-DASH probed version — the declared chip + the "Available:" text are REPLACED (the lie dies with the display), the pick target becomes (link, height); (4) playback pins the height via the engine's one-shot pendingVideoHeight applied at READY through selectVideoTrack — the SAME TrackSelectionOverride the player's quality section uses (a selector, not the round-62 cap; not offered → cleared, ABR continues) — and download rides DownloadRequest.videoQuality ("720p": the queue label, the DB column, data.json's quality, preferredHeightOf's closest-rep pick) with ZERO schema change; siblings stay resolution-independent ("both audio versions at the resolution of the main one" by construction). VERIFICATION MILESTONE — the planner is pure JVM, so the sandbox compiled + EXECUTED the tests for the first time (kotlinc + junit-4.13.2 on the raw sources): DashManifestPlannerTest (NEW, 8 locks incl. the snippet lock) 8/8 green executed, and the D-551 DashManifestHeightsTest had NEVER compiled — "$Number%05d$" is a Kotlin string template (compile error; CI runs assembleDebug only, test sources never compile) AND its fixture left whitespace before the <?xml?> prolog (spec-forbidden, fatal) — both repaired, 6/6 green executed. In passing: SegmentTemplate@duration is ALREADY in timescale ticks (ISO 23009-1 §5.3.9.2) — the old duration*timescale double-scaling fixed to spec, which otherwise would have masked a snippet behind a 96,000s span post-cross-derivation. Untouched: RAW mode, sub/dub grouping, the probe, the publish layout (episodes/ + audio/ + .dashmeta), data.json's schema, the queue/scanner, the player's own quality section. Record: download-research/24.

### D-553 (round 65): the selection TRUTH — the height memory, the honest markers, the reachable quality section
- **What:** (1) CsSourceMemory remembers the picked RESOLUTION per mainId alongside the server (one memory unit); the re-entry + auto-advance play paths pin the recalled height (a null-height pick clears it — declared chips cannot pin, so they must not fake a preference). (2) The player's selected-track marker is computed from group.isTrackSelected (the ACTUALLY-playing rep), refreshed live on videoHeight changes; the accordion lights exactly ONE probed chip (current link × the live decoder height), tints the playing version's label, and shows a "Now playing: <audio> · <label>" line. (3) The player links sheet scrolls as ONE body so "Quality for this stream" is always reachable (the pre-D-553 non-scrolling Column clipped it behind a tall MovieBox accordion). (4) shortAudioLabel: deterministic minimified audio-version labels at 3+ versions in the collapsed header. (5) onVideoSizeChanged logs the decoder's actual WxH (deduped).
- **Why:** the device logcat proved 480p was pinned and played (decoder 856x480) while the user believed 1080p was playing — the app made the right choice but hid it: the marker lied (group.isSelected names the FIRST rep), the quality section was clipped out of reach, and the height did not survive re-entry/auto-advance (the link was remembered, the height was not). A selection the app cannot SHOW is indistinguishable from one it ignored.
- **Status:** ✅ implemented + released in v1.1.27 (record 56).
- **Date:** Round 65.

## D-554 (round 66): the CS quality sheets calm down to 0.60 + the episode-list customization FIRST DRAFT (the dedicated settings page with the stationary live preview)
The v1.1.27 device round verified ALL THREE round-65 fixes ("quite satisfied … exactly like how I hoped"): the resolve-sheet top area, the collapsed short forms, the 480p pick playing at 480p WITH the honest indicators, and the qualities/servers markers. ONE complaint remained: the qualities/servers bottom sheet was "a little bit taller in terms of its height" — the user asked to "adjust it dynamically … maximum height gets reduced properly and smoothly". CODE-VERIFIED DIAGNOSIS: both CS quality sheets already content-fit under a screen-fraction cap — CsPlayerSheets at `csSheetMaxHeight(0.70f)` and CsResolveSheet at a duplicated `screenHeight * 0.70f` — so the fix is a FRACTION change, not new machinery (material3 stays pinned at 1.3.1/D-322, whose ModalBottomSheet has no sheetMaxHeight parameter; the subs sheet's 0.55 precedent from Task 57 P6a). THE FIX (D-554-A): CsLinksSheet → 0.60 (the complained-about sheet), CsResolveSheet → 0.60 (parity — the user's phrase covers both quality surfaces), CsEpisodesSheet pinned at an EXPLICIT 0.70 (a browsing surface, not a complained-about one; a shared-default change must not drag it down), the app-wide default stays 0.70, and the DESIGN-SYSTEM sheets doc carries the exception note. "Dynamically" remains true (content-fitting; the cap binds only on overflow) and "smoothly" remains true (the accordion's expandVertically animates growth under the cap; the D-553 single-body scroll absorbs the rest).

THE FEATURE: the episode-list customization FIRST DRAFT. The user asked for "giving the user customise ability … plan these out and implement the first draft please with an actual live preview at the top in the settings … a dedicated page just like how there is for the poster banner notification". THE PATTERN (research-verified): NotificationPosterSettingsScreen is the exemplar — a dedicated full-screen page whose top region is a STATIONARY live preview rendering the SAME production renderer the feature uses (D-481), options in a LazyColumn below (D-525 single 8dp gutter), persistence = the Koin-singleton prefs class, consumption = reactive `Preference.changes`. THE ARCHITECTURE: (1) `EpisodeListPreferences` grows 7 reactive prefs (`rowStyle` DETAILED/COMPACT/MINIMAL + showSynopsis/showDatePill/showAudioPills/showWatchProgress/dimWatched/showDownloadControl — every default equals TODAY's behavior, so the zero-prefs experience is byte-identical); (2) `EpisodeRow` MOVES out of the 4,243-line DetailsScreen.kt into its own file (with the public `EpisodeListRowStyle` + `EpisodeListDisplayStyle` + the pure `pillsRowVisible` algebra; the dead `DownloadEpisodeButton` deleted) so BOTH the details list and the app-module settings page can call it; (3) DetailsScreen collects the 7 prefs ONCE at screen level (ONE subscription set, not 7×N rows) and folds them into one `EpisodeListDisplayStyle` riding every row; (4) `EpisodeListSettingsScreen` (app/settings) replaces the `EpisodeSettingsKey` placeholder — the D-477/D-481/D-525 scaffold: stationary preview top (THREE static sample rows through the REAL EpisodeRow: a full row with thumbnail/synopsis/date/pills/watch-progress, a WATCHED row showing the dim+grayscale treatment, a bare row exercising the number-disc path; the thumbnail is a generated 120×68 PNG embedded as a data URI so the grayscale/dim effects show REAL offline pixels; rows are inert, callbacks no-op), options below (Layout card = 3-way SegmentedToggle; Elements card = six switch rows with one-line descriptions; a "More" card honestly pointing sort/filter/grouping to the details-page sheet), the preview's style collected from the SAME prefs (one source of truth, zero drift); (5) the Appearance row retitles to "Episode list / Layout, elements, and live preview". SCOPE DISCIPLINE (the D-523 lesson — curated presets, not a free-form editor): grid layouts, title line counts, per-show overrides, watch-screen-clone/CS-sheet application, gestures, and the never-rendered badges are recorded for later rounds, not built. Element toggles are honored WITHIN the style's frame — a style that never renders a section cannot be talked into rendering it. TESTS: the module's FIRST test source set (`testImplementation(libs.junit)` one-liner) + `EpisodeListStyleTest` locking the lenient `fromKey` (D-529 seeding), the defaults, and the `pillsRowVisible` algebra — 11/11 EXECUTED green in the sandbox (kotlinc 2.0.20 + junit; the execution CORRECTED two of my own expectations: the download control relocating to the pills row keeps the row alive when every other resident is gated off — the isolation tests needed download=false). Untouched BY DESIGN: the details-page list-settings SHEET (sort/filter/grouping — list SHAPING; the new page is row APPEARANCE; they coexist), RAW mode, the resolve/play/download pipelines, the watch-screen clone (D-119), the CS episodes sheet, every other sheet's height, data.json's schema. Record: download-research/26.

## D-555 (round 67): the CS sheets find the middle height (0.65) + the episode list becomes FOUR completely different layouts
The v1.1.28 device round rejected the D-554 first draft on BOTH fronts. (A) THE SHEET: 0.60 was "way too smaller" — the user asked for "a bit more taller percentage but smaller than the previous one"; both CS quality surfaces (CsLinksSheet + CsResolveSheet, the D-554-A parity pair) move to **0.65**, the exact middle between the v1.1.27 complaint (0.70) and the over-correction (0.60); CsEpisodesSheet stays explicitly 0.70. (B) THE EPISODE LIST: the user's verdict was explicit — "switching between the three available layouts is definitely not good … I was hoping for a complete UI redesign on switching between each and every single one of them … a completely different layout, a completely different management system, a completely different look and feel … rather than just hiding some features like synopsis or maybe adjusting the shape a little bit", plus FOUR directives: the live preview shows EXACTLY TWO episodes, the samples carry PROPER thumbnail imagery, the release date + metadata render properly everywhere, and there are FOUR options each a genuinely different design. THE REDESIGN: the D-554 three row variations (DETAILED/COMPACT/MINIMAL — one row with sections hidden/shrunk) DIE; `EpisodeListRowStyle` becomes CLASSIC/GRID/TIMELINE/CINEMA, each a different STRUCTURE dispatched through the new **`EpisodeListEntry`** — the ONE entry point both the details list and the settings preview call (the D-481 doctrine at the list level): CLASSIC delegates to the unchanged user-verified `EpisodeRow`; GRID (a two-column poster wall: full-bleed 16:9 cells, EP badge + compact download badge overlays, watch-progress on the image edge, title + date/audio chips below, watched = grayscale + centered check, LONG-PRESS toggles watched — a half-width cell cannot swipe); TIMELINE (an air-date schedule spine: a continuous rail — the call site drops the per-item vertical gap for it — whose node labels carry the release date as the PRIMARY element with an "EP n" fallback when dates are absent/toggle-off, nodes fill when watched, a compact card with trailing thumbnail hangs off every node); CINEMA (full-bleed banner cards: the image IS the card, bottom gradient scrim, huge ghost episode number top-end, overlaid title + translucent date/audio/WATCHED chips, translucent download badge). The compact **`EpisodeDownloadBadge`** mirrors the full control's 8-state contract in ONE 32dp circle (download/pause/resume/retry/play; in-flight states cancel; delete stays on the downloads page — no gesture room); the Phase WP swipe-to-toggle gesture moved VERBATIM into **`SwipeToToggleWatched`** (shared by CLASSIC/TIMELINE/CINEMA; GRID long-presses instead); the display-value resolution (D-306 extension-first + the D-230 fallback + BOTH date label sizes + the HSUB-distinct audio parse) consolidated into **`rememberEpisodeDisplayData`** — one pass shared by every layout. THE PREVIEW: exactly TWO samples (fresh with a 40% progress bar + watched with the dim treatment) through the same dispatcher — GRID pairs them side-by-side like the real wall — both carrying REAL thumbnail imagery: two GENERATED anime stills embedded as PROPER `data:image/jpeg;base64,` URIs (448×252 q64 — the D-554 draft's constant was a BARE base64 payload without the data: scheme prefix, so Coil's DataUriFetcher never matched and the thumbnails rendered EMPTY — the root cause of the user's "the thumbnail images remain empty"), release dates on both, and the four-way Layout toggle states each option's identity line under the selector. THE PLUMBING: the details-list GRID branch chunks `subDubDisplayEpisodes` into paired rows INSIDE the existing LazyColumn (the outer list remains THE virtualizer — no nested grid); the shared per-episode body extracts into one local `renderEpisode` composable lambda both branches call. Legacy stored keys migrate through `fromKey` to CLASSIC. TESTS: `EpisodeListStyleTest` re-anchored + 5 new locks (the legacy migration, `formatShortDate`, `timelineNodeLabel`, `ghostEpisodeNumber`) — 13/13 EXECUTED green in the sandbox against the byte-exact extracted production declarations. Untouched BY DESIGN: the prefs keys, the CLASSIC row's visuals, the swipe algebra, the season/organize systems, the download pipeline, the resolve flow, the other sheets' heights. Record: download-research/27.

## D-556 (round 68): the preview shows YOUR library + the layout refinements (the v1.1.29 device round)
The v1.1.29 device round returned a split verdict: the CS sheets at 0.65 were "proper … exactly like how I wanted" (asked for **0.67** — the user's exact number — CsLinksSheet + CsResolveSheet move; CsEpisodesSheet stays 0.70), the four layouts "are good, unique, and different", and a detailed punch list followed. THE HEADLINE ROOT CAUSE: **Coil 3.0.4 has NO DataUriFetcher** — proven by unzipping coil-core-android-3.0.4.aar and listing coil3/fetch/* (Asset/Bitmap/ByteArray/ByteBuffer/ContentUri/Drawable/FileUri/JarFile/ResourceUri only). D-554's bare base64 AND D-555's `data:image/jpeg;base64,` scheme could therefore NEVER match a fetcher — the preview thumbnails were structurally dead, and the D-555 commit message's "root cause" claim was wrong. THE FIX: the two demo stills ship as real JPEG drawable resources (ep_preview_still_1/2.jpg, 640×366 q82) referenced through `android.resource://` URIs (ResourceUriFetcher's bread and butter) — offline, tiny, same AsyncImage path as network thumbnails; the two ~39KB base64 source constants died. THE REAL PREVIEW (the user: "actual live previews from my library … it will pick any random series … should have its episode list loaded … with images … proper titles, proper descriptions … audio versions … release date"): `loadLibraryPreviewItems` (Dispatchers.IO, zero network) reads ContentRepository.getAllLibraryItems/getAllContentDetailsMap + DataCacheRepository.getAllEpisodeAudioAggregates/getEpisodeMetadata, filters qualifying series (releasedCount ≥ 2 — the data-cache table IS "episode list loaded"), SHUFFLES ("any random series"), probes up to 8 candidates, prefers imagery (episode stills → series cover dataCoverUrl?:extThumbnailUrl), and reconstructs SEpisode + the full D-190 EpisodeMetadata FIELD-FOR-FIELD like DetailsViewModel's cache-restore; audio pills ride the cached scanlator or an honest aggregate-derived token hint (audioScanlatorHint); demo samples only when nothing qualifies. THE LIVE BEHAVIOR: slot 1 boots fresh (40% progress) + slot 2 boots watched (the user's default), swipe/long-press toggles watched (local watchedOverride map), and every download tap advances a 9-step cycle through ALL EIGHT EpisodeDownloadState variants (nothing→resolving→queued→downloading 35→paused→downloading 72→error→retrying→downloaded→wraps). THE SCROLL COLLAPSE (the user: "only this much that the first episode list is hidden, and the second one will remain there"): a custom layout modifier reports contentHeight − collapse×(row1+gap) with the content shifted up under clipToBounds; the fraction tracks the options list's scroll (0 at top; 200dp ramp; list-end ⇒ 1 so short lists complete; a fitting list never collapses on entry); GRID exempt ("already compressed enough"). LAYOUT SWITCHES glide (animateContentSize 360ms FastOutSlowIn — "the layout section should move down slowly"). THE SELECTOR: the "Four completely different designs" line + the identity line DELETED (the preview IS the description); the bottom "More" card DELETED; SegmentedToggle's pill SLIDES on a spring (drawBehind pill from the real segment geometry, height = the row's own; all 8 call sites inherit). THE LAYOUTS: CINEMA's ghost number → a themed EP badge (the details page's own primary/onPrimary — "the episode numbers should actually be in the theme color of the details page"; the ghost survives only as the no-thumbnail placeholder, keeping the ghostEpisodeNumber unit lock honest); TIMELINE's date node gains the BLOB merge (the offset STAYS per the user — a same-color-as-card union of a soft capsule + a neck under the card's edge, drawn spine→blob→node so the junction is seamless); GRID RECREATED (title over the image on a scrim, ringed watched check, hairline border, capsule chips); the TAGS become type-coded capsules (EpisodeDateChip quiet surface; EpisodeAudioChip SUB=primary, DUB=tertiary, HSUB=neutral) shared by CLASSIC+GRID — pillsRowVisible algebra untouched, all 13 test locks hold. Untouched BY DESIGN: prefs keys, swipe algebra, download pipeline, resolve flow, the outer LazyColumn virtualizer. Record: download-research/28.

## D-557 (round 69): the preview works TWICE + the snap collapse + the layout polish (the v1.1.30 device round)
The v1.1.30 device round verified the 0.67 sheets ("100% perfect"), the real-library preview, and the smooth layout switching — then a punch list whose theme was interactions that worked exactly once. (A) THE STALE-CLOSURE BUG: SwipeToToggleWatched's pointerInput(Unit) captured the FIRST onToggleWatched forever; the preview's `{ override = !isWatched }` captured the watched VALUE, so swipe 2 wrote the same value back. FIX: rememberUpdatedState in the gesture (every caller protected) + the preview reads the map at execution time. (B) THE DEAD SPINNER: EpisodeDownloadControl's Resolving was a bare non-clickable CircularProgressIndicator (violating CORE_RULES §23 while the badge already cancelled in-flight) — now tappable→onCancel, visually identical; the opt-in previewTapAll (control + badge + EpisodeRowActions + EpisodeListEntry) makes the WHOLE widget one tap target for the preview demo — production call sites (null) keep the exact per-state contract; the Downloading/Downloaded menu-openers route through it too. THE LIVE DEMO: the Downloading step animates +10%/s (LaunchedEffect keyed on the step), auto-advances to Downloaded at 100%, a tap cancels it mid-flight. (C) THE TWO-PHASE SNAP COLLAPSE: a NestedScrollConnection consumes the WHOLE drag while a phase change is possible (the list does not move in that gesture); crossing halfway (first-row height + gap, measured) snaps open↔collapsed with an animated settle; a crossed-latch makes one drag flip exactly once (the accumulate-reset draft bounced within one gesture — caught in review); the leftover fling is swallowed (onPreFling→Zero); a 180ms silence window re-arms; GRID never engages; a style switch always re-opens. (D) THE NUMBERS LEFT THE IMAGERY: the classic + grid on-image EP pills died ("the image gets covered way too much"); EpisodeNumberLabel renders "EP n" (or the compound S-n/E-m two-shade, verbatim) above the title. (E) GRID redesign 2: borderless pure image plate (no scrim/rim/pill) + the text block below; the watched check became a quiet translucent bubble. (F) THE TIMELINE BLOB as ONE shape: TimelineBlobCardShape unions the rounded card body + a fully-rounded symmetric stadium bump around the node/date in a single Outline.Generic — no seam by construction (the D-556 neck stopped short of the card edge: the spacer + the corner rounding left the cutout); the spine draws in two segments passing behind; content padding clears the bump; the bump bottom clamps to the shape height. (G) THE CINEMA GHOST NUMBER RESTORED: the user LIKED the original huge numbering and asked only to theme it + make it more visible ("you added the bad tag kind of numbering") — the 56sp zero-padded ghost is back in the details page's own primary, near-full alpha + a soft dark shadow; the D-556 badge died. (H) THE PROGRESS GLITCH: the square-capped full-width LinearProgressIndicator overflowed the rounded thumbnail corners; EpisodeWatchProgressBar (rounded inset pill, translucent track) serves classic thumb/grid plate/timeline card/cinema banner. CI LEDGER DISCLOSED: 3 implementation runs (35739592844 RED: LayoutDirection from ui.graphics — it lives in ui.unit; 35739951550 RED: (Int+Float).coerceAtLeast(1) — T fixates to the receiver's Float; 35740400460 GREEN), both repairs single-cause and log-diagnosed. LESSONS: the Shape interface's parameter packages split across geometry/unit namespaces; a generic coercion's literal must be the receiver's type; pointerInput(Unit) callbacks must be read through rememberUpdatedState when they capture state. Record: download-research/29.

## D-558 (round 70): the settings SEARCH + the heading-back redesign + the deterministic collapse (the v1.1.31 device round)
The v1.1.31 round verified the carry (every swipe toggles; every download tap advances; classic + grid done) and set four streams. (A) THE COLLAPSE v3 — SPLIT BY DIRECTION: the D-557 snap's two blind spots (fast flings slipped past the drag-only accumulator; the up-direction consumed PRE-scroll so the preview expanded while the list was scrolled down) die by construction — DOWN-while-open consumes EVERYTHING until the preview snaps collapsed, then RELEASES the same gesture into the list ("after that then the bottom section will begin to scroll over"), and a consumed down-fling hands its momentum to the list through a spline-decay scroll (AnimationState.animateDecay + dispatchRawDelta — the documented fling pattern) so fast swipes collapse AND keep scrolling; UP never consumes in pre-scroll — the list scrolls to the very top FIRST and only the post-scroll leftover (or the finished fling's leftover in onPostFling) expands the preview. Snap/latch/settle/GRID-exemption/re-open carry over. (B) THE TIMELINE THIN GAP: the union blob over-merged; TimelineBlobCardShape keeps ONE outline with DISJOINT sub-shapes — the stadium stays around node+date (2..44dp) and the card starts 8dp right (52dp) — a thin background-colored breath, no junction possible. (C) THE CINEMA SECTION: three new prefs (cinemaNumberCorner RIGHT/LEFT, cinemaNumberStyle SOLID/FROSTED, cinemaWatchedCheck default FALSE) → three EpisodeListDisplayStyle fields (defaults = current behavior; the test lock holds) → the card (corner alignment; frosted = translucent plate BEHIND + frost veil ON TOP of the number; the check only when opted in) → a dedicated LazyColumn item between Layout and Elements that exists only under CINEMA (AnimatedVisibility expand/fade in, shrink/fade out), two single-row SegmentedToggles (no descriptions per the user) + a description-less switch, plus the page-bottom caption "Can be further customized by clicking episodes on the page" (the user's wording). (D) THE SETTINGS SEARCH: five new files in app/settings/search — Models (page enum + entry + result), Engine (pure scorer: all-tokens-match, weights 100/85/75/60 title · 65/50/40 keywords · 45/35 synonyms · 20 subsequence; built-in synonym map theme⇄ui⇄accent; 12-result cap; the weights are the tuning surface), Index (~45 curated entries — the ONE place to add items), Navigator (consume-once pending anchor; mismatched reads leave the request for the top screen), Highlight (anchor→index LaunchedEffect scroll + a three-pulse target wrapper; mode-aware maps where items are conditional). The hub gains a search bar; results show title + breadcrumb; tap → request + chained backstack pushes (Appearance→General) → the target consumes, scrolls, pulses. (E) THE HEADING-BACK: CollapsingHeader gains onBack — a compact 30dp arrow + 8dp breath + the clickable title ("the heading should not be moved to the right that much"); ~24 call sites converted app-wide (all settings screens, history, updates, downloads family, trackers, about, the extensions family, profile — UpdatesSettings gained its FIRST back affordance); the top-right BackAction retires. Independent review: 1 blocker (the search bar's focusManager out of scope) + 2 advisories (the mode-aware Updates index missed the check-now card; a dead import) — ALL fixed pre-push; brace balance machine-verified. Record: cloudstream-v2/30.

## D-559 (round 71): the timeline CONNECTS through a chicken neck, the frosted TEXT is the frost, the heading is ONE button, and the search learns "episode thumbnail" (the v1.1.32 device round)
The v1.1.32 round confirmed the carry ("exactly like how it is meant to be" — the collapse FROZEN; the cinema options "exactly like how I hoped for") and corrected four mis-implementations. (A) THE CHICKEN NECK: the D-558 disjoint gap read as a full disconnection ("instead of making the gap thin, you just outright removed it … like a chicken neck kind of thing, both of them being connected but with a thin chicken neck kind of feel") — TimelineBlobCardShape carries THREE sub-shapes in ONE outline: card + stadium + a ~10dp neck bridging the 8dp gap, centered on the bump's tangent-vertical midline, 2dp overlaps into both bodies (no antialias hairline; nonzero-winding union, no Path.op), clamped against inverted rects. (B) THE FROSTED TEXT: "the text itself being frosted rather than it getting a frosted effect on it" — the plate+veil died; the number is TWO stacked copies: a blurred halo (graphicsLayer alpha 0.45 + BlurEffect(14f) on S+, a plain under-copy below — minSdk 24 safe) and a translucent crisp copy (0.58 + soft shadow); the imagery shows through the glyphs. (C) THE ONE-ELEMENT HEADING: arrow + title are ONE clickable Row (Role.Button, single ripple) — the arrow at 2dp (INSIDE the 16dp padding), a 4dp gap, the title back at ~26dp (D-558's box had pushed it to ~54dp); no-back headers keep the exact 16dp. (D) MY PROFILE: the alpha-0 mini pill still occupied 120dp → the 32sp title truncated to "My PR…" until the first scroll (24sp fit where 32sp didn't); the pill's WIDTH now animates 0↔120dp on the header's own 300ms curve (alpha stays scroll-driven). (E) THE SEARCH BATCH: the clear affordance is a 24dp Box (the 48dp IconButton was the bar's height growth); query is plain remember (SaveableStateProvider keys never reset — rememberSaveable resurrected the stale query on every re-entry); BackHandler(enabled = query.isNotBlank) clears before the device back pops; the results live in ONE grouped card with hairline dividers, per-page icon tiles (a 24-branch page→glyph map) and a trailing chevron; the engine gains the phrase scores (title 90 / keywords 70 — BONUS-only: the reviewer proved the original "rescue" was dead code, every token of a phrase in a field is that field's substring, and a rescue would have double-counted) + episode/thumbnail synonyms; the index gains SEVEN poster-row entries with anchors ("episode thumbnail" hand-traces to Episode-thumbnail-row 250 → Episode list 220 → Notification poster 200) and the poster screen takes its half of the search contract (highlightAnchor + anchor scroll + row pulses — the D-558 gap). Independent review: VERDICT SHIP, zero blockers; the dead-rescue advisory fixed pre-push; brace balance machine-verified on all 8 files (nested-comment-aware). Record: download-research/31.

## D-560 (round 72): the sponsor popup gets its breathing room + the overlay option that appears ONLY while it's missing + the ONE-TIME actual release build (the v1.1.33 device round)
The v1.1.33 round confirmed the round in full (the episode list "quite proper… exactly like how I hoped", the search "proper and everything is clean and good looking", "almost everything has been handled properly") and set two directives. (A) THE SPONSOR POPUP ("a bit more cramped… make it fun… clean… minimal… simple… not say anything too much"): the frozen machinery is untouched byte-for-byte (the observer register/unregister, the AdInProgress-only foreground-return collection, the hide-pill verdict effect, the pill show() INSIDE the tap handlers BEFORE the coordinator calls, the back-cancels Dialog contract, the Idle early return, the Crossfade state coverage) — only the SHELL changed: a 64dp hero bubble (primary @ 0.10) behind each state's 26dp glyph (the card's one piece of fun), a quiet tracked-out SPONSORED eyebrow, ONE short line of copy per state (the two-sentence paragraphs die — Pending: "A quick visit to our sponsor keeps ANI-KUTA free."; Waiting: "Come back to ANI-KUTA when you're ready."; TryAgain: "Stay with our sponsor for ${seconds}s, then come back." with the real threshold), explicit measured-Spacer rhythm (the card grows to 28dp side padding, 28dp corners), and "Not now" as the unified escape. (B) THE OVERLAY OPTION (the user's exact spec: "if the user has not turned on… draw over other apps, then the app will give the user an option there to enable it. But if the user has enabled it, then it will not give the user that option, or it won't even show anything"): OverlayPermissions (an internal :core:ads object — a deliberate COPY of the D-449 wizard pair, not a dependency, because :core:ads depends on NO feature module) exposes hasAccess (canDrawOverlays, API 23+ / minSdk 24 — no guard) + openSettings (the per-package ACTION_MANAGE_OVERLAY_PERMISSION intent → the list fallback); OverlayPermissionRow is ONE Surface(onClick) row (16dp shape, surfaceVariant @ 0.45, a 20dp Layers glyph, "Enable draw over other apps", a trailing chevron, NO second sentence) rendered ONLY inside `if (!overlayGranted)` on the two sitting states (Pending/TryAgain — never the transient waiting state); the consent is remember'd (a DEVICE fact) and re-read by a LifecycleEventObserver on every ON_RESUME so the row vanishes the moment the user returns from the system toggle with consent granted — and consent granted mid-popup makes the return pill (a silent no-op without it) work for THIS very ad. Independent review: VERDICT SHIP, zero blockers (all 49 imports used one-by-one; every symbol verified against in-repo precedents — Surface(onClick) without ExperimentalMaterial3Api, lifecycle-compose LocalLifecycleOwner, Layers/ChevronRight usage); advisories folded pre-push ("5 s"→"5s", the unused state param dropped, a 4dp rhythm spacer). (C) THE ONE-TIME ACTUAL RELEASE BUILD (the user: "the actual release build, not the debug build release… ARM64 V8, the V7 version, the x86 and 64 version and the x86 version and the universal version… this is a one-time thing"): the D-430 machinery is reused unchanged (`-PreleaseAllAbis=true` → assembleRelease → five release-signed APKs staged as ani-kuta-v{version}-{abi}.apk + universal + SHA256SUMS.txt + the RELEASE zip — the exact v1.1.2-era shape) with the proven pre-D-447 gates (tag↔versionName, keystore from secrets + keytool sanity, existence + per-APK lib/ ABI audits, per-APK apksigner with the cert-DN grep, no R8); it ships as WORKFLOW ARTIFACTS, NEVER a GitHub Release on this repo — the D-447 danger is now mechanical because the in-app updater reads /releases?per_page=N and USES PRERELEASES (D-251), so ANY release-signed asset here (stable or prerelease) would be surfaced to the DEBUG app's updater and the D-423 picker would offer an uninstallable APK (signature + applicationId mismatch); artifacts touch no release page — D-447 stands and the APKs stay CI-downloadable; the re-host to Confused-Creature-180/ANI-KUTA remains BLOCKED on the release-agent token (the round-39 pending item — the dev PAT is pull-only there). The release-build-once.yml workflow is workflow_dispatch-ONLY (no automatic trigger — it cannot run by accident) and the v1.1.34 cycle's CI ledger is disclosed as THREE runs: implementation + the debug release + this user-ordered one-time build. Record: download-research/32.

## D-561 (round 73): the sponsor word dies on the popup, the wizard says one line, the hidden Always-sponsor debug page, and the presets are back (the v1.1.34 device round)
The v1.1.34 round confirmed the overlay row ("shows properly, and it works properly too") and set five directives. (A) THE WIZARD COPY: the OVERLAY step's description is replaced word-for-word by the user's dictated line "Let's AniKuta show floating elements, which make things easier." — "And that's it, nothing more." (B) THE SPONSOR-LESS POPUP ("it should never mention sponsored… make sure that it does not mention sponsor anywhere"): the SPONSORED eyebrow composable is deleted outright, "A quick visit to our sponsor…" and "Stay with our sponsor…" are gone; Pending = "Support AniKuta" (the ONLY heading) → "(It just takes a few seconds)" (the user's rounded-brackets line) → Continue → Not now; the approved overlay row gains ONE description line under its title ("Makes things easier for you") and nothing else, its `if (!overlayGranted)` rule untouched; TryAgain ("(Stay for ${seconds}s, then come back)" — the real threshold survives) and Waiting ("(Come back to AniKuta when you're ready)") adopt the parenthetical voice; the D-560 machinery (Crossfade, back-cancels, pill wiring, ON_RESUME consent re-read) is byte-identical. (C) THE HIDDEN DEBUG PAGE ("long presses on the debug options… a toggle to turn on always sponsor… every single time the user tries to open it up, it will open up the sponsor page… by default it will be turned off"): AdPreferences.alwaysSponsor (persisted, default OFF) + alwaysSponsorFlow; AdsCoordinator.onAppOpened() — prefs check → Idle-only → a 2s respawn-suppression after an open-ad completes (the browser-return ALSO lands as an app-open; without the window the popup would loop) → pendingProceed = {} → AdPending; deliberately NO cooldown/grace/offline gate (the toggle means ALWAYS; a failed browser-open completes gracefully); AlwaysSponsorGate() = a ProcessLifecycleOwner ON_START observer composed once next to SmartLinkAdInterstitial() ("opens the app" is a process-foreground fact; the first dispatch posts after composition so cold opens are caught); OFF = a no-op observer, the normal system byte-for-byte. The page: SponsorDebugKey + SponsorDebugScreen (ONE switch), reachable ONLY by long-pressing the "Debug options" row — MoreListRow gained an optional onLongClick (combinedClickable, stable in this Compose 1.10.4 line per LibraryScreen's existing usage) threaded through HubSection; ships in BOTH build types (the Debug-page doctrine). (D) THE PRESET ICONS ("there are no pre-set app icons… add all of these icons… so that they can properly be turned into app icons"): the six RAW_ICONS.zip artworks center-cropped + 512px as drawable-nodpi/preset_icon_{dark,teal,sky,gold,green,pink}.jpg (~120KB total), a Presets grid ABOVE the GitHub catalog (the D-432 full-artwork rounded-cell rule holds), the tap exporting the drawable to filesDir/app-icons/presets/<key>.png and riding the EXACT override machinery (hero label resolves preset names first; clear-override resets) — the round-37 no-presets doctrine superseded by this explicit order, the launcher-baked limitation unchanged. (E) BOTH RELEASES at v1.1.35/10135 from the SAME tag: the debug arm64 via release-apk.yml, the five release-signed variants via release-build-once.yml (artifacts-only, D-447 stands) — the ≤2 budget disclosed as superseded by explicit instruction for a second consecutive cycle. Independent review: VERDICT SHIP, zero blockers; brace balance machine-verified on all ten files. Record: download-research/33.

## D-562 (round 74): the launcher icons become REAL (the D-417 alias system restored), the catalog dies, the popup's heading stands alone, and the Always-sponsor trigger moves to the entry click (the v1.1.35 device round)
The v1.1.35 round reported the icon functionality broken ("when I change the app icon in the settings, then the app icon should actually be changed for the application… this functionality was working previously") and ordered the rest. (A) THE ROOT CAUSE: the "previously working" system was D-417's 8 activity-aliases; D-434 (round 37) removed it and D-561's presets rode the in-app-only override (inAppOverridePath → an exported PNG → the hero AsyncImage) — Android forbids runtime launcher icons from arbitrary bitmaps, so the home-screen icon NEVER moved. The fix: MAIN/LAUNCHER moves OFF MainActivity onto SEVEN aliases (.icons.IconDefault enabled + .icons.Icon{Dark,Teal,Sky,Gold,Green,Pink} disabled — deep-link/OAuth filters stay on the activity; alias class names resolve against the NAMESPACE, the .debug suffix is safe per the D-417 verified pattern); per preset key an ADAPTIVE mipmap (mipmap-anydpi-v26/ic_launcher_<key>.xml: bg = @drawable/preset_icon_<key>, fg = the shared empty ic_launcher_fg_none) + a legacy mipmap-xxxhdpi/ic_launcher_<key>.jpg (minSdk 24 — an adaptive-icon XML cannot inflate below 26); AppIconController.applyLauncherIcon enables the TARGET alias FIRST (DONT_KILL_APP) then disables the previous — never zero launcher entries; AppIconPreferences shrinks to launcherIconKey ("" = default); reconcileLauncherIcon (AnikutaApp.onCreate + every page open) self-heals the app-UPDATE gap (component states can reset to manifest defaults while prefs survive — one binder read when nothing is persisted, one re-apply when the saved alias is not explicitly enabled). (B) THE CATALOG DIES ("remove this functionality and only keep the preset app icons"): fetchCatalog/parseCatalog/loadCatalogIconFile/httpGet/CATALOG_API_URL/CatalogIcon/catalogJson/the grid/the refresh/the empty-note/the exported-PNG override machinery — all deleted; the page is the hero (painting the ACTIVE LAUNCHER resource) + the six baked presets + "Back to the app's icon"; the Appearance row's subtitle becomes "Preset app icons". (C) THE HEADING-ONLY POPUP ("I think we should only… show the heading Support [AniKuta]"): the HeroBubble composable + all three usages deleted — Pending = "Support AniKuta" → "(It just takes a few seconds)" → Continue → Not now (+ the FROZEN overlay row while the consent is missing), Waiting keeps the bare spinner, TryAgain keeps its threshold line; the D-560 machinery byte-identical; zero sponsor strings. (D) THE ENTRY-CLICK TRIGGER ("what I wanted was that every time the user clicks on an entry, then it will show the sponsor rather than every time opening up the app"): AlwaysSponsorGate + AdsCoordinator.onAppOpened + lastOpenAdCompletedAt + the respawn-suppression window are DELETED; the gate lives in requestNavigation right after the in-flight guard — ON = EVERY entry click shows the interstitial with the grace/cooldown/offline/enabled gates ALL bypassed (the toggle means ALWAYS) and the held proceed = the REAL navigation (completing the popup lands on the tapped entry); OFF = the normal system byte-for-byte. The page heading becomes "Debug options" (it IS the debug-options page) and the toggle row is the title + the Switch, NOTHING else. (E) BOTH RELEASES at v1.1.36/10136 from the SAME tag — the ≤2 budget disclosed as superseded by explicit instruction for a third consecutive cycle. Verification: stale-symbol sweep clean; manifest machine-validated (7 aliases); brace balance machine-verified on all ten touched files. Record: download-research/34.
