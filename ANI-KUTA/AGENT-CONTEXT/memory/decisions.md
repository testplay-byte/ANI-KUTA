...
## D-485 (round 48): the poster pipeline hardening — the composer owns Dispatchers.IO, the art loads are time-bounded
...
## D-486 (round 48): the preview's "No episodes available yet" state was UNREACHABLE — the state machine is hasContent-first
Static-analysis find while fixing D-485: the empty path set `failed = false, hasContent = false`, but the UI's when-gate required `failed && !hasContent` — so the honest empty state could never render and an eternal spinner showed instead. The gate is now `!result.hasContent` FIRST (before `failed`), and the empty path keeps `failed = false` (an empty state is not a failure). Verified against the user's Task-2 spec: feed-first → random library content WITH cached episodes (EpisodeDemoPicker reads the real `library_item` table via `getLibraryMainIds` + requires cached episode metadata; re-rolled per open via the roll counter + the Shuffle action) → the honest empty state; no invented "Sample" text anywhere.

## D-487 (round 48): the Notifications screen's episode-type block mirrors the Updates screen's layout EXACTLY
...
## D-488 (round 48): the library badge positions are REAL now — the ViewModel state reaches CoverBadgeRow
