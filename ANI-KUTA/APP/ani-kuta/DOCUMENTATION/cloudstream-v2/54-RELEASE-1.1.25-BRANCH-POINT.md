...
`release/1.1.25` cut from the round-63 feature head `d763ff52`
(`feature/round-57-cloudstream-downloads`, CI **GREEN** — implementation run
35593691913). Version bump rides THIS branch as its first commit:
**1.1.25 / 10125**.
...
## The CI ledger (this cycle — DISCLOSED: 3 runs vs the ≤2 budget)

- Implementation: run 35592484357 **RED** on 392c79b3 — exactly ONE compile
  error: the probe scope's `onDispose { probeScope.cancel() }` used the
  `CoroutineScope.cancel()` EXTENSION without its `kotlinx.coroutines.cancel`
  import (compile-time-only API-shape discoverability — the same class as the
  round-60 finds; no local toolchain exists in the sandbox to pre-compile).
- The one-import repair: run 35593691913 **GREEN** (d763ff52, first-try).
- The release: run 35594620963 **GREEN on the FIRST run** (the tag).
