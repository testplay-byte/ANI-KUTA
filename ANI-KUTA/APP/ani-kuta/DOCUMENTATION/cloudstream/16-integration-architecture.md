...
Checked: 26 factual claims sampled → 26 verified, 0 corrected, 0 flagged-stale (one ±1 line drift noted below). Consistency: ok — the vendor-recommendation facts hold exactly.
Corrections: none.
High-value facts re-verified: library **Kotlin 2.4.0** (CS3 toml:29) vs our **2.2.0** (toml:4); AGP 9.1.1 (toml:5) vs 8.9.1 (toml:3); coroutines 1.11.0/1.9.0 (toml:32/52); serialization-json 1.11.0/1.7.3 (toml:35/53); jsoup 1.22.1 vs 1.19.1 pinned "Source API (binary compat for extensions)" (toml:24 vs :64-65); OkHttp 5.0.0-alpha.14 pinned (:46-48); Jackson `strictly = "2.13.1"` with the minSdk<26 comment (toml:22); compileSdk 37 (toml:62) vs our 36 (AndroidConfig.kt:9); our minSdk 24 (AndroidConfig.kt:10); library deps at build.gradle.kts:55-69 (jackson-module-kotlin, jsoup, kotlinx-*, ksoup, ktor-http, NiceHttp, Rhino, cryptography) + jvmCommonMain kotlin-reflect/NewPipeExtractor (:76-82); maven-publish `com.lagradost.api:1.0.1` (:21, :126-132); `-Xexpect-actual-classes` (:44); **the full §1.4 binary census re-run over all 80 `.cs3`** — nicehttp 80/80, jsoup 64/80, jackson 61/80, CommonActivity 16/80, gson 13/80, DataStore **7/80 = exactly AniDb, AnimePahe, Cinemacity, DoraBash, StremioX, Ultima, XDMovies**, MainActivity exact-descriptor `Lcom/lagradost/cloudstream3/MainActivity;` **3/80** (StreamPlay, StremioX, Ultima — the prefix-grep ambiguity is real: unqualified match hits 78/80 via MainActivityKt), CloudStreamApp 7/80, fuzzywuzzy 0/80, MainActivityKt 78/80; GPL-3.0 root LICENSE + absent library/LICENSE + absent ANI-KUTA LICENSE (all path-checked); 174 Kotlin files / 1.6 MB in library/src; **97** built-in `: ExtractorApi()` classes in library extractors/; TestPlugins/build.gradle.kts:74-75 "Stubs for all cloudstream classes" + `pre-release` coordinate; extensions/build.gradle.kts:74 `-SNAPSHOT`; coreLibraryDesugaring at app/build.gradle.kts:274 (doc says :275 — ±1, trivial); gson shim comment :279-280; CS3 `isMinifyEnabled = false`; ours at anikuta.android.application.gradle.kts:36; toml:17-34 Compose-1.10.4 pinning history + checkDependencyAlignment note. Doc 14's zero-consumer claim (§2.4/§5.3 basis) re-confirmed.
