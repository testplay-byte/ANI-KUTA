...
Checked: 30 claims sampled → 30 verified, 0 corrected, 0 flagged-stale.
Corrections: none.
Confirmed (incl. all high-value targets): **Coil 3 image loading** (`ImageModuleCoil.kt` — coil3 imports + `OkHttpNetworkFetcherFactory`, crossfade 200 ms, 10% heap memory cache, 512 MB/4% disk cache at `cs3_image_cache`, per-request `NetworkHeaders` with default UA + caller headers layered, "headers can be overridden by extensions" comment — IMG:94-116 exact); **posterHeaders flow** (field on SearchResponse:1412 + LoadResponse:1832; threaded through `ResultData` at RVM2:~291; consumed at RFP poster/background/logo and `SearchResultBuilder.kt:135-139` `loadImage(card.posterUrl, card.posterHeaders)`; correctly NOT passed for episode stills EA:~240 and actor portraits AA:~119 — both omissions re-confirmed); **TmdbProvider = open base class, not a built-in provider** (open class at TmdbProvider.kt:53; grep shows zero instantiations of TmdbProvider()/TraktProvider()/MyDramaListAPI()/CrossTmdbProvider() in app+library; the only app-side metaprovider reference is SyncRedirector); **no auto-enrichment** (no cross-cutting metadata service; `applyMeta` at RVM2:1666-1804 is sync-account-driven fill-if-absent, plus anime-tracker lookup + Kitsu episode merge; MetaProvider flag only renders a badge); **TMDb key hardcoded** (`private val tmdbApiKey = "e6333b32409e02a4a6eba6fb7ff866bb"` at TmdbProvider.kt:67, passed as `api_key` query param in getApi:246-248; Trakt/MDL keys from BuildConfig at library/build.gradle.kts:113-122). Also verified: load() contract + fixUrl JSON tolerance (MA:742-762); Trakt/MDL JSON-blob url pattern (`Data(...).toJson()` ↔ `parseJson<Data>(url)`); CrossTmdbProvider fan-out + CrossMetaData replay; TmdbProvider two modes incl. TmdbLink blob as dataUrl; `loadSmall`/LoadResponseFromSearch at RVM2:2549+; timeout clamp; 10-min/20-entry load cache; SyncRedirector MAL/AniList regexes + no in-repo supportedSyncNames overrides; TrailerData shape + loadTrailers cap 3 + raw fallback path; csredirect share flow.
