...
Checked: 18 claims sampled → 18 verified, 0 corrected, 0 flagged-stale. Consistency: ok.
Corrections: none.
Samples re-verified against source: `CloudStreamApp.onCreate` = crash handler + debug flag only (:72-87; ExceptionHandler :41-68 writes `currentlyLoading` + stacktrace to `filesDir/last_error`, restarts, exitProcess); AcraApplication = 78-line `@Deprecated(ERROR)` alias shim (:7-12); favorites key `"$currentAccount/result_favorites_state_data/<id>"` (DSH:52 + folder pattern :530-531; all sibling key constants :50-55 exact); `currentAccount` DSH:181, `setAccount` DSH:198; BookmarkedData DSH:374 / FavoritesData DSH:435 / SubscribedData DSH:310 / ResumeWatchingResult DSH:494 (±1 line); id = `url.replace(mainUrl,"").replace("/","").hashCode()` (ResultViewModel2.kt:376-379); `LocalList : SyncAPI()` at LocalList.kt:19, `requiresLogin=false` :24; SubscriptionWorkManager 6 h TimeUnit.HOURS :46 / NetworkType.CONNECTED :42 / KEEP :53 / work_subscription :30 / foreground FOREGROUND_SERVICE_TYPE_DATA_SYNC / self-cancel-when-empty; the 6 notification channel ids (`cloudstream3.extensions/subscriptions/backups/general/download.queue/updates`) each confirmed at cited file:line; APIRepository CACHE_SIZE=20 :52-64; `PLUGINS_KEY`/`PLUGINS_KEY_LOCAL` PM:70-71, `removeKey(PLUGINS_KEY_LOCAL)` :536, `isSafeMode`/`checkSafeModeFile` :571-579, `isDisabled`→unload :305-308; APIHolder MainAPI.kt:109-172 + AutoDownloadMode :1144; AccountManager = exactly 10 repos (5 SyncRepo + 4 SubtitleRepo + 1 PlainAuthRepo); WatchType.kt:7-13 (5 states + NONE); MA:905-909 DubStatus.
