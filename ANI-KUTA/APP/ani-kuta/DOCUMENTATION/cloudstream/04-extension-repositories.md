...
Corrections: none.
Confirmed (incl. all high-value targets): `parseRepository`/`parsePlugins` 5-minute HTTP cache + `getRepoPlugins` amap/flatten (RepositoryManager.kt:161-191); `repo.json` model quote (5 fields incl. iconUrl) and both real repo.json quotes byte-identical to `extensions/repo.json` / `phisher-builds/repo.json`; 17-key plugins.json field census (re-ran python census over phisher plugins.json — union matches exactly; `languageFromUrl`/`fileMimeType` absent); `SitePlugin` model quote matches source; status constants 0/1/2/3 (MainAPI.kt:391-395 + legacy "30 benenes" comment at ~385-390 + `R.array.extension_statuses` Down/Ok/Slow/Beta); update rule `version > saved || version == PLUGIN_VERSION_ALWAYS_UPDATE(-1)` (PluginManager.kt:229-230, constants at 108-112); sha256-on-download ("Extension hash mismatch" throw, temp file in cacheDir, atomic move, only-if-non-null check); DataStore key literals (`REPOSITORIES_KEY` @ ExtensionsViewModel.kt:30, `PLUGINS_KEY`/`PLUGINS_KEY_LOCAL` @ PluginManager.kt:70-71, `auto_update_plugins`/`auto_download_plugins_key2`/`jsdelivr_proxy_key` @ donottranslate-strings.xml); gradle-plugin claims re-verified against `/tmp/cs-gradle/` (`val apiVersion = 1` :11, `status = 3` default :84, `buildBranch = "builds"` :19, setRepo github/gitlab/codeberg/gitea templates, WriteCacheEntryTask jar fields + excludeNulls + `internalName = name`, PluginEntry "For cross-platform" comment); kill-switch path (isDisabled → unloadPlugin); auto-download gates (blank url/repositoryUrl skip, NSFW, FilterByLang); parseRepoUrl shortcode resolution (py.md `!code` / cutt.ly); cs.repo + cloudstreamrepo deep links in MainActivity; WebView `RepoApi.installRepo`; Levenshtein >80 fuzzy search with description take(64); repos-db.json census (26 entries, 1 verified object, CS.json @ :11, `repo` @ :16, gitlab @ :19, disroot @ :12); ci_check.py full rule list; create_markdown.py preamble; check_repos.yml rentry publish; "vote api is down" PluginAdapter.kt:175-176. This doc's gradle-source citations depend on `/tmp/cs-gradle/` (still present) — future readers should re-fetch if /tmp is cleared.
