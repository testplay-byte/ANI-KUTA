...
- 2026-08-30 (session 9 / Task 48.1): **DEVICE-FEEDBACK ROUND 8 — THE CRASH + THE 428 + watch-page metadata.** Round-8 report (LOGCAT.MD, 337KB, distilled by a dedicated sub-agent): details pages good ✓; (a) AniKoto episode tap → resolving spinner ~20s → APP CRASH (`FATAL EXCEPTION: OkHttp Dispatcher — CloudflareBlockedException` at CloudflareKiller.intercept → process death ~26.7s after the tap — the exception NEVER became a coroutine exception); (b) MovieBox resolves one 720p MP4 → player → `HTTP 428 Precondition Required` from BOTH the cache proxy's upstream learn request AND mpv's direct auto-retry (the proxy's fail-open 301→original-URL hands the request to mpv); (c) no episode description on the player page; (d) no thumbnails/synopsis in the watch episode list; plus two post-crash-restart findings: MovieBoxProvider ClassCastException (app context after ErrorActivity restart) and 7 corrupted .data.json files (each = complete valid JSON + stale tail of the previous LONGER write). **(D-357) THE CRASH — three layers:** CloudflareBlockedException is now `: java.io.IOException` (OkHttp's AsyncCall only routes IOException to onFailure → resumeWithException; every other Throwable is rethrown on the dispatcher thread → uncaught handler → process death — the bridge's partial-links rescue never saw it; all 5 typed catch sites verified unaffected); a terminal **CsInterceptorSafetyNet** rides OUTERMOST on the shared plugin client (IOException passes typed; every other Throwable — including the NoClassDefFoundErrors of the 19 missing census extractor variants — wraps into a descriptive IOException → honest resolver errors instead of process death); the headless WebView solver now FAST-FAILS (1s cookie probe: cf_clearance → early success; still ZERO cookies 8s after the first page finish → unsolvable interactive challenge → immediate null instead of the 20s watchdog ×N serialized hosts — the akirax/kryntal evidence showed page-finish-with-no-cookies-at-all, which no managed-challenge headless solve ever recovers from). **(D-358) THE 428 — two independent comma-truncation bugs:** (1) the proxy's MpvHeaderParser split the header csv on ',' and DROPPED non-`Name:`-shaped fragments → every User-Agent truncated at `(KHTML` on every upstream request (learn/serve/fill); (2) mpv itself splits `http-header-fields` (OPT_STRINGLIST) on ',' and honors ONLY backslash escapes — double-quote quoting is NOT supported (verified against mpv master options/m_option.c `get_nextsep` BEFORE implementing; the natural quotes assumption would have made things worse). Fix: the canonical **MpvHeaderFields** (:core:network — canonical home, :core:player + :core:playback-cache already depend on it): `parse()` = the comma-GLUING algorithm (shared with DownloadHeaderParser; the proxy's MpvHeaderParser and SubtitleEngine's naive split(',') now delegate) + `escapeForMpv()` (`\`→`\\`, `,`→`\,`) applied at ALL 7 MPV boundaries (6 WatchScreen setOptionString sites + AnikutaMPVView's Map-based setPropertyString) — the stored/serialized csv, WatchKey fields, and cache descriptors stay RAW (escaping ONLY at the mpv argument — the R8-PLAN A1 amendment, or the proxy would send literal `\,`). 10 unit tests (MpvHeaderFieldsTest) pin gluing/escaping/a simulated-mpv round-trip. 428/429 join the ladder's looksExpired set + PlayerObserver's capture list — a stale sign/t is unreachable by a same-URL retry; the ladder now goes straight to the pinned re-resolve (fresh sign). Honest scope: the UA truncation is code-verified; the 428's CDN trigger may additionally be sign/t binding — the ladder is the safety net if the UA fix alone doesn't clear it. **(D-359) watch-page episode metadata UNION:** buildEpisodeMetadataSerialized iterated `metadata.entries` ONLY — the AniZip/Jikan map, EMPTY for extension content (fetch gated on currentAnimeId > 0) → WatchKey.episodeMetadataSerialized="" → the player page's "Currently playing" description AND the episode-list rows (thumbnails + synopses) starved even though the bridge populates SEpisode.summary/preview_url. Now the union of extension episode numbers ∪ metadata keys (extension-first per-field, sorted), text newline-sanitized (the wire format is line-oriented), description ≤400 chars (Nav3 backstack/restore-Bundle TransactionTooLargeException safety). Known limitation: the map keys on episode NUMBER alone — CS multi-season shows with per-season numbering collapse (titles self-heal via EpisodeTitleParser; descriptions may cross seasons). **(D-360) self-heal + corruption:** the plugin loader watches for a late-arriving activity when the initial load ran app-context (crash-restart → ErrorActivity → MovieBox errored ALL session; now MainActivity's appearance triggers an idempotent loadAll); `copyFileTruncating` uses SAF "rwt" (Google 146330523 — openOutputStream(uri,"w") does NOT truncate on many providers; "rwt" is the truncating mode) with delete-then-create fallback + a per-folder Mutex serializing .data.json read-modify-write cycles. Deferred: DASH/.mpd surfacing (5 of MovieBox's 6 sources are .mpd manifests mpv cannot play — needs an MPD parser + mpv --audio-file composition designed against REAL manifests; the sandbox is region-blocked from the MovieBox API) → next-round roadmap item. v0.2.70 (versionCode 70).
- 2026-08-30 (session 10 / Task 49): **DEVICE-FEEDBACK ROUND 9 — GENERAL RESOLUTION + THE CONSOLE LOGGING TOOL.** Round-9 report: nothing resolves on ANY extension (MovieBox included) + explicit asks: general CloudStream compatibility ("like the original app"), extractor hardening, HLS quality selection, a filterable console logging tool; aniyomi extensions must not regress. Research FIRST (three agents + plan review): R9-A catalogued 17 failure modes of the resolve path and found **THE DEAD-DISPATCH BUG** — `loadExtractor` normalized the embed URL but compared against the scheme-FUL extractor mainUrl → dispatch never matched → 53/80 census plugins silently resolved 0 links since v0.2.68 (the v0.2.70 "no videos" round was this + CF-blocked hosts + the error black hole, NOT a v0.2.70 regression); R9-B mapped the logging stack (Logger+LogAppender existed; the debug-only console was useless to a release-APK user); R9-C verified 35/35 census extractors registered+real, MovieBox's .mpd = same hcdn3 host + sign + signCookie as the .mp4 (API geo-blocked from sandbox; hcdn3 428s unconditionally at root), and that the bridge never expanded m3u8 masters. **Fixes (D-361..D-363, plan-reviewed GO+amendments):** (1) both-side dispatch normalization (`^(https?://)?(www\.)?` + lowercase + trailing slash — the old regex didn't cover http://) + no-match WARN + CloudflareBlockedException rethrow from getSafeUrl + LoadExtractorDispatchTest (8 cases, CI); (2) VideoResolver error propagation — failures captured with per-hoster isolation kept, rethrown when the final result is empty (the sheet shows the REAL reason; distinct timeout errors; linked==null honest error; bridge itemizes hidden/dropped); (3) the console — RingLogBuffer (:core:common, 10k ring, ALL builds; Logger INFO+ in release, deliberate §20 reversal D-362) + com.lagradost.api.Log sink (plugin logging captured; runCatching-wrapped for JVM tests) + 21 mirrored raw-Log sites + ConsoleLogsScreen (Settings → Developer tools → Console logs: text/level/namespace filters, live 1s + pause, Clear/Copy/Export) with export = version/device header + ring snapshot + own-process `logcat -d --pid` dump → ACTION_SEND share via the existing FileProvider; (4) HLS quality selection — the bridge expands unlabeled M3U8 masters into per-quality variants (M3u8Helper.parseMasterPlaylist extracted PURE + tested; fail-open; ≤4 fetches/resolve, ≤8 variants; labeled variants skip); (5) DASH surfacing — MpdParser (XXE-hardened DocumentBuilderFactory) + bridge sniffer: static single-file .mpd → directly playable VIDEO links (separate single-file audio → audioTracks → the REAL mpv audio-add plumbing), dynamic/multi-segment stay hidden but LOG their structure (MovieBox's 5 .mpd finally diagnosable); (6) CF-solver hardening — ≥2 markers for 200-HTML challenges (Turnstile-embedding pages false-positived), the solver loads the CHALLENGED PATH (not host root), and the WebView ATTACHES to the live activity's decor view (1dp; runCatching + detached fallback) — attached views pass challenge-JS probes detached headless views fail. New tests: dispatch 8 + m3u8 5 + mpd 6 + ring 4 (:core:common joined CI). v0.2.71 (versionCode 71).
- 2026-08-30 (session 11 / Task 50): **DEVICE-FEEDBACK ROUND 10 — THE SEPARATION (aniyomi restored + CloudStream upstream parity).** Round-10 report (last-attempt round): some series show no episodes / resolve-but-zero-videos; "the aniyomi system is not working either. I think you have mixed them up"; "Cloud stream handles things a bit differently so we cannot use the exact same system." Three research agents (R10-A aniyomi audit + old-kuta diff + 341-APK dex survey; R10-B upstream clone → THE semantic spec: `APIRepository.loadLinks` wraps ONLY the provider call in withTimeout, catches it, returns false — streamed links NEVER discarded upstream, cached 20 min, progressive into the picker; R10-C census audit: 0/80 plugins declare loadLinksTimeoutMs → every CS source ran the resolver's 120s withTimeoutOrNull which CANCELLED the coroutine and structurally discarded the ConcurrentLinkedQueue) + R10-PLAN adversarial review (GO + amendments: `kotlin.` exclusion prefix — sb-template v16 extensions bundle a partial stdlib incl. `kotlin.coroutines.Continuation`, a naive child-first port would ICCE every suspend call; label-neutral dub rows instead of a dedup-key change — EpisodeListNormalizer re-dedupes by URL, LazyColumn keys by url, the episode cache is UNIQUE(main_id, episode_number); purge table is `data_cache_episode`). **(D-364) THE ANIYOMI REGRESSION:** D-294's unconditional parent-first loader made the host's serialization-2.x/Kotlin-2.2.0 classes win at class-resolution → extensions bundling 1.x hit NoSuchMethodError/NoClassDefFoundError AT RESOLUTION time (browse/search never touch serialization → "some episodes don't resolve" for two device rounds). ChildFirstPathClassLoader restored (old-kuta) + parent-first exclusion prefixes (API/network/util/com.confused.anikuta/kotlin/okhttp3/okio/jsoup/rx); per-fqcn LinkageError → parent-first retry (D-294's instantiation-crash class still covered). **(D-365) THE SEPARATION:** `AnimeHttpSource.isCloudStreamBridged` (additive open val; bridge → true) dispatches VideoResolver into AniyomiSourcePipeline (restored + probe memoization + honest hoster-only errors + lazy resolveVideo + "null"-URL filter) vs CloudstreamSourcePipeline (cache-first, NO outer timeout). Bridge getVideoList: budget wraps ONLY provider.loadLinks; timeout+partials → WARN+KEEP (THE fix); CF rethrows only at zero links; caller-cancellation propagates (inner catch rethrows all CancellationException; withTimeoutOrNull converts only its own); expansion pass outside the budget. **(D-366) Link cache + purge:** 20-min (sourceId, episodeUrl) cache over raw VideoEntry lists (upstream RepoLinkGenerator parity; CS-only — aniyomi local proxies must never cache); forceRefresh=true on the recovery ladder + ReResolver (dead links never replay); startup `DELETE FROM data_cache_episode WHERE episode_url IS NULL OR ''` (v0.2.68 rows restored with the SERIES url → could never resolve). **(D-367) Honest episodes:** comingSoon → ISE (not silent empty); TorrentLoadResponse → one honest torrent row; shared dub data handles → ONE label-neutral row (no downstream key changes); +8 census mirror extractors (dood.to/.wf/d000d.com, vidhidevip/plus.com, mixdrop.ag, streamsb.net, waaw.to) 35→43; provider-name collision WARN. **(D-368) Selection intelligence:** old-kuta VideoTitleParser rules in groupIntoServers (audio/language tokens never server names; resolution priority; Server A/B/C; "All Videos" raw fallback; pipe videoTitle format kept); 5-tier episode-switch preference (server+audio+quality → … → highest); Link-Source sheet sectioned Aniyomi/CloudStream. Tests: CloudstreamLinkCacheTest 6 + CloudstreamBridgeEpisodesTest 8; :core:video-resolver joined CI. v0.2.72.
