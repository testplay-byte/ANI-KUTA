...
Checked: 38 claims sampled → 35 verified, 2 corrected, 1 annotated.
Corrections:
1. §2.2: `extensions/build.gradle.kts:74` → `:73` (the `library:-SNAPSHOT` dependency line; off-by-one, claim itself correct).
2. §2.2: webMain file list — it contains **6** files (5 matching `*.web.kt` + `Log.kt`); original list omitted `SubtitleHelperPlatform.web.kt`. Fixed inline.
3. §2.3: repo.json model also has an `iconUrl` field (`Repository` has 5 fields, not 4). Annotated inline.
Spot-checked and confirmed against source: README quotes/line numbers (24/26/3/28-33/36/38-40/43-50/109), all 20+ `libs.versions.toml` version+line citations (Kotlin 2.4.0, AGP 9.1.1, minSdk 23/targetSdk 36/compileSdk 37, 4.8.0/68, Media3 1.9.3, Coil 3.3.0 strictly, Rhino 1.8.1 strictly, Jackson 2.13.1 pinned w/ exact comment, zipline 1.27.0, NiceHttp 0.4.18), `app/build.gradle.kts` citations (104 applicationId, 143-179 flavors, 146 minify-off, 199-202 viewBinding, makeJar 299-322), `library/build.gradle.kts` citations (10/21/25-39/41/55-69/76-85/88-97/9/126-132), `PluginManager.kt:611` PathClassLoader + 593-687 load flow + 579-588 safe mode + 92 apiVersion=1, `RepositoryManager.kt` 33-40/50-76/55-59/73-75/99/100-102/109-122, `BasePlugin.kt:20-35/64-77`, `MainAPI.kt` (2860 lines; APIHolder:109; MainAPI:494; TvType:1120 with exactly 18 values; Prerelease:63), extractor count 97 classes/104 files, 61 `values-b+*` + 50 fastlane locales, `settings.gradle.kts`, `AndroidManifest.xml:201` cloudstreamrepo scheme, jitpack.yml, workflows (prerelease.yml secrets-keystore + `pre-release` tag; pull_request.yml checkKotlinAbi), cs-repos `repos-db.json` = 26 entries/1 verified, `ci_check.py` logic, official extensions repo = exactly 5 providers, `extensions/repo.json` contents, AI-POLICY.md existence.
