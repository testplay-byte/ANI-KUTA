...
Checked: 42 claims sampled → 42 verified, 0 corrected, 0 flagged-stale.
Corrections: none.
Confirmed (incl. all high-value targets): the **24 open properties + 9 open functions** inventory (counted `open` members in MainAPI.kt @ efc1915 — exact match; `getVideoInterceptor` correctly identified as the sole non-suspend member); timeout clamps (`DEFAULT_TIMEOUT=120_000L`, `MAX_TIMEOUT=4*DEFAULT` (8 min), `MIN_TIMEOUT=5_000L`, `coerceIn` in `APIRepository.getTimeout`); absence of `login`/`getLoginInfo`/`resolveLink`/`headers`/`mainImageUrl`/`rank` in current MainAPI (repo grep); all line citations spot-checked in MainAPI.kt (493-512 companion+init, 514-546 identity fields, 548-601 flags+timeouts, 604-630 sync/types, 632-712 the 9 functions, 1412/1832 posterHeaders, 410-491 MainPageData/mainPageOf/newHomePageResponse/newSearchResponseList, 735-762 fixUrl), APIRepository.kt (28-64, 85-121, 123-148, 156-196, 204-219), RepoLinkGenerator.kt (dedup + 20-min cache), GeneratorPlayer.kt:264, CS3IPlayer.kt:1941, ExtractorApi.kt (500-519, 526-569 @Prerelease DRM overload, 700-714 ExtractorLink 9 fields, 849-891 Qualities, 914-979 loadExtractor with reverse-order + Levenshtein>80, 985 extractorApis, 1419-1466 abstract class), metaproviders (TmdbProvider:53 + hardcoded key at 64-68; CrossTmdbProvider:22; TraktProvider:42-60; MyDramaList:39-68; SyncRedirector:7), clone-site block at MainActivity.kt:810-839 (quote matches verbatim), ExampleProvider/ExamplePlugin quotes, `library/api/jvm/library.api` exists (592 KB). Line refs were uniformly accurate to ±1-2 lines (e.g. VPNStatus enum actually at 893-897, ExampleProvider class starts :8) — within tolerance, left as-is.
