 ...
 *
 *  - API 30+: [AccessibilityService.takeScreenshot] (2-arg form — deprecated in API 34 but
 ...
 *
 * Returns null on failure (no window, takeScreenshot rejected, PixelCopy error) — the
 * executor reports "NO_WINDOW" or "SCREENSHOT_FAILED" to the agent.
 */
class ScreenshotCapture(
    ...
    /** Capture + downscale + JPEG-compress. Returns JPEG bytes, or null on failure. */
    suspend fun capture(): ByteArray? {
        val raw = captureRaw() ?: return null
        return try {
    ...
    private suspend fun captureViaTakeScreenshot(): Bitmap? =
        suspendCancellableCoroutine { cont ->
            val ok = service.takeScreenshot(
                Runnable::run,
                android.util.Consumer { result: AccessibilityService.ScreenshotResult ->
