...
class UpdateDownloader(
    ...
    fun download(info: AppUpdateInfo): Flow<DownloadProgress> = flow {
        ...
        Logger.i(TAG) { "download: starting download of ${info.downloadUrl} → ${apkFile.absolutePath}" }
        ...
        try {
            ...
            Logger.i(TAG) { "download: total size = ${totalBytes ?: "unknown"} bytes" }
            ...
            if (success) {
                Logger.i(TAG) { "download: complete — ${apkFile.length()} bytes at ${apkFile.absolutePath}" }
    ...
    private companion object {
        private const val TAG = "Anikuta:Core:AppUpdate:Downloader"
