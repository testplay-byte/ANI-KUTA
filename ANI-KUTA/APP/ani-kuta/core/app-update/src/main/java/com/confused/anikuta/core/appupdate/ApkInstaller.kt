...
class ApkInstaller(
    ...
    fun installApk(apkPath: String): Boolean {
        ...
        if (!apkFile.exists()) {
            Logger.e(TAG) { "installApk: file does not exist: $apkPath" }
            return false
        }
        ...
        return try {
            ...
            Logger.i(TAG) { "installApk: launching installer for $apkPath (${apkFile.length()} bytes)" }
            context.startActivity(intent)
            true
        } catch (e: Exception) {
            Logger.e(TAG, e) { "installApk: failed to launch installer" }
            false
        }
    }

    private companion object {
        private const val TAG = "Anikuta:Core:AppUpdate:ApkInstaller"
    }
}
