...
data class UnifiedAnime(
    ...
    /**
     * Which data source's metadata is currently displayed (D-130).
     * - [DataSourcePriority.ANILIST]: AniList metadata is shown (score, synopsis, etc.).
     * - [DataSourcePriority.EXTENSION]: Extension metadata is shown.
     * Only meaningful when BOTH [anilistId] and [sourceId] are non-null (i.e. the
     * entry is linked). For AniList-only or extension-only entries, this field
     * reflects the only available source.
     */
    val dataSourcePriority: DataSourcePriority = DataSourcePriority.EXTENSION,
) {
