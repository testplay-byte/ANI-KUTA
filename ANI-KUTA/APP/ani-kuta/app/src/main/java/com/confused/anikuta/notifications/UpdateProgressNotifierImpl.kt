...
class UpdateProgressNotifierImpl(
    ...
    override fun onFinish(summary: UpdateCheckSummary) {
        lastPostAt = 0L
        // D-426 (round 37 — the D-410 port from release/1.1.1): an EMPTY run
        // is SILENT. The device report: the "Episode check complete — nothing
        // was due for a check this run." notification fired on every app open;
        // it must only show when there was actually something to check.
