...
object AndroidConfig {
    ...
    const val versionCode = 10122
    const val versionName = "1.1.22"
    // D-498 (round 59): release/1.1.22 — cut from the round-59 feature head
    // baa1ee12 (CI green on the FIRST implementation run, 35530923924). What
    // the release carries on top of v1.1.21 (the round-59 device round —
    // full detail: AGENT-CONTEXT/memory/decisions.md +
    // download-research/20):
    // D-546 THE DASH PARSER FACTORY FIX — the REAL "Manifest could not be
    // parsed" root cause, exposed by v1.1.21's body-peek diagnostics: a
    // VALID <?xml manifest body + "This parser does not support
    // specification "Unknown" version "0.0"" (the VERBATIM always-throw
    // message of Android libcore's DocumentBuilderFactory base class, whose
    // setXIncludeAware/isXIncludeAware/setSchema/getSchema throw
    // UnsupportedOperationException on every Android device, even for
    // setXIncludeAware(false)). Both hardened factories (DashManifestPlanner
    // + MpdParser) called isXIncludeAware = false UNWRAPPED inside the
    // factory .apply{} — the factory construction crashed before one byte
    // of XML was read, on EVERY device, since the pipeline existed; v1.1.20
    // and v1.1.21 died at the same line, and MpdParser silently swallowed
    // the same crash (its "DASH hidden" verdicts were never real). The
    // always-throwing line is REMOVED from both factories (the supported
    // hardening stays); parse failures always carry the exception class;
    // the manifest fetch rides a derived patient client (connect/read 20s +
    // call 45s, shared pool/interceptors — the 10s CDN cold-start timeout
    // burned attempt 1/3 in the device log while attempt 2 answered in
    // 823ms); segments/subtitles keep the shared client.
    ...
    // instruction). The release line carries 1.1.22/10122; main is the dev
